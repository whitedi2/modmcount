...
Процедура СформироватьВидимостьПолей()
	...
	Если Фильтр2.Количество() Тогда //формируем видимость по первому значению списка
		...
		ИначеЕсли Фильтр2[0].Значение.ИмяДокумента = "Д_ЗаявкаНаТорговлю" Тогда
			...
			ДопПоля = "ВнутренниеДокументы.Ссылка.ФизическийОбъем КАК ФизическийОбъем,
						|ВнутренниеДокументы.Ссылка.ВидЗаявки КАК ВидЗаявки,
						|ВнутренниеДокументы.Ссылка.Цена КАК Цена";
		...
		ИначеЕсли Фильтр2[0].Значение.ИмяДокумента = "Д_ЗаявкаНаОтгрузку" Тогда
			...
			ДопПоля = "ВнутренниеДокументы.Ссылка.ТипРеализации КАК ТипРеализации,
						|ВнутренниеДокументы.Ссылка.ДатаОтгрузки КАК ДатаОтгрузки,
						|ВнутренниеДокументы.Ссылка.Док КАК Док,
						|ВнутренниеДокументы.Ссылка.Грузополучатель КАК Грузополучатель,
						|ВнутренниеДокументы.Ссылка.КоличествоДок КАК КоличествоДок,
						|ВнутренниеДокументы.Ссылка.Цена КАК Цена,
						|ВнутренниеДокументы.Ссылка.Тема КАК Тема";
		...
		ИначеЕсли Фильтр2[0].Значение.ИмяДокумента = "УЧ_Стр_ДоговорДолевогоУчастия" Тогда
			...
			ДопПоля = "ВнутренниеДокументы.Ссылка.Объект КАК Объект,
						|ВнутренниеДокументы.Ссылка.Застройщик КАК Застройщик,
						|ВнутренниеДокументы.Ссылка.Дольщик КАК Дольщик,
						|ВнутренниеДокументы.Ссылка.Сумма КАК Сумма,
						|ВнутренниеДокументы.Ссылка.Квартира КАК Квартира,
			...
			|ПО ВнутренниеДокументы.Ссылка = ДДУ.ДоговорОснования";
			...
			ЗаполнитьПечать(Документы[Фильтр2[0].Значение.ИмяДокумента].ПустаяСсылка());
		ИначеЕсли Фильтр2[0].Значение = Справочники.Д_ВидыВнутреннихДокументов.ЗаявкаНаСогласованиеДоговора Тогда 
			...
			Список.ТекстЗапроса = "ВЫБРАТЬ РАЗЛИЧНЫЕ
			                      ...
			                      |	ЕСТЬNULL(ВЫБОР
			                      |			КОГДА ТИПЗНАЧЕНИЯ(ВнутренниеДокументы.Ссылка) = ТИП(Документ.Д_ЗаявкаНаФинансирование)
			                      |					И ВнутренниеДокументы.Ссылка.Акцептован
			                      |				ТОГДА ""Оплачено""
			                      ...
			                      |		ПО ВнутренниеДокументы.Ссылка = Д_ТекущаяЗадача.Документ
			                      ...
			                      |		ПО ВнутренниеДокументы.Ссылка = ДоговорыКонтрагентов.ДокументОснование
			...
			ДопПоля = "Подстрока(ВнутренниеДокументы.Ссылка.Текстовка, 1, 150) КАК ПредметДоговора,
						|ВнутренниеДокументы.Ссылка.ЮрЛицоКомпании КАК ЮрЛицоКомпании";
			...
			//Колонка3.Заголовок = "Количество";
			//Колонка3.Ширина = 10;
		ИначеЕсли Фильтр2[0].Значение = Справочники.Д_ВидыВнутреннихДокументов.ЗаявкаНаОплату Тогда 
			сабОбщегоНазначения.УстановитьСвойствоЭлемента(Элементы, "СуммаДокумента", "Видимость", Истина);
			...
			Элементы.СписокКорректировок.Видимость = Ложь;
			//Элементы.СписокКорректировокДДУ.Видимость = Ложь;
		ИначеЕсли Фильтр2[0].Значение = Справочники.Д_ВидыВнутреннихДокументов.ЗаявкаНаФинансирование Тогда 
  			сабОбщегоНазначения.УстановитьСвойствоЭлемента(Элементы, "СуммаДокумента", "Видимость", Истина);
			...
			ДопПоля = "ВнутренниеДокументы.Ссылка.ТипИсточника КАК ТипПлатежа,   
					|ВнутренниеДокументы.Ссылка.Организация КАК Организация,
					|ВнутренниеДокументы.Ссылка.БанковскийСчет КАК БанковскийСчет,
					|ВнутренниеДокументы.Ссылка.Источник КАК Источник,
					|ВнутренниеДокументы.Ссылка.НазначениеПлатежаУчет КАК НазначениеПлатежа,
					|ВнутренниеДокументы.Ссылка.Инициатор КАК Инициатор,
					|ВнутренниеДокументы.Ссылка.СтатьяДДС КАК СтатьяДДС,
					|ВнутренниеДокументы.Ссылка.СуммаОплачено КАК Оплачено,
					|ВнутренниеДокументы.Ссылка.РеестрЗаявок КАК РеестрЗаявок";
