...
Функция ПриОткрытииДокумента(Форма, Объект, ТекФорма, НеСкрыватьПодразделения = Истина , ПроверятьДоступность = Истина) Экспорт
	...
	//Если НЕ БюджетныйНаСервере.РольДоступнаСервер("Администратор") И НЕ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.Д_Бюджет") Тогда
	
	Если НЕ ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.Д_Бюджет") Тогда
...
Процедура УниверсальнаяПечать(ОбъектСсылка, ЭтоСсылка = Ложь, ДопПараметры = Неопределено) Экспорт
	...
	Исключение
		ПоказатьПредупреждение(Неопределено, "Ошибка формы просмотра: " + ОписаниеОшибки());
	КонецПопытки; 
...
Процедура ДобавитьМаршрутВХранилище(Ссылка, Копирование) Экспорт
	...
	#Иначе
		...
		Если НЕ АдресХранилищаМаршрута = Неопределено Тогда
			...
			Если ЗначениеЗаполнено(СтарыйБП) Тогда
				Если ТипЗнч(СтарыйБП) = Тип("СправочникСсылка.СогласованиеОбщее") ИЛИ ТипЗнч(СтарыйБП) = Тип("СправочникСсылка.Согласование1") Тогда
...
Процедура УниверсальноеВыполнениеКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды, ДопПараметры = Неопределено) Экспорт
	...
	ИначеЕсли ИмяКоманды = "ОбщаяПечатьОбсужденийПоДокументу" Тогда
		...
		ФормаПечати.Результат = ТабДок;
		ФормаПечати.Открыть();
	ИначеЕсли ИмяКоманды = "Справочник.СогласованиеОбщее.Команда.ОбсудитьДокумент" Тогда
		...
		КонецЦикла; 
		ТекФорма.Открыть();
	ИначеЕсли ИмяКоманды = "Справочник.СогласованиеОбщее.Команда.ОтчетПоВыполнению" Тогда
		ТабДок = Новый ТабличныйДокумент;
...
Процедура ПроверкаИсполнения(Объект, ПодчиненныеДокументы, НуженВозврат = Ложь, ТекстПредупреждения = "") Экспорт
	...
	Если МассивСтрокБезПП.Количество() Тогда
		ТекстПредупреждения = "Не созданы платежные документы по неотмененным строкам заявки. Строки: ";
...
Процедура ПередНачаломРаботыСистемы(Отказ) Экспорт
	...
	Если СтруктураПоиска.Свойство("ВариантИнтерфейса") Тогда
