...
Процедура ДействияПриСозданииФормыДокумента(Форма, СкрыватьПодразделения = Истина, ДопПараметры = Неопределено) Экспорт
	...
	Если НЕ Объект = Неопределено Тогда
		Если Объект.Свойство("Ссылка") Тогда
			...
			Иначе
				...
				сабОбщегоНазначения.ПроверитьФормуНаДатуЗапрета(Форма, УУДокумент);
				// -  Ничуговская
			КонецЕсли;
		Иначе
			ПодставлятьЗначения = Истина;	
	...
	Если НЕ Объект = Неопределено И Объект.Свойство("Договор") Тогда
		Если НЕ Форма.Элементы.Найти("Договор") = Неопределено Тогда
			...
			Если Объект.Свойство("Организация") И ЗначениеЗаполнено(Объект.Организация) Тогда
	...
	Если НЕ Объект = Неопределено И Объект.Свойство("Ссылка") И Объект.Свойство("Дата") Тогда
		...
		Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
			Если УЧ_Сервер.ДатаБлокировкиИзмененияДокументов(Форма.Объект.Предприятие)>Форма.Объект.Дата И УУДокумент Тогда
	...
	Если ЭтоОбъект Тогда
		РегистрыСведений.сабСоответствияОрганизацийПредприятиям.ДокументЗаблокирован(Форма, Объект, ДопПараметры);
	КонецЕсли;
...
Процедура ДобавитьУсловноеОформление(Форма, Объект, ЦиклОбхода = 0)
	...
	ИначеЕсли ТипЗнч(Объект.Ссылка) = Тип("ДокументСсылка.УЧ_ПоступлениеТоваров") Тогда
		...
		ИначеЕсли ЦиклОбхода = 1 Тогда
			...
			ИмяРеквизитаКорСубконто3 = "Объект.ТабличнаяЧасть.Субконто3";
			ИмяПоляКоличествоДоступность = "Объект.ТабличнаяЧасть.КоличествоДоступность";
			ИмяЭлементаКоличество = "ТабличнаяЧастьКоличество";
			ИмяЭлементаЦена = "ТабличнаяЧастьЦена";
			ИмяПоляПодразделениеДоступность = "Объект.ТабличнаяЧасть.ПодразделениеДоступность";
			ИмяЭлементаПодразделение = "ТабличнаяЧастьПодразделение";
			ИмяРеквизитаПодразделение = "Объект.ТабличнаяЧасть.Подразделение";
			ИмяСвойста = "Видимость";
			ЗначениеСвойства = Ложь;
...
Процедура ФормаТолькоПросмотр(Объект, ЭтаФорма, Достуность, ОграничитьПолностью = Ложь, ИсключитениеДляАдминистратора = Ложь) Экспорт
	
	ИсключитениеДляАдминистратораНастройка = Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройки("РазрешитьРедактированиеДокументовНаСогласованииАдминистратору");
	
	Если БюджетныйНаСервере.РольАдминаДоступнаСервер() И ИсключитениеДляАдминистратора И ИсключитениеДляАдминистратораНастройка Тогда   //заблочил д1 02.11.17
		Возврат;
	КонецЕсли;
	...
	Для каждого ТекЭлемент Из ЭтаФорма.Элементы Цикл
		...
		Если ТекЭлемент.Имя = "ЗадачаКомментарий" Тогда
			ТекЭлемент.ТолькоПросмотр = Ложь;
			ТекЭлемент.Доступность = Истина;						
		КонецЕсли;
		
		Если Лев(ЭтаФорма.ИмяФормы,11) = "Документ.Д_"	Тогда
...
Процедура ПриУстановкеНовогоНомераПриУстановкеНовогоНомера(Источник, СтандартнаяОбработка, Префикс) Экспорт  
	...
	Если сабИспользоватьСквознуюНумерациюПоследовательностиДокументовПоЗаказу Тогда
		НомерЗаказа = сабОперОбщегоНазначения.НомерЗаказаПоЦепочкеДокументов(Источник);
		
		Если Не ЗначениеЗаполнено(НомерЗаказа) И Источник.ДополнительныеСвойства.Свойство("ДокументБУ") И ЗначениеЗаполнено(Источник.ДополнительныеСвойства.ДокументБУ) Тогда
        	НомерЗаказа = сабОперОбщегоНазначения.НомерЗаказаПоЦепочкеДокументов(Источник.ДополнительныеСвойства.ДокументБУ.ПолучитьОбъект());
		КонецЕсли;
		
		Если Не ЗначениеЗаполнено(НомерЗаказа) И Источник.ДополнительныеСвойства.Свойство("ДокументУУ") И ЗначениеЗаполнено(Источник.ДополнительныеСвойства.ДокументУУ) Тогда
        	НомерЗаказа = сабОперОбщегоНазначения.НомерЗаказаПоЦепочкеДокументов(Источник.ДополнительныеСвойства.ДокументУУ.ПолучитьОбъект());
		КонецЕсли;
				
		Если ЗначениеЗаполнено(НомерЗаказа) Тогда
			СтандартнаяОбработка = Ложь;
			Источник.Номер = НомерЗаказа;
		КонецЕсли;
...
//ПодпискаНаСобытие ПриУстановкиНомераДокумента
Процедура ПриУстановкеНовогоНомераПриУстановкеНовогоНомераБУ(Источник, СтандартнаяОбработка, Префикс) Экспорт  
		
	сабИспользоватьСквознуюНумерациюПоследовательностиДокументовПоЗаказу = Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройки("сабИспользоватьСквознуюНумерациюПоследовательностиДокументовПоЗаказу");
	
	Если сабИспользоватьСквознуюНумерациюПоследовательностиДокументовПоЗаказу Тогда
		
		//Префикс = БюджетныйНаСервере.ПолучитьПрефикс(Источник); 02.04.24 Белых пока закомментил, но есть вероятность попасть в неуникальный номер с БУ
...
Функция ОбъектМетаданныхНайтиПоНаименованию(ПолноеИмяОбъектаМетаданных, Наименование, ТочноеСоответствие) Экспорт
	
	Попытка 
		Возврат ОбщегоНазначения.МенеджерОбъектаПоПолномуИмени(ПолноеИмяОбъектаМетаданных).НайтиПоНаименованию(Наименование, ТочноеСоответствие);
	Исключение
		Возврат Неопределено;
	КонецПопытки;
...
Функция РольДоступнаИлиПолныеПрава(Роль) Экспорт
	
	Возврат РольДоступна(Роль) Или РольАдминаДоступнаСервер();
...
Функция ПринадлежитПодсистеме(ТекСсылка,ТекПодсистема) Экспорт
	...
	УстановитьПривилегированныйРежим(Истина);
	Принадлежит = Ложь;
	ТекСсылкаМета = ТекСсылка.Метаданные(); 
	Если ТекПодсистема = "сабУправленческийУчет" Или ТекПодсистема = "модульУправленка" Тогда
		
		Если Метаданные.Подсистемы.модульУправленка.Подсистемы.Найти(ТекПодсистема).Состав.Содержит(ТекСсылкаМета) Тогда
			Возврат Истина;
		КонецЕсли;
		
		Для каждого ТекПодс Из Метаданные.Подсистемы.модульУправленка.Подсистемы.сабУправленческийУчет.Подсистемы Цикл
			Если ТекПодс.Состав.Содержит(ТекСсылкаМета) Тогда
				Возврат Истина;
			КонецЕсли;
			Для каждого ТекПодс2 Из ТекПодс.Подсистемы Цикл
				Если ТекПодс2.Состав.Содержит(ТекСсылкаМета) Тогда
					Возврат Истина;
				КонецЕсли;
				Для каждого ТекПодс3 Из ТекПодс2.Подсистемы Цикл
					Если ТекПодс3.Состав.Содержит(ТекСсылкаМета) Тогда
						Возврат Истина;
					КонецЕсли;
					Для каждого ТекПодс4 Из ТекПодс3.Подсистемы Цикл
						Если ТекПодс4.Состав.Содержит(ТекСсылкаМета) Тогда
							Возврат Истина;
						КонецЕсли;
						Для каждого ТекПодс5 Из ТекПодс4.Подсистемы Цикл
							Если ТекПодс5.Состав.Содержит(ТекСсылкаМета) Тогда
								Возврат Истина;
							КонецЕсли;
							Для каждого ТекПодс6 Из ТекПодс5.Подсистемы Цикл
								Если ТекПодс6.Состав.Содержит(ТекСсылкаМета) Тогда
									Возврат Истина;
								КонецЕсли;
							КонецЦикла;
						КонецЦикла;
	...
	Иначе	
		Возврат Метаданные.Подсистемы.модульУправленка.Подсистемы.Найти(ТекПодсистема).Состав.Содержит(ТекСсылкаМета);
	КонецЕсли;
...
Функция ПринадлежитПодсистемеОбъектМетаданных(ТекСсылкаМета,ТекПодсистема) Экспорт
	...
	Если ТекПодсистема = "сабУправленческийУчет" Или ТекПодсистема = "модульУправленка" Тогда
		...
		РодитПодсистема = ?(ТекПодсистема = "модульУправленка", Метаданные.Подсистемы, Метаданные.Подсистемы.модульУправленка.Подсистемы); 
		
		Если РодитПодсистема.Найти(ТекПодсистема).Состав.Содержит(ТекСсылкаМета) Тогда
			Возврат Истина;
		КонецЕсли;
...
Функция ПроверитьЛимитыПоЗаявкеНаОплату(Заявка) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	
	ЛимитОтсутствует = Ложь;
	РезультатДанные = Документы.Д_ЗаявкаНаОплату.РасчитатьЛимиты(Заявка);	
	ТаблицаЛимиты = РезультатДанные.Выгрузить();
	ТаблицаОтрицательныхОстатков = ТаблицаЛимиты.СкопироватьКолонки();
	
	Если Не ТаблицаЛимиты.Количество() Тогда
		ЛимитОтсутствует = Истина;
	КонецЕсли;
	
	Для Каждого ТекСтрокаЛимит Из ТаблицаЛимиты Цикл
		
		//Если ТекСтрокаЛимит.СтатьяДДС.ВидДеятельности = ВидДеятельностиВХП Тогда
		//	Продолжить;
		//КонецЕсли;
				
		Если Окр(ТекСтрокаЛимит.ОсталосьПоСтатье) < 0 Или Не ЗначениеЗаполнено(ТекСтрокаЛимит.Текущий) Тогда // если лимит отрицательный или его нет вообще (смотрим на текуищий план)
			НСтрокаОтр = ТаблицаОтрицательныхОстатков.Добавить();
			ЗаполнитьЗначенияСвойств(НСтрокаОтр, ТекСтрокаЛимит);
		КонецЕсли;
		
	КонецЦикла;

	Если ТаблицаОтрицательныхОстатков.Количество() Тогда
		ЛимитОтсутствует = истина;		
	КонецЕсли;
	
	УстановитьПривилегированныйРежим(Ложь);
	
	Возврат Новый Структура("ЛимитОтсутствует", ЛимитОтсутствует);
	
КонецФункции

Функция МассивВидовСубконто() Экспорт 
	
	МассивВидовСубконто = Новый Массив;
	
	ТекВидСубконто = БюджетныйНаСервере.ОбъектМетаданныхНайтиПоНаименованию("ПланВидовХарактеристик.ВидыСубконто", "Номенклатура", Истина);
	
	Если Не ТекВидСубконто = Неопределено Тогда
		МассивВидовСубконто.Добавить(ТекВидСубконто);
	КонецЕсли;    
		
	ТекВидСубконто = БюджетныйНаСервере.ОбъектМетаданныхНайтиПоНаименованию("ПланВидовХарактеристик.ВидыСубконто", "Склады", Истина);
...
Функция МассивВидовСубконтоМатериалы() Экспорт 
	
	МассивВидовСубконто = Новый Массив;
	 
	ТекВидСубконто = БюджетныйНаСервере.ОбъектМетаданныхНайтиПоНаименованию("ПланВидовХарактеристик.ВидыСубконто", "Материалы", Истина);
