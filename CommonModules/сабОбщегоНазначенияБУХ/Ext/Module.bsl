...
Процедура АнализЗаменыСтатьи91(НоваяСтрока)

	//анализ статей 91
	Если ТипЗнч(НоваяСтрока.КорСубконто1) = Тип("СправочникСсылка.ПрочиеДоходыИРасходы") Тогда
		РеквизитыСтатьи = БюджетныйНаСервере.ВернутьРеквизиты(НоваяСтрока.КорСубконто1, "СтатьяЗатрат, СчетЗатрат"); 
		Если ЗначениеЗаполнено(РеквизитыСтатьи.СтатьяЗатрат) Тогда
			НоваяСтрока.КорСчет = ?(ЗначениеЗаполнено(РеквизитыСтатьи.СчетЗатрат), РеквизитыСтатьи.СчетЗатрат, ПланыСчетов.Учетный.Счет9003());
			Если НоваяСтрока.КорСчет = ПланыСчетов.Учетный.Счет9003() ИЛИ НоваяСтрока.КорСчет = ПланыСчетов.Учетный.Счет20() Тогда
				НоваяСтрока.КорСубконто1 = Неопределено;
				НоваяСтрока.КорСубконто2 = РеквизитыСтатьи.СтатьяЗатрат;
			Иначе
				НоваяСтрока.КорСубконто1 = РеквизитыСтатьи.СтатьяЗатрат;
			КонецЕсли;
		КонецЕсли;
	КонецЕсли;
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.УЧ_Операция") Тогда
		...
		Пока Выборка.Следующий() Цикл
			...
			Если ТипЗнч(НоваяСтрока["Субконто1Дебет"]) = Тип("СправочникСсылка.Контрагенты") Тогда
				ОрганизацияКонтрагент = КонтрагентВОрганизациях(НоваяСтрока["Субконто1Дебет"]);	
				Если ОрганизацияКонтрагент.ЭтоОрганизация Тогда //ВНХ
					НоваяСтрока.СчетДебет = ПланыСчетов.Учетный.Счет7902();
			...
			Если ТипЗнч(НоваяСтрока["Субконто1Кредит"]) = Тип("СправочникСсылка.Контрагенты") Тогда
				ОрганизацияКонтрагент = КонтрагентВОрганизациях(НоваяСтрока["Субконто1Кредит"]);	
				Если ОрганизацияКонтрагент.ЭтоОрганизация Тогда //ВНХ
					НоваяСтрока.СчетКредит = ПланыСчетов.Учетный.Счет7902();
...
Процедура ЗаполнитьУЧ_НачислениеЗП(Источник, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка, ВыборкаСоотв, СоотвПодразделений)
	...
	ИначеЕсли ТипЗнч(Источник) = Тип("ДокументОбъект.УЧ_НачислениеЗП") И ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ОтражениеЗарплатыВБухучете") Тогда		
		...
		Для каждого ТекСтрока0 Из ДанныеЗаполнения.ФизическиеЛица Цикл
			...
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока0);
			НоваяСтрока.ВидНачисления = ВидНачисления;
			НоваяСтрока.Сотрудник = КадровыйУчет.ОсновнойСотрудникФизическогоЛица(НоваяСтрока.ФизическоеЛицо,ДанныеЗаполнения.Организация,ТекущаяДата());
			
			НайденныеСтроки = ДанныеЗаполнения.НачисленнаяЗарплатаИВзносы.НайтиСтроки(Новый Структура("ФизическоеЛицо", ТекСтрока0.ФизическоеЛицо));
			Для каждого ТекСтрока Из НайденныеСтроки Цикл
			...
			Для каждого ТекСтрока Из НайденныеСтроки Цикл
				НоваяСтрока.СуммаНДФЛ = НоваяСтрока.СуммаНДФЛ + ТекСтрока.Сумма;
			КонецЦикла;
