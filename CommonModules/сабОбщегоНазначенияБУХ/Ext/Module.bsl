...
Процедура ЗаполнитьУЧ_НачислениеЗП(Источник, ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка, ВыборкаСоотв, СоотвПодразделений)
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.УЧ_НачислениеЗП") И ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.НачислениеЗарплаты") Тогда
		...
		Для каждого ТекСтрока Из ДанныеЗаполнения.Начисления Цикл
			НоваяСтрока = Источник.ТабличнаяЧасть.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
			НоваяСтрока.ВидНачисления = ВидНачисления;
			НоваяСтрока.Сумма = ТекСтрока.Результат - ТекСтрока.Результат * 0.13;
			НоваяСтрока.ФизическоеЛицо = НоваяСтрока.Сотрудник.ФизическоеЛицо;
		КонецЦикла;
		
		Для каждого ТекСтрока Из ДанныеЗаполнения.Удержания Цикл
			...
			Для каждого ТекНайдСтрока Из НайденныеСтроки Цикл
				НоваяСтрока.Сотрудник = ТекНайдСтрока.Сотрудник;
				НоваяСтрока.ВидНачисления = ТекНайдСтрока.ВидНачисления;
			КонецЦикла;
			НоваяСтрока.Сумма = ТекСтрока.Результат;
		КонецЦикла;
		
		Для каждого ТекСтрока Из ДанныеЗаполнения.НДФЛ Цикл
			НайденныеСтроки = Источник.ТабличнаяЧасть.НайтиСтроки(Новый Структура("ФизическоеЛицо", ТекСтрока.ФизическоеЛицо));
			Для каждого ТекНайдСтрока Из НайденныеСтроки Цикл
				ТекНайдСтрока.СуммаНДФЛ = ТекСтрока.Налог + ТекСтрока.НалогСПревышения;			
			КонецЦикла;
		КонецЦикла;
		
		Для каждого ТекСтрока Из ДанныеЗаполнения.Взносы Цикл
			НайденныеСтроки = Источник.ТабличнаяЧасть.НайтиСтроки(Новый Структура("ФизическоеЛицо", ТекСтрока.ФизическоеЛицо));
			Для каждого ТекНайдСтрока Из НайденныеСтроки Цикл
				ТекНайдСтрока.СуммаФФОМС = ТекСтрока.ФФОМС;
				ТекНайдСтрока.СуммаФСС = ТекСтрока.ФСС;
				ТекНайдСтрока.СуммаПФР = ТекСтрока.ПФРДоПредельнойВеличины + ТекСтрока.ПФРСПревышения;
				ТекНайдСтрока.СуммаФССНесчСл = ТекСтрока.ФССНесчастныеСлучаи;
			КонецЦикла;
		КонецЦикла; 
