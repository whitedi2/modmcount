...
Функция ДатаБлокировкиИзмененияДокументов(Предприятие)  Экспорт
	...
	Запрос.Текст = 
		...
		|	И УЧ_ЗакрытиеПериода.Предприятие В(&Предприятие)
		|
		|УПОРЯДОЧИТЬ ПО
		|	Дата УБЫВ";
	...
	Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
		Возврат КонецДня(ВыборкаДетальныеЗаписи.Дата);
	КонецЦикла;
	Возврат Дата('00010101');
КонецФункции
...
Функция ПолучитьТаблицуОстатков(Объект, СтруктураИмен, СтруктураСоответствий, ДатаОстатков = Неопределено, ПолучатьВсеСерии = Ложь, НеОтрицОстатки = Ложь, ЭтоЗаказКлиента = Ложь) Экспорт
	...
	ПодразделениеНовое = ?(СтруктураИмен.Свойство("ПодразделениеНовое"), СтруктураИмен.ПодразделениеНовое, Подразделение);
	Товары = СтруктураИмен.Товары;
	Склад = СтруктураИмен.Склад;
	СкладНовый = ?(СтруктураИмен.Свойство("СкладНовый"), СтруктураИмен.СкладНовый, Склад);
	Номенклатура = СтруктураИмен.Номенклатура;
	Количество = СтруктураИмен.Количество;
	Сумма = ?(СтруктураИмен.Свойство("Сумма"), СтруктураИмен.Сумма, Количество);//заглушка
	Счет10 = СтруктураИмен.Счет10;
	СчетНовый = ?(СтруктураИмен.Свойство("СчетНовый"), СтруктураИмен.СчетНовый, Счет10);
	...
	|	ТЧДокумента.СкладНовый КАК СкладНовый
	...
	|	СУММА(ВТ_ТЧДокумента.Сумма) КАК Сумма,
	|	ВТ_ТЧДокумента.СкладНовый КАК СкладНовый
	|ПОМЕСТИТЬ ТЧДока
	|ИЗ
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ЕСТЬNULL(ЕСТЬNULL(УчетныйОстатки.Субконто3, УчетныйОстаткиИОбороты.Субконто3), ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))) <> ТИП(Справочник.СерииНоменклатуры)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ ЕСТЬNULL(ЕСТЬNULL(УчетныйОстатки.Субконто3, УчетныйОстаткиИОбороты.Субконто3), ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))
	...
	|							И &РассчитыватьСебестоимостьПоСериям
	|							И ТИПЗНАЧЕНИЯ(Субконто3) = ТИП(Справочник.СерииНоменклатуры)
	|							И НЕ &ПолучатьВсеСерии
	|						ТОГДА Субконто3 ЕСТЬ NULL
	|								ИЛИ Субконто3 В
	|									(ВЫБРАТЬ
	...
	|				ИЛИ УчетныйОстатки.Субконто3 ЕСТЬ NULL
	|				ИЛИ УчетныйОстаткиИОбороты.Субконто3 ЕСТЬ NULL)
	|			И УчетныйОстатки.Предприятия = УчетныйОстаткиИОбороты.Предприятия
	|			И (&ПолучатьВсеСерии
	|					И НЕ &НеОтрицОстатки
	|				ИЛИ ВЫБОР
	|					КОГДА &РассчитыватьСебестоимостьПоСериям
	|						ТОГДА УчетныйОстатки.КоличествоОстаток > 0
	|					ИНАЧЕ УчетныйОстатки.КоличествоОстаток > 0
	|							ИЛИ НЕ УчетныйОстатки.СуммаОстаток = 0
	|				КОНЕЦ
	|				ИЛИ ТИПЗНАЧЕНИЯ(УчетныйОстатки.Субконто1) = ТИП(Справочник.ОсновныеСредства)
	|				ИЛИ НЕ ТИПЗНАЧЕНИЯ(УчетныйОстатки.Субконто3) = ТИП(Справочник.СерииНоменклатуры))
	|ГДЕ
	|	НЕ ЕСТЬNULL(УчетныйОстаткиИОбороты.КоличествоНачальныйОстаток, 0) + ЕСТЬNULL(УчетныйОстаткиИОбороты.КоличествоОборотДт, 0) = 0
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ЕСТЬNULL(УчетныйОбороты.Субконто3, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))) <> ТИП(Справочник.СерииНоменклатуры)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ ЕСТЬNULL(УчетныйОбороты.Субконто3, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))
	|	КОНЕЦ КАК СерияНоменклатуры,
	|	УчетныйОбороты.КоличествоОборотДт КАК КоличествоОборотДт,
	|	УчетныйОбороты.КоличествоОборотКт КАК КоличествоОборотКт
	|ПОМЕСТИТЬ ОборотыГоризонтПроверки
	|ИЗ
	|	РегистрБухгалтерии.Учетный.Обороты(
	|			&ПозицияДокумента,
	|			&ДатаГоризонтаПроверки,
	|			Регистратор,
	|			Счет В ИЕРАРХИИ (&ВыбСчет),
	|			,
	|			Предприятия = &ВыбПредприятие
	...
	|				КОНЕЦ
	|				И ВЫБОР
	|					КОГДА &РасширенныйГоризонтПроверки = ИСТИНА
	|						ТОГДА ИСТИНА
	|					ИНАЧЕ ЛОЖЬ
	|				КОНЕЦ,
	|			,
	...
	|ГДЕ
	|	ВЫБОР
	|			КОГДА &ТекущийДокумент = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ УчетныйОбороты.Регистратор <> &ТекущийДокумент
	|		КОНЕЦ
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ОборотыГоризонтПроверки.Номенклатура КАК Номенклатура,
	|	ОборотыГоризонтПроверки.Склад КАК Склад,
	|	МИНИМУМ(ОборотыГоризонтПроверки.Счет) КАК Счет,
	|	ОборотыГоризонтПроверки.Предприятия КАК Предприятия,
	|	ОборотыГоризонтПроверки.Подразделение КАК Подразделение,
	|	ОборотыГоризонтПроверки.СерияНоменклатуры КАК СерияНоменклатуры,
	|	СУММА(ОборотыГоризонтПроверки.КоличествоОборотДт) КАК КоличествоОборотДт,
	|	СУММА(ОборотыГоризонтПроверки.КоличествоОборотКт) КАК КоличествоОборотКт,
	|	МИНИМУМ(ОборотыГоризонтПроверки.Счет.УчетПоПодразделениям) КАК СчетУчетПоПодразделениям
	|ПОМЕСТИТЬ ОборотыГоризонтПроверкиСвернуто
	|ИЗ
	|	ОборотыГоризонтПроверки КАК ОборотыГоризонтПроверки
	|
	|СГРУППИРОВАТЬ ПО
	|	ОборотыГоризонтПроверки.Номенклатура,
	|	ОборотыГоризонтПроверки.Склад,
	|	ОборотыГоризонтПроверки.Предприятия,
	|	ОборотыГоризонтПроверки.Подразделение,
	|	ОборотыГоризонтПроверки.СерияНоменклатуры
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ТЧДока.Номенклатура КАК Номенклатура,
	|	ЕСТЬNULL(ОстаткиСвернуто.КоличествоОстаток, 0) - ЕСТЬNULL(ТЧДока.Количество, 0) + ЕСТЬNULL(ОборотыГоризонтПроверкиСвернуто.КоличествоОборотДт, 0) - ЕСТЬNULL(ОборотыГоризонтПроверкиСвернуто.КоличествоОборотКт, 0) КАК КоличествоМинус,
	...
	|	ЕСТЬNULL(ОстаткиСвернуто.КоличествоОстаток, 0) + ЕСТЬNULL(ОборотыГоризонтПроверкиСвернуто.КоличествоОборотДт, 0) - ЕСТЬNULL(ОборотыГоризонтПроверкиСвернуто.КоличествоОборотКт, 0) КАК КоличествоОстаток,
	|	ТЧДока.СерияНоменклатуры КАК СерияНоменклатуры
	|ИЗ
	|	ТЧДока КАК ТЧДока
	...
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ОборотыГоризонтПроверкиСвернуто КАК ОборотыГоризонтПроверкиСвернуто
	|		ПО ТЧДока.Номенклатура = ОборотыГоризонтПроверкиСвернуто.Номенклатура
	|			И ТЧДока.Предприятие = ОборотыГоризонтПроверкиСвернуто.Предприятия
	|			И ТЧДока.Склад = ОборотыГоризонтПроверкиСвернуто.Склад
	|			И (ВЫБОР
	|				КОГДА ОборотыГоризонтПроверкиСвернуто.Подразделение ЕСТЬ NULL
	|						ИЛИ ТЧДока.УчетПоПодразделениям = ЛОЖЬ
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ТЧДока.Подразделение = ОборотыГоризонтПроверкиСвернуто.Подразделение
	|			КОНЕЦ)
	|			И (ВЫБОР
	|				КОГДА &ИспользоватьСерии
	|					ТОГДА ТЧДока.СерияНоменклатуры = ОборотыГоризонтПроверкиСвернуто.СерияНоменклатуры
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|ГДЕ
	|	ЕСТЬNULL(ОстаткиСвернуто.КоличествоОстаток, 0) - ЕСТЬNULL(ТЧДока.Количество, 0) + ЕСТЬNULL(ОборотыГоризонтПроверкиСвернуто.КоличествоОборотДт, 0) - ЕСТЬNULL(ОборотыГоризонтПроверкиСвернуто.КоличествоОборотКт, 0) < 0
	...
	|	ЕСТЬNULL(ОстаткиСвернуто.СуммаОстаток, 0) + ТЧДока.Сумма КАК СуммаКонечное,
	|	ТЧДока.СкладНовый КАК СкладПолучатель,
	...
	|			ТОГДА 0
	|		ИНАЧЕ ТЧДока.Сумма / ТЧДока.Количество
	...
	|		ПО ТЧДока.Номенклатура = ОстаткиСвернуто.Номенклатура
	|			И ТЧДока.Предприятие = ОстаткиСвернуто.Предприятия
	|			И ТЧДока.СкладНовый = ОстаткиСвернуто.Склад
	|			И (ВЫБОР
	|				КОГДА ОстаткиСвернуто.Подразделение ЕСТЬ NULL
	|						ИЛИ ТЧДока.УчетПоПодразделениям = ЛОЖЬ
	|					ТОГДА ИСТИНА
	...
	|	И (&РассчитыватьСебестоимостьПоСериям
	|			ИЛИ НЕ &ИспользоватьСерии)";
	
	Если ЭтоЗаказКлиента Тогда
		Запрос.Текст = СтрЗаменить(Запрос.Текст,
		"ГДЕ НЕ ЕСТЬNULL(УчетныйОстаткиИОбороты.КоличествоНачальныйОстаток, 0) + ЕСТЬNULL(УчетныйОстаткиИОбороты.КоличествоОборотДт, 0) = 0"," ");
	КонецЕсли;
	...
	ВыбСчет.Добавить(ПланыСчетов.Учетный.Счет01());
	...
	ЕстьТЧСерий = Не Объект.Метаданные().ТабличныеЧасти.Найти("СерииНоменклатуры") = Неопределено;
	ЕстьРеквКонтрагент = Не Объект.Метаданные().Реквизиты.Найти("Контрагент") = Неопределено;
	
	ВыбСклад = Новый Массив;
	
	МоментКонца = Объект.МоментВремени();

	Если Объект.Предприятие.УчетнаяПолитика.МетодикаКонтроляОтрицательныхОстатков = "Новая" Тогда	
		РасширенныйГоризонтПроверки = Истина;  
		ДатаГоризонтаПроверки = МоментКонца.Дата + 60*60*24*Объект.Предприятие.УчетнаяПолитика.ГоризонтКонтроляПоследовательности; 
	Иначе
		РасширенныйГоризонтПроверки = Ложь;  
		ДатаГоризонтаПроверки = МоментКонца.Дата;
	КонецЕсли;
	...
	ТЧДокумента.Колонки.Добавить("Товар", Новый ОписаниеТипов(МассивТипов));
	ТЧДокумента.Колонки.Добавить("Количество", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(15,3)));
	ТЧДокумента.Колонки.Добавить("Сумма", Новый ОписаниеТипов("Число",,,Новый КвалификаторыЧисла(17,2)));
	...
	ТЧДокумента.Колонки.Добавить("СкладНовый", Новый ОписаниеТипов(МассивТипов));
	...
	ТЧДокумента.Колонки.Добавить("СчетНовый", Новый ОписаниеТипов("ПланСчетовСсылка.Учетный"));
	...
	ОбъектТЧ = Объект[Товары].Выгрузить();
	НомерСтроки = 1;
	Для Каждого СтрокаТЧ Из ОбъектТЧ Цикл
		СтрокаТЧ.НомерСтроки = НомерСтроки;
		НомерСтроки = НомерСтроки + 1;
		Если СтрокаТЧ[Номенклатура] = Неопределено Или Не ТипЗнч(СтрокаТЧ[Номенклатура]) = Тип("СправочникСсылка.Номенклатура") Тогда
			Продолжить;		
		КонецЕсли; 
		СтрокаТЧДокумента = ТЧДокумента.Добавить();
		СтрокаТЧДокумента.Товар = СтрокаТЧ[Номенклатура];
		Если НЕ ОбъектТЧ.Колонки.Найти("СерияНоменклатуры") = Неопределено Тогда
			СтрокаТЧДокумента.СерияНоменклатуры = СтрокаТЧ["СерияНоменклатуры"];
		ИначеЕсли НЕ ОбъектТЧ.Колонки.Найти("Субконто3Кредит") = Неопределено Тогда
			СтрокаТЧДокумента.СерияНоменклатуры = СтрокаТЧ["Субконто3Кредит"];
		Иначе	
			СтрокаТЧДокумента.СерияНоменклатуры = ПустаяСерия;
		КонецЕсли;
		...
		СтрокаТЧДокумента.Счет10 = СтрокаТЧ[Счет10];
		...
		Если ЕстьРеквТЧСклад И (ЗначениеЗаполнено(СтрокаТЧ[Склад]) ИЛИ Не ЕстьРеквСклад) Тогда
			Если ВыбСклад.Найти(СтрокаТЧ[Склад]) = Неопределено Тогда
				ВыбСклад.Добавить(СтрокаТЧ[Склад]);
			КонецЕсли;
			СтрокаТЧДокумента.Склад = СтрокаТЧ[Склад]; 
		Иначе
			Если ТипЗнч(Объект) = Тип("ДокументОбъект.УЧ_ВыпускПродукции") И ЗначениеЗаполнено(Объект.СкладМатериалов) Тогда
				Если ВыбСклад.Найти(Объект.СкладМатериалов) = Неопределено Тогда
					ВыбСклад.Добавить(Объект.СкладМатериалов);
				КонецЕсли;
				СтрокаТЧДокумента.Склад = Объект.СкладМатериалов;
			Иначе	
				Если ВыбСклад.Найти(Объект[Склад]) = Неопределено Тогда
					ВыбСклад.Добавить(Объект[Склад]);
				КонецЕсли;
				СтрокаТЧДокумента.Склад = Объект[Склад];
			КонецЕсли;
		КонецЕсли;
		Если Склад = СкладНовый Тогда
			СтрокаТЧДокумента.СкладНовый = СтрокаТЧДокумента.Склад;	
		ИначеЕсли ТипЗнч(Объект) <> Тип("ДокументОбъект.УЧ_ПеремещениеТоваров") Тогда 
			СтрокаТЧДокумента.СкладНовый = Объект[СкладНовый];
			ВыбСклад.Добавить(СтрокаТЧДокумента.СкладНовый);
		КонецЕсли;
		
		ТекЦена = ?(СтрокаТЧДокумента.Количество, СтрокаТЧДокумента.Сумма / СтрокаТЧДокумента.Количество, 0);
		
		Если ЕстьТЧСерий Тогда
			НайдСтрокиСерий = Объект.СерииНоменклатуры.НайтиСтроки(Новый Структура("Номенклатура, НомерСтрокиРеализации", СтрокаТЧ[Номенклатура], СтрокаТЧ.НомерСтроки));
			Если НайдСтрокиСерий.Количество() > 1 Тогда
				ТекИндекс = 0;
				Для каждого ТекСтрокаСерий Из НайдСтрокиСерий Цикл
					Если ТекИндекс = 0 Тогда
						ЗаполнитьЗначенияСвойств(СтрокаТЧДокумента, ТекСтрокаСерий);
					Иначе
						СтрокаТЧДокумента2 = ТЧДокумента.Добавить();
						ЗаполнитьЗначенияСвойств(СтрокаТЧДокумента2, СтрокаТЧДокумента);
						ЗаполнитьЗначенияСвойств(СтрокаТЧДокумента, ТекСтрокаСерий);
					КонецЕсли;
					СтрокаТЧДокумента.Товар = СтрокаТЧ[Номенклатура];
					СтрокаТЧДокумента.Сумма = СтрокаТЧДокумента.Количество * ТекЦена;
					ТекИндекс = ТекИндекс + 1;
				КонецЦикла;
			КонецЕсли;
		КонецЕсли;
		
		Если НЕ Счет10 = СчетНовый Тогда
			Если ЗначениеЗаполнено(СтрокаТЧДокумента.СчетНовый) Тогда
				ВыбСчет.Добавить(СтрокаТЧДокумента.СчетНовый);
			КонецЕсли;
		КонецЕсли;
		
	КонецЦикла;
	
	Если ЕстьРеквКонтрагент Тогда //для 10.07 и 45-х счетов
		ВыбСклад.Добавить(Объект.Контрагент);
	КонецЕсли;
	
	ВыюПодразделение = Новый Массив;
	ВыюПодразделение.Добавить(Объект[Подразделение]);
	Если Не Подразделение = ПодразделениеНовое Тогда
		ВыюПодразделение.Добавить(Объект[ПодразделениеНовое]);
	КонецЕсли;
	
	Запрос.УстановитьПараметр("ВыбТовары", ВыбТовары);
	Запрос.УстановитьПараметр("ВыбПредприятие", Объект[Предприятие]);
	Запрос.УстановитьПараметр("Подразделение", ВыюПодразделение);
	...
	Запрос.УстановитьПараметр("ПолучатьВсеСерии",ПолучатьВсеСерии);
	Запрос.УстановитьПараметр("НеОтрицОстатки",НеОтрицОстатки);
	Запрос.УстановитьПараметр("ДатаНач", НачалоМесяца(?(ДатаОстатков = Неопределено, МоментКонца.Дата, ДатаОстатков)));
	Запрос.УстановитьПараметр("ДатаКон", КонецМесяца(?(ДатаОстатков = Неопределено, МоментКонца.Дата, ДатаОстатков)));
	Запрос.УстановитьПараметр("РассчитыватьСебестоимостьПоСериям", РассчитыватьСебестоимостьПоСериям);
	Запрос.УстановитьПараметр("РасширенныйГоризонтПроверки", РасширенныйГоризонтПроверки);
	Запрос.УстановитьПараметр("ДатаГоризонтаПроверки", ДатаГоризонтаПроверки); 
	Запрос.УстановитьПараметр("ТекущийДокумент", ?(ЗначениеЗаполнено(Объект.Ссылка), Объект.Ссылка, Неопределено));
	
	МассивЗапросов = Запрос.ВыполнитьПакетСПромежуточнымиДанными();
	
	РезультатОстатки = МассивЗапросов[2].Выгрузить();
	
	//очищаем остатки от строк с оборотами в случае если нет расчета стоимости по сериям
	//РезультатОстатки = МассивЗапросов[2].Выгрузить();
	МассивУдСтрок = Новый Массив;
	Если Не РассчитыватьСебестоимостьПоСериям Тогда
		НайденныеСтрокиБезОстатков = РезультатОстатки.НайтиСтроки(Новый Структура("КоличествоОстаток, СуммаОстаток", 0, 0));
		Для каждого ТекНайдСтрока Из НайденныеСтрокиБезОстатков Цикл
			НайденныеСтрокиВсе = РезультатОстатки.НайтиСтроки(Новый Структура("Номенклатура, Склад", ТекНайдСтрока.Номенклатура, ТекНайдСтрока.Склад));
			ЕстьОстатки = Ложь;
			Для каждого ТекНайдСтрока2 Из НайденныеСтрокиВсе Цикл
				Если ТекНайдСтрока2.КоличествоОстаток Тогда
					ЕстьОстатки = Истина;
				КонецЕсли;
			КонецЦикла;
			Если ЕстьОстатки Тогда
				МассивУдСтрок.Добавить(ТекНайдСтрока);			
			КонецЕсли;
		КонецЦикла;
		Для каждого ТекУд Из МассивУдСтрок Цикл
			РезультатОстатки.Удалить(ТекУд);	
		КонецЦикла;
	КонецЕсли;

	//Дт в операции добавляем в средние остатки
	ПустойСклад = Справочники.Склады.ПустаяСсылка();
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.УЧ_Операция") Тогда //считаем приход сразу в операции
		Для Каждого ТекСтрока Из Объект.Проводки Цикл
			Если ТипЗнч(ТекСтрока.Субконто1Дебет) = Тип("СправочникСсылка.Номенклатура") И Не ВыбСчет.Найти(ТекСтрока.СчетДебет) = Неопределено Тогда
				...
				Если ТекСтрока.Субконто1Дебет = ТекСтрока.Субконто1Кредит И ТекСтрока.Субконто2Дебет = ТекСтрока.Субконто2Кредит И ТекСтрока.Субконто3Дебет = ТекСтрока.Субконто3Кредит Тогда
					Продолжить;
				КонецЕсли;
				
				НоваяСтрока = РезультатОстатки.Добавить();
				НоваяСтрока.Номенклатура = ТекСтрока.Субконто1Дебет;
				НоваяСтрока.СерияНоменклатуры = ТекСтрока.Субконто3Дебет;
				...
				НоваяСтрока.Подразделение =  ?(СоответствиеУчетаПоПодразделениям.Получить(ТекСтрока.СчетДебет) = Неопределено ИЛИ Не СоответствиеУчетаПоПодразделениям.Получить(ТекСтрока.СчетДебет) = Истина, NULL, Объект[Подразделение]);
			КонецЕсли;
		КонецЦикла;
		РезультатОстатки.Свернуть("Номенклатура, Склад, Счет, Предприятия, Подразделение, СерияНоменклатуры", "Сумма, Количество, Цена");
		Для Каждого ТекСтрока Из РезультатОстатки Цикл
			ТекСтрока.Цена = ?(ТекСтрока.Количество, ТекСтрока.Сумма / ТекСтрока.Количество, 0); 
		КонецЦикла;
	КонецЕсли;	
		
	РезультатКонтроль = МассивЗапросов[6].Выгрузить();
	РезультатКонтрольМинусов  = МассивЗапросов[7].Выгрузить();
	...
	Если НастройкаБронированиеТоваров Тогда
		РезультатТабТЧТовары = МассивЗапросов[1].Выгрузить(); 
		РезультатТабГоризонтПроверки = МассивЗапросов[5].Выгрузить();
		Возврат Новый Структура("РезультатОстатки, РезультатКонтроль, РезультатКонтрольМинусов, ИспользоватьСерии, КорректироватьСебестоимостьПриВыходеОстаткаИзМинуса, РассчитыватьСебестоимостьПоСериям,ТЧДокумента,ОборотыГоризонтПроверки", РезультатОстатки, РезультатКонтроль, РезультатКонтрольМинусов, ИспользоватьСерии, КорректироватьСебестоимостьПриВыходеОстаткаИзМинуса, РассчитыватьСебестоимостьПоСериям,РезультатТабТЧТовары,РезультатТабГоризонтПроверки);
	Иначе
		Возврат Новый Структура("РезультатОстатки, РезультатКонтроль, РезультатКонтрольМинусов, ИспользоватьСерии, КорректироватьСебестоимостьПриВыходеОстаткаИзМинуса, РассчитыватьСебестоимостьПоСериям", РезультатОстатки, РезультатКонтроль, РезультатКонтрольМинусов, ИспользоватьСерии, КорректироватьСебестоимостьПриВыходеОстаткаИзМинуса, РассчитыватьСебестоимостьПоСериям);
	КонецЕсли;

КонецФункции // ()

Функция ОстаткиСУчетомДвижений(Объект, СтруктураИмен, СтруктураСоответствий, ДатаОстатков = Неопределено, ПолучатьВсеСерии = Ложь, НеОтрицОстатки = Ложь, ЭтоЗаказКлиента = Ложь) Экспорт
	...
	|	УчетныйОстатки.Подразделение КАК Подразделение,
	|	ВЫБОР
	|		КОГДА ТИПЗНАЧЕНИЯ(ЕСТЬNULL(УчетныйОстатки.Субконто3, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))) <> ТИП(Справочник.СерииНоменклатуры)
	|			ТОГДА ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|		ИНАЧЕ ЕСТЬNULL(УчетныйОстатки.Субконто3, ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка))
	|	КОНЕЦ КАК СерияНоменклатуры,
	|	ЕСТЬNULL(УчетныйОстатки.КоличествоОстаток, 0) КАК КоличествоОстаток,
	|	ЕСТЬNULL(УчетныйОстатки.СуммаОстаток, 0) КАК СуммаОстаток
	|ПОМЕСТИТЬ Остатки
	...
	Возврат Новый Структура("РезультатОстатки, РезультатКонтроль", РезультатОстатки, РезультатКонтроль);
...
Функция СформироватьСтруктуруИмен(Объект) Экспорт
	...
	Если ТипЗнч(Объект) = Тип("ДокументОбъект.УЧ_СписаниеТоваров") Тогда
		...
		СтруктураИмен.Вставить("ПодразделениеНовое", "Подразделение");
		СтруктураИмен.Вставить("Склад", "Склад");
		СтруктураИмен.Вставить("СкладНовый", "Склад");
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.УЧ_ПереводТМЦ") Тогда
		СтруктураИмен.Вставить("Предприятие", "Предприятие");
		СтруктураИмен.Вставить("Подразделение", "Подразделение");
		СтруктураИмен.Вставить("ПодразделениеНовое", "ПодразделениеПолучатель");
		СтруктураИмен.Вставить("Склад", "Склад");
		СтруктураИмен.Вставить("СкладНовый", "СкладПолучатель");
		СтруктураИмен.Вставить("Номенклатура", "Номенклатура");
	    СтруктураИмен.Вставить("Количество", "Количество");
	    СтруктураИмен.Вставить("Товары", "ТабличнаяЧасть");
	...
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.УЧ_ПеремещениеТоваров") Тогда
		...
		СтруктураИмен.Вставить("Товары", "ТабличнаяЧасть");
		СтруктураИмен.Вставить("Счет10", "СчетУчетаБУ");
		СтруктураИмен.Вставить("СчетНовый", "НовыйСчетУчетаБУ");
	ИначеЕсли ТипЗнч(Объект) = Тип("ДокументОбъект.УЧ_ПоступлениеТоваров") Тогда
		СтруктураИмен.Вставить("Предприятие", "Предприятие");
		СтруктураИмен.Вставить("Подразделение", "Подразделение");
		СтруктураИмен.Вставить("ПодразделениеНовое", "Подразделение");
		СтруктураИмен.Вставить("Склад", "Склад");
		...
		СтруктураИмен.Вставить("Номенклатура", "Номенклатура");
	    СтруктураИмен.Вставить("Количество", "Количество");
		СтруктураИмен.Вставить("Сумма", "Сумма");
	    СтруктураИмен.Вставить("Товары", "ТабличнаяЧасть");
		СтруктураИмен.Вставить("Счет10", "СчетУчета");
		СтруктураИмен.Вставить("СчетНовый", "СчетУчета");
	КонецЕсли;
...
Функция ПоучитьСоответствияСчетовНоменклатуры(Объект, СтруктураИмен, СчетПоУмолчанию = Неопределено) Экспорт
	...
	//добавляем соотв по всем колич счетам
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	...
	|ГДЕ
	|	Учетный.Количественный = ИСТИНА";
	
	Результат = Запрос.Выполнить();
	ВыборкаВсе = Результат.Выбрать();
	
	Пока ВыборкаВсе.Следующий() Цикл
		СоответствияУчетаПодразделений.Вставить(ВыборкаВсе.Счет, ВыборкаВсе.УчетПоПодразделениям);
	КонецЦикла;
	...
	Для каждого ТекСтрока Из Объект[Товары] Цикл
		Если ЗначениеЗаполнено(ТекСтрока[Номенклатура]) И Не ЗначениеЗаполнено(ТекСтрока[Счет10]) И МассивМатериаловБезСчета.Найти(ТекСтрока[Номенклатура]) = Неопределено Тогда
...
Процедура КонтрольОстатковСУчетомБронирования(Объект, СтруктураИмен, СтруктураСоответствий,МассивСообщенийКонтроля,ЗаказКлиента = Неопределено) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	СтруктураТаблицаОстатков = ПолучитьТаблицуОстатков(Объект, СтруктураИмен, СтруктураСоответствий,,Истина); 
	Если ЗначениеЗаполнено(ЗаказКлиента) И ТипЗнч(ЗаказКлиента) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
    	ЗаказОснование = ЗаказКлиента;
	Иначе 
		ЗаказОснование = ПолучитьЗаказКлиентаДляДокументаУУ(Объект.Ссылка);
	КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	|	ВТТовары.Предприятие КАК Предприятие,
	|	ВТТовары.Склад КАК Склад,
	|	ВТТовары.Подразделение КАК Подразделение,
	|	ВТТовары.УчетПоПодразделениям КАК УчетПоПодразделениям,
	|	ВТТовары.ПредприятиеНеКонтролировать10 КАК ПредприятиеНеКонтролировать10
	|ПОМЕСТИТЬ ВТТоварыСвернуто
	|ИЗ
	...
	|	ВТТовары.ПредприятиеНеКонтролировать10,
	|	ВТТовары.УчетПоПодразделениям,
	|	ВТТовары.Склад,
	|	ВТТовары.Предприятие
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТТоварыСвернуто.Номенклатура КАК Номенклатура,
	|	МАКСИМУМ(ВТТоварыСвернуто.Количество) КАК Количество,
	|	ВТТоварыСвернуто.Предприятие КАК Предприятие,
	|	ВТТоварыСвернуто.Склад КАК Склад,
	|	ВТТоварыСвернуто.Подразделение КАК Подразделение,
	|	ВТТоварыСвернуто.УчетПоПодразделениям КАК УчетПоПодразделениям,
	|	ВТТоварыСвернуто.ПредприятиеНеКонтролировать10 КАК ПредприятиеНеКонтролировать10,
	|	СУММА(БронированиеТоваров.Количество) КАК КоличествоЗабронировано
	|ПОМЕСТИТЬ ВТТоварыСБронированием
	|ИЗ
	|	ВТТоварыСвернуто КАК ВТТоварыСвернуто
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрСведений.БронированиеТоваров КАК БронированиеТоваров
	|		ПО ВТТоварыСвернуто.Номенклатура = БронированиеТоваров.Номенклатура
	|			И ВТТоварыСвернуто.Склад = БронированиеТоваров.Склад
	|			И ВТТоварыСвернуто.Предприятие = БронированиеТоваров.Предприятие
	|			И (БронированиеТоваров.Документ <> &Заказ)
	|			И (БронированиеТоваров.Активность)
	|			И (ВЫБОР
	|				КОГДА ВТТоварыСвернуто.УчетПоПодразделениям
	|					ТОГДА ВТТоварыСвернуто.Подразделение = БронированиеТоваров.Подразделение
	|				ИНАЧЕ ИСТИНА
	|			КОНЕЦ)
	|			И (НЕ ВТТоварыСвернуто.ПредприятиеНеКонтролировать10)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТТоварыСвернуто.Номенклатура,
	|	ВТТоварыСвернуто.Предприятие,
	|	ВТТоварыСвернуто.Склад,
	|	ВТТоварыСвернуто.Подразделение,
	|	ВТТоварыСвернуто.УчетПоПодразделениям,
	|	ВТТоварыСвернуто.ПредприятиеНеКонтролировать10
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	|	ОборотыГоризонтПроверки.Предприятия КАК Предприятия,
	|	ОборотыГоризонтПроверки.Подразделение КАК Подразделение,
	|	ОборотыГоризонтПроверки.КоличествоОборотДт - ОборотыГоризонтПроверки.КоличествоОборотКт КАК ОстатокГоризонтПроверки,
	|	ОборотыГоризонтПроверки.Счет КАК Счет
	|ПОМЕСТИТЬ ВТОборотыГоризонтПроверки
	|ИЗ
	|	&ОборотыГоризонтПроверки КАК ОборотыГоризонтПроверки
	|;
	|
	...
	|	ВТОстатки.Предприятия КАК Предприятия,
	|	ВТОстатки.Подразделение КАК Подразделение,
	|	МИНИМУМ(ВТОстатки.Счет.УчетПоПодразделениям) КАК СчетУчетПоПодразделениям,
	|	СУММА(ВТОстатки.КоличествоОстаток) КАК КоличествоОстаток
	|ПОМЕСТИТЬ ВТОстаткиСвернуто
	|ИЗ
	...
	|	ВТОстатки.Склад,
	|	ВТОстатки.Предприятия,
	|	ВТОстатки.Подразделение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТОборотыГоризонтПроверки.Номенклатура КАК Номенклатура,
	|	ВТОборотыГоризонтПроверки.Склад КАК Склад,
	|	ВТОборотыГоризонтПроверки.Предприятия КАК Предприятия,
	|	ВТОборотыГоризонтПроверки.Подразделение КАК Подразделение,
	|	МИНИМУМ(ВТОборотыГоризонтПроверки.Счет.УчетПоПодразделениям) КАК СчетУчетПоПодразделениям,
	|	СУММА(ВТОборотыГоризонтПроверки.ОстатокГоризонтПроверки) КАК ОстатокГоризонтПроверки
	|ПОМЕСТИТЬ ВТОборотыГоризонтПроверкиСвернуто
	|ИЗ
	|	ВТОборотыГоризонтПроверки КАК ВТОборотыГоризонтПроверки
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТОборотыГоризонтПроверки.Номенклатура,
	|	ВТОборотыГоризонтПроверки.Склад,
	|	ВТОборотыГоризонтПроверки.Предприятия,
	|	ВТОборотыГоризонтПроверки.Подразделение
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	|	ВТТоварыСБронированием.Номенклатура КАК Номенклатура,
	|	МАКСИМУМ(ВТТоварыСБронированием.Количество) КАК КоличествоВДокументе,
	|	ВТТоварыСБронированием.Предприятие КАК Предприятие,
	|	ВТТоварыСБронированием.Склад КАК Склад,
	|	ВТТоварыСБронированием.Подразделение КАК Подразделение,
	|	ВТТоварыСБронированием.УчетПоПодразделениям КАК УчетПоПодразделениям,
	|	ВТТоварыСБронированием.ПредприятиеНеКонтролировать10 КАК ПредприятиеНеКонтролировать10,
	|	МАКСИМУМ(ВТТоварыСБронированием.КоличествоЗабронировано) КАК КоличествоЗабронировано,
	|	СУММА(ЕСТЬNULL(ВТОстаткиСвернуто.КоличествоОстаток, 0) + ЕСТЬNULL(ВТОборотыГоризонтПроверкиСвернуто.ОстатокГоризонтПроверки, 0)) КАК КоличетсвоОстаток
	|ИЗ
	|	ВТТоварыСБронированием КАК ВТТоварыСБронированием
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОстаткиСвернуто КАК ВТОстаткиСвернуто
	|		ПО ВТТоварыСБронированием.Номенклатура = ВТОстаткиСвернуто.Номенклатура
	|			И ВТТоварыСБронированием.Склад = ВТОстаткиСвернуто.Склад
	|			И ВТТоварыСБронированием.Предприятие = ВТОстаткиСвернуто.Предприятия
	|			И (ВЫБОР
	|				КОГДА ВТОстаткиСвернуто.Подразделение ЕСТЬ NULL
	|						ИЛИ НЕ ВТТоварыСБронированием.УчетПоПодразделениям
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ВТТоварыСБронированием.Подразделение = ВТОстаткиСвернуто.Подразделение
	|			КОНЕЦ)
	|		ЛЕВОЕ СОЕДИНЕНИЕ ВТОборотыГоризонтПроверкиСвернуто КАК ВТОборотыГоризонтПроверкиСвернуто
	|		ПО ВТТоварыСБронированием.Номенклатура = ВТОборотыГоризонтПроверкиСвернуто.Номенклатура
	|			И ВТТоварыСБронированием.Склад = ВТОборотыГоризонтПроверкиСвернуто.Склад
	|			И ВТТоварыСБронированием.Предприятие = ВТОборотыГоризонтПроверкиСвернуто.Предприятия
	|			И (ВЫБОР
	|				КОГДА ВТОборотыГоризонтПроверкиСвернуто.Подразделение ЕСТЬ NULL
	|						ИЛИ НЕ ВТТоварыСБронированием.УчетПоПодразделениям
	|					ТОГДА ИСТИНА
	|				ИНАЧЕ ВТТоварыСБронированием.Подразделение = ВТОборотыГоризонтПроверкиСвернуто.Подразделение
	|			КОНЕЦ)
	|
	|СГРУППИРОВАТЬ ПО
	|	ВТТоварыСБронированием.Номенклатура,
	|	ВТТоварыСБронированием.Склад,
	|	ВТТоварыСБронированием.УчетПоПодразделениям,
	|	ВТТоварыСБронированием.Предприятие,
	|	ВТТоварыСБронированием.ПредприятиеНеКонтролировать10,
	|	ВТТоварыСБронированием.Подразделение";
	
	//ЕстьЗаказОснование = ЗначениеЗаполнено(ЗаказОснование) И ТипЗнч(ЗаказОснование) = Тип("ДокументСсылка.ЗаказКлиента");
	//Если ЕстьЗаказОснование Тогда
	Запрос.УстановитьПараметр("Заказ", ЗаказОснование);
	//Иначе
	//	Запрос.УстановитьПараметр("Заказ", Документы.ЗаказКлиента.ПустаяСсылка());
	//КонецЕсли;  
	Запрос.УстановитьПараметр("ВТ_ТЧДокумента",СтруктураТаблицаОстатков.ТЧДокумента); 
	Запрос.УстановитьПараметр("Остатки",СтруктураТаблицаОстатков.РезультатОстатки);
	Запрос.УстановитьПараметр("ОборотыГоризонтПроверки",СтруктураТаблицаОстатков.ОборотыГоризонтПроверки);
	РезультатЗапроса = Запрос.Выполнить();
	Выборка = РезультатЗапроса.Выбрать();
	Пока Выборка.Следующий() Цикл
		Нехватка = Выборка.КоличетсвоОстаток - Выборка.КоличествоЗабронировано - Выборка.КоличествоВДокументе;
		Если Нехватка < 0 Тогда
			МассивСообщенийКонтроля.Добавить("Сводный контроль с учетом бронирования в заказах: не достаточно товара " + Выборка.Номенклатура + " на складе " + Выборка.Склад + 
			". Количество в документе: " + Выборка.КоличествоВДокументе + ", остаток: " + Выборка.КоличетсвоОстаток +
			", забронировано: " + Выборка.КоличествоЗабронировано + ". Нехватка: " + (-Нехватка));
		КонецЕсли;
	КонецЦикла;
	УстановитьПривилегированныйРежим(Ложь);
...
Процедура ЗачетБронированиеДляЗаказаОснования(ДокументРеализация, ЗаказКлиента = Неопределено, ОтменаПроведения = Ложь) Экспорт
	
	УстановитьПривилегированныйРежим(Истина);
	Если ЗначениеЗаполнено(ЗаказКлиента) И ТипЗнч(ЗаказКлиента) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		ЗаказКлиента.ПолучитьОбъект().ЗаполнитьДвиженияБронированиеТоваров(ДокументРеализация,ОтменаПроведения);	
	Иначе
		Заказ = ПолучитьЗаказКлиентаДляДокументаУУ(ДокументРеализация);
		Если ЗначениеЗаполнено(Заказ) Тогда
			Заказ.ПолучитьОбъект().ЗаполнитьДвиженияБронированиеТоваров(ДокументРеализация,ОтменаПроведения);
		КонецЕсли;
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
...
//Возвращает заказ клиента для Документа УУ:
//Если в реквизите ДокОснование документа УУ заказ клиента, тогда возвращает значение реквизита ДокОснование.
//Если в реквизите ДокОснование нет заказа клиента, тогда происходит поиск заказа в связаном документе бух.
//Если заказ не найден, возвращается пустая ссылка на документ Заказ клиена.
Функция ПолучитьЗаказКлиентаДляДокументаУУ(ДокументРеализация) Экспорт

	УстановитьПривилегированныйРежим(Истина);
	ЕстьЗаказОснование = Ложь;
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("ДокОснование", ДокументРеализация.Метаданные()) Тогда
		Если ЗначениеЗаполнено(ДокументРеализация.ДокОснование) И ТипЗнч(ДокументРеализация.ДокОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
			Возврат ДокументРеализация.ДокОснование;
		КонецЕсли;
	КонецЕсли;
	НаборЗаписей = РегистрыСведений.сабОбработкаДокументов.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.ДокументУУ.Установить(ДокументРеализация);
	НаборЗаписей.Прочитать();
	Для каждого ТекЗапись Из НаборЗаписей Цикл
		Если ЗначениеЗаполнено(ТекЗапись.ДокументБУ) Тогда
			Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Заказ", ТекЗапись.ДокументБУ.Метаданные()) Тогда
				Если ЗначениеЗаполнено(ТекЗапись.ДокументБУ.Заказ) И ТипЗнч(ТекЗапись.ДокументБУ.Заказ) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
					Возврат ТекЗапись.ДокументБУ.Заказ;
					Прервать;
				Иначе
					Если ОбщегоНазначения.ЕстьРеквизитОбъекта("СчетНаОплатуПокупателю", ТекЗапись.ДокументБУ.Метаданные()) 
						И ЗначениеЗаполнено(ТекЗапись.ДокументБУ.СчетНаОплатуПокупателю) Тогда 	  
						Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Заказ", ТекЗапись.ДокументБУ.СчетНаОплатуПокупателю.Метаданные())
							И ЗначениеЗаполнено(ТекЗапись.ДокументБУ.СчетНаОплатуПокупателю.Заказ) Тогда 
							Возврат ТекЗапись.ДокументБУ.СчетНаОплатуПокупателю.Заказ;
							Прервать;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли;
...
Функция ПодобратьВНХДвижение(ДокСсылка, ПриходРасход, ОбратноеВНХДвижение) Экспорт
	
	Если ТипЗнч(ДокСсылка) = Тип("Структура") И ДокСсылка.Свойство("ЭтоВНХ") Тогда
		ЭтоВНХ = ДокСсылка.ЭтоВНХ; 
	Иначе	
		ЭтоВНХ = ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ОплатаВНХ ИЛИ ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ПоступлениеВНХ ИЛИ ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.СнятиеНаличных;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(ОбратноеВНХДвижение) Или Не ЭтоВНХ Тогда
		Возврат ОбратноеВНХДвижение;	
	КонецЕсли;
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ПО (ВЫБОР
	               |				КОГДА &ИщемРасход
	               |					ТОГДА УЧ_ДвижениеДС.Ссылка = сабСоответствиеОбъектовИнформационнойБазы.Объект1
	               |				ИНАЧЕ УЧ_ДвижениеДС.Ссылка = сабСоответствиеОбъектовИнформационнойБазы.Объект2
	               |			КОНЕЦ)
	               |ГДЕ
	               |	сабСоответствиеОбъектовИнформационнойБазы.Объект1 ЕСТЬ NULL
	               |	И УЧ_ДвижениеДС.СуммаДокумента = &СуммаДокумента
	               |	И НАЧАЛОПЕРИОДА(УЧ_ДвижениеДС.Дата, ДЕНЬ) = НАЧАЛОПЕРИОДА(&Дата, ДЕНЬ)
	               |	И ВЫБОР
	               |			КОГДА &ИщемРасход
	               |				ТОГДА ВЫРАЗИТЬ(УЧ_ДвижениеДС.Контрагент КАК Справочник.Контрагенты).ИНН = ВЫРАЗИТЬ(&Организация КАК Справочник.Организации).ИНН
	               |			ИНАЧЕ ВЫРАЗИТЬ(УЧ_ДвижениеДС.Организация КАК Справочник.Организации).ИНН = &КонтрагентИНН
	               |		КОНЕЦ";
	
	Запрос.УстановитьПараметр("ТекДокумент", ДокСсылка.Ссылка);
	Запрос.УстановитьПараметр("Организация", ДокСсылка.Организация);
	Запрос.УстановитьПараметр("КонтрагентИНН", ?(ТипЗнч(ДокСсылка.Контрагент) = Тип("СправочникСсылка.Контрагенты"), ДокСсылка.Контрагент.ИНН, ?(ТипЗнч(ДокСсылка.Контрагент) = Тип("СправочникСсылка.Организации"), ДокСсылка.Контрагент.ИНН, "")));
	Запрос.УстановитьПараметр("СуммаДокумента", ДокСсылка.СуммаДокумента);
	Запрос.УстановитьПараметр("Дата", ДокСсылка.Дата);
	Запрос.УстановитьПараметр("ПриходРасход", ?(ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ОплатаВНХ, Перечисления.ВидыОперацийПлатежноеПоручение.ПоступлениеВНХ, Перечисления.ВидыОперацийПлатежноеПоручение.ОплатаВНХ));
	Запрос.УстановитьПараметр("ИщемРасход", ?(ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ОплатаВНХ, Ложь, Истина));
	
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выбрать();
	
	Пока Выборка.Следующий() Цикл
		НоваяЗаписьРегистра = РегистрыСведений.сабСоответствиеОбъектовИнформационнойБазы.СоздатьМенеджерЗаписи();
		Если ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ОплатаВНХ И ЗначениеЗаполнено(ДокСсылка.Ссылка) Тогда
			НоваяЗапись = РегистрыСведений.сабСоответствиеОбъектовИнформационнойБазы.СоздатьНаборЗаписей();
			НоваяЗапись.Отбор.Объект1.Установить(ДокСсылка.Ссылка);
			НоваяЗапись.Записать();
			НоваяЗапись = РегистрыСведений.сабСоответствиеОбъектовИнформационнойБазы.СоздатьМенеджерЗаписи();
			НоваяЗапись.Объект1 = ДокСсылка.Ссылка;
			НоваяЗапись.Объект2 = Выборка.Ссылка;
			НоваяЗапись.Записать();
		ИначеЕсли ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ПоступлениеВНХ И ЗначениеЗаполнено(ДокСсылка.Ссылка) Тогда
			//УдалитьСоответствияВНХПоступления(ДокСсылка);
			НоваяЗапись = РегистрыСведений.сабСоответствиеОбъектовИнформационнойБазы.СоздатьНаборЗаписей();
			НоваяЗапись.Отбор.Объект1.Установить(Выборка.Ссылка);
			НоваяЗапись.Записать();
			НоваяЗапись = РегистрыСведений.сабСоответствиеОбъектовИнформационнойБазы.СоздатьМенеджерЗаписи();
			НоваяЗапись.Объект1 = Выборка.Ссылка;
			НоваяЗапись.Объект2 = ДокСсылка.Ссылка;
			НоваяЗапись.Записать();
		КонецЕсли;
