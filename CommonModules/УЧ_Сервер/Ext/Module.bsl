...
Функция СредневзвешенныйКурсУчет(Дата, Источник, Валюта) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РегистрБухгалтерии.Учетный.Остатки(
	               |			&Дата1,
	               |			Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Учетный.Касса))
	               |				ИЛИ Счет В ИЕРАРХИИ (ЗНАЧЕНИЕ(ПланСчетов.Учетный.РасчетныйСчет)),
	               |			,
	               |			Валюта = &Валюта
...
Функция ВернутьОтсрочкуПоДоговору(Договор, Дата) Экспорт
	 Запрос = Новый Запрос;
	 Запрос.Текст = "ВЫБРАТЬ
	 |	ОтсрочкаПоДоговорамСрезПоследних.Договор,
	 |	ОтсрочкаПоДоговорамСрезПоследних.Отсрочка,
	 |	ОтсрочкаПоДоговорамСрезПоследних.Предоплата
	 |ИЗ
	 |	РегистрСведений.ОтсрочкаПоДоговорам.СрезПоследних(
	 |			,
	 |			Договор = &Договор
	 |				И Период = &Период) КАК ОтсрочкаПоДоговорамСрезПоследних";
	 ...
	 Пока Выборка.Следующий() Цикл
		 Возврат Новый Структура("Договор, Отсрочка, Предоплата", Выборка.Договор, Выборка.Отсрочка, Выборка.Предоплата);	
	 КонецЦикла;
	 Возврат Новый Структура("Договор, Отсрочка, Предоплата", Договор, 0, 0);	
...
Функция ПолучитьКурсВалют(Валюта, знач ЭквивалентнаяВалюта = Неопределено, знач Дата = Неопределено, СообщатьОбОтсутвииКурса = Ложь) Экспорт
	...
	Если НайденныеСтроки.Количество() <> 0 Тогда		
		...
		Если НЕ НайденныеСтроки[0].Курс = 1 И (НачалоДня(НайденныеСтроки[0].Период) <> НачалоДня(Дата)) и (ДНКурса <=5 или (ДНКурса=6 и НачалоДня(НайденныеСтроки[0].Период)<НачалоДня(Дата-86400)) или (ДНКурса=7 и НачалоДня(НайденныеСтроки[0].Период)<НачалоДня(Дата-2*86400))) Тогда
			Сообщить("Для пары " + Валюта + "-" + ЭквивалентнаяВалюта + " не загружен актуальный курс на " + Формат(Дата,"ДФЛ=D") + "."); 
		КонецЕсли;
...
Функция ПолучитьТаблицуОстатков(Объект, СтруктураИмен, СтруктураСоответствий) Экспорт
	...
	Если НЕ Объект.Метаданные().ТабличныеЧасти[Товары].Реквизиты.Найти(Склад) = Неопределено Тогда
		ВыбСклад = Новый Массив;
	Иначе
		ВыбСклад = Объект[Склад]; //исправить, т.к. может быть в ТЧ
	КонецЕсли;
	...
	Для Каждого СтрокаТЧ Из Объект[Товары] Цикл
		...
		Если ТипЗнч(ВыбСклад) = Тип("Массив") Тогда
