...
Функция ПолучитьТаблицуОстатков(Объект, СтруктураИмен, СтруктураСоответствий, ДатаОстатков = Неопределено, ПолучатьВсеСерии = Ложь) Экспорт
	...
	|ВЫБРАТЬ
	|	ТЧДока.Номенклатура КАК Номенклатура,
	|	ЕСТЬNULL(ОстаткиСвернуто.Количество, 0) + ТЧДока.Количество КАК КоличествоКонечное,
	|	ТЧДока.Предприятие КАК Предприятие,
	|	ТЧДока.Подразделение КАК Подразделение,
	|	ОстаткиСвернуто.Склад КАК Склад,
	|	ТЧДока.Количество КАК КоличествоНужно,
	|	ЕСТЬNULL(ОстаткиСвернуто.Количество, 0) КАК КоличествоОстаток,
	|	ЕСТЬNULL(ОстаткиСвернуто.Сумма, 0) КАК СуммаКонечное,
	...
	|			КОНЕЦ)
	|ГДЕ
	|	ЕСТЬNULL(ОстаткиСвернуто.Количество, 0) < 0";
