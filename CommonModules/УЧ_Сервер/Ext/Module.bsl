...
Функция ПолучитьТаблицуОстатков(Объект, СтруктураИмен, СтруктураСоответствий, ДатаОстатков = Неопределено, ПолучатьВсеСерии = Ложь, НеОтрицОстатки = Ложь) Экспорт
	...
	|			И (&ПолучатьВсеСерии
	|					И НЕ &НеОтрицОстатки
	|				ИЛИ УчетныйОстатки.КоличествоОстаток > 0
	...
	Для Каждого СтрокаТЧ Из ОбъектТЧ Цикл
		Если СтрокаТЧ[Номенклатура] = Неопределено Или Не ТипЗнч(СтрокаТЧ[Номенклатура]) = Тип("СправочникСсылка.Номенклатура") Тогда
			Продолжить;		
		КонецЕсли; 
		//Доработки для нескольких серий+++ 
		ДляСтрокиНесколькоСерий = Ложь;
		Если НЕ ОбъектТЧ.Колонки.Найти("СерияНоменклатуры") = Неопределено Тогда
			Если ИспользоватьСерии И Не ПолучатьВсеСерии И Не ЗначениеЗаполнено(СтрокаТЧ["СерияНоменклатуры"]) Тогда
				Если ОбщегоНазначенияБП.ЕстьТабЧастьДокумента("СерииНоменклатуры",Объект.Метаданные()) Тогда
					Если ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("СерияНоменклатуры",Объект.Метаданные(),"СерииНоменклатуры") И
						ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("Номенклатура",Объект.Метаданные(),"СерииНоменклатуры") И
						ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("НомерСтрокиРеализации",Объект.Метаданные(),"СерииНоменклатуры") И 
						ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("Количество",Объект.Метаданные(),"СерииНоменклатуры") Тогда 
						СтруктураПоискаСерий = Новый Структура;
						СтруктураПоискаСерий.Вставить("Номенклатура",СтрокаТЧ[Номенклатура]);
						СтруктураПоискаСерий.Вставить("НомерСтрокиРеализации",СтрокаТЧ.НомерСтроки);
						МассивНайденныхСерий = Объект.СерииНоменклатуры.НайтиСтроки(СтруктураПоискаСерий); 
						Если МассивНайденныхСерий.Количество() > 1 Тогда
							ДляСтрокиНесколькоСерий = Истина;
							Для Каждого СтрокаТЧСерии Из МассивНайденныхСерий Цикл
								СтрокаТЧДокумента = ТЧДокумента.Добавить();
								СтрокаТЧДокумента.Товар = СтрокаТЧ[Номенклатура];
								СтрокаТЧДокумента.СерияНоменклатуры = СтрокаТЧСерии["СерияНоменклатуры"];
								СтрокаТЧДокумента.Количество = СтрокаТЧСерии.Количество;
			...
			КонецЕсли;
		КонецЕсли;
		Если Не ДляСтрокиНесколькоСерий Тогда 
		//Доработки для нескольких серий---
			СтрокаТЧДокумента = ТЧДокумента.Добавить();
			СтрокаТЧДокумента.Товар = СтрокаТЧ[Номенклатура];
			Если НЕ ОбъектТЧ.Колонки.Найти("СерияНоменклатуры") = Неопределено Тогда
