...
Функция ПолучитьТипКонтента(ИмяФайлаДляЗагрузки)
	Расширение = ОбщегоНазначенияКлиентСервер.ПолучитьРасширениеИмениФайла(ИмяФайлаДляЗагрузки);
...
Функция ПолучитьИмяФайлаНаСервере(ФайлСсылка) Экспорт
	...
	ИмяФайлаНаСервере = РаботаСФайламиВТомахСлужебный.ПолныйПутьТома(ФайлСсылка.ТекущаяВерсия.Том) + ФайлСсылка.ТекущаяВерсия.ПутьКФайлу;
