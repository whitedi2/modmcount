...
Процедура ПриСозданииНаСервере(Форма, Отказ, СтандартнаяОбработка) Экспорт
	...
	ИначеЕсли Форма.ИмяФормы = "Отчет.УЧ_ОтчетПоУчетномуРегистру.ФормаОбъекта" Тогда 
	...
	Если Параметры.Свойство("ВидРасшифровки") Тогда 
		...
		Если Форма.ИмяФормы = "Отчет.УЧ_ОтчетПоУчетномуРегистру.ФормаОбъекта" Тогда
			
			//Для Каждого ЭлементПользНастроек Из Отчет.КомпоновщикНастроек.ПользовательскиеНастройки.Элементы Цикл
			//	ЭлементПользНастроек.ИдентификаторПользовательскойНастройки = "";
			//КонецЦикла;
	...
	Если ИдентификаторОбъекта = "сабАнализСубконто"
		...
		Если ПервыйЭлемент <> Неопределено Тогда
			Если ИдентификаторОбъекта = "сабОборотноСальдоваяВедомость" Тогда
				...
				СписокПунктовМеню.Добавить("УЧ_ОтчетПоУчетномуРегистру"       , ПредставлениеДляОтчетаПоРегистру);
...
Процедура ВывестиОтчет(ОбъектОтчет, Результат, ДанныеРасшифровки, Схема = Неопределено, ВыводитьПолностью = Истина) Экспорт
	...
	//	КонецЦикла; 
	//Иначе
		ТекущиеНастройки = ОбъектОтчет.КомпоновщикНастроек.ПолучитьНастройки();
	//КонецЕсли;
	...
	Если ВыводитьПолностью Тогда
		...
		Если ИспользоватьПередКомпоновкойМакета Тогда
			...
			НастройкиДляКомпоновкиМакета = ОбъектОтчет.КомпоновщикНастроек.ПолучитьНастройки();
