...
Процедура ДобавитьОтборПоОрганизации(ОбъектОтчет, ВПользовательскиеНастройки = Истина) Экспорт
	...
	Если ЗначениеЗаполнено(ОбъектОтчет.Организация) Тогда
		НовыйОтбор = СтандартныеОтчетыКлиентСервер.ДобавитьОтбор(ОбъектОтчет.КомпоновщикНастроек, "Организация", ОбъектОтчет.Организация,,, );
	...
	Попытка
		...
		Если ЗначениеЗаполнено(ОбъектОтчет.СценарийПлана) Тогда
			НовыйОтбор = СтандартныеОтчетыКлиентСервер.ДобавитьОтбор(ОбъектОтчет.КомпоновщикНастроек, "СценарийПлана", ОбъектОтчет.СценарийПлана,,, );
...
Функция ПолучитьСчетаЗатрат(Прочие = Ложь) Экспорт
	
	МассивСчетов = Новый Массив;
	Счет20 = ПланыСчетов.Учетный.НайтиПоКоду("20");
	Если ЗначениеЗаполнено(Счет20) Тогда
		МассивСчетов.Добавить(Счет20);
	КонецЕсли;
	Счет25 = ПланыСчетов.Учетный.НайтиПоКоду("25");
	Если ЗначениеЗаполнено(Счет25) Тогда
		МассивСчетов.Добавить(Счет25);
	КонецЕсли;
	Счет26 = ПланыСчетов.Учетный.НайтиПоКоду("26");
	Если ЗначениеЗаполнено(Счет26) Тогда
		МассивСчетов.Добавить(Счет26);
	КонецЕсли;
	Счет44 = ПланыСчетов.Учетный.НайтиПоКоду("44");
	Если ЗначениеЗаполнено(Счет44) Тогда
		МассивСчетов.Добавить(Счет44);
	КонецЕсли;
	Если Прочие Тогда
		Счет91 = ПланыСчетов.Учетный.НайтиПоКоду("91");
		Если ЗначениеЗаполнено(Счет91) Тогда
			МассивСчетов.Добавить(Счет91);
		КонецЕсли;
	КонецЕсли;
