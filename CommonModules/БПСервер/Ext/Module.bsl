...
Процедура ОповеститьОДоработке(БП, Исполнитель, СтрОснование) Экспорт
	...
	Если МассивОзн.Количество() Тогда
		СоздатьОповещение(МассивОзн, "Документ " +Строка(БП.Заявка) + ", отправлен на доработку пользователем "+ Строка(Исполнитель) + ?(ЗначениеЗаполнено(СтрОснование), " по причине: "+ СтрОснование +".", "."), "Документ " +Строка(БП.Заявка) + ", отправлен на доработку.", БП.Заявка,,Справочники.Пользователи.Система);
	КонецЕсли;
...
Процедура ОповеститьОПросроченныхЗаданиях() экспорт
	...
	Пока ВыборкаГруппировки.Следующий() Цикл
		...
		Если КолвоСвоих ИЛИ КолвоПодчиненных Тогда
			...
			СоздатьОповещение(ВыборкаГруппировки.Замещающий, СообщениеПользователюИтоговое, "Уведомление о просроченных заданиях",,,Справочники.Пользователи.Система, "Уведомление о просроченных заданиях", СтруктураОтчета); 
		КонецЕсли;
