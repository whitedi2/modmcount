...
Функция ВыполнитьКомандуЗадачиБП(Задача, ИмяКоманды, знач ДанныеФормы = Неопределено, ВызваноИзФормыЗадачи = истина, ИмяСоответствующейФормы = Неопределено, ВыполнитьЗадачуБезПроверок = Ложь) экспорт
	...
	ИначеЕсли (ИмяСоответствующейФормы = "ФормаОплаты" 
		...
		ИЛИ	Задача.ТочкаМаршрута = Перечисления.Согласование1ТочкиМаршрута.Действие6
...
Процедура СоздатьСледующуюЗадачу(Задача, ИмяКоманды, ДопДанные, ЭтоФормаДоработки)
	...
	ИначеЕсли ТипЗнч(Задача.Заявка) = Тип("ДокументСсылка.Д_ЗаявкаНаОплату") Тогда
		...
		Если НЕ КомандыДалее.Найти(ИмяКоманды) = Неопределено Тогда
			...
			Если Не ФормируемыеЗадачи.Количество() Тогда
				ТекБПОбъект.Действие6ПередСозданиемЗадач(Перечисления.Согласование1ТочкиМаршрута.Действие6, ФормируемыеЗадачи, Истина);
			КонецЕсли; 
...
Процедура ЗаголовокЗадачиИзЗаявки(Объект, ЭтаФорма) Экспорт
	...
	Если  ЗначениеЗаполнено(Объект.Заявка) И Метаданные.Документы.Содержит(Объект.Заявка.Метаданные()) Тогда
		...
		Если ТипЗнч(РеквизитыЗаявки) = Тип("Структура") Тогда
			ЭтаФорма.Заголовок = Объект.Наименование + " №" + ?(РеквизитыЗаявки.Свойство("Номер"), РеквизитыЗаявки.Номер, "") + " от " + ?(РеквизитыЗаявки.Свойство("Дата"), РеквизитыЗаявки.Дата, "");
		КонецЕсли;	
	КонецЕсли;
...
Функция СоздатьЗапросСписокМоихЗадач() Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		КОГДА ТИПЗНАЧЕНИЯ(ВыбранныеСправочники.Заявка) = ТИП(Документ.Д_ЗаявкаНаФинансирование)
	               |			ТОГДА ВЫРАЗИТЬ(ВыбранныеСправочники.Заявка КАК Документ.Д_ЗаявкаНаФинансирование).Сумма
...
Функция ПолучитьТекущуюТочкуМаршрута(Док) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(Согласование1.НеСогласовано, ЕСТЬNULL(Согласование2.НеСогласовано, ЕСТЬNULL(СогласованиеОбщее.НеСогласовано, ЕСТЬNULL(УтверждениеБюджета.НеСогласовано, ЛОЖЬ)))) КАК НеСогласовано
	               |ИЗ
	...
	Для каждого ТекСтрока Из ТЗ_Рез Цикл
		...
		Иначе
			...
			Если ТекСтрока.НеСогласовано Тогда
...
Функция ТочкиПроцессов(ВидТочки = "согласование") Экспорт
	...
	ИначеЕсли ВидТочки = "ознакомление" Тогда
		...
		МассивТочек.Добавить(Перечисления.Согласование1ТочкиМаршрута.Действие6);
...
Процедура ДобавитьРецензентовВМаршрут(БП, ИмяТЧ, ДокументОснование, Предприятие = Неопределено, Дата = Неопределено, ТипБюджета = Неопределено) Экспорт
	...
	Если ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.Д_ЗаявкаНаОплату") Тогда 
		...
		|	ВЫБОР
		|		КОГДА Д_ЗаявкаНаОплатуЗаявкаБезнал.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
		|			ТОГДА Д_ЗаявкаНаОплатуЗаявкаБезнал.Ссылка.Подразделение
		|		ИНАЧЕ Д_ЗаявкаНаОплатуЗаявкаБезнал.Подразделение
		|	КОНЕЦ КАК Подразделение,
		|	Д_ЗаявкаНаОплатуЗаявкаБезнал.Ссылка КАК Ссылка,
		|	Д_ЗаявкаНаОплатуЗаявкаБезнал.Ссылка.ТипИсточника КАК ТипИсточника,
		|	Д_ЗаявкаНаОплатуЗаявкаБезнал.Ссылка.ВидОперации КАК ВидОперации,
		...
		|	И МаршрутыДвиженияЗаявок.ПометкаУдаления = ЛОЖЬ
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("ВидДокумента", Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаОплату"));
	...
	ИначеЕсли ТипЗнч(ДокументОснование) = Тип("ДокументСсылка.Д_ЗаявкаНаСогласованиеДоговора") Тогда 
		...
		|	Д_ЗаявкаНаСогласованиеДоговора.ЮрЛицоКомпании КАК Организация
		|ПОМЕСТИТЬ ВТ_ДанныеЗаявки
		|ИЗ
	...
	Для Каждого ТекСтрокаПользователь Из ПользователиПоДействию Цикл
		...
		Попытка
			...
			Если ТекСтрокаПользователь.ТекПользователь = Справочники.Пользователи.ПолучитьЭлементПредопределенный("Автор документа") Тогда
				ОтобранныеСтроки = БП[ИмяТЧ].НайтиСтроки(Новый Структура(ИмяПользователяВТч, ДокументОснование.Автор));
			Иначе
...
Процедура ВернутьОтмененныеЗаякиНаДоработку(Задача) Экспорт
	...
	Если Задача.ТочкаМаршрута = Перечисления.Согласование1ТочкиМаршрута.Действие1 Тогда
		...
		Если ТекОбРеестр = Неопределено Тогда
			Возврат;		
		КонецЕсли;
...
Процедура АкцептоватьЗаявкиНаОплату(Ссылка, ТекДокумент = Неопределено) Экспорт
	...
	Если ТипЗнч(ТекущееПП) = Тип("ДокументСсылка.ПлатежноеПоручение") Тогда
		Если ТипЗнч(ТекущееПП.Заявка) = Тип("ДокументСсылка.Д_ЗаявкаНаФинансирование") И ЗначениеЗаполнено(ТекущееПП.Заявка) Тогда
			...
			Если ЗначениеЗаполнено(ТекДокумент) Тогда
				МенЗаписи = РегистрыСведений.сабСтатусыЗаявокНаОплату.СоздатьМенеджерЗаписи();
				МенЗаписи.Документ = ТекущееПП.Заявка;
				МенЗаписи.Статус = ТекСтатус;
				МенЗаписи.ДатаОплалы = ТекДокумент.Дата;
				МенЗаписи.Записать();
			КонецЕсли;
	...
	ИначеЕсли ТипЗнч(ТекущееПП) = Тип("ДокументСсылка.УЧ_ДвижениеДС") Тогда
		Если ТипЗнч(ТекущееПП.ДокОснование) = Тип("ДокументСсылка.Д_ЗаявкаНаФинансирование") И ЗначениеЗаполнено(ТекущееПП.ДокОснование) Тогда
			...
			//статус заявки
			МенЗаписи = РегистрыСведений.сабСтатусыЗаявокНаОплату.СоздатьМенеджерЗаписи();
			МенЗаписи.Документ = ТекущееПП.ДокОснование;
			МенЗаписи.Статус = ТекСтатус;
			МенЗаписи.ДатаОплалы = ТекущееПП.Дата;
			МенЗаписи.Записать();
...
Функция Оплата_ВыполненоСервер(Задача, ПроверкаУспешно, Пройден, ДопДанные)
	...
	ТочкаОзнакомленияСогласование1 = Перечисления.Согласование1ТочкиМаршрута.Действие6;
	...
	Если ТипЗнч(ЗадачаЗаявка) = Тип("ДокументСсылка.Д_ЗаявкаНаОплату") Тогда
		...
		Если Пройден Тогда // если заявка согласована или нет
			...
			Если Задача.ТочкаМаршрута = ТочкаОзнакомленияСогласование1 Тогда 
...
Процедура ВыполнитьЗадачиИсполненияЗаявокаОплату(ЗадачаЗаявка, Комментарий) Экспорт
	...
	Пока Выборка.Следующий() Цикл 
		...
		ИначеЕсли Выборка.ТочкаМаршрута = Перечисления.СогласованиеОбщееТочкиМаршрута.Действие3 Тогда 
...
Функция ПолучитьПППоЗаявке(ЗаявкаНаФинансирование, УИДСтроки = Неопределено) Экспорт
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ПлатежноеПоручение.Ссылка КАК Ссылка
	               |ИЗ
	               ...
	               |	И ПлатежноеПоручение.ПометкаУдаления = ЛОЖЬ
	               |	И ВЫБОР
	               |			КОГДА &УИДСтроки = НЕОПРЕДЕЛЕНО
	               |				ТОГДА ИСТИНА
	               |			ИНАЧЕ ПлатежноеПоручение.УИДСтроки = &УИДСтроки
	               |					ИЛИ ПлатежноеПоручение.ТабличнаяЧасть.УИДСтрокиЗаявки = &УИДСтроки
	               |					ИЛИ ПлатежноеПоручение.СтрокиЗаявкиНаОплату.УИДСтрокиДокОснования = &УИДСтроки
	               |		КОНЕЦ
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               |
	               ...
	               |			КОГДА &УИДСтроки = НЕОПРЕДЕЛЕНО
	               |				ТОГДА ИСТИНА
	               |			ИНАЧЕ УЧ_ДвижениеДС.Проводки.УИДСтрокиЗаявки = &УИДСтроки
	               |		КОНЕЦ";
	
	Запрос.УстановитьПараметр("Заявка", ЗаявкаНаФинансирование);
	Запрос.УстановитьПараметр("УИДСтроки", УИДСтроки);
...
Процедура ДобавитьКомандыПроцессов(ЭтаФорма, ИмяГруппыРодителя, ОбъектСсылка) Экспорт
	
	Если ЭтаФорма.Элементы.Найти("ГруппаБизнессПроцессы1") = Неопределено Тогда
		
		МассТип = Новый Массив;
		МассТип.Добавить(Тип("Строка"));
		ТипСтрока = Новый ОписаниеТипов(МассТип);
		
		МассТип = Новый Массив;
		МассТип.Добавить(Тип("СправочникСсылка.Задача"));
		ТипЗадача = Новый ОписаниеТипов(МассТип);
		
		МассТип = Новый Массив;
		МассТип.Добавить(Тип("СправочникСсылка.ДоговорыКонтрагентов"));
		//МассТип.Добавить(Тип("ДокументСсылка.БП_Поручение"));
		//МассТип.Добавить(Тип("ДокументСсылка.БП_Поручение"));
		ТипДоки = Новый ОписаниеТипов(МассТип);
		
		ДобавляемыеРеквизиты = Новый Массив;
		РеквизитКомментарий = Новый РеквизитФормы("ЗадачаКомментарий", ТипСтрока,	, "Задача комментарий");
		ДобавляемыеРеквизиты.Добавить(РеквизитКомментарий);
		РеквизитКомментарий = Новый РеквизитФормы("ЗадачаТекИмяКоманды1", ТипСтрока,	, "ЗадачаТекИмяКоманды1");
		ДобавляемыеРеквизиты.Добавить(РеквизитКомментарий);
		РеквизитКомментарий = Новый РеквизитФормы("ЗадачаТекИмяКоманды2", ТипСтрока,	, "ЗадачаТекИмяКоманды2");
		ДобавляемыеРеквизиты.Добавить(РеквизитКомментарий);
		РеквизитКомментарий = Новый РеквизитФормы("ЗадачаТекИмяКоманды3", ТипСтрока,	, "ЗадачаТекИмяКоманды3");
		ДобавляемыеРеквизиты.Добавить(РеквизитКомментарий);
		РеквизитКомментарий = Новый РеквизитФормы("Задача", ТипЗадача,	, "Задача");
		ДобавляемыеРеквизиты.Добавить(РеквизитКомментарий);
		РеквизитКомментарий = Новый РеквизитФормы("ПодчиненныеДокументы", Новый ОписаниеТипов("ТаблицаЗначений"),	, "ПодчиненныеДокументы");
		ДобавляемыеРеквизиты.Добавить(Новый РеквизитФормы("Документ", ТипДоки, "ПодчиненныеДокументы"));
		
		ДобавляемыеРеквизиты.Добавить(РеквизитКомментарий);
		
		ЭтаФорма.ИзменитьРеквизиты(ДобавляемыеРеквизиты);

		НоваяКоманда = ЭтаФорма.Команды.Добавить("ЗаявкаСогласована");
		НоваяКоманда.Действие = "ТекКоманда1Нажатие";//Имя процедуры
		НоваяКоманда.Заголовок = "Согласовано";
		
		НоваяКоманда = ЭтаФорма.Команды.Добавить("ОтправитьДалее");
		НоваяКоманда.Действие = "ТекКоманда3Нажатие";//Имя процедуры
		НоваяКоманда.Заголовок = "ОтправитьДалее";
		
		НоваяКоманда = ЭтаФорма.Команды.Добавить("НаДоработку");
		НоваяКоманда.Действие = "ТекКоманда2Нажатие";//Имя процедуры
		НоваяКоманда.Заголовок = "Не согласовано";
		
		НоваяГруппа = ЭтаФорма.Элементы.Вставить("ГруппаБизнессПроцессы1", Тип("ГруппаФормы"), ЭтаФорма, ЭтаФорма.Элементы[ИмяГруппыРодителя]);
		...
		НоваяГруппа.Заголовок = "Команды Бизнес-процесса";
		НоваяГруппа.ЦветФона = Новый Цвет(255, 250, 217);
		
		НоваяГруппа2 = ЭтаФорма.Элементы.Добавить("ГруппаБизнессПроцессы2", Тип("ГруппаФормы"), НоваяГруппа);
		НоваяГруппа2.Вид = ВидГруппыФормы.ОбычнаяГруппа;
		НоваяГруппа2.Группировка = ГруппировкаПодчиненныхЭлементовФормы.ГоризонтальнаяВсегда;
		НоваяГруппа2.ОтображатьЗаголовок = Ложь;
		
		НовыйЭлемент = ЭтаФорма.Элементы.Добавить("ЗаявкаСогласована", Тип("КнопкаФормы"), НоваяГруппа2);
		НовыйЭлемент.ИмяКоманды = "ЗаявкаСогласована";
		НовыйЭлемент.ЦветТекста = Новый Цвет(51, 153, 102);
		НовыйЭлемент.Шрифт = Новый Шрифт(,,Истина);
		//НовыйЭлемент.Заголовок = "Согласовано";
		
		НовыйЭлемент = ЭтаФорма.Элементы.Добавить("ОтправитьДалее", Тип("КнопкаФормы"), НоваяГруппа2);
		НовыйЭлемент.ИмяКоманды = "ОтправитьДалее";
		НовыйЭлемент.ЦветТекста = Новый Цвет(51, 153, 102);
		НовыйЭлемент.Шрифт = Новый Шрифт(,,Истина);
		//НовыйЭлемент.Заголовок = "Согласовано с замечаниями";
		
		НовыйЭлемент = ЭтаФорма.Элементы.Добавить("НаДоработку", Тип("КнопкаФормы"), НоваяГруппа2);
		НовыйЭлемент.ИмяКоманды = "НаДоработку";
		НовыйЭлемент.ЦветТекста = Новый Цвет(255, 0, 0);
		НовыйЭлемент.Шрифт = Новый Шрифт(,,Истина);
		
		НовыйЭлемент = ЭтаФорма.Элементы.Добавить("ЗадачаКомментарий", Тип("ПолеФормы"), НоваяГруппа2);
		НовыйЭлемент.ПутьКДанным = "ЗадачаКомментарий";
		НовыйЭлемент.Вид = ВидПоляФормы.ПолеВвода;
		НовыйЭлемент.ПодсказкаВвода = "Комментарий к выполнению...";
		НовыйЭлемент.ПоложениеЗаголовка = ПоложениеЗаголовкаЭлементаФормы.Нет;
		
	КонецЕсли;
	
	БПСервер.НастроитьДоступностьКнопок(ОбъектСсылка, ЭтаФорма.Элементы.ЗаявкаСогласована.Заголовок, ЭтаФорма.Элементы.ОтправитьДалее.Заголовок, ЭтаФорма.Элементы.НаДоработку.Заголовок, ЭтаФорма.ЗадачаТекИмяКоманды1, ЭтаФорма.ЗадачаТекИмяКоманды2, ЭтаФорма.ЗадачаТекИмяКоманды3, Ложь, ЭтаФорма.Задача);
	Если Не ЗначениеЗаполнено(ЭтаФорма.ЗадачаТекИмяКоманды1) И Не ЗначениеЗаполнено(ЭтаФорма.ЗадачаТекИмяКоманды2) И Не ЗначениеЗаполнено(ЭтаФорма.ЗадачаТекИмяКоманды3) Тогда
		ЭтаФорма.Элементы.ГруппаБизнессПроцессы1.Видимость = Ложь;
	Иначе
				
		ЭтаФорма.Элементы.ЗаявкаСогласована.Видимость = ЗначениеЗаполнено(ЭтаФорма.Элементы.ЗаявкаСогласована.Заголовок);
		ЭтаФорма.Элементы.ОтправитьДалее.Видимость = ЗначениеЗаполнено(ЭтаФорма.Элементы.ОтправитьДалее.Заголовок);
		ЭтаФорма.Элементы.НаДоработку.Видимость = ЗначениеЗаполнено(ЭтаФорма.Элементы.НаДоработку.Заголовок);
		...
		Если ЗначениеЗаполнено(ЭтаФорма.Задача.РольИсполнителя) И ТипЗнч(ЭтаФорма.Задача.РольИсполнителя) = Тип("СправочникСсылка.РолиИсполнителей") Тогда
			ИмяКомандыУтвердительно = ЭтаФорма.Задача.РольИсполнителя.ИмяКомандыУтвердительно;
			ЦветКомандыУтвердительно = ЭтаФорма.Задача.РольИсполнителя.ЦветКомандыУтвердительно.Получить();
			Если ЗначениеЗаполнено(ИмяКомандыУтвердительно) Тогда
				ЭтаФорма.Элементы.ЗаявкаСогласована.Заголовок = ИмяКомандыУтвердительно;
			КонецЕсли;
			Если НЕ ЦветКомандыУтвердительно = Неопределено Тогда
				ЭтаФорма.Элементы.ЗаявкаСогласована.ЦветТекста = ЦветКомандыУтвердительно;
			КонецЕсли;
			ИмяКомандыОтрицательно = ЭтаФорма.Задача.РольИсполнителя.ИмяКомандыОтрицательно;
			ЦветКомандыОтрицательно = ЭтаФорма.Задача.РольИсполнителя.ЦветКомандыОтрицательно.Получить();
			Если ЗначениеЗаполнено(ИмяКомандыОтрицательно) Тогда
				ЭтаФорма.Элементы.НаДоработку.Заголовок = ИмяКомандыОтрицательно;
			КонецЕсли;
			Если НЕ ЦветКомандыОтрицательно = Неопределено Тогда
				ЭтаФорма.Элементы.НаДоработку.ЦветТекста = ЦветКомандыОтрицательно;
			КонецЕсли;
			ИмяКомандыСЗамечаниями = ЭтаФорма.Задача.РольИсполнителя.ИмяКомандыСЗамечаниями;
			ЦветКомандыСЗамечаниями = ЭтаФорма.Задача.РольИсполнителя.ЦветКомандыСЗамечаниями.Получить();
			Если ЗначениеЗаполнено(ИмяКомандыСЗамечаниями) Тогда
				ЭтаФорма.Элементы.ОтправитьДалее.Заголовок = ИмяКомандыСЗамечаниями;
			Иначе
				ЭтаФорма.Элементы.ОтправитьДалее.Видимость = Ложь; // для пустого реквизита прячем команду	
			КонецЕсли;
			Если НЕ ЦветКомандыСЗамечаниями = Неопределено Тогда
				ЭтаФорма.Элементы.ОтправитьДалее.ЦветТекста = ЦветКомандыСЗамечаниями;
			КонецЕсли;
...
Процедура НастроитьДоступностьКнопок(ЗадачаСсылка, ЗаголовокДа, ЗаголовокДа2, ЗаголовокНет, ИмяКомандыДа, ИмяКомандыДа2, ИмяКомандыНет, ВидимостьРазрешенияРедактирования, ЗадачаВозврат = Неопределено) Экспорт
	...
	Если Не ТипЗнч(Задача) = Тип("СправочникСсылка.Задача") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("Заявка", Задача);
		...
		Пока Выборка.Следующий() Цикл
			...
			ЗадачаВозврат = Задача;
		КонецЦикла;
		
		Если Не ЗначениеЗаполнено(ЗадачаВозврат) Или (Не ТекПользовательИсполнительЗадачи(ЗадачаВозврат) И БПСервер.ПолучитьМассивПользователей().Найти(ЗадачаВозврат.Автор) = Неопределено) Тогда
			Возврат;		
		КонецЕсли;
	...
	Если ТипЗнч(Объект.РольИсполнителя) = Тип("СправочникСсылка.РолиИсполнителей") Тогда
		СвойстваРолиИсполнителя = БюджетныйНаСервере.ВернутьРеквизиты(Объект.РольИсполнителя, "ИмяКомандыУтвердительно, ИмяКомандыОтрицательно, НаименованиеЗадачи, ИмяКомандыСЗамечаниями"); 
	Иначе
	...
	Если Не СвойстваРолиИсполнителя = Неопределено Тогда
		...
		Если ЗначениеЗаполнено(СвойстваРолиИсполнителя.ИмяКомандыСЗамечаниями) Тогда
			ЗаголовокДа2 = ?(ЭтоИсполнитель, СвойстваРолиИсполнителя.ИмяКомандыСЗамечаниями, "");
		КонецЕсли;
