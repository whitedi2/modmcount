...
Функция ПолучитьДатуЗапрета(знач Объект, знач Пользователь = Неопределено) Экспорт
	...
	|	сабДатыЗапретаИзмененияСрезПоследних.ДатаЗапрета
	|ИЗ
	|	РегистрСведений.сабДатыЗапретаИзменения.СрезПоследних(&ТекДата, ) КАК сабДатыЗапретаИзмененияСрезПоследних
	|ГДЕ
	|	сабДатыЗапретаИзмененияСрезПоследних.Объект = ЗНАЧЕНИЕ(Перечисление.ВидыНазначенияДатЗапрета.ПоВсемОбъектам)
	|	И сабДатыЗапретаИзмененияСрезПоследних.Пользователь = ЗНАЧЕНИЕ(Перечисление.ВидыНазначенияДатЗапрета.ПоВсемПользователям)";
	...
	|	ВложенныйЗапрос.ДатаЗапрета КАК ДатаЗапрета,
	|	ВЫБОР
	...
	|			ТОГДА ВЫБОР
	|					КОГДА ВложенныйЗапрос.Пользователь.Родитель = ЗНАЧЕНИЕ(Справочник.ГруппыПользователей.ПустаяСсылка)
	|						ТОГДА 2.9
	|					КОГДА ВложенныйЗапрос.Пользователь.Родитель.Родитель = ЗНАЧЕНИЕ(Справочник.ГруппыПользователей.ПустаяСсылка)
	|						ТОГДА 2.8
	|					ИНАЧЕ 2
	|				КОНЕЦ
	...
	|ИЗ
	|	(ВЫБРАТЬ
	|		сабДатыЗапретаИзмененияСрезПоследних.Объект КАК Объект,
	|		сабДатыЗапретаИзмененияСрезПоследних.Пользователь КАК Пользователь,
	|		МИНИМУМ(сабДатыЗапретаИзмененияСрезПоследних.ДатаЗапрета) КАК ДатаЗапрета
	|	ИЗ
	|		РегистрСведений.сабДатыЗапретаИзменения.СрезПоследних(&ТекДата, ) КАК сабДатыЗапретаИзмененияСрезПоследних
	|			ЛЕВОЕ СОЕДИНЕНИЕ Справочник.ГруппыПользователей.Состав КАК ГруппыПользователейСостав
	|			ПО (сабДатыЗапретаИзмененияСрезПоследних.Пользователь = ГруппыПользователейСостав.Ссылка
	|					ИЛИ сабДатыЗапретаИзмененияСрезПоследних.Пользователь = ГруппыПользователейСостав.Ссылка.Родитель)
	|	ГДЕ
	|		(сабДатыЗапретаИзмененияСрезПоследних.Пользователь = &Пользователь
	|				ИЛИ ГруппыПользователейСостав.Пользователь = &Пользователь
	|				ИЛИ сабДатыЗапретаИзмененияСрезПоследних.Пользователь = ЗНАЧЕНИЕ(Перечисление.ВидыНазначенияДатЗапрета.ПоВсемПользователям))
	|		И (сабДатыЗапретаИзмененияСрезПоследних.Объект = &Предприятие
	|				ИЛИ сабДатыЗапретаИзмененияСрезПоследних.Объект = ЗНАЧЕНИЕ(Перечисление.ВидыНазначенияДатЗапрета.ПоВсемОбъектам))
	|	
	|	СГРУППИРОВАТЬ ПО
	|		сабДатыЗапретаИзмененияСрезПоследних.Объект,
	|		сабДатыЗапретаИзмененияСрезПоследних.Пользователь) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
...
Процедура ПроверитьФормуНаДатуЗапрета(Форма, УУДокумент = Ложь) Экспорт
	...
	УУДокумент = Ложь;
	Если НЕ УУДокумент И ЗначениеЗаполнено(Объект.Ссылка) И БюджетныйНаСервере.ПринадлежитПодсистеме(Объект.Ссылка, "сабУправленческийУчет") Тогда
		УУДокумент = Истина;
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Объект.Ссылка) И УУДокумент Тогда
		...
		Если БюджетныйНаСервере.ПринадлежитПодсистеме(Объект.Ссылка,"сабУправленческийУчет") Тогда
			...
			ДатаЗакрытияУчетПредприятие = УЧ_Сервер.ДатаБлокировкиИзмененияДокументов(Объект.Предприятие);
		...
		Если Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройки("сабПредупреждатьОДокументахВЗакрытомПериоде") И ИсточникДата <= ДатаЗакрытияУчетПредприятие Тогда
			Группа1 = Форма.Элементы.Найти("Группа1");
					
			НоваяКнопка = Форма.Элементы.Вставить("саб_ГруппаЗакрытыйПериод", Тип("ГруппаФормы"),, Группа1);
			...
			НоваяКнопка.ЦветФона = Новый Цвет(255, 250, 217);
			
			ЭлементФормыНовый = Форма.Элементы.Вставить("саб_ДокументВЗакрытомПериоде0", Тип("ДекорацияФормы"), НоваяКнопка);
			ЭлементФормыНовый.Вид = ВидДекорацииФормы.Картинка;
			ЭлементФормыНовый.Картинка = БиблиотекаКартинок.ОформлениеЗнакВосклицательныйЗнак;
			
			ЭлементФормыНовый = Форма.Элементы.Вставить("саб_ДокументВЗакрытомПериоде", Тип("ДекорацияФормы"), НоваяКнопка); 
			ЭлементФормыНовый.Вид = ВидДекорацииФормы.Надпись;
			ЭлементФормыНовый.Заголовок = "Документ в закрытом периоде. Дата закрытия периода " + ДатаЗакрытияУчетПредприятие;
		КонецЕсли;
	...
	//	ДатаБлокировки = УЧ_Сервер.ДатаБлокировкиИзмененияДокументов(Объект.Предприятие);
	//	Если ДатаБлокировки <> Неопределено Тогда	
	//		Если Объект.Дата <= ДатаБлокировки  Тогда
	//			Форма.ТолькоПросмотр = Истина
	//		КонецЕсли;
	//	КонецЕсли;
	//КонецЕсли;
...
Процедура ОтобразитьСостояниеДокумента(Форма, ТекущийОбъект = Неопределено) Экспорт
	...
	//убираем признак изменения при интерактивном проведении объекта
	Если Не ТекущийОбъект = Неопределено Тогда
		Если БюджетныйНаСервере.ЕстьСвойствоОбъекта(Форма, "ДокументБУ") И ЗначениеЗаполнено(Форма.ДокументБУ) Тогда
			
			НаборЗаписей = РегистрыСведений.сабОбработкаДокументов.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.ДокументБУ.Установить(Форма.ДокументБУ);
			НаборЗаписей.Прочитать();
			Если НаборЗаписей.Количество() Тогда
				Для каждого ТекЗапись Из НаборЗаписей Цикл
					//Если ТекЗапись.Модифицирован Тогда
					//	ТекЗапись.Автор = ПараметрыСеанса.ТекущийПользователь;
					//	//Коммент для отображения на вкладке измененных
					//	//ТекЗапись.Модифицирован = Ложь;
					//	ТекЗапись.ДатаОбработки = ТекущаяДата();
					//КонецЕсли;
					Если БюджетныйНаСервере.ЕстьСвойствоОбъекта(Форма, "АвтообновленияЗаблокированы") Тогда
						ТекЗапись.АвтообновленияЗаблокированы = Форма.АвтообновленияЗаблокированы;
					КонецЕсли;
				КонецЦикла; 
...
&НаСервере
Функция ПолучитьСчетУчетаНоменклатуры(Номенклатура, Организация = Неопределено, Склад = Неопределено) Экспорт
	
	Счет = Неопределено;
	
	ЭтоМассивНоменклатур = Ложь;
	Если ТипЗнч(Номенклатура) = Тип("Массив") Тогда
		ЭтоМассивНоменклатур = Истина;
	КонецЕсли;
	
	ВидыТовары = Новый Массив;
	ВидыТовары.Добавить(Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Товары", Истина));
	
	ВидыГП = Новый Массив;
	ВидыГП.Добавить(Справочники.ВидыНоменклатуры.НайтиПоНаименованию("Продукция", Истина));

	Если Не ЭтоМассивНоменклатур Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		|	Номенклатура.Счет10 КАК Счет10,
		...
		Пока Выборка.Следующий() Цикл
			Если ЗначениеЗаполнено(Выборка.Счет10) Тогда
				Счет = Выборка.Счет10;
				Возврат Счет;
			КонецЕсли; 
		КонецЦикла;
		
		Если Не ВидыГП.Найти(Номенклатура.ВидНоменклатуры) = Неопределено Тогда
			Счет = ПланыСчетов.Учетный.Счет43();
		ИначеЕсли Номенклатура.ТипНоменклатуры = Перечисления.сабТипыНоменклатуры.Товар Или Номенклатура.ТипНоменклатуры = Перечисления.сабТипыНоменклатуры.Комплектующие
			Или Не ВидыТовары.Найти(Номенклатура.ВидНоменклатуры) = Неопределено Тогда
	...
	Организация, СписокНоменклатуры, Склад, Неопределено, Неопределено);
	
	Для каждого ТекНоменклатура Из СведенияОНоменклатуре Цикл
		
		Если ЗначениеЗаполнено(ТекНоменклатура.Значение.СчетУчета) Тогда
			ЕстьСоответствие = Ложь;
			Соотв = ВыборкаСоотв.НайтиСтроки(Новый Структура("СчетБУ", ТекНоменклатура.Значение.СчетУчета));
			Для каждого ТекСчет Из Соотв Цикл
				ТекНоменклатура.Значение.Вставить("СчетУчетаУУ", ТекСчет.СчетУУ);
				ЕстьСоответствие = Истина;
				Прервать;
			КонецЦикла;
			Если Не ЕстьСоответствие Тогда
				ТекНоменклатура.Значение.Вставить("СчетУчетаУУ", ПланыСчетов.Учетный.ПустаяСсылка());			
			КонецЕсли;
		Иначе
			ТекНоменклатура.Значение.Вставить("СчетУчетаУУ", ПланыСчетов.Учетный.ПустаяСсылка());	
		КонецЕсли;
	...
	Если ЭтоМассивНоменклатур Тогда
...
Процедура сабПлатКалПриЗаписиОбъектовПриЗаписи(Источник, Отказ) Экспорт
	...
	ФормироватьСтрокиГрафикаОплат = Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройки("ФормироватьСтрокиГрафикаОплат");
	...
	Если ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.Д_ЗаявкаНаОплату") Тогда
		...
		Если Источник.ДополнительныеСвойства.Модифицирован Тогда
			...
			Для каждого ТекСтрока Из Источник.ЗаявкаБезнал Цикл
				...
				КонецЕсли;
				НоваяСтрока.ПодразделениеЦФО = ?(ЗначениеЗаполнено(ТекСтрока.Подразделение), ТекСтрока.Подразделение, Источник.Подразделение);
	...
	ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.Д_ЗаявкаНаФинансирование") Тогда
		...
		Если НЕ ЗначениеЗаполнено(ТекТочкаМаршрута) Тогда
			...
			ИначеЕсли ЗначениеЗаполнено(ТекБП) И ТочкаМаршрутаЗадачи = Перечисления.СогласованиеОбщееТочкиМаршрута.Действие1 Тогда 
				ТекТочкаМаршрута = Перечисления.Согласование1ТочкиМаршрута.Действие1;	
			Иначе 	
				ТекТочкаМаршрута = Перечисления.Согласование1ТочкиМаршрута.Старт;
			КонецЕсли;   
		...
		//очищаем строки по поставкам, т.к. собираем их в заявку
		Для каждого ТекСтрока Из Источник.ТабличнаяЧасть Цикл
			Если ЗначениеЗаполнено(ТекСтрока.Документ) Тогда
				НаборЗаписей = РегистрыСведений.сабГрафикПлатежей.СоздатьНаборЗаписей();
				НаборЗаписей.Отбор.Документ.Установить(ТекСтрока.Документ);
				НаборЗаписей.Записать();
			КонецЕсли;
		КонецЦикла;
		...
		//график платежей
		Если Источник.ДополнительныеСвойства.Модифицирован И НЕ ЗначениеЗаполнено(Источник.РеестрЗаявок) Тогда
			НаборЗаписей = РегистрыСведений.сабГрафикПлатежей.СоздатьНаборЗаписей();
			НаборЗаписей.Отбор.Документ.Установить(Источник.Ссылка);
			НаборЗаписей.Прочитать();
			...
			Если Источник.Сумма <> 0 Тогда
				РегистрыСведений.сабИсторияИзмененийПлатежногоКалендаря.ДобавитьЗапись(Источник.ДатаОплаты,ПользователиКлиентСервер.ТекущийПользователь(),Перечисления.сабДействияПриИзмененииПлатежногоКалендаря.СозданиеЗаписи,Источник.Ссылка,Источник.Сумма,);
			КонецЕсли;
		...
		Если Источник.Сценарий.СоздаватьГрафикОплат Тогда
			...
			Если Источник.ДополнительныеСвойства.Модифицирован Тогда
				...
				Запрос.Текст = "ВЫБРАТЬ
							   ...
							   |	Д_БюджетЗатраты.Контрагент КАК Контрагент,
							   |	Д_БюджетЗатраты.Договор КАК Договор,
							   ...
							   |	Д_БюджетЗатратыКП.Контрагент КАК Контрагент,
							   |	Д_БюджетЗатратыКП.Договор КАК Договор,
	...
	ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.СчетНаОплатуПокупателю") Тогда
		Если (Источник.ДополнительныеСвойства.Свойство("Модифицирован") И Источник.ДополнительныеСвойства.Модифицирован) ИЛИ (Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") И Источник.ДополнительныеСвойства.ЭтоНовый) Тогда
			
			Если Источник.ДоговорКонтрагента.сабГрафикОплаты.Количество() Тогда
				Запрос = Новый Запрос;
				Запрос.Текст = 
				"ВЫБРАТЬ
				...
				Запрос.УстановитьПараметр("Предприятие", Источник.Организация.Предприятие);
				Запрос.УстановитьПараметр("Счета", ПланыСчетов.Учетный.Счет6201());
				Запрос.УстановитьПараметр("Субконто1", Источник.Контрагент);
				Запрос.УстановитьПараметр("Субконто2", Источник.ДоговорКонтрагента);
				...
				СуммаИтого = Источник.СуммаДокумента;
				Для каждого ТекСтрока Из ВыборкаИсход Цикл
					Если Не ЗначениеЗаполнено(ТекСтрока.Субконто3) Тогда
						Продолжить;					
					КонецЕсли;
					НаборЗаписей = РегистрыСведений.сабГрафикПлатежей.СоздатьНаборЗаписей();
					НаборЗаписей.Отбор.Документ.Установить(ТекСтрока.Субконто3);
					НаборЗаписей.Прочитать();
					Если НаборЗаписей.Количество() Тогда
						НаборЗаписей.Очистить();
						НаборЗаписей.Записать();
					КонецЕсли;	
					СуммаИтого = СуммаИтого - ТекСтрока.СуммаОстатокДт;
			...
			Иначе 
				...
				|		ПО РеализацияТоваровУслуг.Ссылка = сабОбработкаДокументов.ДокументБУ
				|ГДЕ
				|	РеализацияТоваровУслуг.СчетНаОплатуПокупателю = &Основание";
				...
				Пока Выборка.Следующий() Цикл
					НаборЗаписей = РегистрыСведений.сабГрафикПлатежей.СоздатьНаборЗаписей();
					НаборЗаписей.Отбор.Документ.Установить(Выборка.ДокументУУ);
					НаборЗаписей.Прочитать();
					Если НаборЗаписей.Количество() Тогда
						НаборЗаписей.Очистить();
						НаборЗаписей.Записать();
			...
			НоваяСтрока.Документ = Источник.Ссылка;
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Источник);
			НоваяСтрока.Сумма = ?(Источник.СуммаВключаетНДС, Источник.Товары.Итог("Сумма"), Источник.Товары.Итог("Сумма") + Источник.Товары.Итог("СуммаНДС"));
			...
			СтатьяДоговора = Источник.ДоговорКонтрагента.СтатьяДДС;
			Если ЗначениеЗаполнено(СтатьяДоговора) Тогда
				НоваяСтрока.СтатьяДДС = СтатьяДоговора;
			Иначе	
		...
		Иначе //обычная перезапись, например при удалении заявки
			...
			НаборЗаписей.Записать();
		КонецЕсли;
	ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.УЧ_Реализация") И ФормироватьСтрокиГрафикаОплат Тогда
		Если (Источник.ДополнительныеСвойства.Свойство("Модифицирован") И Источник.ДополнительныеСвойства.Модифицирован) ИЛИ (Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") И Источник.ДополнительныеСвойства.ЭтоНовый) Тогда
			
			МассивСсылок = Новый Массив;
			
			ЕстьГрафик = Ложь;
			Если ТипЗнч(Источник.Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") И Источник.Договор.сабГрафикОплаты.Количество() Тогда
				ЕстьГрафик = Истина;
				Запрос = Новый Запрос;
				Запрос.Текст = "ВЫБРАТЬ
				|	сабСтрокиГрафиковДоговоров.Ссылка КАК Ссылка,
				|	сабСтрокиГрафиковДоговоров.Документ КАК Документ,
				|	сабСтрокиГрафиковДоговоров.НомерСтроки КАК НомерСтроки,
				|	сабСтрокиГрафиковДоговоров.Дата КАК Дата
				|ИЗ
				|	Справочник.сабСтрокиГрафиковДоговоров КАК сабСтрокиГрафиковДоговоров
				|ГДЕ
				|	сабСтрокиГрафиковДоговоров.Документ = &Документ";
				
				Запрос.УстановитьПараметр("Документ", Источник.Ссылка);
				
				Результат = Запрос.Выполнить();
				ВыборкаСтрокиГрафика = Результат.Выгрузить();
				МассивСсылок = ОбщегоНазначения.ТаблицаЗначенийВМассив(ВыборкаСтрокиГрафика);
				ТекУсловия = Новый Структура("Отсрочка", 0);
			Иначе
				ТекУсловия = УЧ_Сервер.ВернутьОтсрочкуПоДоговору(Источник.Договор, Источник.Дата);
				Если Не ЗначениеЗаполнено(Источник.Договор) Или ТекУсловия.НеУстановлена Тогда
					Возврат;		
				КонецЕсли;
				МассивСсылок.Добавить(Новый Структура("Ссылка", Источник.Ссылка));
			КонецЕсли;
			
			
			Для каждого ТекСсылка Из МассивСсылок Цикл
				
				ИсточникСсылки = ТекСсылка.Ссылка.ПолучитьОбъект();
				...
				|		ПО сабОбработкаДокументов.ДокументБУ = РеализацияТоваровУслуг.Ссылка
				|			И (НЕ РеализацияТоваровУслуг.СчетНаОплатуПокупателю = ЗНАЧЕНИЕ(Документ.СчетНаОплатуПокупателю.ПустаяСсылка))
				|ГДЕ
				|	сабОбработкаДокументов.ДокументУУ = &ДокументУУ";
				Запрос.УстановитьПараметр("ДокументУУ", ИсточникСсылки.Ссылка);
				Результат = Запрос.Выполнить();
				Выборка = Результат.Выбрать();
				...
				НаборЗаписей = РегистрыСведений.сабГрафикПлатежей.СоздатьНаборЗаписей();
				НаборЗаписей.Отбор.Документ.Установить(ИсточникСсылки.Ссылка);
				НаборЗаписей.Прочитать();
				сабОбщегоНазначения.ВернутьСуммуБюджета(ИсточникСсылки, НаборЗаписей);
				НаборЗаписей.Очистить();
				НоваяСтрока = НаборЗаписей.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ИсточникСсылки);
				НоваяСтрока.Документ = ИсточникСсылки.Ссылка;
				Если ТипЗнч(ТекСсылка.Ссылка) = Тип("СправочникСсылка.сабСтрокиГрафиковДоговоров") Тогда
					НоваяСтрока.Договор = ИсточникСсылки.Владелец;
					НоваяСтрока.Сумма = ИсточникСсылки.Сумма;
					НоваяСтрока.Договор = ИсточникСсылки.Владелец;
				Иначе
					НоваяСтрока.Сумма = ?(НЕ ИсточникСсылки.УчитыватьНДС ИЛИ ИсточникСсылки.СуммаВключаетНДС, ИсточникСсылки.Товары.Итог("Сумма") + ИсточникСсылки.Услуги.Итог("Сумма"), ИсточникСсылки.Товары.Итог("Сумма") + ИсточникСсылки.Товары.Итог("СуммаНДС") + ИсточникСсылки.Услуги.Итог("Сумма") + ИсточникСсылки.Услуги.Итог("СуммаНДС"));
					НоваяСтрока.СуммаНДС = ИсточникСсылки.Товары.Итог("СуммаНДС") + ИсточникСсылки.Услуги.Итог("СуммаНДС");
					Если ИсточникСсылки.Товары.Количество() Тогда
						НоваяСтрока.СтавкаНДС = ИсточникСсылки.Товары[0].СтавкаНДС;
					КонецЕсли;
					Если ИсточникСсылки.Услуги.Количество() Тогда
						НоваяСтрока.СтавкаНДС = ИсточникСсылки.Услуги[0].СтавкаНДС;
					КонецЕсли;
				КонецЕсли;
				НоваяСтрока.ЦФО = ИсточникСсылки.Предприятие;
				НоваяСтрока.ПодразделениеЦФО = ИсточникСсылки.Подразделение;
				НоваяСтрока.ДатаПлатежа = ИсточникСсылки.Дата + ТекУсловия.Отсрочка*24*60*60;
				НоваяСтрока.ВидОперации = Перечисления.ВидыОперацийПлатежноеПоручение.Поступление;
				НоваяСтрока.Источник = ИсточникСсылки.Организация.ОсновнойБанковскийСчет;
				НоваяСтрока.Валюта = ?(ЗначениеЗаполнено(ИсточникСсылки.Валюта), ИсточникСсылки.Валюта, УЧ_Сервер.НациональнаяВалюта());
				СтатьяДоговора = НоваяСтрока.Договор.СтатьяДДС;
				Если ЗначениеЗаполнено(СтатьяДоговора) Тогда
					НоваяСтрока.СтатьяДДС = СтатьяДоговора;
				Иначе	
					НоваяСтрока.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.ОплатаОтПокупателя;
		...
		Иначе //обычная перезапись, например при удалении заявки
			...
			Если ТипЗнч(Источник.Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") И Источник.Договор.сабГрафикОплаты.Количество() Тогда
				...
				|	сабСтрокиГрафиковДоговоров.НомерСтроки КАК НомерСтроки,
				|	сабСтрокиГрафиковДоговоров.Дата КАК Дата,
				|	сабСтрокиГрафиковДоговоров.ПометкаУдаления КАК ПометкаУдаления
				|ИЗ
				|	Справочник.сабСтрокиГрафиковДоговоров КАК сабСтрокиГрафиковДоговоров
				|ГДЕ
				|	сабСтрокиГрафиковДоговоров.Документ = &Документ";
			...
			Для каждого ТекСсылка Из МассивСсылок Цикл
				...
				Если ТипЗнч(ТекСсылка.Ссылка) = Тип("СправочникСсылка.сабСтрокиГрафиковДоговоров") Тогда
					Если НЕ ТекСсылка.ПометкаУдаления = Источник.ПометкаУдаления Тогда
						ТекСсылкаОбъект = ТекСсылка.Ссылка.ПолучитьОбъект();
						ТекСсылкаОбъект.ПометкаУдаления = Источник.ПометкаУдаления;	
						ТекСсылкаОбъект.Записать();			
					КонецЕсли;
				КонецЕсли;

				НаборЗаписей = РегистрыСведений.сабГрафикПлатежей.СоздатьНаборЗаписей();
				НаборЗаписей.Отбор.Документ.Установить(ТекСсылка.Ссылка);
				НаборЗаписей.Прочитать();
	...
	ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.УЧ_ПоступлениеТоваров") И ФормироватьСтрокиГрафикаОплат Тогда
		Если (Источник.ДополнительныеСвойства.Свойство("Модифицирован") И Источник.ДополнительныеСвойства.Модифицирован) ИЛИ (Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") И Источник.ДополнительныеСвойства.ЭтоНовый) Тогда
			...
			Для каждого ТекСсылка Из МассивСсылок Цикл
				...
				Запрос = Новый Запрос;
				Запрос.Текст = "ВЫБРАТЬ
				|	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка КАК Ссылка
				|ИЗ
				|	Документ.Д_ЗаявкаНаФинансирование.ТабличнаяЧасть КАК Д_ЗаявкаНаФинансированиеТабличнаяЧасть
				|ГДЕ
				|	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Документ = &Документ
				|	И НЕ Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка.ПометкаУдаления
				|
				|СГРУППИРОВАТЬ ПО
				|	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка";
				Запрос.УстановитьПараметр("Документ", ТекСсылка.Ссылка);
				Результат = Запрос.Выполнить();
				Выборка = Результат.Выбрать();
				Если Выборка.Количество() Тогда
					Возврат;	
				...
				НоваяСтрока = НаборЗаписей.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ИсточникСсылки);
				НоваяСтрока.Документ = ТекСсылка.Ссылка;
				Если ТипЗнч(ТекСсылка.Ссылка) = Тип("СправочникСсылка.сабСтрокиГрафиковДоговоров") Тогда
					НоваяСтрока.Договор = ИсточникСсылки.Владелец;
					НоваяСтрока.Сумма = ИсточникСсылки.Сумма;
				Иначе
					НоваяСтрока.Сумма = ?(НЕ ИсточникСсылки.УчитыватьНДС ИЛИ ИсточникСсылки.СуммаВключаетНДС, ИсточникСсылки.ТабличнаяЧасть.Итог("Сумма") + ИсточникСсылки.Услуги.Итог("Сумма"), ИсточникСсылки.ТабличнаяЧасть.Итог("Сумма") + ИсточникСсылки.ТабличнаяЧасть.Итог("СуммаНДС") + ИсточникСсылки.Услуги.Итог("Сумма") + ИсточникСсылки.Услуги.Итог("СуммаНДС"));
					НоваяСтрока.СуммаНДС = ИсточникСсылки.ТабличнаяЧасть.Итог("СуммаНДС") + ИсточникСсылки.Услуги.Итог("СуммаНДС");
					Если ИсточникСсылки.ТабличнаяЧасть.Количество() Тогда
						НоваяСтрока.СтавкаНДС = ИсточникСсылки.ТабличнаяЧасть[0].СтавкаНДС;
					КонецЕсли;
					Если ИсточникСсылки.Услуги.Количество() Тогда
						НоваяСтрока.СтавкаНДС = ИсточникСсылки.Услуги[0].СтавкаНДС;
					КонецЕсли;
				...
				Если ЗначениеЗаполнено(СтатьяДоговора) Тогда
					НоваяСтрока.СтатьяДДС = СтатьяДоговора;
				Иначе	
					НоваяСтрока.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.ПолучитьСтатьюОплатаПоставщику(НоваяСтрока.Предприятие);
				КонецЕсли;
				НоваяСтрока.ТочкаМаршрута = Перечисления.Согласование1ТочкиМаршрута.Старт;
				НаборЗаписей.Записать();
		...
		КонецЕсли;
	ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.УЧ_ВыпускПродукции") И ФормироватьСтрокиГрафикаОплат Тогда
		Если (Источник.ДополнительныеСвойства.Свойство("Модифицирован") И Источник.ДополнительныеСвойства.Модифицирован) ИЛИ (Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") И Источник.ДополнительныеСвойства.ЭтоНовый) Тогда
			...
			НоваяСтрока.Документ = Источник.Ссылка;
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Источник);
			НоваяСтрока.Сумма = Источник.Услуги.Итог("Сумма");
			НоваяСтрока.СуммаНДС = Источник.Услуги.Итог("СуммаНДС");
			Если Источник.Услуги.Количество() Тогда
				НоваяСтрока.СтавкаНДС = Источник.Услуги[0].СтавкаНДС;
			КонецЕсли;
			НоваяСтрока.Предприятие = Источник.Предприятие;
			НоваяСтрока.ЦФО = Источник.Предприятие;
			НоваяСтрока.ПодразделениеЦФО = Источник.Подразделение;
			НоваяСтрока.Комментарий = Источник.Комментарий;
			НоваяСтрока.ДатаПлатежа = Источник.Дата + ТекУсловия.Отсрочка*24*60*60;
			НоваяСтрока.ВидОперации = Перечисления.ВидыОперацийПлатежноеПоручение.Оплата;
			НоваяСтрока.Источник = Источник.Организация.ОсновнойБанковскийСчет;
			НоваяСтрока.Договор = Источник.Договор;
			НоваяСтрока.Валюта = ?(ЗначениеЗаполнено(Источник.Валюта), Источник.Валюта, УЧ_Сервер.НациональнаяВалюта());
			СтатьяДоговора = Источник.Договор.СтатьяДДС;
			Если ЗначениеЗаполнено(СтатьяДоговора) Тогда
				НоваяСтрока.СтатьяДДС = СтатьяДоговора;
			Иначе	
				НоваяСтрока.СтатьяДДС = Справочники.СтатьиДвиженияДенежныхСредств.ПолучитьСтатьюОплатаПоставщику(НоваяСтрока.Предприятие);
...
Функция КоличествоПрикрепленныхФайлов(Ссылка) экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	И НЕ ПрикрепленныеОбъекты.Объект.Служебный";
...
Процедура ОбновитьКоличествоПрикрепленныхФайловСервер(Форма) экспорт
	//Если ТипЗнч(Форма.Объект.Ссылка) = Тип("СправочникСсылка.Задача") тогда
...
Процедура сабПриЗаписиОрганизацииСчетаПриЗаписи(Источник, Отказ) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("СправочникОбъект.БанковскиеСчета") Тогда
		...
		Если Не Выборка.Количество() Тогда
			...
			НоваяЗапись.Записать();
		Иначе 
			НЗ_ИсточникПП = РегистрыСведений.Д_ИсточникПП.СоздатьНаборЗаписей();
			НЗ_ИсточникПП.Отбор.БанковскиеСчета.Установить(Источник.Ссылка);    
			НЗ_ИсточникПП.Прочитать(); 
			НоваяЗапись = НЗ_ИсточникПП[НЗ_ИсточникПП.Количество() - 1];
			НеобходимаАктуализацияПоследнейЗаписи = Ложь;
			
			Если Источник.Предприятие <> НоваяЗапись.Предприятие Тогда
				НоваяЗапись.Предприятие = Источник.Предприятие;
				НеобходимаАктуализацияПоследнейЗаписи = Истина; 
			КонецЕсли;
			
			Если Источник.Ответственный <> НоваяЗапись.ОтветственноеЛицо Тогда
				НоваяЗапись.ОтветственноеЛицо = Источник.Ответственный; 
				НеобходимаАктуализацияПоследнейЗаписи = Истина;
			КонецЕсли;
				
			Если НеобходимаАктуализацияПоследнейЗаписи Тогда
				НЗ_ИсточникПП.Записать();
			КонецЕсли;
...
#Область РаботаСЗагрузкойИзФайлов 

Функция КоличествоСтрокВФайле(Адрес, Расширение) Экспорт
...
Функция ПрочитатьДанныеФайла(Адрес, СтрокаНачала=0, КолВоКолонокФайла=0, Расширение="xlsx") Экспорт
