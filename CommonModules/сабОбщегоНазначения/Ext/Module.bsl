...
Процедура ОтобразитьСостояниеДокумента(Форма, ТекущийОбъект = Неопределено) Экспорт
	...
	Если Не ТекущийОбъект = Неопределено Тогда
		Если БюджетныйНаСервере.ЕстьСвойствоОбъекта(Форма, "ДокументБУ") И ЗначениеЗаполнено(Форма.ДокументБУ) Тогда
			...
			//ОбновленнаяЗапись.ДокументБУ = Форма.ДокументБУ;
			//ОбновленнаяЗапись.ДокументУУ = ТекущийОбъект.Ссылка;
			...
			//Если БюджетныйНаСервере.ЕстьСвойствоОбъекта(Форма, "АвтообновленияЗаблокированы") Тогда
			//	ОбновленнаяЗапись.АвтообновленияЗаблокированы = Форма.АвтообновленияЗаблокированы;
			//КонецЕсли;
...
Процедура сабПлатКалПриЗаписиОбъектовПриЗаписи(Источник, Отказ) Экспорт
	...
	ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.УЧ_Реализация") И ФормироватьСтрокиГрафикаОплат Тогда
		Если (Источник.ДополнительныеСвойства.Свойство("Модифицирован") И Источник.ДополнительныеСвойства.Модифицирован) ИЛИ (Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") И Источник.ДополнительныеСвойства.ЭтоНовый) Тогда
			...
			Запрос.УстановитьПараметр("ДокументУУ", Источник.Ссылка);
			Результат = Запрос.Выполнить();
			Выборка = Результат.Выбрать();
			...
			НоваяСтрока.Документ = Источник.Ссылка;
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Источник);
			НоваяСтрока.Сумма = ?(НЕ Источник.УчитыватьНДС ИЛИ Источник.СуммаВключаетНДС, Источник.Товары.Итог("Сумма") + Источник.Услуги.Итог("Сумма"), Источник.Товары.Итог("Сумма") + Источник.Товары.Итог("СуммаНДС") + Источник.Услуги.Итог("Сумма") + Источник.Услуги.Итог("СуммаНДС"));
			НоваяСтрока.СуммаНДС = Источник.Товары.Итог("СуммаНДС") + Источник.Услуги.Итог("СуммаНДС");
			Если Источник.Товары.Количество() Тогда
				НоваяСтрока.СтавкаНДС = Источник.Товары[0].СтавкаНДС;
			КонецЕсли;
	...
	ИначеЕсли ТипЗнч(Источник.Ссылка) = Тип("ДокументСсылка.УЧ_ПоступлениеТоваров") И ФормироватьСтрокиГрафикаОплат Тогда
		Если (Источник.ДополнительныеСвойства.Свойство("Модифицирован") И Источник.ДополнительныеСвойства.Модифицирован) ИЛИ (Источник.ДополнительныеСвойства.Свойство("ЭтоНовый") И Источник.ДополнительныеСвойства.ЭтоНовый) Тогда
			...
			НоваяСтрока.Документ = Источник.Ссылка;
			ЗаполнитьЗначенияСвойств(НоваяСтрока, Источник);
			НоваяСтрока.Сумма = ?(НЕ Источник.УчитыватьНДС ИЛИ Источник.СуммаВключаетНДС, Источник.ТабличнаяЧасть.Итог("Сумма") + Источник.Услуги.Итог("Сумма"), Источник.ТабличнаяЧасть.Итог("Сумма") + Источник.ТабличнаяЧасть.Итог("СуммаНДС") + Источник.Услуги.Итог("Сумма") + Источник.Услуги.Итог("СуммаНДС"));
			НоваяСтрока.СуммаНДС = Источник.ТабличнаяЧасть.Итог("СуммаНДС") + Источник.Услуги.Итог("СуммаНДС");
			Если Источник.ТабличнаяЧасть.Количество() Тогда
				НоваяСтрока.СтавкаНДС = Источник.ТабличнаяЧасть[0].СтавкаНДС;
			КонецЕсли;
			Если Источник.Услуги.Количество() Тогда
				НоваяСтрока.СтавкаНДС = Источник.Услуги[0].СтавкаНДС;
			КонецЕсли;
