...
Функция УУ_ТабличныйДокументУПД(Макет, МассивОбъектов, ОбъектыПечати, ТабДокумент, ТекстЗапросаДокументам, ТолькоПередаточныйДокумент, ПараметрыПечати, ПараметрыВывода)
	...
	Для Каждого ВыборкаУПД ИЗ ДанныеУниверсальныхПередаточныхДокументов Цикл
		...
		//Изменено
		Если ТипЗнч(ВыборкаУПД.Ссылка) = ТипЗнч(Документы.РеализацияТоваровУслуг.ПустаяСсылка()) Тогда
			ДокументСсылка = ВыборкаУПД.Ссылка; 
		ИначеЕсли ТипЗнч(ВыборкаУПД.Ссылка) = ТипЗнч(Документы.СчетФактураВыданный.ПустаяСсылка()) Тогда
			ДокументСсылка = ВыборкаУПД.Ссылка.ДокументОснование;
		КонецЕсли;
		Если Не ПустаяСтрока(ДокументСсылка) И (ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.РеализацияТоваровУслуг") ИЛИ ТипЗнч(ДокументСсылка) = Тип("ДокументСсылка.КорректировкаРеализации")) Тогда
			Если НЕ ПустаяСтрока(ДокументСсылка.ЗаРуководителяНаОсновании) Тогда
				ВыборкаУПД.ДанныеШапки.ФИОРуководителя = ДокументСсылка.ЗаРуководителяНаОсновании.Должность.Наименование+" "+ДокументСсылка.ЗаРуководителяНаОсновании.ФизическоеЛицо.Фамилия+" "+ДокументСсылка.ЗаРуководителяНаОсновании.ФизическоеЛицо.Инициалы+" "+ДокументСсылка.ЗаРуководителяНаОсновании.Наименование;
			КонецЕсли;
			Если НЕ ПустаяСтрока(ДокументСсылка.ЗаГлавногоБухгалтераНаОсновании) Тогда
				ВыборкаУПД.ДанныеШапки.ФИОГлавногоБухгалтера = ДокументСсылка.ЗаГлавногоБухгалтераНаОсновании.Должность.Наименование+" "+ДокументСсылка.ЗаГлавногоБухгалтераНаОсновании.ФизическоеЛицо.Фамилия+" "+ДокументСсылка.ЗаГлавногоБухгалтераНаОсновании.ФизическоеЛицо.Инициалы+" "+ДокументСсылка.ЗаГлавногоБухгалтераНаОсновании.Наименование;
			КонецЕсли;
			//Если НЕ ПустаяСтрока(ДокументСсылка.ОтветственныйЗаОформление) Тогда
			//	ВыборкаУПД.ДанныеШапки.ФИООтветственного = ВыборкаУПД.ДанныеШапки.ДолжностьОтветственного.Наименование+" "+ДокументСсылка.ОтветственныйЗаОформление.Фамилия+" "+ДокументСсылка.ОтветственныйЗаОформление.Инициалы;
			//	ВыборкаУПД.ДанныеШапки.ДолжностьОтветственного = "";
			//КонецЕсли;
		КонецЕсли;
		//Изменено
