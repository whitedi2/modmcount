Функция ПодобратНоменклатуруПоШК(ШтрихКод) Экспорт
	...
	|	РегистрСведений.ШтрихкодыНоменклатуры КАК ШтрихкодыНоменклатуры
	|ГДЕ
	|	ПОДСТРОКА(ШтрихкодыНоменклатуры.Штрихкод, 1, 7) = &Штрихкод";
...
Процедура РаспределитьПроводкиПоДокументамОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	Для каждого ТекСтрока Из ВремТЧ Цикл
		...
		Если ЭтоОплата И ЗначениеЗаполнено(ТекСтрока["Субконто" + Префикс + "3"]) Тогда //если принудительно субконто3 выбрано, тогда пропускаем
		...
		Для каждого ТекОстаток Из НайденныеОстаткиИсход Цикл
			...
			Если НЕ ТекОстаток["СуммаОстаток" + Префикс] = Выборка[ИндексСтроки]["СуммаОстаток" + Префикс] И (ЭтоОплата ИЛИ Не ЗначениеЗаполнено(ТекОстаток.Субконто3)) Тогда
				НоваяСтрока = НоваяТЧ.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
				НоваяСтрока["ВидСубконто" + Префикс + "3"] = ?(НЕ ЭтоОплата Или ЭтоСторно, Неопределено, СубконтоДокумент);
				НоваяСтрока["Субконто" + Префикс + "3"] = ?(НЕ ЭтоОплата Или ЭтоСторно, Неопределено, ТекОстаток.Субконто3);
				...
				ТекСтрока.Сумма = ТекСтрока.Сумма - ?(НЕ ЭтоОплата Или ЭтоСторно, 0, НоваяСтрока.Сумма);
				ТекОстаток["СуммаОстаток" + Префикс] = Выборка[ИндексСтроки]["СуммаОстаток" + Префикс];
			КонецЕсли;
		КонецЦикла;
		
		Если ЭтоОплата И СуммаОбщая Тогда //сумма осталась, кидаем на аванс
			НоваяСтрока = НоваяТЧ.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
	...
	Для каждого ТекСтрока Из НоваяТЧ Цикл
		НоваяСтрока = ВремТЧ.Добавить();
		ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
		Если НЕ ЭтоОплата Или ЭтоСторно Тогда //тогда сторнируем
