...
Процедура РаспределитьПроводкиПоДокументамОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	Для каждого ТекСтрока Из ВремТЧ Цикл
		...
		Если СуммаОбщая = 0 И НайденныеОстатки.Количество() = 1 И Не ЗначениеЗаполнено(НайденныеОстатки[0].Субконто3) Тогда //не раскладываем зачет аванса по документам, пропускаем
			ТекСтрока["Субконто" + Префикс + "3"] = Неопределено;
			Продолжить;		
		КонецЕсли;
