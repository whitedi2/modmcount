...
Функция ПолучитьСозданныеДокументыНаОСновании(ДокументОснование, СсылкаТекущего = Неопределено, ПроверяемыйТип = Неопределено) Экспорт
	...
	|	И УЧ_ВозвратТоваровПоставщику.ПометкаУдаления = ЛОЖЬ
	|	И ВЫБОР
	...
	|	И УЧ_Возврат.ПометкаУдаления = ЛОЖЬ
	|	И ВЫБОР
	|			КОГДА &СсылкаТекущего = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ НЕ УЧ_Возврат.Ссылка = &СсылкаТекущего
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА НЕ &ПроверятьТип
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ТИПЗНАЧЕНИЯ(УЧ_Возврат.Ссылка) = &ПроверяемыйТип
	|		КОНЕЦ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|ГДЕ
	|	ЗаказНаВозвратОтКлиента.ПометкаУдаления = ЛОЖЬ
	|	И ВЫБОР
	|			КОГДА &СсылкаТекущего = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ НЕ ЗаказНаВозвратОтКлиента.Ссылка = &СсылкаТекущего
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА НЕ &ПроверятьТип
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ТИПЗНАЧЕНИЯ(ЗаказНаВозвратОтКлиента.Ссылка) = &ПроверяемыйТип
	|		КОНЕЦ
	|	И ЗаказНаВозвратОтКлиента.ДокументОснование = &ДокОснование
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ЗаказНаПроизводство.ПометкаУдаления = ЛОЖЬ
	|	И ВЫБОР
	|			КОГДА &СсылкаТекущего = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ НЕ ЗаказНаПроизводство.Ссылка = &СсылкаТекущего
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА НЕ &ПроверятьТип
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ТИПЗНАЧЕНИЯ(ЗаказНаПроизводство.Ссылка) = &ПроверяемыйТип
	|		КОНЕЦ
	|	И ЗаказНаПроизводство.ДокОснование = &ДокОснование
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|			КОГДА &СсылкаТекущего = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ НЕ ЗаказПоставщику.Ссылка = &СсылкаТекущего
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА НЕ &ПроверятьТип
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ТИПЗНАЧЕНИЯ(ЗаказПоставщику.Ссылка) = &ПроверяемыйТип
	|		КОНЕЦ
	|	И ЗаказПоставщику.ДокОснование = &ДокОснование
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	|ВЫБРАТЬ
	...
	|ГДЕ
	|	ПеремещениеТоваров.Заказ = &ДокОснование
	|	И ПеремещениеТоваров.ПометкаУдаления = ЛОЖЬ
	|	И ВЫБОР
	|			КОГДА &СсылкаТекущего = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ НЕ ПеремещениеТоваров.Ссылка = &СсылкаТекущего
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА НЕ &ПроверятьТип
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ТИПЗНАЧЕНИЯ(ПеремещениеТоваров.Ссылка) = &ПроверяемыйТип
	|		КОНЕЦ
	|
	|ОБЪЕДИНИТЬ ВСЕ
	|
	...
	|ГДЕ
	|	ТребованиеНакладная.Заказ = &ДокОснование
	|	И ТребованиеНакладная.ПометкаУдаления = ЛОЖЬ
	|	И ВЫБОР
	|			КОГДА &СсылкаТекущего = НЕОПРЕДЕЛЕНО
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ НЕ ТребованиеНакладная.Ссылка = &СсылкаТекущего
	|		КОНЕЦ
	|	И ВЫБОР
	|			КОГДА НЕ &ПроверятьТип
	|				ТОГДА ИСТИНА
	|			ИНАЧЕ ТИПЗНАЧЕНИЯ(ТребованиеНакладная.Ссылка) = &ПроверяемыйТип
	|		КОНЕЦ";
	...
	Для каждого ТекТип Из МассивТипов Цикл
		...
		Для Каждого ТекСтрока Из Продажи Цикл
			...
			Если НЕ ЗначениеЗаполнено(НайденнаяНоменклатура) Тогда
				...
				НовНоменклатура.Код = ТекСтрока.Артикул;
				НовНоменклатура.Родитель = ГруппаНеопознаннойНоменклатуры;
				НовНоменклатура.ТипНоменклатуры = Перечисления.сабТипыНоменклатуры.Товар;
...
Процедура РаспределитьСуммыПоГрафикуДоговора(Движения, Источник) Экспорт
	...
	Если ТипЗнч(Источник.Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") И Источник.Договор.сабГрафикОплаты.Количество() Тогда
		...
		ЭтоРеализация = ТипЗнч(Источник) = Тип("ДокументОбъект.УЧ_Реализация");
		
		ТЧДвижений0 = Движения.Учетный.Выгрузить();
		
		ТЧДвижений = Движения.Учетный.Выгрузить();
		ТЧДвижений.Очистить();
		
		СчетКонтрагента = ?(ЭтоРеализация, Источник.Счет, Источник.СчетКонтрагента); 
		
		Для каждого ТекСтрока Из ТЧДвижений0 Цикл
			НоваяСтрока = ТЧДвижений.Добавить();			
		...
		ТЧДвижений0.Очистить();
		
		ИтогоПоГрафику = Источник.Договор.сабГрафикОплаты.Итог("Сумма");
		
		Если ИтогоПоГрафику Тогда
			...
			Если Не ВыборкаСтрокиГрафика.Количество() Тогда
				Для каждого ТекСтрокаГрафика Из Источник.Договор.сабГрафикОплаты Цикл
					НоаяСтрокаГрафика = Справочники.сабСтрокиГрафиковДоговоров.СоздатьЭлемент();
					ЗаполнитьЗначенияСвойств(НоаяСтрокаГрафика, Источник.Ссылка);
					ЗаполнитьЗначенияСвойств(НоаяСтрокаГрафика, ТекСтрокаГрафика);
					НоаяСтрокаГрафика.Документ = Источник.Ссылка;
					НоаяСтрокаГрафика.Владелец = Источник.Договор;
					НоаяСтрокаГрафика.Записать();
					
					НоваяСтрокТЗ = ВыборкаСтрокиГрафика.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокТЗ, НоаяСтрокаГрафика);					
				КонецЦикла;
			КонецЕсли;
			
			Для каждого ТекДвижение Из ТЧДвижений Цикл
				Если ТекДвижение.СчетКт = СчетКонтрагента ИЛИ ТекДвижение.СчетДт  = СчетКонтрагента Тогда
					ИндексСтроки = 0;
					СуммаДвижения = ТекДвижение.Сумма;
					ТекСумма = 0;
					НоваяСтрока = ТЧДвижений0.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекДвижение);
					Для каждого ТекСтрокаГрафика Из Источник.Договор.сабГрафикОплаты Цикл
						Если ИндексСтроки = 0 Тогда
							НоваяСтрока.Сумма = ТекСтрокаГрафика.Сумма / ИтогоПоГрафику * СуммаДвижения;
							ТекСумма = ТекСумма + НоваяСтрока.Сумма;
						Иначе
							НоваяСтрока = ТЧДвижений0.Добавить();
							ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекДвижение);
						...
						КонецЕсли;
						Если ЭтоРеализация Тогда
							НоваяСтрока.СубконтоДт3 = ВыборкаСтрокиГрафика[ИндексСтроки].Ссылка;							
						Иначе	
							НоваяСтрока.СубконтоКт3 = ВыборкаСтрокиГрафика[ИндексСтроки].Ссылка;							
						КонецЕсли;
						ИндексСтроки = ИндексСтроки + 1;
					КонецЦикла;
					НоваяСтрока.Сумма = НоваяСтрока.Сумма + (СуммаДвижения - ТекСумма); //копейки
				КонецЕсли;
			КонецЦикла;	
			Движения.Учетный.Загрузить(ТЧДвижений0);
		КонецЕсли;
...
Процедура РаспределитьПроводкиПоДокументамОбработкаПроведения(Источник, Отказ, РежимПроведения) Экспорт
	...
	Если ТипЗнч(Источник) = Тип("ДокументОбъект.УЧ_Реализация") Тогда
		Если Источник.ВидОперации = Перечисления.ВидыОперацийРеализация.ZОтчет Тогда
			...
			Для каждого ТекСтрока Из Источник.Товары Цикл
				Если (ЗначениеЗаполнено(ТекСтрока.Контрагент) И НЕ ТекСтрока.Контрагент = Источник.Контрагент) Или ЗначениеЗаполнено(Источник.Контрагент) Тогда
					ЕстьКонтрики = Истина;
				КонецЕсли;
			КонецЦикла;
	...
	Для каждого ТекСтрока Из ВремТЧ Цикл
		...
		Для каждого ТекОстаток Из НайденныеОстатки Цикл
			СуммаПоследнийОстаток = ТекОстаток["СуммаОстаток" + Префикс];
			Если СуммаОбщая > ТекОстаток["СуммаОстаток" + Префикс] Тогда
				СуммаОбщая = СуммаОбщая - ТекОстаток["СуммаОстаток" + Префикс];
				ТекОстаток["СуммаОстаток" + Префикс] = 0;
			Иначе
		...
		Если (СуммаОбщая = 0 Или ЭтоОплата) И НайденныеОстатки.Количество() = 1 И Не ЗначениеЗаполнено(НайденныеОстатки[0].Субконто3) Тогда //не раскладываем зачет аванса по документам, пропускаем
			ТекСтрока["Субконто" + Префикс + "3"] = Неопределено;
			Продолжить;		
		КонецЕсли;
		...
		Для каждого ТекОстаток Из НайденныеОстаткиИсход Цикл
			...
			Если НЕ ТекОстаток["СуммаОстаток" + Префикс] = Выборка[ИндексСтроки]["СуммаОстаток" + Префикс] И ((ЭтоОплата ИЛИ НужноПринудительноеРаспределение) ИЛИ Не ЗначениеЗаполнено(ТекОстаток.Субконто3)) Тогда
				...
				Если ИндексСтрокиОстатков = 0 Тогда
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
				Иначе
					ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока, ,"КоличествоДт, КоличествоКт");				
				КонецЕсли;				
				...
				КонецЕсли;
				НоваяСтрока.Сумма = ?((НЕ (ЭтоОплата ИЛИ НужноПринудительноеРаспределение) Или ЭтоСторно) И СуммаОбщая, СуммаПоследнийОстаток, ТекОстаток["СуммаОстаток" + Префикс] - Выборка[ИндексСтроки]["СуммаОстаток" + Префикс]);
				...
				КонецЕсли;
				Распределено = Истина;
				ИндексСтрокиОстатков = ИндексСтрокиОстатков + 1;
			КонецЕсли;
		КонецЦикла;
		
		Если (ЭтоОплата ИЛИ НужноПринудительноеРаспределение) И СуммаОбщая И Распределено Тогда //сумма осталась, кидаем на аванс
			НоваяСтрока = НоваяТЧ.Добавить();
			Если ИндексСтрокиОстатков = 0 Тогда
				ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
			Иначе
	...
	Для каждого ТекУдСтрока Из МассивУдаляемыхСтрок Цикл
		ВремТЧ.Удалить(ТекУдСтрока);
	КонецЦикла;
...
Функция ПроверкаСозданияНаОснованииНаСервере(ДанныеЗаполнения, Комментарий, СтандартнаяОбработка, ПроверяемыйТип = Неопределено) Экспорт
	...
	Если СозданныеДокументы.Количество() Тогда
		...
		СписокДокументов = "
		|";
		Для каждого ТекДок Из СозданныеДокументы Цикл
			СписокДокументов = СписокДокументов + Строка(ТекДок.Ссылка) + "
			|";		
		КонецЦикла;
		Возврат Новый Структура("Признак, Ссылка, СозданныеДокументы, СписокДокументов", Признак, СозданныеДокументы[0].Ссылка, СозданныеДокументы, СписокДокументов);		
	КонецЕсли;
	...
	Если НЕ ДанныеЗаполнения.Проведен Тогда
		...
		Признак = "##НеПроведен"; 
		//Комментарий = Признак;
		Возврат Новый Структура("Признак, Ссылка, СозданныеДокументы", Признак);			
	КонецЕсли;
	
	Возврат Новый Структура("Признак, Ссылка, СозданныеДокументы", "");
...
Функция ЗаполнитьКешНаСервере(ДатаНачала, ДатаКонца, Магазин) Экспорт
	МассивСтруктур = Новый Массив;	
	...
	Пока ТекДата <= ДатаКонца Цикл
		...
		Пока Выборка.Следующий() Цикл
			...
			Выборка.Счет, Выборка.Номенклатура, Выборка.Склад, Выборка.Предприятие, Неопределено, Выборка.Сумма, Выборка.Количество, ТекДата));  
		КонецЦикла;
...
Процедура ЗаписатьОстатки(Выборка) Экспорт
...
Процедура ПерезаполнитьТЧСерииНоменклатурыПередЗаписью(Товары,СерииНоменклатуры,ДляБух = Ложь,ИмяНоменклатура = "Номенклатура") Экспорт
	
	//для корректировки поступления
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("КоличествоДоИзменения",Метаданные.НайтиПоТипу(ТипЗнч(СерииНоменклатуры))) Тогда
		ЕстьКоличествоДоИзменения = Истина;
	Иначе
		ЕстьКоличествоДоИзменения = Ложь;
	КонецЕсли;
	МассивУдаляемыхСтрок = Новый Массив;
	Если ДляБух Тогда 
		ТЗТовары = Товары.Выгрузить(,"Номенклатура, НомерСтроки,СерияНоменклатуры"); 
	Иначе
		ТЗТовары = Товары.Выгрузить(,ИмяНоменклатура + ",НомерСтроки,НесколькоСерий");
	КонецЕсли;
	Для Каждого СтрокаТЧСерии Из СерииНоменклатуры Цикл
		Если ДляБух Тогда
			СтруктураПоиска = Новый Структура("Номенклатура, НомерСтроки,СерияНоменклатуры",СтрокаТЧСерии.Номенклатура,СтрокаТЧСерии.НомерСтрокиРеализации,Справочники.СерииНоменклатуры.ПустаяСсылка());
		Иначе
			СтруктураПоиска = Новый Структура(ИмяНоменклатура + ",НомерСтроки, НесколькоСерий",СтрокаТЧСерии.Номенклатура,СтрокаТЧСерии.НомерСтрокиРеализации,Истина);
		КонецЕсли;	
		НайденныеСтрокиОсновнаяТЧ = ТЗТовары.НайтиСтроки(СтруктураПоиска);
	...
	Для Каждого СтрокаМассива Из МассивУдаляемыхСтрок Цикл
		СерииНоменклатуры.Удалить(СтрокаМассива);	
	КонецЦикла;
	Для Каждого СтрокаТЧТовары Из Товары Цикл
		Если ДляБух Тогда
			Если ЗначениеЗаполнено(СтрокаТЧТовары.СерияНоменклатуры) Тогда
				...
				//для корректировки поступления
				Если ЕстьКоличествоДоИзменения Тогда
					НоваяСтрокаСерий.КоличествоДоИзменения = СтрокаТЧТовары.КоличествоДоИзменения;
				КонецЕсли;
			Иначе	
				СтруктураПоискаДляКоличества = Новый Структура("Номенклатура,НомерСтрокиРеализации",СтрокаТЧТовары.Номенклатура,СтрокаТЧТовары.НомерСтроки);
				МассивСтрокДляКоличества = СерииНоменклатуры.НайтиСтроки(СтруктураПоискаДляКоличества);
				
				Если МассивСтрокДляКоличества.Количество() > 0 Тогда
					ОбщееКоличествоНесколькихСерий = 0;
					//для корректировки поступления
					//Если ЕстьКоличествоДоИзменения Тогда
					//	ОбщееКоличествоДоИзмененияНесколькихСерий = 0; 
					//КонецЕсли;
					
					Для Каждого СтрМассиваДляКоличества Из МассивСтрокДляКоличества Цикл 
						ОбщееКоличествоНесколькихСерий = ОбщееКоличествоНесколькихСерий + СтрМассиваДляКоличества.Количество; 
						//для корректировки поступления
						//Если ЕстьКоличествоДоИзменения Тогда
						//	ОбщееКоличествоДоИзмененияНесколькихСерий = ОбщееКоличествоДоИзмененияНесколькихСерий + СтрМассиваДляКоличества.КоличествоДоИзменения; 
						//КонецЕсли;
					КонецЦикла;
					...
					Если СтрокаТЧТовары.Количество < ОбщееКоличествоНесколькихСерий Тогда 
						...
						Если ЕстьКоличествоДоИзменения Тогда
							...
							//МассивУдаляемыхСтрокКоличества = Новый Массив;
							...
							//Для Каждого СтрМассиваДляКоличества Из МассивСтрокДляКоличества Цикл
							//	Если Разница >= СтрМассиваДляКоличества.Количество Тогда
							//		КоличествоВычитаемое = СтрМассиваДляКоличества.Количество;
							//		
							//		Разница = Разница - СтрМассиваДляКоличества.Количество;
							//		СтрМассиваДляКоличества.Количество = 0;
							//	Иначе 
							//		СтрМассиваДляКоличества.Количество = СтрМассиваДляКоличества.Количество - Разница;
							//		Прервать;
							//	КонецЕсли;								
							//КонецЦикла; 
						Иначе
							...
							Для Каждого СтрМассиваДляКоличества Из МассивСтрокДляКоличества Цикл
								Если Разница >= СтрМассиваДляКоличества.Количество Тогда
									МассивУдаляемыхСтрокКоличества.Добавить(СтрМассиваДляКоличества);
									Разница = Разница - СтрМассиваДляКоличества.Количество;
								Иначе 
									СтрМассиваДляКоличества.Количество = СтрМассиваДляКоличества.Количество - Разница;
									Прервать;
								КонецЕсли;								
							КонецЦикла; 
							Для Каждого СтрДляУдаления Из МассивУдаляемыхСтрокКоличества Цикл
								СерииНоменклатуры.Удалить(СтрДляУдаления);
							КонецЦикла;
							МассивСтрокДляПроверкиНесколькихСерий = СерииНоменклатуры.НайтиСтроки(Новый Структура("Номенклатура,НомерСтрокиРеализации",СтрокаТЧТовары.Номенклатура,СтрокаТЧТовары.НомерСтроки));	
							Если МассивСтрокДляПроверкиНесколькихСерий.Количество() = 1 Тогда
								СтрокаТЧТовары.СерияНоменклатуры = МассивСтрокДляПроверкиНесколькихСерий[0].СерияНоменклатуры;
							КонецЕсли;
						КонецЕсли;
					КонецЕсли;	
				КонецЕсли;
		...
		Иначе
			Если Не СтрокаТЧТовары.НесколькоСерий Тогда
				...
				//	НоваяСтрокаСерий.Номенклатура = СтрокаТЧТовары.Товар;	
				//Иначе
					НоваяСтрокаСерий.Номенклатура = СтрокаТЧТовары[ИмяНоменклатура];
				//КонецЕсли;
				НоваяСтрокаСерий.ДатаПроизводства = СтрокаТЧТовары.СерияНоменклатуры.ДатаПроизводства;
				НоваяСтрокаСерий.СерияНоменклатуры = СтрокаТЧТовары.СерияНоменклатуры;
				НоваяСтрокаСерий.Количество = СтрокаТЧТовары.Количество;
			...
			Иначе
				//Если ЭтоСписаниеТоваров Тогда
				//	СтруктураПоискаДляКоличества = Новый Структура("Номенклатура,НомерСтрокиРеализации",СтрокаТЧТовары.Товар,СтрокаТЧТовары.НомерСтроки);
				//Иначе
					СтруктураПоискаДляКоличества = Новый Структура("Номенклатура,НомерСтрокиРеализации",СтрокаТЧТовары[ИмяНоменклатура],СтрокаТЧТовары.НомерСтроки);
					//КонецЕсли;	
					МассивСтрокДляКоличества = СерииНоменклатуры.НайтиСтроки(СтруктураПоискаДляКоличества);
	...
	Запрос.Текст = 
		"ВЫБРАТЬ
		|	ПоступлениеТоваровУслугСерииНоменклатуры.НомерСтрокиРеализации КАК НомерСтрокиРеализации,
		|	ПоступлениеТоваровУслугСерииНоменклатуры.СерияНоменклатуры КАК СерияНоменклатуры,
		|	ПоступлениеТоваровУслугСерииНоменклатуры.Номенклатура КАК Номенклатура
		|ПОМЕСТИТЬ ВТСерииНоменклатуры
		|ИЗ
		|	&ТЧСерииНоменклатуры КАК ПоступлениеТоваровУслугСерииНоменклатуры
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ВТСерииНоменклатуры.НомерСтрокиРеализации КАК НомерСтрокиРеализации
		|ИЗ
		|	ВТСерииНоменклатуры КАК ВТСерииНоменклатуры
		|ГДЕ
		|	ВТСерииНоменклатуры.СерияНоменклатуры.Владелец <> ВТСерииНоменклатуры.Номенклатура";
	
	Запрос.УстановитьПараметр("ТЧСерииНоменклатуры",СерииНоменклатуры.Выгрузить());
	РезультатЗапроса = Запрос.Выполнить();
	
	Выборка = РезультатЗапроса.Выбрать();
	
	Пока Выборка.Следующий() Цикл
		Товары[Выборка.НомерСтрокиРеализации-1].СерияНоменклатуры = Справочники.СерииНоменклатуры.ПустаяСсылка();
		СтрокиСерии = СерииНоменклатуры.НайтиСтроки(Новый Структура("НомерСтрокиРеализации",Выборка.НомерСтрокиРеализации));
		Для каждого СтрокаСерии Из СтрокиСерии Цикл
		   СерииНоменклатуры.Удалить(СтрокаСерии);
		КонецЦикла;
	КонецЦикла;
	КонецЕсли;
...
Процедура СинхронизацияСерийПриСозданииФормы(Товары,СерииНоменклатуры,ИмяНоменклатура = "Номенклатура") Экспорт
	...
	Для Каждого СтрокаТЧ Из Товары Цикл
		...
		//Иначе
			НайденныеСерии = СерииНоменклатуры.НайтиСтроки(Новый Структура("Номенклатура, НомерСтрокиРеализации", СтрокаТЧ[ИмяНоменклатура], СтрокаТЧ.НомерСтроки));
		//КонецЕсли;
...
Процедура СинхронизацияСерийПриЗаписиНаСервереФормы(ПараметрыЗаписи,Форма,ТекущийОбъект,Отказ,ДокументСоздан,ТекстОшибки) Экспорт
	...
	Если ПараметрыЗаписи.Свойство("СинхронизироватьСерииНоменклатурыСДокументомБух") Тогда
		Если Не Отказ И ПараметрыЗаписи.СинхронизироватьСерииНоменклатурыСДокументомБух	Тогда
			...
			ДокументБУОбъект = Форма.ДокументБУ.ПолучитьОбъект();
		Если ОбщегоНазначенияБП.ЕстьТабЧастьДокумента("СерииНоменклатуры",ДокументБУОбъект.Метаданные())Тогда 
			...
			Попытка
				...
				ДокументБУОбъект.ДополнительныеСвойства.Вставить("ЭтоСинхронизацияСерийПриПроведенииУпрДокумента", Истина);
				ДокументБУОбъект.ДополнительныеСвойства.Вставить("СчитатьРеквизитыПроверенными", Истина); // добавлено для пропуска проверки на закрытый период
...
Процедура СинхронизацияСерийПослеЗаписиНаСервереФормы(Товары,СерииНоменклатуры,ИмяНоменклатура = "Номенклатура") Экспорт
...
Процедура СинхронизацияНомераСтрокиОсновнойТЧССериямиБух(ОсновнаяТЧ,ИсходнаяТЧ,СерииНоменклатуры, ДокОбъект) Экспорт
	
	Если ОбщегоНазначенияБП.ЕстьРеквизитТабЧастиДокумента("ИдентификаторСтроки",ДокОбъект.Метаданные(),Метаданные.НайтиПоТипу(ТипЗнч(ОсновнаяТЧ)).Имя) Тогда
		Для Каждого СтрокаТЧ Из ОсновнаяТЧ Цикл	
			СтруктураПоиска = Новый Структура("ИдентификаторСтроки,Номенклатура",СтрокаТЧ.ИдентификаторСтроки,СтрокаТЧ.Номенклатура); 
			МассивНайденныхСтрок = ИсходнаяТЧ.НайтиСтроки(СтруктураПоиска);
			Для Каждого НайденнаяСтрокаИзМассива Из МассивНайденныхСтрок Цикл
				Если СтрокаТЧ.НомерСтроки <> НайденнаяСтрокаИзМассива.НомерСтроки Тогда
					СтруктураПоискаСерии = Новый Структура("НомерСтрокиРеализации,Номенклатура", НайденнаяСтрокаИзМассива.НомерСтроки,НайденнаяСтрокаИзМассива.Номенклатура);
					МассивСтрокСерииНоменклатура = СерииНоменклатуры.НайтиСтроки(СтруктураПоискаСерии);
					Для Каждого НайденнаяСтрокаСерииНоменклатуры Из МассивСтрокСерииНоменклатура Цикл
						НайденнаяСтрокаСерииНоменклатуры.НомерСтрокиРеализации = СтрокаТЧ.НомерСтроки;
					КонецЦикла;
				КонецЕсли;
			КонецЦикла;
		КонецЦикла; 
...
Процедура ОчиститьСериюВОсновнойТЧДляНесколькихСерий(ТЧОбъект,ТЧТекОбъект) Экспорт
	
	Для каждого СтрОсновняТЧ Из ТЧОбъект Цикл
		Если СтрОсновняТЧ.НесколькоСерий Тогда 
			ТЧТекОбъект[ТЧОбъект.Индекс(СтрОсновняТЧ)].СерияНоменклатуры = Справочники.СерииНоменклатуры.ПустаяСсылка();
		КонецЕсли;
	КонецЦикла;
...
Функция НомерЗаказаПоЦепочкеДокументов(ТекущийДокументОбъект) Экспорт
	
	НомерЗаказа = "";
	
	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("Заказ", ТекущийДокументОбъект.Метаданные()) И ЗначениеЗаполнено(ТекущийДокументОбъект.Заказ) Тогда
		НомерЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийДокументОбъект.Заказ, "Номер");
	ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("ДокОснование", ТекущийДокументОбъект.Метаданные()) Тогда
		
		Если ТипЗнч(ТекущийДокументОбъект.ДокОснование) = Тип("ДокументСсылка.ЗаказКлиента") И ЗначениеЗаполнено(ТекущийДокументОбъект.ДокОснование) Тогда 
			НомерЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийДокументОбъект.ДокОснование, "Номер");
		КонецЕсли;
		
	ИначеЕсли ОбщегоНазначения.ЕстьРеквизитОбъекта("ДокументОснование", ТекущийДокументОбъект.Метаданные()) Тогда 
		
		Если ЗначениеЗаполнено(ТекущийДокументОбъект.ДокументОснование) Тогда
			
			Если ТипЗнч(ТекущийДокументОбъект.ДокументОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда 
				НомерЗаказа = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийДокументОбъект.ДокументОснование, "Номер"); 
			ИначеЕсли ТипЗнч(ТекущийДокументОбъект.ДокументОснование) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда 
				Заказ = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекущийДокументОбъект.ДокументОснование, "Заказ"); 
