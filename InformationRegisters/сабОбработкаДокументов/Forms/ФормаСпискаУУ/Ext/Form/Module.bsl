...
Процедура ПринятьИзмененияНаСервере()
	...
	ВыделенныеСтроки = Элементы.СписокИзмененные.ВыделенныеСтроки;
	МассивСтрок = Новый Массив();
	Для каждого ИдентификаторСтрокиСписка Из ВыделенныеСтроки Цикл
		МассивСтрок.Добавить(ИдентификаторСтрокиСписка);
	КонецЦикла;
	
	Для каждого ТекОбр Из МассивСтрок Цикл
		Попытка
			ТекОбъект = ТекОбр.ДокументБУ.ПолучитьОбъект();
			ТекОбъект.ОбработкаЗаполненияСФормы(ТекОбр.ДокументУУ, Истина);
