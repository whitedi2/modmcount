...
Процедура ОбработатьДокументыУУНаСервере(ТекДокументСтруктура, МассивСтруктурВозврата = Неопределено)
	
	РегистрыСведений.сабОбработкаДокументов.ОбработатьДокументыУУНаСервере(ТекДокументСтруктура, МассивСтруктурВозврата);
...
Процедура СоздатьНаОсновании(Команда)
	...
	Для каждого ТекОбр Из ДокументыКОбработке Цикл
		...
		ОбработатьДокументыУУНаСервере(ТекОбр, МассивСтруктурДанных);
		
		Для Каждого ТекЭлементМассиваСтруктурДанных Из МассивСтруктурДанных Цикл
			
			Если ТекЭлементМассиваСтруктурДанных.Свойство("Ссылка") Тогда
				ТекОбр.Ссылка = ТекЭлементМассиваСтруктурДанных.Ссылка;
			КонецЕсли;
			
			Если НЕ ТекЭлементМассиваСтруктурДанных.Количество() Тогда
				Прервать;		
			КонецЕсли;
			
			ДопПараметр = Новый Структура("ДокументУУ", УУСсылка); 
			ТекФормаУУ = ПолучитьФорму("Документ." + ТекЭлементМассиваСтруктурДанных.ИмяФормы +".ФормаОбъекта", Новый Структура("Основание, ДокументУУ, ЗначенияЗаполнения, ВидОперации", ТекОбр.Ссылка, УУСсылка, ТекЭлементМассиваСтруктурДанных, ТекЭлементМассиваСтруктурДанных.ВидОперации));
...
Процедура ПринятьИзмененияНаСервере(ТекОбр)
	...
	Попытка
		...
		Если ОбщегоНазначенияКлиентСервер.ЕстьРеквизитИлиСвойствоОбъекта(ТекОбр.ДокументБУ, "АвтообновленияЗаблокированы") 
			И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекОбр.ДокументБУ, "АвтообновленияЗаблокированы") Тогда 
