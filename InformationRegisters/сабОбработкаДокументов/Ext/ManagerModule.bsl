Функция ПолучитьНеобработанныеДокументы(ВыбратьПервые = Неопределено, ТолькоСАвтопроведением = Ложь, Параметры = Неопределено) Экспорт
	...
	Если ТипЗнч(Параметры) = Тип("Структура") Тогда
		Если Параметры.Свойство("НачалоПериода") Тогда
			ДатаВводаОстатков = Параметры.НачалоПериода;
		КонецЕсли;
...
Процедура ОбработатьДокументыБУНаСервере(ТекДокументСтруктура, СтруктураВозврата = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ТипДокументаБУ = "РеализацияУслугПоПереработке" И Выборка.ТипДокументаУУ = "УЧ_Реализация" Тогда
		...
		Если Выборка.ТипДокументаБУ = "АктОбОказанииПроизводственныхУслуг" И Выборка.ТипДокументаУУ = "УЧ_Реализация" Тогда
		...
		Если Выборка.ТипДокументаБУ = "СписаниеСРасчетногоСчета"  И Выборка.ТипДокументаУУ = "УЧ_ДвижениеДС" Тогда
			...
			//Иначе
				...
				Если СтруктураВозврата = Неопределено Тогда
					...
					ОбратноеВНХДвижение = УЧ_Сервер.ПодобратьВНХДвижение(НоваяОперация.Ссылка, НоваяОперация.ПриходРасход, Неопределено);
		...
		Если Выборка.ТипДокументаБУ = "ОтчетКомиссионераОПродажах" И Выборка.ТипДокументаУУ = "УЧ_Реализация" Тогда
...
Процедура ОбработатьДокументыУУНаСервере(ТекДокументСтруктура, МассивСтруктурВозврата = Неопределено) Экспорт
	...
	ЕстьСоответствие = Ложь; 
	МассивДокументовСоответствия = Новый Массив;
	
	Пока Выборка.Следующий() Цикл	
		НоваяОперация = Неопределено;  
		
		Если МассивСтруктурВозврата = Неопределено Тогда
		...
		Если Выборка.ТипДокументаБУ = "РеализацияТоваровУслуг" И Выборка.ТипДокументаУУ = "УЧ_Реализация"
			И Не ТекДокументСтруктура.Ссылка.ВидОперации = Перечисления.ВидыОперацийРеализация.УслугиПоПереработке Тогда
		...
		Если Выборка.ТипДокументаБУ = "РеализацияУслугПоПереработке" И Выборка.ТипДокументаУУ = "УЧ_Реализация" Тогда
			...
			Если СтруктураВозврата = Неопределено Тогда
				НоваяОперация = Документы.РеализацияУслугПоПереработке.СоздатьДокумент();
				НоваяОперация.Заполнить(ТекДокументСтруктура.Ссылка); 
				НоваяОперация.ДополнительныеСвойства.Вставить("ЭтоНовый", Истина);
				НоваяОперация.ДополнительныеСвойства.Вставить("ДокументУУ", ТекДокументСтруктура.Ссылка);
				НоваяОперация.Дата = ТекДокументСтруктура.Ссылка.Дата;
				НоваяОперация.Записать(РежимЗаписиДокумента.Проведение);
			Иначе
				СтруктураВозврата.Вставить("ИмяФормы", "РеализацияУслугПоПереработке");
		...
		Если Выборка.ТипДокументаБУ = "ОтчетПроизводстваЗаСмену" И Выборка.ТипДокументаУУ = "УЧ_ВыпускПродукции" 
			И Не ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекДокументСтруктура.Ссылка, "ПередачаВПереработку") Тогда
			
			Если (ТекДокументСтруктура.Ссылка.ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.Комплектация) ИЛИ (ТекДокументСтруктура.Ссылка.ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.Разукомплектация) Тогда
				Продолжить;
			КонецЕсли;
		...
		Если Выборка.ТипДокументаБУ = "КомплектацияНоменклатуры" И Выборка.ТипДокументаУУ = "УЧ_ВыпускПродукции" Тогда
			...
			Если (ТекДокументСтруктура.Ссылка.ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.Комплектация) ИЛИ (ТекДокументСтруктура.Ссылка.ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.Разукомплектация) Тогда
				...
				Если СтруктураВозврата = Неопределено Тогда
					НоваяОперация = Документы.КомплектацияНоменклатуры.СоздатьДокумент();
					НоваяОперация.Заполнить(ТекДокументСтруктура.Ссылка);
					НоваяОперация.ДополнительныеСвойства.Вставить("ЭтоНовый", Истина);
					НоваяОперация.ДополнительныеСвойства.Вставить("ДокументУУ", ТекДокументСтруктура.Ссылка);
					НоваяОперация.Дата = ТекДокументСтруктура.Ссылка.Дата;
					НоваяОперация.Записать(РежимЗаписиДокумента.Проведение);
				Иначе
					СтруктураВозврата.Вставить("ИмяФормы", "КомплектацияНоменклатуры");
					Если ТекДокументСтруктура.Ссылка.ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.Комплектация Тогда 
						СтруктураВозврата.Вставить("ВидОперации", Перечисления.ВидыОперацийКомплектацияНоменклатуры.Комплектация);	
					Иначе 
						СтруктураВозврата.Вставить("ВидОперации", Перечисления.ВидыОперацийКомплектацияНоменклатуры.Разукомплектация);
					КонецЕсли;
				КонецЕсли;
		...
		Если Выборка.ТипДокументаБУ = "ПередачаТоваров" И Выборка.ТипДокументаУУ = "УЧ_ВыпускПродукции" 
			 И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекДокументСтруктура.Ссылка, "ПередачаВПереработку") Тогда
			
			Если СтруктураВозврата = Неопределено Тогда
				НоваяОперация = Документы.ПередачаТоваров.СоздатьДокумент();
				НоваяОперация.Заполнить(ТекДокументСтруктура.Ссылка);
				НоваяОперация.ДополнительныеСвойства.Вставить("ЭтоНовый", Истина);
				НоваяОперация.ДополнительныеСвойства.Вставить("ДокументБУ", ТекДокументСтруктура.Ссылка);
				НоваяОперация.Записать(РежимЗаписиДокумента.Проведение);
			Иначе
				СтруктураВозврата.Вставить("ИмяФормы", "ПередачаТоваров");
				СтруктураВозврата.Вставить("ВидОперации", Перечисления.ВидыОперацийПередачаТоваров.ВПереработку);
			КонецЕсли;
			
			//ЕстьСоответствие = Истина;
			
			Если Не НоваяОперация = Неопределено Тогда
				МассивДокументовСоответствия.Добавить(Новый Структура("СтруктураВозврата, ДокументБУ", СтруктураВозврата, НоваяОперация.Ссылка));
			КонецЕсли;
			
		КонецЕсли;
		
		Если Выборка.ТипДокументаБУ = "ПоступлениеИзПереработки" И Выборка.ТипДокументаУУ = "УЧ_ВыпускПродукции" 
			 И ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекДокументСтруктура.Ссылка, "ПередачаВПереработку") Тогда
			
			Если СтруктураВозврата = Неопределено Тогда
				НоваяОперация = Документы.ПоступлениеИзПереработки.СоздатьДокумент();
				НоваяОперация.Заполнить(ТекДокументСтруктура.Ссылка);
				НоваяОперация.ДополнительныеСвойства.Вставить("ЭтоНовый", Истина);
				НоваяОперация.ДополнительныеСвойства.Вставить("ДокументБУ", ТекДокументСтруктура.Ссылка);
				НоваяОперация.Записать(РежимЗаписиДокумента.Проведение); 
			Иначе
				СтруктураВозврата.Вставить("ИмяФормы", "ПоступлениеИзПереработки");
				СтруктураВозврата.Вставить("ВидОперации", "");	
			КонецЕсли;
		...
		Если Выборка.ТипДокументаБУ = "ВозвратТоваровОтПокупателя" И Выборка.ТипДокументаУУ = "УЧ_Возврат" Тогда
			...
			Если СтруктураВозврата = Неопределено Тогда
				НоваяОперация = Документы.ВозвратТоваровОтПокупателя.СоздатьДокумент();
				НоваяОперация.Заполнить(ТекДокументСтруктура.Ссылка);
				НоваяОперация.ДополнительныеСвойства.Вставить("ЭтоНовый", Истина);
				НоваяОперация.ДополнительныеСвойства.Вставить("ДокументУУ", ТекДокументСтруктура.Ссылка);
				НоваяОперация.Дата = ТекДокументСтруктура.Ссылка.Дата;
				НоваяОперация.Записать(РежимЗаписиДокумента.Проведение);
			Иначе
				СтруктураВозврата.Вставить("ИмяФормы", "ВозвратТоваровОтПокупателя");
				СтруктураВозврата.Вставить("ВидОперации", Перечисления.ВидыОперацийВозвратТоваровОтПокупателя.Товары);
			КонецЕсли;
		...
		Если Выборка.ТипДокументаБУ = "ВозвратТоваровПоставщику" И Выборка.ТипДокументаУУ = "УЧ_ВозвратТоваровПоставщику" Тогда
			...
			Если СтруктураВозврата = Неопределено Тогда
				...
				НоваяОперация.Записать(РежимЗаписиДокумента.Проведение);
			Иначе
				СтруктураВозврата.Вставить("ИмяФормы", "ВозвратТоваровПоставщику");
				СтруктураВозврата.Вставить("ВидОперации", Перечисления.ВидыОперацийВозвратТоваровПоставщику.ПокупкаКомиссия);
			КонецЕсли;
			
			ЕстьСоответствие = Истина;
		КонецЕсли;

		Если Не МассивДокументовСоответствия.Количество() И Не НоваяОперация = Неопределено Тогда
			МассивДокументовСоответствия.Добавить(Новый Структура("СтруктураВозврата, ДокументБУ", СтруктураВозврата, НоваяОперация.Ссылка));
		КонецЕсли;
		
		Если Не СтруктураВозврата = Неопределено И СтруктураВозврата.Количество() Тогда
	...
	Если НЕ ЕстьСоответствие Тогда
		...
		Если СтруктураВозврата = Неопределено Тогда
			НоваяОперация = Документы.ОперацияБух.СоздатьДокумент();
			НоваяОперация.ДополнительныеСвойства.Вставить("ДокументУУ", ТекДокументСтруктура.Ссылка);
			НоваяОперация.Заполнить(ТекДокументСтруктура.Ссылка);
			НоваяОперация.Дата = ТекДокументСтруктура.Ссылка.Дата;
			НоваяОперация.Записать(РежимЗаписиДокумента.Проведение);
		Иначе
			СтруктураВозврата.Вставить("ИмяФормы", "ОперацияБух");
			СтруктураВозврата.Вставить("ВидОперации", "");
			МассивСтруктурВозврата.Добавить(СтруктураВозврата);
		КонецЕсли;
	...
	Для Каждого ТекЭлементДокументовСоответствия Из МассивДокументовСоответствия Цикл
		
		Если ТекЭлементДокументовСоответствия.СтруктураВозврата = Неопределено Тогда
			НаборЗаписей = РегистрыСведений.сабОбработкаДокументов.СоздатьМенеджерЗаписи();
			НаборЗаписей.ДокументУУ = ТекДокументСтруктура.Ссылка;
			НаборЗаписей.ДокументБУ = ТекЭлементДокументовСоответствия.ДокументБУ;
			НаборЗаписей.Автор = ПараметрыСеанса.ТекущийПользователь;
			НаборЗаписей.ДатаОбработки = ТекущаяДата();
...
Функция ТекстЗапросаНеобработанных(ВыбратьПервые = Неопределено) Экспорт
	...
	|	И (НЕ Хозрасчетный.Регистратор ЕСТЬ NULL ИЛИ ТипЗначения(ЖурналДокументовЖурналОпераций.Ссылка) = Тип(Документ.ПоступлениеНаРасчетныйСчет) ИЛИ ТипЗначения(ЖурналДокументовЖурналОпераций.Ссылка) = Тип(Документ.СписаниеСРасчетногоСчета))
	...
	|	И Выбор Когда &КонецПериода = ДатаВремя(1,1,1) Тогда Истина Иначе ЖурналДокументовЖурналОпераций.Дата <= &КонецПериода Конец
...
Функция ТекстЗапросаНеобработанныхУУ(ФоноваяОбработка = Неопределено) Экспорт
	...
	Возврат "ВЫБРАТЬ
	        ...
	        |	УЧ_Возврат.Дата,
	        |	УЧ_Возврат.Номер,
	        |	УЧ_Возврат.Организация,
	        |	"""",
	        |	УЧ_Возврат.СуммаДокумента,
	        |	УЧ_Возврат.ВидОперации,
	        |	УЧ_Возврат.Комментарий,
	        |	ТИП(Документ.УЧ_Возврат),
	        |	сабСоответствиеДокументов.ДокументУУ,
	        |	сабСоответствиеДокументов.ДокументБУ,
	        |	УЧ_Возврат.Ссылка,
	        |	Учетный.Регистратор,
	        ...
	        |ИЗ
	        |	РегистрСведений.сабОбработкаДокументов КАК сабСоответствиеДокументов
	        |		ПРАВОЕ СОЕДИНЕНИЕ Документ.УЧ_Возврат КАК УЧ_Возврат
	        |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Учетный КАК Учетный
	        |			ПО УЧ_Возврат.Ссылка = Учетный.Регистратор
	        |				И (Учетный.НомерСтроки = 1)
	        |		ПО (УЧ_Возврат.Ссылка = сабСоответствиеДокументов.ДокументУУ)
	        |ГДЕ
	        |	сабСоответствиеДокументов.ДокументБУ ЕСТЬ NULL
	        |	И НЕ Учетный.Регистратор ЕСТЬ NULL
	        |	И (НЕ ТипЗначения(УЧ_Возврат.Ссылка) В (&МассивИсклТипов) ИЛИ УЧ_Возврат.ВидОперации В(&МассивВклВидовОпераций))
	        |	И УЧ_Возврат.Дата >= &Дата
	        |	И УЧ_Возврат.ПометкаУдаления = ЛОЖЬ
	        |	И УЧ_Возврат.Проведен = ИСТИНА
	        |	И (УЧ_Возврат.Организация В
	        |			(ВЫБРАТЬ
	        |				сабСоответствияОрганизацийПредприятиям.Организация КАК Организация
	        |			ИЗ
	        |				РегистрСведений.сабСоответствияОрганизацийПредприятиям КАК сабСоответствияОрганизацийПредприятиям
			...
			|
			|ВЫБРАТЬ
	        |	УЧ_ВозвратТоваровПоставщику.Дата,
	        |	УЧ_ВозвратТоваровПоставщику.Номер,
	        |	УЧ_ВозвратТоваровПоставщику.Организация,
	        |	"""",
	        |	УЧ_ВозвратТоваровПоставщику.СуммаДокумента,
	        |	УЧ_ВозвратТоваровПоставщику.ВидОперации,
	        |	УЧ_ВозвратТоваровПоставщику.Комментарий,
	        |	ТИП(Документ.УЧ_ВозвратТоваровПоставщику),
	        |	сабСоответствиеДокументов.ДокументУУ,
	        |	сабСоответствиеДокументов.ДокументБУ,
	        ...
	        |ИЗ
	        |	РегистрСведений.сабОбработкаДокументов КАК сабСоответствиеДокументов
	        |		ПРАВОЕ СОЕДИНЕНИЕ Документ.УЧ_ВозвратТоваровПоставщику КАК УЧ_ВозвратТоваровПоставщику
	        |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Учетный КАК Учетный
	        |			ПО УЧ_ВозвратТоваровПоставщику.Ссылка = Учетный.Регистратор
	        |				И (Учетный.НомерСтроки = 1)
	        |		ПО (УЧ_ВозвратТоваровПоставщику.Ссылка = сабСоответствиеДокументов.ДокументУУ)
	        |ГДЕ
	        |	сабСоответствиеДокументов.ДокументБУ ЕСТЬ NULL
	        |	И НЕ Учетный.Регистратор ЕСТЬ NULL
	        |	И (НЕ ТипЗначения(УЧ_ВозвратТоваровПоставщику.Ссылка) В (&МассивИсклТипов) ИЛИ УЧ_ВозвратТоваровПоставщику.ВидОперации В(&МассивВклВидовОпераций))
	        |	И УЧ_ВозвратТоваровПоставщику.Дата >= &Дата
	        |	И УЧ_ВозвратТоваровПоставщику.ПометкаУдаления = ЛОЖЬ
	        |	И УЧ_ВозвратТоваровПоставщику.Проведен = ИСТИНА
	        |	И (УЧ_ВозвратТоваровПоставщику.Организация В
	        |			(ВЫБРАТЬ
	        |				сабСоответствияОрганизацийПредприятиям.Организация КАК Организация
	        |			ИЗ
	        |				РегистрСведений.сабСоответствияОрганизацийПредприятиям КАК сабСоответствияОрганизацийПредприятиям
...
Функция ТекстЗапросаИзмененные(ВыбратьПервые = Неопределено) Экспорт
	...
	|И сабСоответствиеДокументов.ПервичныйДокументУУ = ЛОЖЬ"
	+ ?(ВыбратьПервые = Неопределено, "", " И сабДокументОшибкаФоновогоПроведения.ДокументБУ ЕСТЬ NULL");
...
Функция ПолучитьИзмененныеДокументы(ВыбратьПервые, ТолькоСАвтопроведением = Ложь) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ " + ?(ВыбратьПервые = Неопределено, "", "ПЕРВЫЕ " + Формат(ВыбратьПервые, "ЧРГ=''")) + "
