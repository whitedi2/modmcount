...
Функция ТекстЗапросаНеобработанныхУУ(ВыбратьПервые = Неопределено) Экспорт
	//выбрать первые используется только в фоновом задании
	...
	Возврат "ВЫБРАТЬ
	        ...
	        |		ПО (УЧ_ПеремещениеТоваров.Ссылка = сабСоответствиеДокументов.ДокументБУ)
	        |ГДЕ
	        |	сабСоответствиеДокументов.ДокументБУ ЕСТЬ NULL
	        |	И НЕ Учетный.Регистратор ЕСТЬ NULL
	        |	И НЕ ТИПЗНАЧЕНИЯ(УЧ_ПеремещениеТоваров.Ссылка) В (&МассивИсклТипов)
	        ...
	        |				И (Учетный.НомерСтроки = 1)
	        |		ПО (УЧ_ПоступлениеТоваров.Ссылка = сабСоответствиеДокументов.ДокументБУ)
	        |ГДЕ
	        |	сабСоответствиеДокументов.ДокументБУ ЕСТЬ NULL
	        |	И НЕ Учетный.Регистратор ЕСТЬ NULL
	        |	И НЕ ТИПЗНАЧЕНИЯ(УЧ_ПоступлениеТоваров.Ссылка) В (&МассивИсклТипов)
	        ...
	        |				И (Учетный.НомерСтроки = 1)
	        |		ПО (УЧ_Реализация.Ссылка = сабСоответствиеДокументов.ДокументБУ)
	        |ГДЕ
	        |	сабСоответствиеДокументов.ДокументБУ ЕСТЬ NULL
	        |	И НЕ Учетный.Регистратор ЕСТЬ NULL
	        |	И НЕ ТИПЗНАЧЕНИЯ(УЧ_Реализация.Ссылка) В (&МассивИсклТипов)
	        ...
	        |				И (Учетный.НомерСтроки = 1)
	        |		ПО (УЧ_СписаниеТоваров.Ссылка = сабСоответствиеДокументов.ДокументБУ)
	        |ГДЕ
	        |	сабСоответствиеДокументов.ДокументБУ ЕСТЬ NULL
	        |	И НЕ Учетный.Регистратор ЕСТЬ NULL
	        |	И НЕ ТИПЗНАЧЕНИЯ(УЧ_СписаниеТоваров.Ссылка) В (&МассивИсклТипов)
	        ...
	        |				И (Учетный.НомерСтроки = 1)
	        |		ПО (УЧ_ОприходованиеТоваров.Ссылка = сабСоответствиеДокументов.ДокументБУ)
	        |ГДЕ
	        |	сабСоответствиеДокументов.ДокументБУ ЕСТЬ NULL
	        |	И НЕ Учетный.Регистратор ЕСТЬ NULL
	        ...
	        |				И (Учетный.НомерСтроки = 1)
	        |		ПО (УЧ_ВыпускПродукции.Ссылка = сабСоответствиеДокументов.ДокументБУ)
	        |ГДЕ
	        |	сабСоответствиеДокументов.ДокументБУ ЕСТЬ NULL
	        |	И НЕ Учетный.Регистратор ЕСТЬ NULL
	        |	И НЕ ТИПЗНАЧЕНИЯ(УЧ_ВыпускПродукции.Ссылка) В (&МассивИсклТипов)
	        ...
	        |	УЧ_ПереводТМЦ.Дата,
	        |	УЧ_ПереводТМЦ.Номер,
	        |	УЧ_ПереводТМЦ.Организация,
	        |	"""",
	        |	УЧ_ПереводТМЦ.СуммаИтог,
	        |	УЧ_ПереводТМЦ.ВидОперации,
	        |	УЧ_ПереводТМЦ.Комментарий,
	        |	ТИП(Документ.УЧ_ПереводТМЦ),
	        |	сабСоответствиеДокументов.ДокументУУ,
	        |	сабСоответствиеДокументов.ДокументБУ,
	        |	УЧ_ПереводТМЦ.Ссылка,
	        |	Учетный.Регистратор
	        |ИЗ
	        |	РегистрСведений.сабОбработкаДокументов КАК сабСоответствиеДокументов
	        |		ПРАВОЕ СОЕДИНЕНИЕ Документ.УЧ_ПереводТМЦ КАК УЧ_ПереводТМЦ
	        |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Учетный КАК Учетный
	        |			ПО УЧ_ПереводТМЦ.Ссылка = Учетный.Регистратор
	        |				И (Учетный.НомерСтроки = 1)
	        |		ПО (УЧ_ПереводТМЦ.Ссылка = сабСоответствиеДокументов.ДокументБУ)
	        |ГДЕ
	        |	сабСоответствиеДокументов.ДокументБУ ЕСТЬ NULL
	        |	И НЕ Учетный.Регистратор ЕСТЬ NULL
	        |	И НЕ ТИПЗНАЧЕНИЯ(УЧ_ПереводТМЦ.Ссылка) В (&МассивИсклТипов)
	        |	И УЧ_ПереводТМЦ.Дата >= &Дата
	        |	И УЧ_ПереводТМЦ.ПометкаУдаления = ЛОЖЬ
	        |	И УЧ_ПереводТМЦ.Проведен = ИСТИНА
	        |	И УЧ_ПереводТМЦ.Организация В
	        |			(ВЫБРАТЬ
	        |				сабСоответствияОрганизацийПредприятиям.Организация КАК Организация
	        |			ИЗ
	        |				РегистрСведений.сабСоответствияОрганизацийПредприятиям КАК сабСоответствияОрганизацийПредприятиям
