...
Функция ПолучитьНеобработанныеДокументыУУ(ВыбратьПервые = Неопределено, ТолькоСАвтопроведением = Ложь) Экспорт
	...
	Запрос = Новый Запрос;
	Запрос.Текст = ТекстЗапросаНеобработанныхУУ(ВыбратьПервые);
	Запрос.УстановитьПараметр("Дата", ДатаВводаОстатков);
	Запрос.УстановитьПараметр("МассивИсклТипов", МассивИсклТипов);
	Результат = Запрос.Выполнить();
	Выборка = Результат.Выгрузить();
