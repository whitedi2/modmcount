...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.ДатыЗапретаЗагрузкиДанных Тогда
		...
		ТипыПользователя =
			Метаданные.РегистрыСведений.сабДатыЗапретаИзменения.Измерения.Пользователь.Тип.Типы();
...
Процедура ОбработкаВыбора(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ВРег(ИсточникВыбора.ИмяФормы)
	   = ВРег("РегистрСведений.сабДатыЗапретаИзменения.Форма.РедактированиеДатыЗапрета") Тогда
...
////////////////////////////////////////////////////////////////////////////////
// Одинаковые обработчики событий форм сабДатыЗапретаИзменения и РедактированиеДатыЗапрета.
...
Процедура ПользователиПолноеПредставлениеОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если Строки.Количество() = 0 Тогда
		...
		ОповеститьОбИзменении(Тип("РегистрСведенийКлючЗаписи.сабДатыЗапретаИзменения"));
...
Процедура ПрочитатьПользователей()
	...
	Попытка
		...
		Запрос.Текст =
		"ВЫБРАТЬ РАЗЛИЧНЫЕ
		|	ПРЕДСТАВЛЕНИЕ(сабДатыЗапретаИзменения.Пользователь) КАК ПолноеПредставление,
		|	сабДатыЗапретаИзменения.Пользователь,
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Перечисление.ВидыНазначенияДатЗапрета)
		|			ТОГДА 0
		|		ИНАЧЕ 1
		|	КОНЕЦ КАК ОбщееНазначение,
		|	ПРЕДСТАВЛЕНИЕ(сабДатыЗапретаИзменения.Пользователь) КАК Представление,
		|	МАКСИМУМ(сабДатыЗапретаИзменения.Комментарий) КАК Комментарий,
		|	ЛОЖЬ КАК БезДатыЗапрета,
		|	-1 КАК НомерКартинки
		|ИЗ
		|	РегистрСведений.сабДатыЗапретаИзменения КАК сабДатыЗапретаИзменения
		|ГДЕ
		|	НЕ(сабДатыЗапретаИзменения.Раздел <> сабДатыЗапретаИзменения.Объект
		|				И ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Объект) = ТИП(ПланВидовХарактеристик.РазделыДатЗапретаИзменения))
		|	И НЕ(ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Объект) <> ТИП(ПланВидовХарактеристик.РазделыДатЗапретаИзменения)
		...
		|
		|ИМЕЮЩИЕ
		|	сабДатыЗапретаИзменения.Пользователь <> НЕОПРЕДЕЛЕНО И
		|	ВЫБОР
		|		КОГДА ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.Пользователи)
		|				ИЛИ ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.ГруппыПользователей)
		|				ИЛИ ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.ВнешниеПользователи)
		|				ИЛИ ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.ГруппыВнешнихПользователей)
		|				ИЛИ сабДатыЗапретаИзменения.Пользователь = ЗНАЧЕНИЕ(Перечисление.ВидыНазначенияДатЗапрета.ДляВсехПользователей)
...
Функция ПрочитатьДанныеПользователяСРазделами(Знач Пользователь,
	...
	Попытка
		...
		|	
		|	ВЫБРАТЬ
		|		сабДатыЗапретаИзменения.Раздел,
		|		сабДатыЗапретаИзменения.Раздел.Наименование,
		|		NULL
		|	ИЗ
		|		РегистрСведений.сабДатыЗапретаИзменения КАК сабДатыЗапретаИзменения
		|	ГДЕ
		|		сабДатыЗапретаИзменения.Раздел <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
		|		И сабДатыЗапретаИзменения.Пользователь <> НЕОПРЕДЕЛЕНО
		|		И ВЫБОР
		|				КОГДА ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.Пользователи)
		|						ИЛИ ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.ГруппыПользователей)
		|						ИЛИ ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.ВнешниеПользователи)
		...
		|	сабДатыЗапретаИзменения.Объект,
		|	ПРЕДСТАВЛЕНИЕ(сабДатыЗапретаИзменения.Объект) КАК ПолноеПредставление,
		|	ПРЕДСТАВЛЕНИЕ(сабДатыЗапретаИзменения.Объект) КАК Представление,
		|	ВЫБОР
		|		КОГДА сабДатыЗапретаИзменения.Объект ЕСТЬ NULL 
		|			ТОГДА ИСТИНА
		|		ИНАЧЕ ЛОЖЬ
		|	КОНЕЦ КАК БезДатыЗапрета,
		|	сабДатыЗапретаИзменения.ДатаЗапрета,
		|	сабДатыЗапретаИзменения.ОписаниеДатыЗапрета,
		|	ЛОЖЬ КАК ЭтоРаздел,
		|	0 КАК КоличествоДнейРазрешения,
		|	ИСТИНА КАК ЗаписьСуществует
		|ИЗ
		|	Разделы КАК Разделы
		|		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.сабДатыЗапретаИзменения КАК сабДатыЗапретаИзменения
		|		ПО Разделы.Ссылка = сабДатыЗапретаИзменения.Раздел
		|			И (сабДатыЗапретаИзменения.Пользователь = &Пользователь)
		|			И (НЕ(сабДатыЗапретаИзменения.Раздел <> сабДатыЗапретаИзменения.Объект
		|					И ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Объект) = ТИП(ПланВидовХарактеристик.РазделыДатЗапретаИзменения)))
		|			И (НЕ(ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Объект) <> ТИП(ПланВидовХарактеристик.РазделыДатЗапретаИзменения)
		|					И &ВсеРазделыБезОбъектов))
		|ГДЕ
		|	НЕ(&Пользователь <> &ЗначениеДляВсехПользователей
		|				И сабДатыЗапретаИзменения.Раздел ЕСТЬ NULL )
		|
		|УПОРЯДОЧИТЬ ПО
...
Функция ПрочитатьДанныеПользователяБезРазделов(Знач Пользователь, Знач ЕдинственныйРаздел, Знач ЗначениеДляВсехПользователей)
	...
	Попытка
		...
		|			сабДатыЗапретаИзменения.ДатаЗапрета КАК ДатаЗапрета,
		|			сабДатыЗапретаИзменения.ОписаниеДатыЗапрета КАК ОписаниеДатыЗапрета
		|		ИЗ
		|			РегистрСведений.сабДатыЗапретаИзменения КАК сабДатыЗапретаИзменения
		|		ГДЕ
		|			сабДатыЗапретаИзменения.Пользователь = &Пользователь
		|			И сабДатыЗапретаИзменения.Раздел = ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
		|			И сабДатыЗапретаИзменения.Объект = ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)) КАК ОбщаяДата
		|		ПО (ИСТИНА)
		|ГДЕ
		|	ВЫБОР
		...
		|	&ЕдинственныйРаздел,
		|	сабДатыЗапретаИзменения.Объект,
		|	ПРЕДСТАВЛЕНИЕ(сабДатыЗапретаИзменения.Объект),
		|	ПРЕДСТАВЛЕНИЕ(сабДатыЗапретаИзменения.Объект),
		|	сабДатыЗапретаИзменения.ДатаЗапрета,
		|	сабДатыЗапретаИзменения.ОписаниеДатыЗапрета,
		|	ЛОЖЬ,
		...
		|ГДЕ
		|	сабДатыЗапретаИзменения.Пользователь = &Пользователь
		|	И сабДатыЗапретаИзменения.Раздел = &ЕдинственныйРаздел
		|	И ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Объект) <> ТИП(ПланВидовХарактеристик.РазделыДатЗапретаИзменения)";
...
Процедура ЗаблокироватьНаборЗаписейПользователя(Пользователь, ТекущаяСтрока = Неопределено)
	...
	Попытка
		...
		|	сабДатыЗапретаИзменения.ДатаЗапрета,
		|	сабДатыЗапретаИзменения.ОписаниеДатыЗапрета,
		|	сабДатыЗапретаИзменения.Комментарий
		|ИЗ
		|	РегистрСведений.сабДатыЗапретаИзменения КАК сабДатыЗапретаИзменения
		|ГДЕ
		|	сабДатыЗапретаИзменения.Пользователь = &Пользователь";
	...
	Попытка
		Для каждого ОписаниеЗаписи Из Выгрузка Цикл
			...
			Если НайденныеСтроки.Количество() = 0 Тогда
				...
				КлючЗаписи = РегистрыСведений.сабДатыЗапретаИзменения.СоздатьКлючЗаписи(
					ЗначенияКлючаЗаписи);
...
Процедура РазблокироватьВсеЗаписиНаСервере(ЗначенияКлючейЗаблокированныхЗаписей, ИдентификаторФормы)
	...
	Для каждого ЗначенияКлючаЗаписи Из ЗначенияКлючейЗаблокированныхЗаписей Цикл
		КлючЗаписи = РегистрыСведений.сабДатыЗапретаИзменения.СоздатьКлючЗаписи(ЗначенияКлючаЗаписи);
...
Функция ТекущаяУстановкаДатыЗапрета(ДатыЗапретаЗагрузкиДанных)
	...
	Попытка
		...
		|	РегистрСведений.сабДатыЗапретаИзменения КАК сабДатыЗапретаИзменения
		|ГДЕ
		|	(сабДатыЗапретаИзменения.Пользователь = НЕОПРЕДЕЛЕНО
		|			ИЛИ ВЫБОР
		|				КОГДА ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.Пользователи)
		|						ИЛИ ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.ГруппыПользователей)
		|						ИЛИ ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) = ТИП(Справочник.ВнешниеПользователи)
		...
		|				ИНАЧЕ &ДатыЗапретаЗагрузкиДанных = ИСТИНА
		|			КОНЕЦ)
		|	И ТИПЗНАЧЕНИЯ(сабДатыЗапретаИзменения.Пользователь) <> ТИП(Перечисление.ВидыНазначенияДатЗапрета)";
...
Функция ТекущийСпособУказанияДатыЗапрета(Знач Пользователь, Знач ЕдинственныйРаздел, Знач ЗначениеДляВсехПользователей, Знач НачалоДня, Данные = Неопределено)
	...
	Попытка
		...
		|	РегистрСведений.сабДатыЗапретаИзменения КАК сабДатыЗапретаИзменения
		|ГДЕ
		|	НЕ(&Пользователь <> сабДатыЗапретаИзменения.Пользователь
		|				И &Пользователь <> ""*"")
		|	И НЕ(сабДатыЗапретаИзменения.Раздел = ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
		|				И сабДатыЗапретаИзменения.Объект = ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка))
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|	И сабДатыЗапретаИзменения.Раздел <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
		|	И сабДатыЗапретаИзменения.Объект <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
		|	И сабДатыЗапретаИзменения.Объект <> сабДатыЗапретаИзменения.Раздел
		|;
		|
		|////////////////////////////////////////////////////////////////////////////////
		|ВЫБРАТЬ ПЕРВЫЕ 1
		...
		|				И &Пользователь <> ""*"")
		|	И сабДатыЗапретаИзменения.Раздел <> ЗНАЧЕНИЕ(ПланВидовХарактеристик.РазделыДатЗапретаИзменения.ПустаяСсылка)
		|	И сабДатыЗапретаИзменения.Раздел = сабДатыЗапретаИзменения.Объект";
...
Процедура РедактироватьДатуЗапретаВФормеЗавершение(ВыделенныеСтроки) Экспорт
	...
	ОткрытьФорму("РегистрСведений.сабДатыЗапретаИзменения.Форма.РедактированиеДатыЗапрета",
...
////////////////////////////////////////////////////////////////////////////////
// Одинаковые процедуры и функции форм сабДатыЗапретаИзменения и РедактированиеДатыЗапрета.
