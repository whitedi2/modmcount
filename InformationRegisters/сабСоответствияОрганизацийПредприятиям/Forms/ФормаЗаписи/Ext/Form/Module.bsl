...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |		ПО ДоговорыКонтрагентов.Организация = сабСоответствияОрганизацийПредприятиям.Организация И НЕ сабСоответствияОрганизацийПредприятиям.Предприятие = Значение(Справочник.Предприятия.ПустаяСсылка)
	               |ГДЕ
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.УстановленСрокОплаты Тогда
			НоваяЗапись = РегистрыСведений.ОтсрочкаПоДоговорам.СоздатьМенеджерЗаписи();
			НоваяЗапись.Договор = Выборка.Ссылка;
			НоваяЗапись.Период = ?(Выборка.Дата = Дата('00010101'), Дата('20000101'), Выборка.Дата);
