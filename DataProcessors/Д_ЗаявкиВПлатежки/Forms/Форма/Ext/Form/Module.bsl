...
Процедура СоздатьПлатежку(Команда)
	...
	Для каждого ТекСтрока Из Элементы.Заявки.ВыделенныеСтроки Цикл
		...
		СтруктураЗначений.Вставить("Заявка", Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка);
...
Процедура СоздатьДвижениеДС(Команда)
	...
	Для каждого ТекСтрока Из Элементы.Заявки.ВыделенныеСтроки Цикл
		...
		УспешноСозданы = Выгрузить(НомерСтроки, Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка);		
	КонецЦикла; 
...
Процедура СоздатьРКО(Команда)
	...
	Для каждого ТекСтрока Из Элементы.Заявки.ВыделенныеСтроки Цикл
		...
		ФормаПлатежки.Объект.ДокОснование = Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка;
		
		ИспользуетсяМеханизмЧачтичнойОплаты = БПСервер.ИспользуетсяМеханизмЧастичнойОплаты(БюджетныйНаСервере.ВернутьРеквизит(Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка, "Предприятие"));
		...
		Для каждого ТекЦФО Из МассивСтрок Цикл
			...
			Если НЕ ЗначениеЗаполнено(ТекПП) Тогда
				ТекПП = БюджетныйНаСервере.ВернутьРеквизит(Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка, "Предприятие");
			КонецЕсли; 
			...
			Иначе
				ФормаПлатежки.Объект.Подразделение = БюджетныйНаСервере.ВернутьРеквизит(Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка, "Подразделение");
			КонецЕсли;
...
Процедура ЗаписатьРКО(Команда)
	ЗаписатьРКСервер(Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка);
	ПриОткрытии(0);
КонецПроцедуры
...
Процедура ЗаписатьРКСервер(ЗаявкаСсылка)
	...
	Для каждого ТекСтрока Из Элементы.Заявки.ВыделенныеСтроки Цикл
		...
		ФормаПлатежки.Объект.ВидОперации = ?(ТипЗнч(ТекущаяСтрока.Контрагент) = Тип("СправочникСсылка.Организации"), ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.ОплатаВНХ"), ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.Оплата"));
		ФормаПлатежки.ДокОснование = Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка;
		ИспользуетсяМеханизмЧачтичнойОплаты = БПСервер.ИспользуетсяМеханизмЧастичнойОплаты(БюджетныйНаСервере.ВернутьРеквизит(Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка, "Предприятие"));
		...
		Для каждого ТекЦФО Из МассивСтрок Цикл
			...
			Иначе
				ФормаПлатежки.Подразделение = БюджетныйНаСервере.ВернутьРеквизит(Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка, "Подразделение");
			КонецЕсли;
...
Процедура СформироватьПлатежиИОтправитьВБанк(Команда)
	...
	Для каждого ТекСтрока Из Заявки Цикл
		...
		СоздатьПлатежкуНаСервере(Заявки.Индекс(ТекСтрока), Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка);
		
	КонецЦикла;
	
	ОткрытьФорму("Обработка.сабКлиентБанк.Форма", Новый Структура("РеестрЗаявок", Элементы.ЗаявкиТаблица.ТекущиеДанные.Ссылка));
...
Процедура СформироватьРКО(Команда)
	...
	Для каждого ТекСтрока Из Заявки Цикл
		...
		Если НЕ (ТекСтрока.СуммаОплачено >= ТекСтрока.СуммаДокумента ИЛИ НЕ ТипЗнч(ТекСтрока.Плательщик) = Тип("СправочникСсылка.Кассы") Или ТекСтрока.ИсточникПредопределенный) Тогда
			...
			СоздатьРКОНаСервере(Заявки.Индекс(ТекСтрока), Объект1, Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка);
		КонецЕсли;
				
		Если НЕ ТекСтрока.СуммаПоступление = ТекСтрока.СуммаДокумента И ЗначениеЗаполнено(ТекСтрока.ИсточникПолучатель) Тогда
			СоздатьПКОНаСервере(Заявки.Индекс(ТекСтрока), Объект1, ТекСтрока.ИсточникПолучатель, Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка);
		КонецЕсли;
...
Процедура СформироватьВзаимозачеты(Команда)
	...
	Для каждого ТекСтрока Из Заявки Цикл
		...
		СоздатьРКОНаСервере(Заявки.Индекс(ТекСтрока), Неопределено, Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка);
...
Процедура ОтменитьСтрокуЗавершение(Строка, ДополнительныеПараметры) Экспорт
	...
	Если ТипЗнч(ТекСтрока) = Тип("Число") И НЕ Строка = Неопределено Тогда
		...
		ОткрепитьЗаявку(ДанныеСтроки.ЗаявкаНаФинансирование, Строка, Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка);
