...
Процедура ЗаполнитьТЧ(ЗаявкаСсылка1)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ РАЗЛИЧНЫЕ
				   ...
				   |	Д_ЗаявкаНаОплатуЗаявкаБезнал.КлючАналитикиБюджета КАК КлючАналитикиБюджета,
	               |	Д_ЗаявкаНаОплатуЗаявкаБезнал.ВалютаКонтрагента КАК ВалютаКонтрагента,
				   |	Д_ЗаявкаНаОплатуЗаявкаБезнал.ДокументОснование КАК ДокументОснование
	               |ПОМЕСТИТЬ Данные
	               |ИЗ
	               ...
	               |	NULL,
	               |	сабГрафикПлатежей1.Валюта,
				   |	сабГрафикПлатежей1.КлючАналитикиБюджета,
	               |	сабГрафикПлатежей1.ВалютаКонтрагента,
				   |	NULL
	               |ИЗ
	...
	Пока ЕстьНераскиданные Цикл
		...
		Для каждого ТекСтрока Из Заявки Цикл
			...
			Для каждого ТекПодчиненный Из Найденные Цикл
				...
				КонецЕсли;				
			КонецЦикла;
			Если ТекСтрока.СуммаОплачено = 0 Тогда
				
				Запрос = Новый Запрос;
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	Д_ЗаявкаНаФинансирование.Ссылка КАК ЗаявкаНаФинансирование,
				|	ВедомостьНаВыплатуЗарплаты.Ссылка КАК ВедомостьНаВыплатуЗарплаты,
				...
				|		ПО Д_ЗаявкаНаФинансирование.ДокОснование = ВедомостьНаВыплатуЗарплаты.Ссылка
				|		ЛЕВОЕ СОЕДИНЕНИЕ Документ.ПлатежноеПоручение КАК ПлатежноеПоручение
				|		ПО (ВедомостьНаВыплатуЗарплаты.Ссылка = ПлатежноеПоручение.ДокументОснование)
				|ГДЕ
				|	Д_ЗаявкаНаФинансирование.Ссылка = &Ссылка";
				
				Запрос.УстановитьПараметр("Ссылка", ТекСтрока.ЗаявкаНаФинансирование);
				...
				Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
					//Если ПустаяСтрока(ВыборкаДетальныеЗаписи.ПлатежноеПоручение.Заявка) Тогда
						Док = ВыборкаДетальныеЗаписи.ПлатежноеПоручение.ПолучитьОбъект();
						Док.Заявка = ВыборкаДетальныеЗаписи.ЗаявкаНаФинансирование;
						Док.Записать();
					//КонецЕсли;
					ТекСтрока.СуммаОплачено = ТекСтрока.СуммаОплачено + ВыборкаДетальныеЗаписи.СуммаДокумента;
				КонецЦикла;
...
Процедура СоздатьРКО(Команда)
	...
	Для каждого ТекСтрока Из Элементы.Заявки.ВыделенныеСтроки Цикл
		...
		ИначеЕсли ТекущаяСтрока.ПодотчетИзЗаявки Тогда
			...
			Если ТипЗнч(ТекущаяСтрока.Контрагент) = Тип("СправочникСсылка.ФизическиеЛица") Тогда
				ФормаПлатежки.Объект.КорСчет = УЧ_Сервер.СчетПоКоду("71.01");
				ФормаПлатежки.Объект.КорСубконто1 = ТекущаяСтрока.Контрагент;
			КонецЕсли;
...
Процедура ЗаявкиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ЗаявкиСуммаОплачено" ИЛИ Поле.Имя = "ЗаявкиСуммаПоступление" Тогда
		...
		Иначе
			СтандартнаяОбработка = Ложь;
			РеквизитыКассы = БюджетныйНаСервере.ВернутьРеквизиты(Элементы.Заявки.ТекущиеДанные.Плательщик, "ВидИсточника");
			ЭтоУчетнаяКасса = РеквизитыКассы.ВидИсточника = ПредопределенноеЗначение("Перечисление.ГД_ВидыИсточников.КассаУчет");
			Если ЭтоУчетнаяКасса Тогда
				ТекФорма = ПолучитьФорму("Документ.УЧ_ДвижениеДС.ФормаСписка"); 
			Иначе
				ТекФорма = ПолучитьФорму("Документ.РасходныйКассовыйОрдер.ФормаСписка", Новый Структура("ОтключитьОжиданиеПриОткрытииФорм", Истина)); 
			КонецЕсли;
			ТекФорма.РежимОткрытияОкна = РежимОткрытияОкнаФормы.БлокироватьОкноВладельца;
			НовыйОтбор = ТекФорма.Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
...
Процедура ЗаявкиТаблицаПриАктивизацииСтроки(Элемент)
	...
	Если Элементы.ЗаявкиТаблица.ТекущаяСтрока = Неопределено Тогда
		Возврат;
	КонецЕсли;	

	ТекДанныеСсылка = ?(ЗначениеЗаполнено(Заявка), Заявка, Элементы.ЗаявкиТаблица.ДанныеСтроки(Элементы.ЗаявкиТаблица.ТекущаяСтрока).Ссылка);
	//Если Не ТекДанные = Неопределено Тогда
		ЗаполнитьТЧ(ТекДанныеСсылка);
		ЗаявкаОснование = ТипЗнч(ТекДанныеСсылка) = Тип("Строка");
	//КонецЕсли;
...
Процедура Обновить(Команда)
	ПриОткрытии(Неопределено);
	ЗаявкиТаблицаПриАктивизацииСтроки(Неопределено);
	ЗаявкаПриИзменении(Неопределено);
	Элементы.ЗаявкиТаблица.Обновить();
КонецПроцедуры
