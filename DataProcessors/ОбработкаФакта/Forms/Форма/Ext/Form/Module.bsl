...
Процедура Объект3Выбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	ИначеЕсли Элемент.ТекущийЭлемент.Имя = "Объект3ЦФО" Тогда
		...
		ТекПодразделение = "";
		//ТекПодразделение = Элемент.ТекущиеДанные.ФинПодразделение;
		НовоеЦФО = ОткрытьФормуМодально("Справочник.Предприятия.Форма.ФормаВыбора", Новый Структура("ТекущаяСтрока", ?(ЗначениеЗаполнено(ТекЦФО), ТекЦФО, Элемент.ТекущиеДанные.Предприятие))); 
	...
	//Для Каждого ТекДок Из МассивДокументов Цикл
		...
		Если ТипЗнч(ТекДок) = Тип("ДокументСсылка.УЧ_ДвижениеДС") Тогда
			...
			Для Каждого ТекСтрока Из МассивСтрок Цикл
				...
				Иначе
					ТекДокОбъект.Проводки[ТекСтрока.НомерСтроки - 1].ФинПодразделение = ТекСтрока.ЦФО.ВидДеятельности;
				КонецЕсли;
