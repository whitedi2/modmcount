...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.ГоризонтПланирования = Дата('00010101') ИЛИ Объект.ГоризонтПланирования < КонецНедели(ТекущаяДата()) Тогда
...
Процедура ОбновитьНаСервере()
	...
	КоличествоСтрокДо = Данные.Количество() - 1;
	...
	ЗапросТекст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	              ...
	              |				,
	              |			,
	              |			НЕ &ПоОбъектам = ""Объекты"" И Предприятия В ИЕРАРХИИ(&ОтборПредприятия) И 
	              |			ВЫБОР
	              |					КОГДА &ИспользоватьПлатежку
	              |						ТОГДА Выразить(Субконто2 КАК Справочник.Кассы).ВидИсточника = Значение(Перечисление.ГД_ВидыИсточников.КассаУчет)
	              |					ИНАЧЕ Истина
	              ...
	              |				,
	              |			,
	              |			НЕ &ПоОбъектам = ""Объекты"" И (Организация В (&ОтборОрганизации)
	              |			ИЛИ ЗНАЧЕНИЕ(Справочник.Организации.ПустаяСсылка) В (&ОтборОрганизации))) КАК УчетныйОстатки
	              |;
	              |
	              |////////////////////////////////////////////////////////////////////////////////
	              |ВЫБРАТЬ РАЗРЕШЕННЫЕ
	...
	ДнейПланирования = ЧислоРабочихДнейВПериоде(ТекущаяДата(), МИН(Объект.ГоризонтПланирования, КонецМесяца(ТекущаяДата())));
	
	Если Объект.УчитыватьВыходные ИЛИ Не ДнейПланирования Тогда
		ДнейПланирования = Цел((Объект.ГоризонтПланирования - ТекущаяДата()) / (24*60*60)) + 2;	
	КонецЕсли;
	...
	УстановитьЗначениеПараметраНастроек(Отбор.Настройки, "ПоОбъектам", Объект.ВариантГруппировки);
	УстановитьЗначениеПараметраНастроек(Отбор.Настройки, "КорректировкаОстатковПоНеоплаченнымЗаявкам", Объект.КорректировкаОстатковПоНеоплаченнымЗаявкам);
	...
	РезультатВыборка = ПроцессорВывода.Вывести(ПроцессорКомпоновкиДанных);
	...
	Если ИндексСтроки > КоличествоСтрокДо Тогда
		НоваяСтрока = Данные.Вставить(ИндексСтроки);	
	Иначе	
		НоваяСтрока = Данные[ИндексСтроки];
	КонецЕсли;
	...
	Для Каждого Выборка Из РезультатВыборка Цикл
		...
		Если ТекИсточник = Неопределено Тогда
			...
			Для т = 0 По ДнейПланирования Цикл
				...
				Данные[0]["День" + Строка(т)] = Данные[0]["День" + Строка(т)] + ТекСумма[т];
				Данные[0]["БудущиеПлатежи"] = 0;
				Данные[0]["ПлатежиПоРеестрам"] = 0;
			КонецЦикла;
		ИначеЕсли НЕ ТекИсточник = Выборка.Источник ИЛИ НЕ ТекПредприятие = Выборка.Предприятие Тогда
			...
			НоваяСтрока.ДатаПоследнегоДокумента = Данные[ИндексСтрокиОстатковНаНачало].ДатаПоследнегоДокумента;
			НоваяСтрока.СтатьяДДС = "Остаток на конец";
			НоваяСтрока.ЦФО = Данные[ИндексСтрокиОстатковНаНачало].ЦФО;
			НоваяСтрока.Предприятие = Данные[ИндексСтрокиОстатковНаНачало].Предприятие;
			
			НоваяСтрока["БудущиеПлатежи"] = 0;
			
			Для т = 0 По ДнейПланирования Цикл
				Если т > 0 Тогда
					Данные[ИндексСтрокиОстатковНаНачало]["День" + Строка(т)] = ?(Объект.ВариантГруппировки = "Объекты", 0, ТекОстатокКонец[т-1]);
					ТекОстатокКонец[т] = ?(Объект.ВариантГруппировки = "Объекты", 0, ТекОстатокКонец[т]) + Данные[ИндексСтрокиОстатковНаНачало]["День" + Строка(т)];
				КонецЕсли;
				Если Объект.РежимРаботы = "Календарь" Тогда
					НоваяСтрока["День" + Строка(т)] = ТекОстатокКонец[т];
				Иначе
					НоваяСтрока["День" + Строка(т)] = -(ТекОстатокКонец[т] - Данные[ИндексСтрокиОстатковНаНачало]["День" + Строка(т)]);
				КонецЕсли;
			КонецЦикла;
			...
			Данные[0].ПлатежиПоРеестрам = Данные[0].ПлатежиПоРеестрам + ТекОстатокПоРеестрамКонец;
			НоваяСтрока.ПлатежиПоРеестрам = ТекОстатокПоРеестрамКонец;
			
			Данные[0].ОстатокНаНачало = Данные[0]["День0"];
			НоваяСтрока.ОстатокНаНачало = Данные[ИндексСтрокиОстатковНаНачало]["День0"];
			
			Данные[0].ОстатокНаНачалоРабочий = Данные[0].ОстатокНаНачало + Данные[0].ПлатежиПоРеестрам;
			НоваяСтрока.ОстатокНаНачалоРабочий = НоваяСтрока.ОстатокНаНачало + ТекОстатокПоРеестрамКонец;
			
			Данные[0].ОстатокНаКонец = Данные[0]["День1"]  + ?(Объект.КорректировкаОстатковПоНеоплаченнымЗаявкам, Данные[0].ПлатежиПоРеестрам, 0);
			НоваяСтрока.ОстатокНаКонец = Данные[ИндексСтрокиОстатковНаНачало]["День1"] + ?(Объект.КорректировкаОстатковПоНеоплаченнымЗаявкам, ТекОстатокПоРеестрамКонец, 0);
		...
		Если Выборка.СтатьяДДС = "Остаток на начало" Тогда
			ИндексСтрокиОстатковНаНачало = Данные.Индекс(НоваяСтрока);
		КонецЕсли;
	КонецЦикла;
	...
	Для т = 0 По ДнейПланирования Цикл
		Если т > 0 Тогда
			Данные[0]["День" + Строка(т)] = НоваяСтрока["День" + Строка(т-1)];
		ИначеЕсли НЕ Объект.РежимРаботы = "Календарь" Тогда
			Данные[0]["День" + Строка(т)] = - ДенежныйПотокИтого[т];
		КонецЕсли;
		Если Объект.РежимРаботы = "Календарь" Тогда
			НоваяСтрока["День" + Строка(т)] = Данные[0]["День" + Строка(т)] + ДенежныйПотокИтого[т];
		КонецЕсли;
	КонецЦикла;
	
	Данные[0].ОстатокНаКонец = Данные[0]["ОстатокНаНачалоРабочий"] - Данные[0]["День0"];
	
	МассивСтрокКУдалению = Новый Массив;
	Если КоличествоСтрокДо > ИндексСтроки Тогда
		Для ТекСтрокаИндекс = ИндексСтроки + 1 По КоличествоСтрокДо Цикл
			МассивСтрокКУдалению.Добавить(Данные[ТекСтрокаИндекс]);
		КонецЦикла;
	КонецЕсли;
	...
	Если Индекс < 31 Тогда
		Для ТекСтрокаИндекс = Индекс По 31 Цикл
	...
	Иначе
		...
		Для ТекСтрокаИндекс = 1 По ИндексСтроки Цикл
			Если Объект.РежимРаботы = "Календарь" Тогда // удаляем пустые строки
				ПустаяСтрока = Истина;
				Для т = 0 По ДнейПланирования Цикл
					Если Данные[ТекСтрокаИндекс]["День" + Строка(т)] Тогда
						ПустаяСтрока = Ложь;					
					КонецЕсли;
				КонецЦикла;
...
Процедура ДнейПланированияПриИзменении(Элемент)
	Объект.ГоризонтПланирования = МИН(Объект.ГоризонтПланирования, КонецМесяца(ТекущаяДата()));
	Обновить(Неопределено);
КонецПроцедуры
...
&НаКлиенте
Процедура ПеренестиЗаявкуНаКлиенте(НоваяДата, НоваяСумма = 0, НовыйИсточник = Неопределено, ВыдСтроки = Неопределено, Поле = Неопределено)
	...
	Для каждого ТекСтрокаГрафика Из ВыделенныеСтроки Цикл
		...
		ПеренестиЗаписьНаСервере(ТекСтрокаГрафика, НоваяДата, НоваяСумма, НовыйИсточник);
...
&НаСервереБезКонтекста
Процедура ПеренестиЗаписьНаСервере(КлючЗаписи, НоваяДата, НоваяСумма, НовыйИсточник, НомерИтерации = 1) Экспорт
...
Процедура ГрафикРазбить(Команда)
	...
	Если НЕ ТекДанные = Неопределено Тогда
		...
		ТекФорма.СуммаПлатежа = Элементы.РасшифровкаГрафика.ТекущиеДанные.Сумма;
		ТекФорма.Открыть();
...
Функция ПолучитьСтрокиРасшифровки()
	...
	Для Каждого Выборка Из Результат Цикл
		...
		Если Не СтрОтбор.Свойство("ПодразделениеЦФО") Тогда
			СтрОтбор.Вставить("ПодразделениеЦФО", Справочники.СтруктураПредприятия.ПустаяСсылка());
		КонецЕсли;
		НовыйКлючЗаписи = РегистрыСведений.сабГрафикПлатежей.СоздатьКлючЗаписи(СтрОтбор);
		МассивКлючей.Добавить(НовыйКлючЗаписи);
		
		//Если Не СтрОтбор.Свойство("ПодразделениеЦФО") Тогда
		СтрОтбор.Вставить("ПодразделениеЦФО", Выборка.Предприятие.ВидДеятельности);
		//КонецЕсли;
		НовыйКлючЗаписи = РегистрыСведений.сабГрафикПлатежей.СоздатьКлючЗаписи(СтрОтбор);
		МассивКлючей.Добавить(НовыйКлючЗаписи);
	КонецЦикла;
...
&НаСервере
Процедура ПечатьНаСервере(ДокументРезультат, ДанныеРасшифровки)
	...
	Для каждого ТекСтрока Из ДанныеТЧ Цикл
		ТекСтрока.ОстатокНаНачало = ?(Объект.КорректировкаОстатковПоНеоплаченнымЗаявкам, ТекСтрока.ОстатокНаНачалоРабочий, ТекСтрока.ОстатокНаНачало);
		Если ТекСтрока.СтатьяДДС = "Остаток на конец" Тогда
...
Процедура Печать(Команда)
	...
	ПечатьНаСервере(ФормаПечати.Результат, ФормаПечати.ДанныеРасшифровки);
	ФормаПечати.Открыть();
