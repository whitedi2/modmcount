...
Процедура ОбновитьНаСервере()
	...
	ЗапросТекст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	              ...
	              |						ТОГДА ЛОЖЬ
	              |					ИНАЧЕ Счет В ИЕРАРХИИ (&Счет51) ИЛИ Счет В ИЕРАРХИИ (&Счет50)
	              |				КОНЕЦ
	              |				,
	              |			,
	              ...
	              |						ТОГДА ЛОЖЬ
	              |					ИНАЧЕ Счет В ИЕРАРХИИ (&Счет51БУ) ИЛИ Счет В ИЕРАРХИИ (&Счет50БУ)
	              |				КОНЕЦ
	              |				,
	              |			,
	              |			НЕ &ПоОбъектам = ""Объекты"") КАК УчетныйОстатки
...
Процедура СобратьПлатежНаСервере(КлючЗаписи, МассивДат) Экспорт
	...
	Для каждого ТекЗапись Из НаборЗаписей Цикл
		Если ТекЗапись.УИДСтрокиДокумента = "" Или ТекЗапись.УИДСтрокиДокумента = МенеджерЗаписи.УИДСтрокиДокумента Тогда
...
Функция ВернутьМассивВсехКлючейСтрокГрафика()
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |	И (сабГрафикПлатежей.Сценарий = ЗНАЧЕНИЕ(Справочник.СценарииПланирования.ПустаяСсылка)
	               |			ИЛИ сабГрафикПлатежей.Сценарий = ЗНАЧЕНИЕ(Справочник.СценарииПланирования.Факт))
	               |АВТОУПОРЯДОЧИВАНИЕ";
