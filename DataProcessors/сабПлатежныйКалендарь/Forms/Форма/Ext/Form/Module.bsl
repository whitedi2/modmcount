...
Процедура ОбновитьНаСервере()
	...
	СтруктураЗаполненияПК = ВернутьТаблицуНачальногоЗаполненияПК(Объект.ВариантГруппировки, Объект.РежимРаботы);
	...
	ЗаполнитьТаблицуДанныхПКНаФорме(Данные, РезультатВыборка, Индекс, ДнейПланирования, Объект.ВариантГруппировки, Объект.РежимРаботы);
	
	СтруктураЗаполненияПК = ВернутьТаблицуНачальногоЗаполненияПК("Источники", "Календарь");
	
	РезультатВыборка = СтруктураЗаполненияПК.ТаблицаНачальногоЗаполнения;
	Индекс = СтруктураЗаполненияПК.ТекущийИндекс; 
	ДнейПланирования = СтруктураЗаполненияПК.КоличествоДнейПланирования;
	
	ЗаполнитьТаблицуДанныхПКНаФорме(ДанныеПоИсточникам, РезультатВыборка, Индекс, ДнейПланирования, "Источники", "Календарь");
...
&НаСервере
Функция ВернутьТаблицуНачальногоЗаполненияПК(ВариантГруппировкиПК, РежимРаботыПК)
...
&НаСервере
Процедура ЗаполнитьТаблицуДанныхПКНаФорме(ЗаполняемаяТаблица, РезультатВыборка, Индекс, ДнейПланирования, ВариантГруппировкиПК, РежимРаботыПК)
	...
	Для Каждого Выборка Из РезультатВыборка Цикл
		...
		ИначеЕсли НЕ ТекИсточник = Выборка.Источник ИЛИ НЕ ТекПредприятие = Выборка.Предприятие Тогда
			...
			ЗаполняемаяТаблица[0].ОстатокНаКонец = ЗаполняемаяТаблица[0]["День1"]  + ?(Объект.КорректировкаОстатковПоНеоплаченнымЗаявкам, ЗаполняемаяТаблица[0].ПлатежиПоРеестрам, 0);
			НоваяСтрока.ОстатокНаКонец = ЗаполняемаяТаблица[ИндексСтрокиОстатковНаНачало]["День1"] + ?(Объект.КорректировкаОстатковПоНеоплаченнымЗаявкам, ТекОстатокПоРеестрамКонец, 0);
