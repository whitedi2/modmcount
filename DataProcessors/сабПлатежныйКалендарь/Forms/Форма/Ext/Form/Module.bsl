...
Процедура ПеренестиЗаписьНаСервере(КлючЗаписи, НоваяДата, НоваяСуммаСтруктура, НовыйИсточник, НомерИтерации = 1) Экспорт
	...
	Если ДатаТекущая = Дата(1,1,1) Тогда
		РегистрыСведений.сабИсторияИзмененийПлатежногоКалендаря.ДобавитьЗапись(НоваяДата,ПользователиКлиентСервер.ТекущийПользователь(),Перечисления.сабДействияПриИзмененииПлатежногоКалендаря.СозданиеЗаписи,КлючЗаписи.Документ,СуммаТекущая,);
	ИначеЕсли (НоваяСумма = 0) И (НоваяДата <> ДатаТекущая) Тогда
		РегистрыСведений.сабИсторияИзмененийПлатежногоКалендаря.ДобавитьЗапись(НоваяДата,ПользователиКлиентСервер.ТекущийПользователь(),Перечисления.сабДействияПриИзмененииПлатежногоКалендаря.ПереносОплаты,КлючЗаписи.Документ,СуммаТекущая,) ;
	ИначеЕсли НоваяСумма <> 0 Тогда
		Если МенеджерЗаписи2.Сумма <> 0 Тогда
			ИтогоСумма = МенеджерЗаписи2.Сумма;
		ИначеЕсли  МенеджерЗаписи1.Сумма <> 0 Тогда
			ИтогоСумма = МенеджерЗаписи1.Сумма;			
		КонецЕсли;
		
		РегистрыСведений.сабИсторияИзмененийПлатежногоКалендаря.ДобавитьЗапись(НоваяДата,ПользователиКлиентСервер.ТекущийПользователь(),Перечисления.сабДействияПриИзмененииПлатежногоКалендаря.ДроблениеСуммы,КлючЗаписи.Документ,НоваяСумма,"Итого сумма: "+ИтогоСумма);
...
Процедура ДанныеПеретаскивание(Элемент, ПараметрыПеретаскивания, СтандартнаяОбработка, Строка, Поле)
	...
	Если МассивСтрок.Количество() И (СтрНайти(Поле.Имя, Элемент.Имя + "День") ИЛИ СтрНайти(Поле.Имя, Элемент.Имя + "БудущиеПлатежи")) Тогда
...
Процедура ГрафикУдалитьПлатежНаСервере(КлючЗаписи)
	...
	ЗаполнитьЗначенияСвойств(МенеджерЗаписи1, КлючЗаписи);
	МенеджерЗаписи1.Прочитать();
	СуммаПлатежа = МенеджерЗаписи1.Сумма;
	МенеджерЗаписи1.Удалить();
	РегистрыСведений.сабИсторияИзмененийПлатежногоКалендаря.ДобавитьЗапись(КлючЗаписи.ДатаПлатежа,ПользователиКлиентСервер.ТекущийПользователь(),Перечисления.сабДействияПриИзмененииПлатежногоКалендаря.УдалениеСтрокиГрафика,КлючЗаписи.Документ,СуммаПлатежа,);
КонецПроцедуры
...
Процедура УстанвитьКОплате(КлючЗаписи)
	...
	Если ТипЗнч(МенеджерЗаписи.Документ) = Тип("ДокументСсылка.Д_ЗаявкаНаФинансирование") И НЕ МенеджерЗаписи.ТочкаМаршрута = Перечисления.Согласование1ТочкиМаршрута.Действие5 И НЕ МенеджерЗаписи.ТочкаМаршрута = Перечисления.Согласование1ТочкиМаршрута.Завершение Тогда
		БылСтатус = Строка(МенеджерЗаписи.ТочкаМаршрута);
	...
	РегистрыСведений.сабИсторияИзмененийПлатежногоКалендаря.ДобавитьЗапись(МенеджерЗаписи.ДатаПлатежа,ПользователиКлиентСервер.ТекущийПользователь(),Перечисления.сабДействияПриИзмененииПлатежногоКалендаря.ИзменениеТочкиМаршрута,МенеджерЗаписи.Документ,МенеджерЗаписи.Сумма,"Был статус: "+БылСтатус);
...
Функция ВернутьТаблицуНачальногоЗаполненияПК(ВариантГруппировкиПК, РежимРаботыПК, КорректировкаОстатковПоНеоплаченнымЗаявкам)
	...
	ЗапросТекст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	              ...
	              |	Выразить(УчетныйОстатки.Субконто2 КАК Справочник.БанковскиеСчета).Владелец КАК Организация
	              |ПОМЕСТИТЬ Остатки
	              |ИЗ
	              ...
	              |				,
	              |			,
	              |			НЕ &ПоОбъектам = ""Объекты"" И СценарийПлана = &СценарийФакт) КАК УчетныйОстатки
	              |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.Б_ПлановыеКурсыВалют.СрезПоследних(
	              |				,
	              |				Валюта2 = &ЭквивалентнаяВалюта
	              |					И Сценарий = &АктуальныйСценарий) КАК Б_ПлановыеКурсыВалютСрезПоследних
	...
	УстановитьЗначениеПараметраНастроек(Отбор.Настройки, "СценарийФакт", Справочники.СценарииПланирования.СценарийФакт());
