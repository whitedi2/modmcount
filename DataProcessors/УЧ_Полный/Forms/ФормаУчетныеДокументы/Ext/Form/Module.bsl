...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИндексГруппы = 1;
	ОбщийИндекс = 1;
	Для каждого ТекПодсистема Из Метаданные.Подсистемы.модульУправленка.Подсистемы.сабУправленческийУчет.Подсистемы.сабУправленческийУчетСлужебная.Подсистемы.сабГруппировкаУчетныхДокументовПереопределяемый.Подсистемы Цикл
...
Процедура СоздатьЭлементыГруппы(Родитель, ТекПодсистема)
	...
	Для каждого ВидОперации Из ВремТЧ Цикл
			...
			Для каждого ТекКомандаДокумента Из Метаданные.Документы[ВидОперации.Имя].Команды Цикл
				Если НЕ ВидимостьИДоступность(ТекКомандаДокумента) Тогда
					Продолжить;			
				КонецЕсли;
...
Процедура ВыборВидаОперации(Команда)
	...
	//частные случаи
	Если Команда.Имя = "КомандаУЧ_ПоступлениеТоваров___ВидОперации__Поступление" Тогда
		ПараметрыФормы = Новый Структура("ВидимостьРасчетовПредприятий", Истина);
		ОткрытьФорму("Документ.УЧ_ПоступлениеТоваров.Форма.ФормаСпискаВнутреннихРеализаций", ПараметрыФормы, , , , );
		Возврат;		
	КонецЕсли;
	...
	ТекФорма = ПолучитьФорму("Документ." + КодВидаДокумента + ".ФормаСписка", Новый Структура("ВидОперации", ЗначениеПеречисления),,ЗначениеПеречисления);
