...
Процедура УУ_СформироватьПечатнуюФорму(МассивОбъектов, ПараметрыПечати, КоллекцияПечатныхФорм, ИмяМакета, СинонимМакета, ОбъектыПечати, ПараметрыВывода)
	...
	Для каждого ОбъектыТипа Из ТипыОбъектов Цикл
		...
		Попытка
			...
			Если ИмяМакета = "СчетЗаказКомплект" Тогда
				Индекс = ТаблицаСведенийСчетНаОплату.Количество() - 1; 
				Пока Индекс >= 0 Цикл
					ПорядокОплатыПоДоговору = ТаблицаСведенийСчетНаОплату[Индекс].ДоговорКонтрагента.ПорядокОплатыПоДоговору;
					Если ПорядокОплатыПоДоговору <> Перечисления.сабПорядокОплатПоДоговору.ПоСчетам И ЗначениеЗаполнено(ПорядокОплатыПоДоговору) Тогда 
						ТаблицаСведенийСчетНаОплату.Удалить(Индекс);  
					КонецЕсли; 
					Индекс = Индекс - 1; 
				КонецЦикла;
			КонецЕсли;
