...
Процедура ВыполнитьЗагрузкуВыписок()
	...
	Настройка = ИзвлечьНастройкуИзХранилища();
...
Функция ЗагрузитьСообщения(Знач УчетнаяЗапись, Отбор)
	...
	Попытка
		НаборСообщений = Соединение.Выбрать(Ложь, МассивНовыеСообщений);
...
Функция ЗагрузитьБанковскуюВыпискуНаСервере(ДвоичныеДанные, Кодировка)
	...
	НастройкиЗапуска.ЗапуститьВФоне = Ложь;
	НастройкиЗапуска.ЗапуститьНеВФоне = Истина;
