...
Процедура ВыгрузитьФайлыНаКлиенте()
	...
	Для Каждого СтрокаКонтрагенты Из Объект.Контрагенты Цикл
		...
		КонецЕсли;
		СтруктураСтрокаКонтрагенты = Новый Структура("Поставщик,ВсяНоменклатура,ИД,Алти,Лакталис,Экомилк,Бонфесто",СтрокаКонтрагенты.Поставщик,СтрокаКонтрагенты.ВсяНоменклатура,СтрокаКонтрагенты.ИД,СтрокаКонтрагенты.Алти,СтрокаКонтрагенты.Лакталис,СтрокаКонтрагенты.Экомилк,СтрокаКонтрагенты.Бонфесто);
		Если СтрокаКонтрагенты.Лакталис Тогда
...
&НаСервере
Процедура СформироватьФайлПоКонтрагенту(Файл_delivery,Файл_sku,Файл_ttoptions,Файл_stocks,Файл_receive,Файл_cancellations,Файл_ta,Файл_warehouses=Неопределено,НоменклатураДляОтбора,Поставщик,ИДДистрибьютора,ЭтоАлти,ЭтоЛакталис,ЭтоЭкомилк,ЭтоБонфесто)
//Формирования файла sku +++
	...
	КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.Текст = ВернутьТекстЗапросаdelivery(ЭтоЛакталис ИЛИ ЭтоБонфесто);
...
Процедура ВыгрузитьФайлыСервер(Файл_delivery,Файл_sku,Файл_ttoptions,Файл_stocks,Файл_receive,Файл_cancellations,Файл_ta,Файл_warehouses = Неопределено, СтрокаКонтрагенты = Неопределено)
	...
	Если СтрокаКонтрагенты <> Неопределено Тогда
		...
		СформироватьФайлПоКонтрагенту(Файл_delivery,Файл_sku,Файл_ttoptions,Файл_stocks,Файл_receive,Файл_cancellations,Файл_ta,Файл_warehouses,НоменклатураДляОтбора,Поставщик,ИДДистрибьютора,ЭтоАлти,ЭтоЛакталис,ЭтоЭкомилк,ЭтоБонфесто);
	КонецЕсли;
КонецПроцедуры 
