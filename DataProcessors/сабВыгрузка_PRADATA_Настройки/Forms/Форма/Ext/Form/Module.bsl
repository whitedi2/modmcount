...
Процедура ВыгрузитьФайлыНаКлиенте()
	...
	Для Каждого СтрокаКонтрагенты Из Объект.Контрагенты Цикл
		...
		КонецЕсли;
		СтруктураСтрокаКонтрагенты = Новый Структура("Поставщик,ВсяНоменклатура,ИД,Алти,Лакталис,Экомилк",СтрокаКонтрагенты.Поставщик,СтрокаКонтрагенты.ВсяНоменклатура,СтрокаКонтрагенты.ИД,СтрокаКонтрагенты.Алти,СтрокаКонтрагенты.Лакталис,СтрокаКонтрагенты.Экомилк);
		Если СтрокаКонтрагенты.Лакталис Тогда
		...
		КонецЕсли;
		ПутьКФайлу = СтрокаКонтрагенты.ПапкаДляВыгрузки;
...
&НаСервере
Процедура СформироватьФайлПоКонтрагенту(Файл_delivery,Файл_sku,Файл_ttoptions,Файл_stocks,Файл_receive,Файл_cancellations,Файл_ta,Файл_warehouses=Неопределено,НоменклатураДляОтбора,Поставщик,ИДДистрибьютора,ЭтоАлти,ЭтоЛакталис,ЭтоЭкомилк)
	...
	КонецЕсли;
	Запрос = Новый Запрос;
	Запрос.Текст = ВернутьТекстЗапросаdelivery(ЭтоЛакталис);
	ДатаНачала = ?(Объект.ПроизвольныйПериод, НачалоДня(Объект.ДатаНачала), НачалоДня(ТекущаяДата() - 3888000)); 
	ДатаОкончания = ?(Объект.ПроизвольныйПериод, КонецДня(Объект.ДатаОкончания - 86400), КонецДня(ТекущаяДата() - 86400));
	Запрос.УстановитьПараметр("ДатаНачала", ДатаНачала);
	Запрос.УстановитьПараметр("ДатаОкончания", ДатаОкончания);
	Запрос.УстановитьПараметр("ИДДистрибьютора", Объект.Ид_дистрибьютора);
...
Процедура ВыгрузитьФайлыСервер(Файл_delivery,Файл_sku,Файл_ttoptions,Файл_stocks,Файл_receive,Файл_cancellations,Файл_ta,Файл_warehouses = Неопределено, СтрокаКонтрагенты = Неопределено)
	...
	Если СтрокаКонтрагенты <> Неопределено Тогда
		...
		СформироватьФайлПоКонтрагенту(Файл_delivery,Файл_sku,Файл_ttoptions,Файл_stocks,Файл_receive,Файл_cancellations,Файл_ta,Файл_warehouses,НоменклатураДляОтбора,Поставщик,ИДДистрибьютора,ЭтоАлти,ЭтоЛакталис,ЭтоЭкомилк);
	КонецЕсли;
КонецПроцедуры 
