...
Процедура ЗаполнитьТаблицу(ВыбранныеДанныеТЗ)
	...
	ИспользоватьНесколькоПредприятий = Истина;
	...
	Если ЗначениеЗаполнено(АктуальныйСценарий) Тогда		
		Если АктуальныйСценарий = Справочники.СценарииПланирования.ПустаяСсылка()  Тогда
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |		КОГДА ВложенныйЗапрос.Ссылка.ТочкаМаршрута = ЗНАЧЕНИЕ(Перечисление.Согласование1ТочкиМаршрута.Действие1)
	               |				ИЛИ ВложенныйЗапрос.Ссылка.ТочкаМаршрута ЕСТЬ NULL
	               |					И НЕ Согласование1.Завершен = ИСТИНА
	               |				ИЛИ Согласование1.Ссылка ЕСТЬ NULL
	               |			ТОГДА ""На подготовке""
	               |		КОГДА ВложенныйЗапрос.Ссылка.ТочкаМаршрута = ЗНАЧЕНИЕ(Перечисление.Согласование1ТочкиМаршрута.Действие2)
	               |			ТОГДА ""На согласовании""
	               |		КОГДА ВложенныйЗапрос.Ссылка.ТочкаМаршрута = ЗНАЧЕНИЕ(Перечисление.Согласование1ТочкиМаршрута.Действие3)
	               |			ТОГДА ""На проверке""
	               |		КОГДА ВложенныйЗапрос.Ссылка.ТочкаМаршрута = ЗНАЧЕНИЕ(Перечисление.Согласование1ТочкиМаршрута.Действие4)
	               |			ТОГДА ""На результирующем согласовании""
	               |		КОГДА ВложенныйЗапрос.Ссылка.ТочкаМаршрута = ЗНАЧЕНИЕ(Перечисление.Согласование1ТочкиМаршрута.Действие5)
	               |			ТОГДА ""На исполнении""
	               |		КОГДА ВложенныйЗапрос.Ссылка.ТочкаМаршрута = ЗНАЧЕНИЕ(Перечисление.Согласование1ТочкиМаршрута.ПроверкаКонтрагентов)
	               ...
	               |	И ТекущаяЗаявка.ЗаявкаНаФинансирование = ЗНАЧЕНИЕ(Документ.Д_ЗаявкаНаФинансирование.ПустаяСсылка)
	               |
	               |ОБЪЕДИНИТЬ ВСЕ
	               |
	               |ВЫБРАТЬ
	               ...
	               |
	               |ВЫБРАТЬ
	               |	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.СтатьяДДС,
	               |	ВЫБОР
	               |		КОГДА Д_ЗаявкаНаАвансовыйОтчетСтрокиЗаявкиНаОплату.Ссылка ЕСТЬ NULL
	               |			ТОГДА ЕСТЬNULL(Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Сумма, Д_ЗаявкаНаФинансирование.Сумма)
	               |		ИНАЧЕ 0
	               |	КОНЕЦ,
	               |	ЕСТЬNULL(Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Предприятие, Д_ЗаявкаНаФинансирование.ЦФО),
	               |	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка,
	               |	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка.Дата,
	               |	NULL,
	               |	Д_ЗаявкаНаАвансовыйОтчетСтрокиЗаявкиНаОплату.Ссылка.Предприятие,
	               |	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка.Представление,
	               |	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка.Дата,
	               |	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка.Номер,
	               |	NULL,
	               |	ВЫБОР
	               ...
	               |		ИНАЧЕ 0
	               |	КОНЕЦ,
	               |	Д_ЗаявкаНаФинансирование.НазначениеПлатежаУчет,
	               |	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка.ДатаОплаты,
	               |	ЕСТЬNULL(Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Подразделение, Д_ЗаявкаНаФинансирование.Подразделение),
	               |	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.ИнвПроект,
	               |	ВЫБОР
	               |		КОГДА Д_ЗаявкаНаАвансовыйОтчетСтрокиЗаявкиНаОплату.Ссылка ЕСТЬ NULL
	               |			ТОГДА ЕСТЬNULL(Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Сумма, Д_ЗаявкаНаФинансирование.Сумма)
	               |		ИНАЧЕ 0
	               ...
	               |		СГРУППИРОВАТЬ ПО
	               |			Задача.Заявка) КАК ВложенныйЗапрос
	               |		ПО Д_ЗаявкаНаФинансирование.Ссылка = ВложенныйЗапрос.Заявка
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.Д_ЗаявкаНаАвансовыйОтчет.СтрокиЗаявкиНаОплату КАК Д_ЗаявкаНаАвансовыйОтчетСтрокиЗаявкиНаОплату
	               |		ПО Д_ЗаявкаНаФинансирование.Ссылка = Д_ЗаявкаНаАвансовыйОтчетСтрокиЗаявкиНаОплату.ДокОснование
	               |			И (НЕ &ИсключатьАвансовые)
	               |ГДЕ
	               |	Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка.ПометкаУдаления = ЛОЖЬ
	               |	И ВЫБОР
	               |			КОГДА Д_ЗаявкаНаФинансирование.ДатаОплаты < Д_ЗаявкаНаФинансирование.Дата
	               |				ТОГДА Д_ЗаявкаНаФинансирование.Дата
	               |			ИНАЧЕ Д_ЗаявкаНаФинансирование.ДатаОплаты
	               |		КОНЕЦ МЕЖДУ &ДатаНачала И &ДатаОкончания
	               |	И Д_ЗаявкаНаФинансирование.ПометкаУдаления = ЛОЖЬ
	               |	И (Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Ссылка.Предприятие В (&Предприятия)
	               |			ИЛИ Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Предприятие В (&Предприятия)
	               |			ИЛИ Д_ЗаявкаНаФинансирование.Автор В (&Автор))
	               |	И НЕ Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Сумма = 0
	               |	И Д_ЗаявкаНаФинансированиеТабличнаяЧасть.Предприятие В(&ЦФО)
	               |
	               |УПОРЯДОЧИТЬ ПО
	               |	ЦФО,
	               |	Дата";
	
	Запрос.УстановитьПараметр("ПредприятиеКазна", Неопределено);
