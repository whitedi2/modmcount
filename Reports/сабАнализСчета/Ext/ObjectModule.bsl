...
Процедура ПередКомпоновкойМакета(Схема, ОбъектОтчет) Экспорт
	...
	Если ЗначениеЗаполнено(ОбъектОтчет.Организация) Тогда
		СтандартныеОтчетыКлиентСервер.ДобавитьОтбор(КомпоновщикНастроекПроизвольный, "Организация", ОбъектОтчет.Организация,,, );
	КонецЕсли;
	Если ЗначениеЗаполнено(ОбъектОтчет.СценарийПлана) Тогда
		СтандартныеОтчетыКлиентСервер.ДобавитьОтбор(КомпоновщикНастроекПроизвольный, "СценарийПлана", ОбъектОтчет.СценарийПлана,,, );
	КонецЕсли;
