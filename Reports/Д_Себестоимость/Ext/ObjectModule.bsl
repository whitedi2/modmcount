...
Процедура ЗаполнитьТаблицу(ТабДок, ПараметрКоманды) Экспорт
	...
	Если ПараметрКоманды.Корректировки И ПараметрКоманды.Сценарий1 <> Справочники.СценарииПланирования.Факт Тогда
	...
	Для Каждого Предпр Из ПараметрКоманды.Предприятие Цикл
		...
		Если СтрокаНач.Количество() Тогда
			...
			Если ТЗ_Сорт.Найти(Справочники.СтатьиЗатрат.ЗатратыОсн, "Значение") <> Неопределено Тогда	
				НСтр = ТЗ_Сорт2.Добавить();
				ЗаполнитьЗначенияСвойств(НСтр, ТЗ_Сорт.Найти(Справочники.СтатьиЗатрат.ЗатратыОсн, "Значение"));
				ТЗ_Сорт.Удалить(ТЗ_Сорт.Индекс(ТЗ_Сорт.Найти(Справочники.СтатьиЗатрат.ЗатратыОсн, "Значение")));
			КонецЕсли;

			Если ТЗ_Сорт.Найти(Справочники.СтатьиЗатрат.ЗатратыВсп, "Значение") <> Неопределено Тогда	
				НСтр = ТЗ_Сорт2.Добавить();
				ЗаполнитьЗначенияСвойств(НСтр, ТЗ_Сорт.Найти(Справочники.СтатьиЗатрат.ЗатратыВсп, "Значение"));
				ТЗ_Сорт.Удалить(ТЗ_Сорт.Индекс(ТЗ_Сорт.Найти(Справочники.СтатьиЗатрат.ЗатратыВсп, "Значение")));
			КонецЕсли;
...
Функция ОбработатьПредприятия(СписокПредприятий, ПараметрКоманды, ДатаНач, ДатаКон, ПланСчетов1, ПланСчетов2)
	...
	Если НЕ ПараметрКоманды.Сценарий1 = Справочники.СценарииПланирования.Факт И НЕ ПараметрКоманды.Сценарий2 = Справочники.СценарииПланирования.Факт Тогда
		Возврат Новый Структура("ЗакрытыеПредприятия, ОткрытыеПредприятия", СписокПредприятий, Новый Массив);	
	КонецЕсли;
	
	Если НЕ ПараметрКоманды.Сценарий1 = Справочники.СценарииПланирования.Факт Тогда
...
Процедура ПросуммироватьТаблицу(ТЗ, ПараметрКоманды, ДваСценария)
	...
	ГруппыИсключения.Добавить(Справочники.СтатьиЗатрат.ЗатратыРеализация);
