...
Процедура ТекЗадачиПриАктивизацииСтроки(Элемент)
	...
	Если ТипыСовпадают И НЕ ТекДанные = Неопределено И ЗначениеЗаполнено(ТекДанные.Ссылка) Тогда
		...
		БПСервер.НастроитьДоступностьКнопок(Задача, ЗадачаТекКоманда1, ЗадачаТекКоманда3, ЗадачаТекКоманда2, ЗадачаТекИмяКоманды1, ЗадачаТекИмяКоманды3, ЗадачаТекИмяКоманды2, ВидимостьРазрешенияРедактирования);
...
Процедура ТекЗадачиВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТекЗадачиЗаявка" ИЛИ Поле.Имя = "ТекЗадачиОтступ" Тогда
