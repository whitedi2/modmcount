...
Процедура НовыйСрокВыполненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.ФормаВыбораДатыИВремени", Новый Структура("ЗаголовокФормы, ТекДата", "Выберите время", ?(СрокВыполнения > ТекущаяДата(), СрокВыполнения, ТекущаяДата() + 24*60*60)),,,,, Новый ОписаниеОповещения("НовыйСрокВыполненияНачалоВыбораЗавершение", ЭтаФорма), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
КонецПроцедуры

&НаКлиенте
Процедура НовыйСрокВыполненияНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
