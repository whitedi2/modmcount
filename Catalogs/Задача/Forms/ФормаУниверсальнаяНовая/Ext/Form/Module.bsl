...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	НастроитьДоступностьКнопок(Объект.Ссылка, Элементы.ЗаявкаСогласована.Заголовок, Элементы.ОтправитьДалее.Заголовок, Элементы.НаДоработку.Заголовок, ЗадачаТекИмяКоманды1, ЗадачаТекИмяКоманды2, ЗадачаТекИмяКоманды3, ВидимостьРазрешенияРедактирования);
...
Функция НайтиДолжность(Наименование)
	...
	|	Д_Должности.Ссылка
	|ИЗ
	|	Справочник.Д_Должности КАК Д_Должности
	|ГДЕ
...
Процедура ЗаполнитьСложныйМаршрут()
	...
	Для каждого ТекМаршрутСтрока Из ТекМассМаршрутов Цикл
		сабБПКлиентСервер.ДобавитьДеревоМаршрута(Элементы, Команды, ТекущаяСтрокаГруппы, МаршрутДерево, ТекМаршрутСтрока, УсловноеОформление, Новый Структура("КнопкиУправления, ДопУсловияВидимость, ТолькоПросмотр, Согласовано", Ложь, Ложь, Истина, Истина));
	КонецЦикла;
