...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения.Ссылка) = Тип("ДокументСсылка.Д_ЗаявкаНаФинансирование") Тогда
		ЗаполнитьНаСервере(ЭтотОбъект, ДанныеЗаполнения.Ссылка);
		Описание = БюджетныйНаСервере.ВернутьРеквизит(ДанныеЗаполнения.Ссылка, "НазначениеПлатежаУчет");
...
Процедура ПриЗаписи(Отказ)
	...
	Если Не ЗначениеЗаполнено(ТекТочкаМаршрута) И Не Завершен Тогда //старт процесса
		Действие2ПередСозданиемЗадач(Перечисления.СогласованиеОбщееТочкиМаршрута.Действие2, Новый Массив, Истина);
	КонецЕсли;
