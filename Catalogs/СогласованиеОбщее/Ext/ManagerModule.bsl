...
Процедура ОтчетОВыполнении(ТабДок, Ссылка) Экспорт
	...
	Макет = Справочники.СогласованиеОбщее.ПолучитьМакет("ВыполненныеЗадачи");
	Если Ссылка.Количество() Тогда
		...
		ИначеЕсли ТипЗнч(Ссылка[0]) = Тип("СправочникСсылка.Задача") Тогда
	...
	|ИЗ
	|	Справочник." + ТекБпМетаданные + " КАК СогласованиеОбщее
	|ГДЕ
