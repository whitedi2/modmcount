...
Процедура сабПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)	
	...
	Если Объект.ИспользоватьПоУмолчанию Тогда
		...
		|	И НЕ Предприятия.ПометкаУдаления
		|	И Предприятия.ИспользоватьПоУмолчанию";
		...
		Если Не РезультатЗапроса.Пустой() Тогда
			...
			Сообщение.Текст = "Нельзя использовать эту организацию по умолчанию. По умолчанию уже используется организация: " + Выборка.Наименование;
			Сообщение.Поле = "Объект.ИспользоватьПоУмолчанию";
			Сообщение.Сообщить();
			Возврат;
		КонецЕсли;
	КонецЕсли;
