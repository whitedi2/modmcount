...
Процедура УстановитьВидимость()
	...
	Элементы.ГруппаСогласованиеДоговора.Видимость = СтрВидов.ЭтоЗаявкаНаСогласованиеДоговора;
	Элементы.ГруппаЗаявкаНаАвансовыйОтчет.Видимость = СтрВидов.ЭтоЗаявкаНаАО;
...
Функция ПолучитьСтруктуруВидаДока(ВидДокумента)
	...
	ЭтоЗаявкаНаОплату = ВидДокумента = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаОплату");
	Стру.Вставить("ЭтоЗаявкаНаОплату", ЭтоЗаявкаНаОплату);
	
	ЭтоЗаявкаНаФинансирование = ВидДокумента = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаФинансирование");
	Стру.Вставить("ЭтоЗаявкаНаФинансирование", ЭтоЗаявкаНаФинансирование);

	ЭтоЗаявкаНаСогласованиеДоговора = ВидДокумента = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаСогласованиеДоговора");
	Стру.Вставить("ЭтоЗаявкаНаСогласованиеДоговора", ЭтоЗаявкаНаСогласованиеДоговора);

	ЭтоЗаявкаНаКадровоеДвижение = ВидДокумента = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаКадровоеДвижение");
	Стру.Вставить("ЭтоЗаявкаНаКадровоеДвижение", ЭтоЗаявкаНаКадровоеДвижение);
	
	ЭтоЗаявкаНаКорректировкуБюджета = ВидДокумента = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("КорректировкаБюджета");
	Стру.Вставить("ЭтоЗаявкаНаКорректировкуБюджета", ЭтоЗаявкаНаКорректировкуБюджета);
	
	ЭтоЗаявкаНаОтгрузку = ВидДокумента = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаОтгрузку");
	Стру.Вставить("ЭтоЗаявкаНаОтгрузку", ЭтоЗаявкаНаОтгрузку);
	
	ЭтоЗаявкаНаАО = ВидДокумента = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаАвансовыйОтчет");
	Стру.Вставить("ЭтоЗаявкаНаАО", ЭтоЗаявкаНаАО);
...
Процедура ТаблицаПредприятийПодразделениеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	|ГДЕ
	|	СтруктураПредприятия.Предприятие = &Владелец";
	...
	ОткрытьФорму("ОбщаяФорма.УниверсальнаяФормаВыбора", ПараметрыФормы,,,,, Новый ОписаниеОповещения("ТаблицаПредприятийПодразделениеНачалоВыбораЗавершение", ЭтаФорма, Новый Структура("СписокПодразделений", СписокПодразделений)), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	
КонецПроцедуры

&НаКлиенте
Процедура ТаблицаПредприятийПодразделениеНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(Объект.Предприятие) И НЕ ТекущийОбъект.ПредприятияПодразделения.Количество() Тогда
		СтрокаПредприятийПодразделений = ТекущийОбъект.ПредприятияПодразделения.Добавить();
...
Процедура ВывестиПредприятияПодразделения()
	...
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ВходящиеДанные.Предприятие КАК Предприятие,
	...
	Пока ВыборкаПредприятия.Следующий() Цикл
		...
		Пока ВыборкаПодразделения.Следующий() Цикл
			Если СтрокаТаблицы.СписокПодразделений.НайтиПоЗначению(ВыборкаПодразделения.Подразделение) = Неопределено Тогда
...
Процедура Реквизит1ДопУсловияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("Справочник.МаршрутыДвиженияЗаявок.Форма.ФормаДопУсловий", Новый Структура("ИД", Элемент.Родитель.ТекущиеДанные.ИДСтроки), ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
