...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	Для Каждого СтрокаТаблицы Из Объект.Проводки Цикл
		...
		ПредставлениеСчетаДт = СтрЗаменить(ПредставлениеСчетаДт," ","");
		ПредставлениеСчетаКт = СтрЗаменить(ПредставлениеСчетаКт," ","");
...
Процедура СвернутьСчета(Команда)
	...
	КонецЕсли;
	ОткрытьФорму("Документ.УЧ_Операция.Форма.ФормаВыбораСчетов",,,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);	
КонецПроцедуры
