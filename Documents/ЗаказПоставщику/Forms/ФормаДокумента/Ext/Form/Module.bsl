	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если Параметры.Свойство("ЗначениеКопирования") Тогда
			...
			Иначе
				...
				Если Объект.ТранзитныйГруз Тогда
					Объект.ДатаТранзитногоПоступления = КонецДня(ТекущаяДата()) + 1*24*60*60;
				КонецЕсли;
			КонецЕсли;
		Иначе
			Объект.ДатаПоступления = КонецДня(ТекущаяДата()) + 1*24*60*60;
...
Процедура РассчитатьКоличество(ТекДанные = Неопределено)
	...
	Если Не АльтернативнаяФорма И ТекДанные.КоэффициентПересчетаУпаковок Тогда
	...
	Если Изменять Тогда
		...
		Если Выборка.Количество() = 1 Тогда 
			...
			Если ЗначениеЗаполнено(Выборка.Склад) Тогда
				...
				КонецЦикла;
			ИначеЕсли ЗначениеЗаполнено(Объект.Контрагент) И ТипЗнч(Объект.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
...
Процедура ДатаДоставкиПриИзменении(Элемент)
	...
	Если Объект.ТранзитныйГруз Тогда
		Объект.ДатаТранзитногоПоступления = Объект.ДатаДоставки;
	КонецЕсли;
