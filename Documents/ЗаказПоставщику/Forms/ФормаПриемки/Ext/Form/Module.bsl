...
Процедура СоздатьПоступлениеНаСервере()
	...
	Иначе
		...
		Для Каждого ТекСтрока Из ТЗ_ПланФакт Цикл
			...
			НоваяСтрокаТЧ.СуммаНДС							= ТекСтрока.Сумма / ((100+ТекСтрока.СтавкаНДССтавка)/100) * (ТекСтрока.СтавкаНДССтавка/100);
			//НоваяСтрокаТЧ.СуммаНДСПоПервичнымДокументам		= ТекСтрока.СуммаНДСДок;
			НоваяСтрокаТЧ.СтранаПроисхождения = ТекСтрока.Номенклатура.СтранаПроисхождения;
...
Процедура ЗаполнитьПоВЕТИС(Команда)
	...
	ЗаполнитьПоВЕТИСНаСервере();
	ЗаполнитьПодвалТЗ();

КонецПроцедуры

&НаСервере
Процедура ЗаполнитьПоВЕТИСНаСервере()
...
Процедура ЗаполнитьПоУПДЭДО(Команда)
	...
	ЗаполнитьПоУПДЭДОНаСервере(ПоказатьСписокЭДО,СписокДокументовЭДО);
...
&НаСервере
Процедура ЗаполнитьПоУПДЭДОНаСервере(ПоказатьСписокЭДО,СписокДокументовЭДО)
