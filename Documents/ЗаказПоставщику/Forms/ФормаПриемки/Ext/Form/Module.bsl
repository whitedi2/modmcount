...
Процедура НазадПродолжить0()
	Если  СозданныйДокумент = Неопределено И ТЗ_ПланФакт.Итог("Сумма") <> 0 Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("НазадЗавершение", ЭтаФорма), "Сохранить в черновик введенные данные?", РежимДиалогаВопрос.ДаНет);
	Иначе
...
Процедура СоздатьПоступлениеНаСервере()
	...
	Если Не ЗначениеЗаполнено(ОбрабатываемыйДокумент.Организация) Тогда
		...
		Для Каждого ТекСтрока Из ТЗ_ПланФакт Цикл
			...
			НоваяСтрокаТЧ.СуммаНДС							= ТекСтрока.Сумма / ((100+НоваяСтрокаТЧ.СтавкаНДССтавка)/100) * (НоваяСтрокаТЧ.СтавкаНДССтавка/100);
...
Процедура РеквизитыСчФСовпадаютПриИзменении(Элемент)
	Элементы.НомерСчФ.Доступность = Не РеквизитыСчФСовпадают;
	Элементы.ДатаСчФ.Доступность = Не РеквизитыСчФСовпадают;
КонецПроцедуры
