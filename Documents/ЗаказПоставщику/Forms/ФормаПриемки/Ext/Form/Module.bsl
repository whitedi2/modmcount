...
Процедура ДалееПродолжить1(ПриемкаВосстановленаИзЧерновика)
	...
	Иначе
		...
		Если Не ПриемкаВосстановленаИзЧерновика Тогда
			...
			ДатаФактическогоПоступления = ТекущаяДата();
		КонецЕсли;  
...
Процедура ЗаполнитьТаблицуПланФакт()
	...
	Для Каждого ТекСтрока Из ТЗ_ПланФакт Цикл  //пока так, потом в запросе рассчитать
		...
		КонецЕсли; 
		УстановитьЗначениеЕстьРазницаВЦене(ТекСтрока);
	КонецЦикла; 
...
Процедура СоздатьПоступлениеПослеВопроса()
	...
	Иначе 
		...
		СтруктураСообщенийПриемки = Новый Структура("ДокументОтгрузки,Ветис,ЭДО,РасхожденияЦен",Новый Массив,Новый Массив,Новый Массив,Новый Массив);
...
&НаСервере
Процедура ОтправитьСообщениеТелеграмНаСервере(ТестСообщенияДляТелеграм)
	
	УстановитьПривилегированныйРежим(Истина);
	Если Не ПараметрыСеанса.РаботаСВнешнимиРесурсамиЗаблокирована Тогда
		Если Константы.сабОбменСЛогистическимиПрограммами.Получить() Тогда
			TokenTelegramm = ОбщегоНазначения.ПрочитатьДанныеИзБезопасногоХранилища("ТокенТелеграмДляБота", "Токен");
			ОтправляемСообщение = Ложь; 
			Если TokenTelegramm <> Неопределено Тогда
				Для Каждого СтрокаТЧ Из ТЗ_ПланФакт Цикл
					Если СтрокаТЧ.ВключатьВДокументОтгрузки И СтрокаТЧ.ЕстьРасхождениеЦены > 0.2 Тогда 
						ТестСообщенияДляТелеграм = ТестСообщенияДляТелеграм + Символы.ПС + 
						" № строки: " + СтрокаТЧ.НомерСтроки + ", расхождение: " + СтрокаТЧ.ЕстьРасхождениеЦены + "%"; 
						ОтправляемСообщение = Истина;	
					КонецЕсли
				КонецЦикла;
				Если ОтправляемСообщение Тогда
					Запрос = Новый Запрос;
					Запрос.Текст = 
					"ВЫБРАТЬ РАЗЛИЧНЫЕ
					|	ФизическиеЛица.ИДТелеграм КАК ИДТелеграм
					|ИЗ
					|	Справочник.ФизическиеЛица КАК ФизическиеЛица
					|ГДЕ
					|	ФизическиеЛица.ОтпралятьСообщенияИзПриемки
					|	И СОКРЛ(ФизическиеЛица.ИДТелеграм) <> """"";
					...
					Если Не РезультатЗапроса.Пустой() Тогда	
						Пока Выборка.Следующий() Цикл
							...
							Ресурс = "bot" + СокрЛП(TokenTelegramm) + "/SendMessage?chat_id=" + Выборка.ИДТелеграм + "&text=" + ТестСообщенияДляТелеграм; 
							Попытка
								Соединение  =  Новый HTTPСоединение(Сервер,443,,,,15,Новый ЗащищенноеСоединениеOpenSSL());
								Запрос = Новый HTTPЗапрос(Ресурс);
								Ответ = Соединение.Получить(Запрос);
								Если Не Ответ.КодСостояния = 200 тогда
									Попытка
										ЗаписьЖурналаРегистрации("Отправка сообщения в телеграм при создании документа отгрузки из приёмки (расхождения в ценах)", 
...
Процедура СоздатьПоступлениеНаСервере(ДокументВетисСсылка,СтруктураСообщенийПриемки,ВетисСформирован)
	...
	Попытка
		...
		ТестСообщенияДляТелеграм = "Из приемки по заказу: " + ОбрабатываемыйДокумент.Метаданные().Синоним + " № " + 
		ОбрабатываемыйДокумент.Номер + " от " + Формат(ОбрабатываемыйДокумент.Дата,"ДФ=dd.MM.yyyy") + " был создан документ поступления: " +
		ПоступлениеОбъект.Ссылка.Метаданные().Синоним + " № " + 
		ПоступлениеОбъект.Ссылка.Номер + " от " + Формат(ПоступлениеОбъект.Ссылка.Дата,"ДФ=dd.MM.yyyy") + ". Разница цен в созданном документе и цен в документе ЭДО более 0,2%: ";
		ОтправитьСообщениеТелеграмНаСервере(ТестСообщенияДляТелеграм);
	Исключение
	...
	УстановитьПривилегированныйРежим(Истина);
	ЕстьТекстРасхожденияЦен = Ложь;
	ТестСообщенияРасхожденияЦен = "";
	Для каждого СтрокаТЧ Из ТЗ_ПланФакт Цикл
		...
		Если СтрокаТЧ.ВключатьВДокументОтгрузки И СтрокаТЧ.ЕстьРасхождениеЦены > 0.2 Тогда 
			ТестСообщенияРасхожденияЦен = ТестСообщенияРасхожденияЦен + 
			"№ строки: " + СтрокаТЧ.НомерСтроки + ", Номенклатура: """ + СтрокаТЧ.Номенклатура + """, цена заказа/цена ЭДО: " +
			СтрокаТЧ.ЦенаЗаказ + "/" + СтрокаТЧ.ЦенаЭДО + ", расхождение: " + СтрокаТЧ.ЕстьРасхождениеЦены + "%" + Символы.ПС; 
			ЕстьТекстРасхожденияЦен = Истина;	
		КонецЕсли
		
	КонецЦикла; 
	Если ЕстьТекстРасхожденияЦен Тогда
	СтруктураСообщенийПриемки.РасхожденияЦен.Добавить("В приемке есть расхождения цен (более 0.2%): " + Символы.ПС + ТестСообщенияРасхожденияЦен);
	КонецЕсли;
	УстановитьПривилегированныйРежим(Ложь);
			...
			Если ДанныеЭДО.Свойство("ДанныеТабличныхЧастей") Тогда	
				Если ДанныеЭДО.ДанныеТабличныхЧастей.Свойство("Товары") Тогда
					...
					Для каждого СтрокаТЧ Из ТЗ_ПланФакт Цикл
						...
						КонецЕсли;
						УстановитьЗначениеЕстьРазницаВЦене(СтрокаТЧ);
					КонецЦикла;
				КонецЕсли;
			КонецЕсли;
		КонецЕсли; 
...
Процедура УстановитьЗначениеЕстьРазницаВЦене(СтрокаТЧ)
	
	Если СтрокаТЧ.ЦенаЗаказ > 0 И СтрокаТЧ.ЦенаЭДО > 0 И СтрокаТЧ.ЦенаЗаказ <> СтрокаТЧ.ЦенаЭДО Тогда
		БольшаяЦена = Макс(СтрокаТЧ.ЦенаЗаказ,СтрокаТЧ.ЦенаЭДО);
		МеньшаяЦена = Мин(СтрокаТЧ.ЦенаЗаказ,СтрокаТЧ.ЦенаЭДО);
		ПроцентРасхождения = -((МеньшаяЦена - БольшаяЦена) / БольшаяЦена * 100);
		СтрокаТЧ.ЕстьРасхождениеЦены = Окр(ПроцентРасхождения,2,РежимОкругления.Окр15как10);
	Иначе
		СтрокаТЧ.ЕстьРасхождениеЦены = 0;	
	КонецЕсли;
...
Процедура ТЗ_ПланФактЦенаЭДОПриИзменении(Элемент)
	...
	РассчитатьСуммуЭДОПоСтроке(ТД);
	ЗаполнитьПодвалТЗ();
	УстановитьЗначениеЕстьРазницаВЦене(ТД);
...
Процедура УстановитьДоступностьКладовщика()
	...
	Элементы.СтранаПроисхожденияГТД.Видимость = Не Кладовщик;
	//Элементы.ГруппаПодПланФактом.Видимость = Не Кладовщик;
	Элементы.ЗполнитьПоКоличествуЗаказа.Видимость = Не Кладовщик;
	Элементы.ЗаполнитьПоКоличествуЭДО.Видимость = Не Кладовщик;
	Элементы.ЗаполнитьПоКоличествуВетис.Видимость = Не Кладовщик;
	Элементы.ЦенаЗаказ.Видимость = Не Кладовщик;
	Элементы.ЦенаЭДО.Видимость = Не Кладовщик;
	Элементы.СуммаЗаказ.Видимость = Не Кладовщик;
	Элементы.СуммаЭДО.Видимость = Не Кладовщик;
    Элементы.ЗаполнениДляВсейТЧ.Видимость = Не Кладовщик;
