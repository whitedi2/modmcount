	...
	Для Каждого ТекСтрока Из ТЗ_ПланФакт Цикл 
		...
		//КонецЕсли;
		 Если ТекСтрока.КоличествоФакт = 0 Тогда
			...
			КонецЕсли;
		КонецЕсли;
		Если ТекСтрока.ЦенаДокБезНДС = 0  Тогда
			Сообщить("В строке с номенклатурой " + ТекСтрока.Номенклатура + " не заполнена цена!");
			Возврат;
		КонецЕсли;
		//Если ТекСтрока.КоличествоЭДО = 0 И ТекСтрока.КоличествоФакт <> 0 Тогда
		//	Сообщить("В строке с номенклатурой " + ТекСтрока.Номенклатура + " не заполнено количество по ЭДО!");
		//	Возврат;
		//КонецЕсли;
        //---
...
Процедура ЗаполнитьПоВЕТИС(Команда)
	...
	//Если ПоляЗаполены И ЗначениеЗаполнено(НакладнаяВЕТИС) Тогда
	//	ОписаниеОповещения = Новый ОписаниеОповещения("ВыполнитьПослеОтветаЗаполдениеНомерТТНДатаТТНВЕТИС",ЭтотОбъект);
	//	ПоказатьВопрос(ОписаниеОповещения, ТекстВопроса,РежимДиалогаВопрос.ДаНетОтмена);
	//Иначе
	//	ТекстВопроса = ?(ЗначениеЗаполнено(НакладнаяВЕТИС),"Заполнить данные из документа ВетИС?", 
	//	"Заполнить данные из Ветеринарно-сопроводительных документов в табличной части?");
	...
	Для Каждого СтрокаТЧ Из ТЗ_ПланФакт Цикл
		...
		Если СтрокаТЧ.НакладныеВетИС.Количество() = 1 Тогда 
			...
			Если Выборка.НайтиСледующий(СтроктураПоискаДляВыборки) Тогда
				Если ЗначениеЗаполнено(Выборка.КоэффициентЕдиницыВЕТИС) Тогда
					СтрокаТЧ.КоличествоДок = ?(СтрокаТЧ.Номенклатура.ЕдиницаИзмерения = Выборка.ЕдиницаИзмерения,КоличествоВСД,КоличествоВСД / Выборка.КоэффициентЕдиницыВЕТИС); 
				Иначе
				    //СтрокаТЧ.КоличествоДок = ?(ЗначениеЗаполнено(СтрокаТЧ.Номенклатура.Вес),СтрокаТЧ.Номенклатура.Вес * КоличествоВСД,КоличествоВСД);
					//СтрокаТЧ.КоличествоДок = ?(ЗначениеЗаполнено(СтрокаТЧ.Номенклатура.Вес),КоличествоВСД / СтрокаТЧ.Номенклатура.Вес,КоличествоВСД);
			...
			Иначе
				...
				СтрокаТЧ.КоличествоДок = КоличествоВСД;
			КонецЕсли;
			//СтрокаТЧ.КоличествоДок = СправочникВСДПолучитьКоличество(СтрокаТЧ.НакладныеВетИС[0].Значение);	
		...
		Если  СтрокаТЧ.НакладныеВетИС.Количество() > 1 Тогда
			...
			Если ТЗСерийВСД.Количество() = 1 Тогда
				...
				Если Выборка.НайтиСледующий(СтроктураПоискаДляВыборки) Тогда
					Если ЗначениеЗаполнено(Выборка.КоэффициентЕдиницыВЕТИС) Тогда
						//СтрокаТЧ.КоличествоДок = ТЗСерийВСД[0].Количество * Выборка.КоэффициентЕдиницыВЕТИС;
						СтрокаТЧ.КоличествоДок = ?(СтрокаТЧ.Номенклатура.ЕдиницаИзмерения = Выборка.ЕдиницаИзмерения,ТЗСерийВСД[0].Количество,ТЗСерийВСД[0].Количество / Выборка.КоэффициентЕдиницыВЕТИС); 
					Иначе
					    //СтрокаТЧ.КоличествоДок = ?(ЗначениеЗаполнено(СтрокаТЧ.Номенклатура.Вес),СтрокаТЧ.Номенклатура.Вес * ТЗСерийВСД[0].Количество,ТЗСерийВСД[0].Количество);
			...
			ИначеЕсли  ТЗСерийВСД.Количество() > 1 Тогда
				...
				Если Выборка.НайтиСледующий(СтроктураПоискаДляВыборки) Тогда
					Если ЗначениеЗаполнено(Выборка.КоэффициентЕдиницыВЕТИС) Тогда
						//СтрокаТЧ.КоличествоДок = ОбщееКоличество * Выборка.КоэффициентЕдиницыВЕТИС; 
						СтрокаТЧ.КоличествоДок = ?(СтрокаТЧ.Номенклатура.ЕдиницаИзмерения = Выборка.ЕдиницаИзмерения,ОбщееКоличество,ОбщееКоличество / Выборка.КоэффициентЕдиницыВЕТИС); 
					Иначе
...
&НаСервере
Процедура ПодобратьВЕТИСДляСтрокНаСервере()
	Отказ = Ложь;
	Для Каждого СтрокаТЧ ИЗ ТЗ_ПланФакт Цикл
	...
	Иначе
		...
		КонецЕсли;
		//ТЗПланФактДляЗапроса = ТЗ_ПланФакт.Выгрузить(,"НомерСтроки, Номенклатура");
	КонецЕсли;
...
Процедура ВыполнитьПослеОтветаЗаполнениеТЧВетис(Результат,ДопПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда 
		ПодобратьВЕТИСДляСтрокНаСервере();
	КонецЕсли;
...
&НаСервере
Процедура ЗаполнитьТЧЭДОНаСервере()
	Если ЗначениеЗаполнено(ВходящаяУПДЭДО) Тогда 
		...
		Иначе
			...
			Если ДанныеЭДО.Свойство("ДанныеТабличныхЧастей") Тогда	
				Если ДанныеЭДО.ДанныеТабличныхЧастей.Свойство("Товары") Тогда
					...
					//|		ПО (ВТТоварыЭДО.Номенклатура = ОписаниеНоменклатурыИС.Номенклатура
					//|				И ВТТоварыЭДО.ЕдиницаИзмерения <> ВТТоварыЭДО.Номенклатура.ЕдиницаИзмерения)
					
					|СГРУППИРОВАТЬ ПО
					|	ВТТоварыЭДО.НомерГТД,
					|	ВТТоварыЭДО.СтранаПроисхождения,
					|	ВТТоварыЭДО.Цена,
					//|	ЕСТЬNULL(ОписаниеНоменклатурыИС.КоличествоВПотребительскойУпаковке, 1),
					...
					Для каждого СтрокаТЧ Из ТЗ_ПланФакт Цикл
						...
						Если Выборка.НайтиСледующий(СтруктураОтбораЭДО) Тогда   
							...
							КонецЕсли;
							//Если СтрокаТЧ.ЕдиницаИзмерения <> Выборка.ЕдиницаИзмерения И Выборка.Коэффициент <> 1 Тогда
								//СтрокаТЧ.СуммаЭДО = СтрокаТЧ.СуммаЭДО * Выборка.КоличествоВПотребительскойУпаковке;
							//КонецЕсли;
							СтрокаТЧ.СуммаЭДО = ?(ДанныеЭДО.Шапка.СуммаВключаетНДС,Выборка.Сумма,Выборка.Сумма * ((100+СтрокаТЧ.СтавкаНДССтавка)/100));
...
Процедура ЗаполнитьПоКоличествуЭДО(Команда)
	...
	Если ЗначениеЗаполнено(ЗаполнениДляВсейТЧ) Тогда
		//Для всех строк
        ЗаполнитьКоличествоДляВсехСтрокТЧПланФакт("ЭДО");
	Иначе 
		//Для текущей строки 
		ТД = Элементы.ТЗ_ПланФакт.ТекущиеДанные;
		ЗаполнитьКоличествоДляСтрокиТЧПланФакт(ТД, "ЭДО");	
	КонецЕсли;
	ЗаполнитьПодвалТЗ(); 
...
Процедура ЗполнитьПоКоличествуЗаказа(Команда)
	...
	Если ЗначениеЗаполнено(ЗаполнениДляВсейТЧ) Тогда
		//Для всех строк
        ЗаполнитьКоличествоДляВсехСтрокТЧПланФакт("Заказ");
	Иначе 
		//Для текущей строки
		ТД = Элементы.ТЗ_ПланФакт.ТекущиеДанные;
		ЗаполнитьКоличествоДляСтрокиТЧПланФакт(ТД, "Заказ");	
	КонецЕсли;
    ЗаполнитьПодвалТЗ(); 
...
Процедура ЗаполнитьПоКоличествуВетис(Команда)
	...
	Если ЗначениеЗаполнено(ЗаполнениДляВсейТЧ) Тогда
		//Для всех строк
        ЗаполнитьКоличествоДляВсехСтрокТЧПланФакт("Ветис");
	Иначе 
		//Для текущей строки
		ТД = Элементы.ТЗ_ПланФакт.ТекущиеДанные;
		ЗаполнитьКоличествоДляСтрокиТЧПланФакт(ТД, "Ветис");
	КонецЕсли;
    ЗаполнитьПодвалТЗ(); 
...
&НаКлиенте
Процедура ЗаполнитьКоличествоДляСтрокиТЧПланФакт(ТД, ВыбранноеЗначениеДляКоличества)
...
&НаСервере
Процедура ЗаполнитьКоличествоДляВсехСтрокТЧПланФакт(ВыбранноеЗначениеДляКоличества)
