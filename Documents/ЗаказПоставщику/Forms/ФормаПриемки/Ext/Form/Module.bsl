...
Процедура СоздатьПоступлениеНаСервере()
	...
	Если Не ЗначениеЗаполнено(ОбрабатываемыйДокумент.Организация) ИЛИ ОбрабатываемыйДокумент.ПокупкаВРозницу Тогда
		...
		ПоступлениеОбъект.Дата = ДатаТТН; //?(ЗначениеЗаполнено(ДатаФактическогоПоступления), ДатаФактическогоПоступления,ДатаТТН);
		...
		Если СтрокаТЧ.НакладныеВетИС.Количество() = 1 Тогда 
			...
			Если Выборка.НайтиСледующий(СтроктураПоискаДляВыборки) Тогда
				Если ЗначениеЗаполнено(Выборка.КоэффициентЕдиницыВЕТИС) Тогда
					СтрокаТЧ.КоличествоДок = КоличествоВСД * Выборка.КоэффициентЕдиницыВЕТИС; 
				Иначе
		...
		Если  СтрокаТЧ.НакладныеВетИС.Количество() > 1 Тогда
			...
			ИначеЕсли  ТЗСерийВСД.Количество() > 1 Тогда
				...
				Если Выборка.НайтиСледующий(СтроктураПоискаДляВыборки) Тогда
					...
					Иначе
					 СтрокаТЧ.КоличествоДок = ?(ЗначениеЗаполнено(СтрокаТЧ.Номенклатура.Вес),СтрокаТЧ.Номенклатура.Вес * ОбщееКоличество,ОбщееКоличество);
