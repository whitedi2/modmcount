&НаКлиенте
Перем БудетИзменениеСтатусовДругихДокументов;
...
Процедура ЗаполнитьОстаткиИРезервы(РасчетПоСтроке = Ложь, СтруктураДанных = Неопределено)
	...
	Если РасчетПоСтроке Тогда
		...
		Для каждого ТекСтрокаНайденная Из НайденныеСтроки Цикл
			Если НеУчитыватьРезервыПриходаДляКонтроляОстатковВЗаказахКлиентов И НачалоДня(ТекСтрокаНайденная.ДатаЗаказа) <= НачалоДня(Объект.Дата) И НЕ ТипЗнч(ТекСтрокаНайденная.Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") И НЕ ТипЗнч(ТекСтрокаНайденная.Заказ) = Тип("ДокументСсылка.ЗаказНаВозвратОтКлиента") Тогда
	...
	Для каждого ТекСтрока Из Выборка Цикл
		Если НеУчитыватьРезервыПриходаДляКонтроляОстатковВЗаказахКлиентов И НачалоДня(ТекСтрока.Дата) <= НачалоДня(Объект.Дата) И НЕ ТипЗнч(ТекСтрока.Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") И НЕ ТипЗнч(ТекСтрока.Заказ) = Тип("ДокументСсылка.ЗаказНаВозвратОтКлиента") Тогда		
			Продолжить;	
		КонецЕсли;
...
Процедура ТабличнаяЧастьПередУдалением(Элемент, Отказ)
	...
	СтрокиРабот = Объект.РаботыИРасценки.НайтиСтроки(Новый Структура("УИДТЧ", ТекДанные.УИД));
	Для Каждого СтрокаРабот Из СтрокиРабот Цикл
		Объект.РаботыИРасценки.Удалить(СтрокаРабот);	
	КонецЦикла;
...
Процедура УстановитьМатериалыПоСпецификации(ТекСпецификация, ПервоначальноеКоличество, ТекУИД)
	...
	|		КОГДА СпецификацииСостав.Ссылка.Количество > 0
	|			ТОГДА СпецификацииСостав.Ссылка.Количество
	|		ИНАЧЕ 1
	...
	Если ЗначениеЗаполнено(ТекСпецификация) Тогда
		...
		Для Каждого СоставСпецификаций Из НайденныйСоставСпецификаций Цикл
			...
			Если СоставПодчиненнойСпецификации.Количество() > 0 Тогда
				Для Каждого СтрокаПодчиненнойСпецификации Из СоставПодчиненнойСпецификации Цикл
					...
					Если СоставПодчиненнойСпецификации2.Количество() > 0 Тогда
						Для Каждого СтрокаПодчиненнойСпецификации2 Из СоставПодчиненнойСпецификации2 Цикл
								...
								Для Каждого СтрокаПодчиненнойСпецификации3 Из СоставПодчиненнойСпецификации3 Цикл
									...
									Если СоставПодчиненнойСпецификации4.Количество() > 0 Тогда
										Для Каждого СтрокаПодчиненнойСпецификации4 Из СоставПодчиненнойСпецификации4 Цикл
											...
											СтрокаТаблМатериалов.Сумма = СтрокаПодчиненнойСпецификации4.Цена*СтрокаТаблМатериалов.Количество;//СтрокаПодчиненнойСпецификации4.Сумма;
									...
									Иначе
										...
										СтрокаТаблМатериалов.Сумма = СтрокаПодчиненнойСпецификации3.Цена*СтрокаТаблМатериалов.Количество;//СтрокаПодчиненнойСпецификации3.Сумма;
							...
							Иначе
								...
								СтрокаТаблМатериалов.Сумма = СтрокаПодчиненнойСпецификации2.Цена*СтрокаТаблМатериалов.Количество;//СтрокаПодчиненнойСпецификации2.Сумма;
					...
					Иначе
						...
						СтрокаТаблМатериалов.Сумма = СтрокаПодчиненнойСпецификации.Цена*СтрокаТаблМатериалов.Количество;//СтрокаПодчиненнойСпецификации.Сумма;
			...
			Иначе
				...
				СтрокаТаблМатериалов.Сумма = СоставСпецификаций.Цена*СтрокаТаблМатериалов.Количество;//СоставСпецификаций.Сумма;
...
Процедура УстановитьОтборСтрокРаботыИРасценки()
	...
	Иначе
		Элементы.РаботыИРасценки.ОтборСтрок = Новый ФиксированнаяСтруктура();
	КонецЕсли;	
...
Процедура ПриОткрытии(Отказ)
	...
	Элементы.Группа8.Видимость = Не СкрытьРесурсы;
...
Процедура ПослеЗаписи(ПараметрыЗаписи)
	...
	Если ПараметрыЗаписи.Свойство("ИзмСтатусовЗаказовСОдинаковымЛотом") Тогда
		Если ПараметрыЗаписи.ИзмСтатусовЗаказовСОдинаковымЛотом Тогда
		МЗаказыСОдинПартиями = Новый Массив; 
		ТекстЗаказыСОдинПартиями = "";
		МСсылокТекЗаказов = Новый Массив;
		МСсылокТекЗаказов.Добавить(Объект.Ссылка);
		ЗаполнитьМассивЗаказыСОдинаковымиПартиями(МЗаказыСОдинПартиями,ТекстЗаказыСОдинПартиями,МСсылокТекЗаказов,Объект.Статус);
		Если МЗаказыСОдинПартиями.Количество() > 0 Тогда
            ВопросДляИзмененияСтатусаЗаказовСОдинаковымЛотом(МЗаказыСОдинПартиями,ТекстЗаказыСОдинПартиями,Объект.Статус);
		КонецЕсли;
		КонецЕсли;
	КонецЕсли;
...
Асинх Процедура ВопросДляИзмененияСтатусаЗаказовСОдинаковымЛотом(МЗаказыСОдинПартиями,ТекстЗаказыСОдинПартиями,НовыйСтатус)
	...
	БудетИзменениеСтатусовДругихДокументов = 1;
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		...
		ОбещаниеПр = ПредупреждениеАсинх(РезультатИзмененияСтатусаЗаказовТекст); 
		РезультатПр = Ждать ОбещаниеПр;
	КонецЕсли;
	Если БудетИзменениеСтатусовДругихДокументов = 2 Тогда
		БудетИзменениеСтатусовДругихДокументов = Неопределено;
		Закрыть();
	КонецЕсли; 
	Если БудетИзменениеСтатусовДругихДокументов = 1 Тогда
		БудетИзменениеСтатусовДругихДокументов = Неопределено;
	КонецЕсли;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	МассивКУд = Новый Массив;
	Для каждого ТекСтрока Из ТекущийОбъект.РаботыИРасценки Цикл
		НайденныеСтрокиТовары = ТекущийОбъект.ТабличнаяЧасть.НайтиСтроки(Новый Структура("УИД", ТекСтрока.УИДТЧ));
		Если Не НайденныеСтрокиТовары.Количество() Тогда
			МассивКУд.Добавить(ТекСтрока);	
		КонецЕсли;
	КонецЦикла;
	Для каждого ТекУд Из МассивКУд Цикл
		ТекущийОбъект.РаботыИРасценки.Удалить(ТекУд);	
	КонецЦикла;
	
	//Изменение статусов заказов с таким же лотом 
	Если ЗначениеЗаполнено(Объект.Ссылка) И Объект.Статус <> Объект.Ссылка.Статус Тогда
		ПараметрыЗаписи.Вставить("ИзмСтатусовЗаказовСОдинаковымЛотом",Истина);
	КонецЕсли;
...
Процедура УстановитьДоступностьДокумента()
	...
	ТекДоступность = НЕ Объект.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовНаПроизводство.Новый");
...
&НаКлиенте
Процедура СтатусОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	
	ВыбранноеЗначениеСтатуса = ВыбранноеЗначение;
	Если НЕ ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовНаПроизводство.Новый") Тогда
...
Процедура ПередЗакрытием(Отказ, ЗавершениеРаботы, ТекстПредупреждения, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(БудетИзменениеСтатусовДругихДокументов) Тогда
		БудетИзменениеСтатусовДругихДокументов = 2;
		Отказ = Истина;
	КонецЕсли;
