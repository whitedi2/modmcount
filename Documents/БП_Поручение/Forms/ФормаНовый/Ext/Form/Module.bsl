...
Процедура СрокИсполненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	ОткрытьФорму("ОбщаяФорма.ФормаВыбораДатыИВремени", Новый Структура("ЗаголовокФормы", "Установить срок исполнения"),,,,, Новый ОписаниеОповещения("СрокИсполненияНачалоВыбораЗавершение", ЭтаФорма), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	
КонецПроцедуры

&НаКлиенте
Процедура СрокИсполненияНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
...
Процедура СписокИсполнителейСрокИсполненияНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ТекДата = Неопределено;
	ОткрытьФорму("ОбщаяФорма.ФормаВыбораДатыИВремени", Новый Структура("ЗаголовокФормы", "Установить срок исполнения"),,,,, Новый ОписаниеОповещения("СписокИсполнителейСрокИсполненияНачалоВыбораЗавершение", ЭтаФорма), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
КонецПроцедуры

&НаКлиенте
Процедура СписокИсполнителейСрокИсполненияНачалоВыбораЗавершение(Результат, ДополнительныеПараметры) Экспорт
