...
Функция ПечатьНакладная(МассивОбъектов) Экспорт
	...
	иначе     
		...
		ОбластьШапкаТаблицыНачало 	= Макет.ПолучитьОбласть("ШапкаТаблицы1|НачалоСтроки1");
		ОбластьСтрокаТаблицыНачало 	= Макет.ПолучитьОбласть("СтрокаТаблицы1|НачалоСтроки1");
		ОбластьПодвалТаблицыНачало 	= Макет.ПолучитьОбласть("ПодвалТаблицы|НачалоСтроки");
		
		ОбластьШапкаТаблицыКолонкаКодов 	= Макет.ПолучитьОбласть("ШапкаТаблицы1|КолонкаКодов1");
		ОбластьСтрокаТаблицыКолонкаКодов 	= Макет.ПолучитьОбласть("СтрокаТаблицы1|КолонкаКодов1");
		ОбластьПодвалТаблицыКолонкаКодов 	= Макет.ПолучитьОбласть("ПодвалТаблицы|КолонкаКодов");
		
		ОбластьШапкаТаблицыКолонкаКодов.Параметры.ИмяКолонкиКодов = КолонкаКодов; 
		
		ОбластьШапкаТаблицыКолонкаУпаковок 		= Макет.ПолучитьОбласть("ШапкаТаблицы1|КолонкаУпаковок1");
		ОбластьСтрокаТаблицыКолонкаУпаковок 	= Макет.ПолучитьОбласть("СтрокаТаблицы1|КолонкаУпаковок1");
		ОбластьПодвалТаблицыКолонкаУпаковок		= Макет.ПолучитьОбласть("ПодвалТаблицы|КолонкаУпаковок");
		
		ОбластьКолонкаТоваров = Макет.Область("КолонкаТоваров1");
		
		Если НЕ ВыводитьКоды Тогда
			
			ОбластьКолонкаТоваров.ШиринаКолонки = ОбластьКолонкаТоваров.ШиринаКолонки + Макет.Область("КолонкаКодов1").ШиринаКолонки;
			
		КонецЕсли;
		
		Если НЕ ВыводитьУпаковки Тогда
			
			ОбластьКолонкаТоваров.ШиринаКолонки = ОбластьКолонкаТоваров.ШиринаКолонки + Макет.Область("КолонкаУпаковокКоличество1").ШиринаКолонки
			+ Макет.Область("КолонкаУпаковокПредставление1").ШиринаКолонки; 		
			
		КонецЕсли;
		
		ОбластьШапкаТаблицыКолонкаТоваров 	= Макет.ПолучитьОбласть("ШапкаТаблицы1|КолонкаТоваров1");
		ОбластьСтрокаТаблицыКолонкаТоваров 	= Макет.ПолучитьОбласть("СтрокаТаблицы1|КолонкаТоваров1");
		ОбластьПодвалТаблицыКолонкаТоваров 	= Макет.ПолучитьОбласть("ПодвалТаблицы1|КолонкаТоваров1"); 
		
		ОбластьШапкаТаблицыСерияНоменклатуры  = Макет.ПолучитьОбласть("ШапкаТаблицы1|СерияНоменклатуры1");
		ОбластьСтрокаТаблицыСерияНоменклатуры = Макет.ПолучитьОбласть("СтрокаТаблицы1|СерияНоменклатуры1");
		ОбластьПодвалТаблицыСерияНоменклатуры = Макет.ПолучитьОбласть("ПодвалТаблицы1|СерияНоменклатуры1");
		
		ОбластьШапкаТаблицыКонец 	= Макет.ПолучитьОбласть("ШапкаТаблицы1|КонецСтроки1");
		ОбластьСтрокаТаблицыКонец 	= Макет.ПолучитьОбласть("СтрокаТаблицы1|КонецСтроки1");
		ОбластьПодвалТаблицыКонец 	= Макет.ПолучитьОбласть("ПодвалТаблицы1|КонецСтроки1");
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Если СерииНоменклатурыИспользуются тогда
			...
			ТабличныйДокумент.Присоединить(ОбластьШапкаТаблицыСерияНоменклатуры);
		...
		СуммаТоваров      = 0;
		СуммаУслуг        = 0;
		ТипУслуга = Перечисления.сабТипыНоменклатуры.Услуга;
		// СТРОКИ ТЧ
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл
			...
			Если СерииНоменклатурыИспользуются тогда
				ОбластьСтрокаТаблицыСерияНоменклатуры.Параметры.Заполнить(ВыборкаПоСтрокамТЧ);
				ТабличныйДокумент.Присоединить(ОбластьСтрокаТаблицыСерияНоменклатуры);	
			КонецЕсли;
		...
		Если СерииНоменклатурыИспользуются тогда	
			ТабличныйДокумент.Присоединить(ОбластьПодвалТаблицыСерияНоменклатуры);
		КонецЕсли;
...
Функция ПечатьМ4(МассивОбъектов, ОбъектыПечати)
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|	ГДЕ
		|		Документ.Ссылка В (&МассивДокументов)
		|	И НЕ Документ.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.сабТипыНоменклатуры.Услуга)
		|
		|	) КАК ВложенныйЗапрос
		|
		|УПОРЯДОЧИТЬ ПО
...
Функция ПечатьТТН(МассивОбъектов, ОбъектыПечати = Неопределено, ПараметрыПечати = Неопределено) Экспорт
	...
	|	ПеремещениеТоваров.Номенклатура.Весовой = 0 КАК Весовой
	|ИЗ
	...
	Запрос.УстановитьПараметр("МассивДокументов", МассивОбъектов);
	Запрос.УстановитьПараметр("СрокДоставки", "");
	Запрос.УстановитьПараметр("МаркаАвтомобиля", "");
	Запрос.УстановитьПараметр("МаркаПрицепа", "");
	Запрос.УстановитьПараметр("ГосНомерАвтомобиля", "");
	Запрос.УстановитьПараметр("ГосНомерПрицепа", "");
	Запрос.УстановитьПараметр("ПунктПогрузки", "");
	Запрос.УстановитьПараметр("ПунктРазгрузки", "");
	Запрос.УстановитьПараметр("Водитель", "");
	Запрос.УстановитьПараметр("Перевозчик", "");
	Запрос.УстановитьПараметр("Заказчик", "");
	Запрос.УстановитьПараметр("ВидПеревозки", "");
	Запрос.УстановитьПараметр("ЛицензионнаяКарточка", "");
	Запрос.УстановитьПараметр("ВодительскоеУдостоверение", "");
	...
	ОбъектыПечати.Добавить(МассивОбъектов, "Документ_1");
...
Процедура СформироватьТекстЗапросаТОРГ2(ТекстЗапроса)
	...
	|ГДЕ
	|	ПеремещенияТоваровТовары.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.сабТипыНоменклатуры.Товар)
	...
	|ГДЕ
	|	ПеремещенияТоваровТабличнаяЧасть.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.сабТипыНоменклатуры.Товар)
