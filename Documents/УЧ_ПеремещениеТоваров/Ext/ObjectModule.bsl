	...
	Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
		...
		Для каждого ТекСерия Из МассивСерий Цикл
			...
			Если ЕстьАвтоПереводНоменклатуры Тогда
				СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, Склад, Подразделение, ТекСерия.СерияНоменклатуры);
			Иначе
				Если ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.СчетУчетаБУ) Тогда
					Если УчетПоподразделениямСчетУчета Тогда
						Если ЭтоПоступлениеСОбособленногоПодразделения Тогда
							СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение, Счет, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, Подразделение, ПодразделениеПолучатель, ТекСтрокаТабличнаяЧасть.СчетУчетаБУ, ТекСерия.СерияНоменклатуры);
						Иначе	
							СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение, Счет, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, Склад, Подразделение, ТекСтрокаТабличнаяЧасть.СчетУчетаБУ, ТекСерия.СерияНоменклатуры);
						КонецЕсли;	
					Иначе
						Если ЭтоПоступлениеСОбособленногоПодразделения Тогда
							СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, Подразделение, ТекСтрокаТабличнаяЧасть.СчетУчетаБУ, ТекСерия.СерияНоменклатуры);
						Иначе	
							СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, Склад, ТекСтрокаТабличнаяЧасть.СчетУчетаБУ, ТекСерия.СерияНоменклатуры);
						КонецЕсли;	
					КонецЕсли;
				Иначе	
					Если УчетПоподразделениямСчетУчета Тогда
						Если ЭтоПоступлениеСОбособленногоПодразделения Тогда
							СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, Подразделение, ПодразделениеПолучатель, ТекСерия.СерияНоменклатуры);
						Иначе	
							СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, Склад, Подразделение, ТекСерия.СерияНоменклатуры);
						КонецЕсли;	
					Иначе
						Если ЭтоПоступлениеСОбособленногоПодразделения Тогда
							СтруктураПоиска = Новый Структура("Номенклатура, Склад, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, Подразделение, ТекСерия.СерияНоменклатуры);
						Иначе	
							СтруктураПоиска = Новый Структура("Номенклатура, Склад, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, Склад, ТекСерия.СерияНоменклатуры);
						КонецЕсли;	
					КонецЕсли;
				КонецЕсли;
			КонецЕсли;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
		...
		Если ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.СчетУчетаБУ) Тогда
			Если УчетПоподразделениямСчетУчета Тогда
				Если ЭтоПоступлениеСОбособленногоПодразделения Тогда
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, Подразделение", ТекСтрокаТабличнаяЧасть.Номенклатура, Подразделение, ТекСтрокаТабличнаяЧасть.СчетУчетаБУ, ПодразделениеПолучатель);
				Иначе	
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, Подразделение", ТекСтрокаТабличнаяЧасть.Номенклатура, Склад, ТекСтрокаТабличнаяЧасть.СчетУчетаБУ, Подразделение);
				КонецЕсли;	
			Иначе
				Если ЭтоПоступлениеСОбособленногоПодразделения Тогда
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет", ТекСтрокаТабличнаяЧасть.Номенклатура, Подразделение, ТекСтрокаТабличнаяЧасть.СчетУчетаБУ);
				Иначе	
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет", ТекСтрокаТабличнаяЧасть.Номенклатура, Склад, ТекСтрокаТабличнаяЧасть.СчетУчетаБУ);
				КонецЕсли;	
			КонецЕсли;			
		Иначе
			Если УчетПоподразделениямСчетУчета Тогда
				Если ЭтоПоступлениеСОбособленногоПодразделения Тогда
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение", ТекСтрокаТабличнаяЧасть.Номенклатура, Подразделение, ПодразделениеПолучатель);
				Иначе	
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение", ТекСтрокаТабличнаяЧасть.Номенклатура, Склад, Подразделение);
				КонецЕсли;	
			Иначе
				Если ЭтоПоступлениеСОбособленногоПодразделения Тогда
		...
		Если ТекСтрокаТабличнаяЧасть.Количество <> 0 Тогда
			...
			Если ТекСтрокаТабличнаяЧасть.Количество = КоличествоОстаток Тогда
				ТекСтрокаТабличнаяЧасть.Цена = ЦенаОстаток;
