...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ТабличнаяЧастьСверн.Свернуть("Номенклатура, СчетУчетаБУ, НовыйСчетУчетаБУ, Характеристика", "Количество, Сумма, СуммаНДС");  
	
	Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧастьСверн Цикл
		//Супербоб для возврата алкашки, удалить по окончании
		...
		Иначе
			...
			Иначе
				Если ЭтоПоступлениеСОбособленногоПодразделения Тогда
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет", ТекСтрокаТабличнаяЧасть.Номенклатура, Подразделение, Движение.СчетКт);
				Иначе	
		...
		Если ПеренесеноИзАстор Тогда
			НайденныеСтроки = УчетПартий.НайтиСтроки(Новый Структура("Номенклатура, ВидДвижения", ТекСтрокаТабличнаяЧасть.Номенклатура, ?(ЭтоПоступлениеСОбособленногоПодразделения, Перечисления.ВидДвиженияНакопления.Приход, Перечисления.ВидДвиженияНакопления.Расход)));
			ПолученнаяСумма = 0;
			ПолученноеКоличество = 0;
			Для Каждого НайденнаяСтрока Из НайденныеСтроки Цикл
				ПолученнаяСумма = ПолученнаяСумма + НайденнаяСтрока.СуммаНДС + НайденнаяСтрока.СуммаБезНДС;
				ПолученноеКоличество = ПолученноеКоличество + ТекСтрокаТабличнаяЧасть.Количество;
			КонецЦикла;
			Движение.Сумма = ПолученнаяСумма;
			ТекЦена = ?(ПолученноеКоличество = 0, 0, ПолученнаяСумма / ПолученноеКоличество);
		Иначе	
			ВыборкаДетальныеЗаписи = РезультатОстатки.НайтиСтроки(СтруктураПоиска);
			Если ВыборкаДетальныеЗаписи.Количество() Тогда
				Движение.Сумма = ВыборкаДетальныеЗаписи[0].Цена * ТекСтрокаТабличнаяЧасть.Количество;
	...
	//		
	//		//заказ поставщика
	//		Если ТипЗнч(ДокОснование) = Тип("ДокументСсылка.УЧ_ПеремещениеТоваров") И ВидОперации = Перечисления.ВидыПеремещений.ПоступлениеСОбособленногоПодразделения Тогда
	//			Если ЗначениеЗаполнено(ДокОснование.ДокОснование) И ТипЗнч(ДокОснование.ДокОснование) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
	//				Движение = Движения.Учетный.Добавить();
	...
	//				Движение.КоличествоКт = ТекСтрокаТабличнаяЧасть.Количество;
	//				Движение.Содержание = Комментарий;
	//				Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.ЗаказыПоставщикам] = ДокОснование.ДокОснование;
	//				Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаТабличнаяЧасть.Номенклатура;
	//			КонецЕсли;
	//		КонецЕсли;
	//		
	//		//заказ клиента
	//		Если ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ЗаказПоставщику") И ВидОперации = Перечисления.ВидыПеремещений.ПеремещениеНаОбособленноеПодразделение Тогда
	//			Движение = Движения.Учетный.Добавить();
	//			Движение.СчетДт = ПланыСчетов.Учетный._ЗаказКлиентов;
	//			Движение.Период = Дата;
	//			Движение.Предприятия = Предприятие;
	//			Движение.ПодразделениеДт = Подразделение;
	//			Движение.СценарийПлана = Справочники.СценарииПланирования.Факт;
	...
	//			Движение.КоличествоДт = ТекСтрокаТабличнаяЧасть.Количество;
	//			Движение.Содержание = Комментарий;
	//			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.ЗаказыПоставщикам] = ДокОснование;
	//			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаТабличнаяЧасть.Номенклатура;
	//		КонецЕсли;
	//	КонецЦикла;
	//КонецЕсли;
	...
	Если НЕ Отказ И ЭтоПоступлениеСОбособленногоПодразделения И ТипЗнч(ДокОснование) = Тип("ДокументСсылка.УЧ_ПеремещениеТоваров") Тогда
		Заказ = ДокОснование.ДокОснование;
		Если ЗначениеЗаполнено(ДокОснование) И ТипЗнч(ДокОснование) = Тип("ДокументСсылка.УЧ_ПеремещениеТоваров") 
			И ЗначениеЗаполнено(Заказ) И ТипЗнч(Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") И Не Заказ.Статус = Перечисления.СтатусыЗаказовПоставщикам.Закрыт Тогда		
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		...
		Если ДанныеЗаполнения.ВидОперации = Перечисления.ВидыЗаказов.ВозвратБрака Тогда
			ЭтоЗаказНаВозврат = Истина;
		ИначеЕсли ДанныеЗаполнения.ВидОперации = Перечисления.ВидыЗаказов.ПеремещениеМеждуСкладами Тогда
			ЭтоПеремещениеМеждуСкладами = Истина;
		КонецЕсли;	
		
		// Заполнение шапки
		Комментарий 	= ДанныеЗаполнения.Комментарий;
		ПодразделениеПолучатель = ДанныеЗаполнения.Подразделение;
		Если ЭтоЗаказНаВозврат Тогда
			Подразделение = ДанныеЗаполнения.Подразделение;
			СкладПолучатель = Справочники.Склады.ПолучитьСкладПоЗначениюДопРеквизита("Склад возврата", ДанныеЗаполнения.Предприятие);
			ВидОперации 	= Перечисления.ВидыПеремещений.Перемещение;
		ИначеЕсли ЭтоПеремещениеМеждуСкладами Тогда
			Подразделение 	= ДанныеЗаполнения.Контрагент;
			СкладПолучатель = ПодразделениеПолучатель.Склад;
			ВидОперации 	= Перечисления.ВидыПеремещений.Перемещение;
		Иначе	
			Подразделение 	= ДанныеЗаполнения.Контрагент;
			СкладПолучатель = ПодразделениеПолучатель.Склад;
			ВидОперации 	= Перечисления.ВидыПеремещений.ПеремещениеНаОбособленноеПодразделение;
		КонецЕсли;
		Организация 	= Подразделение.Организация;
		ОрганизацияПолучатель = ПодразделениеПолучатель.Организация;
		Предприятие 	= ДанныеЗаполнения.Предприятие;
		ДокОснование 	= ДанныеЗаполнения.Ссылка;
		СуммаДокумента 	= ДанныеЗаполнения.СуммаДокумента;
		Склад 			= ?(ЗначениеЗаполнено(ДанныеЗаполнения.СкладОтправитель), ДанныеЗаполнения.СкладОтправитель, Подразделение.Склад);
		...
		Для Каждого ТекСтрокаТабличнаяЧасть Из ДанныеЗаполнения.ТабличнаяЧасть Цикл
			...
			Иначе	
				НоваяСтрока.Количество = ?(ЭтоПеремещениеМеждуСкладами, ТекСтрокаТабличнаяЧасть.Количество, 0);//ТекСтрокаТабличнаяЧасть.Количество;
			КонецЕсли;
