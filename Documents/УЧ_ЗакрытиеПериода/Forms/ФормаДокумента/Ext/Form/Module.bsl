...
Процедура ЗаполнитьПродукциюСервер(ИмяКоманды)
	...
	Запрос.Текст = 
		...
		|ПОМЕСТИТЬ Продукция
		|ИЗ
		|	РегистрБухгалтерии.Учетный.Обороты(&НачПериода, &КонПериода, , Счет = &Счет43, , Предприятия = &ВыбПредприятие, КорСчет = &Счет40, ) КАК УчетныйОбороты
		|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ РегистрБухгалтерии.Учетный.Обороты(&НачПериода, &КонПериода, , Счет = &Счет43, , Предприятия = &ВыбПредприятие, КорСчет = &Счет40, ) КАК УчетныйОбороты1
