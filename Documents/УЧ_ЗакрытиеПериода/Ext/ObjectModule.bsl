	...
	Если Выборка.Количество() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			Для каждого ТекСтрока Из Выборка Цикл
				...
				ОтобранныеСтрокиТЧ = Выборка43.НайтиСтроки(Новый Структура("Субконто1", ТекСтрока.Субконто1));
				Для каждого ТекСтрокаТЧ Из ОтобранныеСтрокиТЧ Цикл
					Количество = Количество + ТекСтрокаТЧ.КоличествоНачальныйОстаток;
					Сумма = Сумма + ТекСтрокаТЧ.СуммаНачальныйОстаток;
				КонецЦикла;
				...
				Для каждого ТекСтрокаПроводка Из Проводки Цикл
					...
					ИначеЕсли ТекСтрокаПроводка.СчетДт = Сч43 И ТекСтрокаПроводка.СубконтоДт[СубНом] = ТекСтрока.Субконто1 И НЕ ТекСтрокаПроводка.СчетКт = Сч40 Тогда
						ТекСтрокаПроводка.Сумма = ТекСтрокаПроводка.КоличествоДт * СебестоимостьСредняя;
					ИначеЕсли ТекСтрокаПроводка.СчетКт = Сч43 И ТекСтрокаПроводка.СубконтоКт[СубНом] = ТекСтрока.Субконто1 Тогда
