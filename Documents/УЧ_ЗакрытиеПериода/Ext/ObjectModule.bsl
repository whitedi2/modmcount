...
Процедура Закрытие40(ВыборкаОстатки)
	...
	Суб3 = ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты и организации", Истина);
	...
	Если Выборка.Количество() Тогда
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			...
			//Если ВыборкаДетальныеЗаписи.Регистратор.Номер = "ПЛ-00000217" Тогда
			//	а = 1;			
			//КонецЕсли;
			...
			Для каждого ТекСтрока Из Выборка Цикл
				...
				Количество = 0;
				Сумма = 0;
				ОтобранныеСтрокиТЧ = Номенклатура.НайтиСтроки(Новый Структура("Субконто1, Субконто2, Субконто3", ТекСтрока.Субконто1, ТекСтрока.Субконто2, ТекСтрока.Субконто3));
				...
				Для каждого ТекСтрокаТЧ Из ОтобранныеСтрокиТЧ Цикл
					Количество = Количество + ТекСтрокаТЧ.КоличествоНачальныйОстаток;
				...
				Для каждого ТекСтрокаПроводка Из Проводки Цикл
					...
					//	НужнаЗапись = Истина;
					ИначеЕсли ТекСтрокаПроводка.СчетКт = Сч43 И ТекСтрокаПроводка.СубконтоКт[СубНом] = ТекСтрока.Субконто1 И (ТекСтрокаПроводка.СубконтоКт[СубСерия] = ТекСтрока.Субконто3 ИЛИ Не ЗначениеЗаполнено(ТекСтрокаПроводка.СубконтоКт[СубСерия])) Тогда
						НужнаЗапись = Истина;
						...
						Если ТекСтрокаПроводка.СчетДт = Счет9002 Тогда
							...
							НоваяСтрока9002.Субконто3 = ТекСтрокаПроводка.СубконтоДт[Суб3];
