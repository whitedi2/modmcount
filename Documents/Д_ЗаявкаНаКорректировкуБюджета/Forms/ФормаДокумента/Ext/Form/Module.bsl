...
Процедура ПриОткрытии(Отказ)
	...
	РольАдминистратораДоступна = БюджетныйНаСервере.РольДоступнаСервер("Администратор");
	УстановитьДоступностьЛимитов(РольАдминистратораДоступна);
	
	Если НЕ РольАдминистратораДоступна И НЕ ТекБП = Неопределено Тогда
...
Процедура ПредприятиеПриИзменении(Элемент)
	...
	АктуальныйСценарий = БюджетныйНаСервере.ПолучитьАктуальныйСценарий(Объект.Дата, Ложь);
...
Функция РасчитатьЛимиты(СтатьяДДС, ЦФО, Подразделение, СтатьяЗатрат, Статья91, ИнвПроект, ДатаПолучения)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				ИЛИ Затраты.СценарийПлана = ЗНАЧЕНИЕ(Справочник.СценарииПланирования.ПланПоСЗ)
	               ...
	               |	ВЫБОР
	               |		КОГДА Б_ПараметрыБюджета.Утвержден
	               |				ИЛИ Затраты903.СценарийПлана = ЗНАЧЕНИЕ(Справочник.СценарииПланирования.ПланПоСЗ)
	...
	КонецЦикла;
	 ...
	 Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	                |	СУММА(ВЫБОР
	                |			КОГДА Б_ПараметрыБюджета.Утвержден
	                |					ИЛИ Деньги.СценарийПлана = ЗНАЧЕНИЕ(Справочник.СценарииПланирования.ПланПоСЗ)
...
Процедура ПолучитьЛимитыСервер(ИндексТекСтроки)
	...
	Если НЕ ТекущаяСтрока = Неопределено Тогда
		...
		СтруктураЛимитов = РасчитатьЛимиты(ТекущаяСтрока.СтатьяБДДС, ЦФО, ТекущаяСтрока.ВидДеятельности, ТекущаяСтрока.СтатьяЗатрат, ТекущаяСтрока.СтатьяДоходовИРасходов, ИнвПроект, ДатаПолученияЛимита);
		...
		Если Не БюджетныйНаСервере.РольДоступнаСервер("Администратор") Тогда
			УстановитьДоступностьЛимитов(Ложь);
		КонецЕсли;
		//КонецЕсли; 
		ТекущаяСтрока.ФактПоЗаявкам = РассчитатьФактПоЗаявкамНаСервере(ТекущаяСтрока.ЦФО, ТекущаяСтрока.ВидДеятельности,ТекущаяСтрока.СтатьяБДДС, ИнвПроект);
...
Процедура СЗЦФОНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если СписокВыбора_Предприятие.Количество() Тогда
		//Элементы.СписокВыбора_Предприятие.СписокВыбора.Добавить(ПредопределенноеЗначение("Справочник.Предприятия.Казна"));
		ТекЗначение = ВыбратьИзСписка(СписокВыбора_Предприятие, , СписокВыбора_Предприятие.НайтиПоЗначению(?(ЗначениеЗаполнено(ТекДанные.ЦФО), ТекДанные.ЦФО, Объект.Предприятие)));	
		БюджетныйНаКлиенте.ПриНачалеВыбораРеквизитаВСпискеДубль2(ЭтаФорма, "СписокВыбора_Предприятие", ТекЗначение, "Предприятия", ТекДанные.ЦФО, СтандартнаяОбработка, Ложь);
		СЗЦФОПриИзменении(Элемент);
	КонецЕсли;
...
Процедура ЗаполнитьСтрокиТЧ(ПересчитатьФакт = Истина)
	Для каждого ТекСтрока Из Объект.СЗ Цикл
		...
		Если ПересчитатьФакт Тогда
			ТекСтрока.ФактПоЗаявкам = РассчитатьФактПоЗаявкамНаСервере(ТекСтрока.ЦФО, ТекСтрока.ВидДеятельности,ТекСтрока.СтатьяБДДС, ТекСтрока.ИнвПроект);
		КонецЕсли;
	КонецЦикла; 
КонецПроцедуры
...
Процедура РассчитатьФактПоЗаявкам()
	Элементы.СЗ.ТекущиеДанные.ФактПоЗаявкам = РассчитатьФактПоЗаявкамНаСервере(Элементы.СЗ.ТекущиеДанные.ЦФО, Элементы.СЗ.ТекущиеДанные.ВидДеятельности,Элементы.СЗ.ТекущиеДанные.СтатьяБДДС, Элементы.СЗ.ТекущиеДанные.ИнвПроект);	
КонецПроцедуры
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	СоздаватьГрафикБюджета = АктуальныйСценарий.СоздаватьГрафикОплат;
...
Процедура СоздатьОбновитьКлючАналитики(СтрокаТЧ, ТекущийОбъект)
	...
	НовыйКлючАналитики.СценарийПлана = АктуальныйСценарий;
