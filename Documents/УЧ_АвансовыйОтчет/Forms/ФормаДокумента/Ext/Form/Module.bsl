...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	Пока Выборка.Следующий() Цикл
		Соотв.Вставить(Выборка.Счет, Новый Структура("Количественный, Валютный, УчетПоПодразделениям, КоличествоСубконто", Выборка.Количественный, Выборка.Валютный, Выборка.УчетПоПодразделениям, ?(ЗначениеЗаполнено(Выборка.Счет), Выборка.Счет.ВидыСубконто.Количество(), 0)));
	КонецЦикла;
