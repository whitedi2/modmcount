...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
		...
		КонецЕсли;
		Движение.Сумма = ?(УчитыватьНДС, ?(СуммаВключаетНДС, ТекСтрокаСумма - ТекСтрокаСуммаНДС, ТекСтрокаСумма), ТекСтрокаСумма) * КурсВалюты / КратностьВзаиморасчетов;
		...
		Если УчитыватьНДС И ТекСтрокаСуммаНДС Тогда
			...
			Если Движение.СчетДт.УчетПоПодразделениям Тогда
				Движение.ПодразделениеДт = ?(ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.КорПодразделение), ТекСтрокаТабличнаяЧасть.КорПодразделение, Подразделение);
			КонецЕсли;
			...
			Если Не КурсВалюты = 1 И Движение.СчетКт.Валютный Тогда
			...
			Движение.Содержание = "возмещение НДС при поступлении а/о";
