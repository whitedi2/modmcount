...
Процедура ОпределитьКорсчет(ТекСтрокаБюджетСебестоимости, Движение, Счет, ПризнакСуммы = "ФР")
	...
	Иначе
		//Если ТекСтрокаБюджетСебестоимости.СтатьяЗатрат = Справочники.ИздержкиОбращения.ЗатратыВсп Тогда 
			Движение[Счет] = ПланыСчетов.Учетный.Счет6001();
			УЧ_Сервер.УстановитьСубконто(Движение[Субконто],Движение[Счет], 1, Справочники.Контрагенты.ПустаяСсылка());
