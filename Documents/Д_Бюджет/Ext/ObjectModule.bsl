...
Процедура СформироватьПроводкиОплаты(ТекСтрокаБюджетСебестоимости, ТекДата)
	...
	Если ТекСтрокаБюджетСебестоимости.СуммаБДДС Тогда
		...
		Если ТекСтрокаБюджетСебестоимости.СтатьяБДДС.Доход Тогда
			...
			ОпределитьКорсчет(ТекСтрокаБюджетСебестоимости, Движение, "СчетКт", "ДДС", Истина);
	...
	Если ТекСтрокаБюджетСебестоимости.СуммаСеб Тогда
		...
		Если ТипЗнч(ТекСтрокаБюджетСебестоимости.СтатьяЗатрат) = Тип("СправочникСсылка.СтатьиЗатрат") И Предприятие.СчетЗакрытияЗатрат = ПланыСчетов.Учетный.Счет91() И ЗначениеЗаполнено(Предприятие.СтатьяЗакрытияЗатрат) Тогда // закрытие всего предприятия на 91 счет и фиксированную статью доходов и расходов с прогоном через 25 счет
			...
			Если ТекСтрокаБюджетСебестоимости.СтатьяБДДС.Доход Тогда
				...
				ОпределитьКорсчет(ТекСтрокаБюджетСебестоимости, Движение, "СчетДт",, Истина);
...
Процедура Реализация()
	...
	Для Каждого ТекСтрокаБюджетПродаж Из БюджетПродаж Цикл
		...
		Если ЗначениеЗаполнено(ТекСтрокаБюджетПродаж.СтавкаНДС) Тогда
			
			Движение = Движения.Учетный.Добавить();
			
			Движение.СчетДт = ПланыСчетов.Учетный.Счет9003();
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ТекСтрокаБюджетПродаж.Номенклатура);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 2, СтатьяНДС);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 3, ТекСтрокаБюджетПродаж.Контрагент);
			...
			Движение.Предприятия = Предприятие;
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетПродаж.Количество * ТекСтрокаБюджетПродаж.Цена - ТекСтрокаБюджетПродаж.Количество * ТекСтрокаБюджетПродаж.Цена / (1 + ТекСтрокаБюджетПродаж.СтавкаНДС.Ставка / 100);
			Движение.НомерСтрокиДок = ТекСтрокаБюджетПродаж.НомерСтроки;
			...
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетПродаж.Месяц, - Движение.Сумма);
...
Процедура Закупки()
	...
	Для Каждого ТекСтрокаБюджетЗакупок Из БюджетЗакупок Цикл
		
		ТекДата = ТекСтрокаБюджетЗакупок.Месяц;
		Количество = ТекСтрокаБюджетЗакупок.Количество;
		
		Если ТипЗнч(ТекСтрокаБюджетЗакупок.Номенклатура) = Тип("СправочникСсылка.ОсновныеСредства") Тогда // если реализуется основное средство, делаем проводки как в учете 1) Д62.03К90.05 2) Д90.05К01
			
			Цена = ТекСтрокаБюджетЗакупок.Цена;
			
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = ПланыСчетов.Учетный.Счет6203();
			Движение.СчетДт = ПланыСчетов.Учетный.Счет9005();
			
			Если Движение.СчетДт.УчетПоПодразделениям Тогда
				Движение.ПодразделениеДт = ТекСтрокаБюджетЗакупок.Подразделение;
			КонецЕсли;
			
			Движение.Период = ТекДата;
			Движение.Предприятия = Предприятие;
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетЗакупок.ПоступлениеДС;
			Движение.Содержание = "начислена выручка (" + Строка(ТекСтрокаБюджетЗакупок.ПоступлениеДС) + ")"; 
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)] = ТекСтрокаБюджетЗакупок.Контрагент;
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Основные средства", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = ПланыСчетов.Учетный.Счет9005();
			
			Если Движение.СчетКт.УчетПоПодразделениям Тогда
				Движение.ПодразделениеКт = ТекСтрокаБюджетЗакупок.Подразделение;
			КонецЕсли;
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = Количество * Цена;
			Движение.Содержание = "реализовано основное средство (" + Строка(ТекСтрокаБюджетЗакупок.Номенклатура) + " " + Строка(Количество) + " * " + Строка(Цена) + ")"; 
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Основные средства", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Основные средства", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
						
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, Количество * Цена);
		...
		Если Сценарий.ДатаНачала < Дата('20111030000000') Тогда
			Если ПустаяСтрока(ТекСтрокаБюджетЗакупок.ВозмещениеТр) Тогда
				Возмещение = 0;
			Иначе
				Возмещение = Число(ТекСтрокаБюджетЗакупок.ВозмещениеТр);			
			КонецЕсли;
			Цена = ТекСтрокаБюджетЗакупок.Цена;
		Иначе
			Возмещение = 0;
			...
			Цена = ТекСтрокаБюджетЗакупок.Цена + ТекСтрокаБюджетЗакупок.Верх;
			Если НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = НРег("сверх цены") ИЛИ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = НРег("сверх цены (прямая)") Тогда
				ВозмещениеЖД = ВозмещениеЖД + ТекСтрокаБюджетЗакупок.ЖД;
			ИначеЕсли НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = "в цене" Тогда
				Цена = Цена + ТекСтрокаБюджетЗакупок.ЖД; 	
			КонецЕсли;
			Если НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = НРег("сверх цены") ИЛИ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = НРег("сверх цены (прямая)") Тогда
				ВозмещениеАвто = ВозмещениеАвто + ТекСтрокаБюджетЗакупок.Авто;
			ИначеЕсли НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = "в цене" Тогда
				Цена = Цена + ТекСтрокаБюджетЗакупок.Авто; 	
			КонецЕсли;
			Если НРег(ТекСтрокаБюджетЗакупок.ВозмещениеСопровождения) = НРег("сверх цены") ИЛИ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеСопровождения) = НРег("сверх цены (прямая)") Тогда
				ВозмещениеСопровождения = ВозмещениеСопровождения + ТекСтрокаБюджетЗакупок.Сопровождение;
			ИначеЕсли НРег(ТекСтрокаБюджетЗакупок.ВозмещениеСопровождения) = "в цене" Тогда
				Цена = Цена + ТекСтрокаБюджетЗакупок.Сопровождение; 	
			КонецЕсли;
		...
		//если ВНХ контрагент
		Если ТипЗнч(ТекСтрокаБюджетЗакупок.Контрагент) = Тип("СправочникСсылка.Предприятия") Тогда
			СчетПокупатель = ПланыСчетов.Учетный.Счет7902();
			СчетВерх = ПланыСчетов.Учетный.Счет7901();
			СчетИсточник = ПланыСчетов.Учетный.Счет5501();
		иначе
			СчетВерх = ПланыСчетов.Учетный.Счет7601();
			СчетПокупатель = ПланыСчетов.Учетный.Счет6201();
			СчетИсточник = БюджетныйНаСервере.ПолучитьИсточник(ТекСтрокаБюджетЗакупок.Источник);
		КонецЕсли;
		
		//если ВНХ продавец товара
		Если ЗначениеЗаполнено(ТекСтрокаБюджетЗакупок.Покупной) И Не ТекСтрокаБюджетЗакупок.Покупной = Перечисления.Д_ПокупнойСпирт.Транзитный Тогда
			СчетПродавца = ПланыСчетов.Учетный.Счет6003();
			СчетИсточникПродавца = БюджетныйНаСервере.ПолучитьИсточник(ТекСтрокаБюджетЗакупок.Источник);
		иначе
			СчетПродавца = ПланыСчетов.Учетный.Счет7902();
			СчетИсточникПродавца = ПланыСчетов.Учетный.Счет5501();
		КонецЕсли;
		...
		НоменклатураПоиска = Новый Структура("Номенклатура, Месяц", ТекСтрокаБюджетЗакупок.Номенклатура, ТекСтрокаБюджетЗакупок.Месяц);
		
		
		// ищем единую номенклатуру в соответствии
		СоответствияПроизводственныхИПродажныхНом = СоответствиеНоменклатуры.НайтиСтроки(Новый Структура("Соответствие", ТекСтрокаБюджетЗакупок.Номенклатура));
		
		Если Не СоответствияПроизводственныхИПродажныхНом.Количество() Тогда
			СоответствияПроизводственныхИПродажныхНом = СоответствиеНоменклатуры.НайтиСтроки(Новый Структура("Номенклатура", ТекСтрокаБюджетЗакупок.Номенклатура));
		КонецЕсли;
		...
		Если СоответствияПроизводственныхИПродажныхНом.Количество() Тогда
			СтрокиСоответствия = ОбъемПроизводства.НайтиСтроки(Новый Структура("ЕдинаяНоменклатура, Месяц", СоответствияПроизводственныхИПродажныхНом[0].Соответствие, ТекСтрокаБюджетЗакупок.Месяц));
		КонецЕсли;	
		...
		Если ЗначениеЗаполнено(ЕдинаяНом) Тогда
			СтрокиЕдинойНоменклатуры = Новый Массив;
			Т = ОбъемПроизводства.НайтиСтроки(Новый Структура("ЕдинаяНоменклатура, Месяц", ЕдинаяНом, ТекСтрокаБюджетЗакупок.Месяц));
			Для каждого ПП Из Т Цикл
				СтрокиЕдинойНоменклатуры.Добавить(ПП.Номенклатура);	
			КонецЦикла;
			СтрокиЕдинойНоменклатуры.Добавить(ЕдинаяНом);	
		...
		Если СтрокиЕдинойНоменклатуры.Количество() Тогда
			...
			Если ПустаяСтрока(ТекСтрокаБюджетЗакупок.Покупной) Тогда//если не выбрана закупка товара
				//Себестоимость = ?(ОтобранныеСтроки[0].РеализацияКоличество = 0, 0 ,ОтобранныеСтроки[0].РеализацияСумма / ОтобранныеСтроки[0].РеализацияКоличество);
				Себестоимость = ВычислитьСебестоимостьЕдинойНоменклатуры(СтрокиЕдинойНоменклатуры, ТекСтрокаБюджетЗакупок.Месяц);
			Иначе//если выбрана закупка товара
				Если ТекСтрокаБюджетЗакупок.ЦенаЗакупки тогда
					Себестоимость = ТекСтрокаБюджетЗакупок.ЦенаЗакупки;
				Иначе
					//Если ТекСтрокаБюджетЗакупок.Количество Тогда
					//	сообщить("Себестоимость номенклатуры " + строка(ТекСтрокаБюджетЗакупок.Номенклатура) 
					//	+ " неопределена.");
					//КонецЕсли;
					Себестоимость = 0;
				КонецЕсли;
		...
		Иначе	
			...
			Если НЕ ОтобранныеСтроки.Количество() Тогда //поиск в СоответствииНоменклатуры
				НайденоСоответствие = СоответствиеНоменклатуры.НайтиСтроки(Новый Структура("Номенклатура", ТекСтрокаБюджетЗакупок.Номенклатура)); 
				Если НайденоСоответствие.Количество() Тогда
					НоменклатураПоиска = Новый Структура("Номенклатура, Месяц", НайденоСоответствие[0].Соответствие, ТекСтрокаБюджетЗакупок.Месяц);
					ОтобранныеСтроки = ТаблицаПрозведенныхНоменклатур.НайтиСтроки(НоменклатураПоиска);
					НоменклатураСоответствие = НайденоСоответствие[0].Соответствие;							
				КонецЕсли;
			КонецЕсли;
		...
		//Если ПустаяСтрока(ТекСтрокаБюджетЗакупок.Номенклатура.Д_НомерНоменклатуры) Тогда
		//	Сообщить("Номер номенклатуры " + строка(ТекСтрокаБюджетЗакупок.Номенклатура) + " неопределен.");
		//КонецЕсли;
		...
		Если Количество * Себестоимость И НЕ НоменклатураСоответствие = Неопределено Тогда
			Движение = Движения.Учетный.Добавить();
			Если ЗначениеЗаполнено(ТекСтрокаБюджетЗакупок.Покупной) Тогда
				Движение.СчетКт = ПланыСчетов.Учетный.Счет41();
				Движение.СчетДт = ПланыСчетов.Учетный.Счет41();
			Иначе
				Движение.СчетКт = ПланыСчетов.Учетный.Счет43();
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = Количество * Себестоимость;
			Движение.КоличествоКт = ТекСтрокаБюджетЗакупок.Количество;
			Движение.КоличествоДт = ТекСтрокаБюджетЗакупок.Количество;
			Движение.Содержание = "переворот";
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = НоменклатураСоответствие;
		КонецЕсли;
		...
		Если Количество * Цена Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = СчетПокупатель;
			Движение.СчетДт = ПланыСчетов.Учетный.Счет9001();//90.1
			Движение.Период = ТекДата;
			Движение.Предприятия = Предприятие;
			...
			Движение.Содержание = "начислена выручка (" + Строка(ТекСтрокаБюджетЗакупок.Номенклатура) + " " + Строка(Количество) + " * " + Строка(Цена) + ")"; 
			Если ТипЗнч(ТекСтрокаБюджетЗакупок.Контрагент) = Тип("СправочникСсылка.Предприятия") Тогда
				Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Предприятия", Истина)] = ТекСтрокаБюджетЗакупок.Контрагент;
			КонецЕсли;
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			//Если ЗначениеЗаполнено(ТекСтрокаБюджетЗакупок.СтатьяБДДС) Тогда
			//	Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)Бюджет] = ТекСтрокаБюджетЗакупок.СтатьяБДДС;
			//КонецЕсли;
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, Количество * Цена);
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ТекСтрокаБюджетЗакупок.СтавкаНДС) Тогда
			
			Движение = Движения.Учетный.Добавить();
			
			Движение.СчетКт = ПланыСчетов.Учетный.Счет9003();
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, ТекСтрокаБюджетЗакупок.Номенклатура);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 2, СтатьяНДС);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 3, ТекСтрокаБюджетЗакупок.Контрагент);
			...
			Движение.Предприятия = Предприятие;
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетЗакупок.Количество * ТекСтрокаБюджетЗакупок.Цена - ТекСтрокаБюджетЗакупок.Количество * ТекСтрокаБюджетЗакупок.Цена / (1 + ТекСтрокаБюджетЗакупок.СтавкаНДС.Ставка / 100);
			Движение.НомерСтрокиДок = ТекСтрокаБюджетЗакупок.НомерСтроки;
			//Движение.Содержание = ?(Не ЕстьДоговор, "", Строка(ТекКонтрагент)) + ", Кол-во:" + ТекСтрокаРеализация.Количество + " Цена:" + ТекСтрокаРеализация.Цена + ", " + Комментарий;
			...
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, - Движение.Сумма);
			
		КонецЕсли;
		
		// начислен верх 
		Если ТекСтрокаБюджетЗакупок.Верх * Количество Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = ПланыСчетов.Учетный.Счет9003();
			Движение.СчетДт = СчетВерх;
			Движение.Период = ТекДата;
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетЗакупок.Верх * Количество;
			Движение.Содержание = "Начислен верх";
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			//Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.ИздержкиОбращения.Верхи;
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, - ТекСтрокаБюджетЗакупок.Верх * Количество);
			//Если ТипЗнч(ТекСтрокаБюджетЗакупок.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
			//Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)Бюджет] = ТекСтрокаБюджетЗакупок.Контрагент;
			//КонецЕсли;
		КонецЕсли;
		// оплачен верх
		Если ТекСтрокаБюджетЗакупок.КЗНачало + ТекСтрокаБюджетЗакупок.ПоступлениеДС Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = СчетВерх;
			Движение.СчетДт = СчетИсточник;
			Движение.Период = ТекДата;
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ?(Цена = 0, 0, (ТекСтрокаБюджетЗакупок.ПоступлениеДС - Возмещение * Количество) * ТекСтрокаБюджетЗакупок.Верх /
			Цена) + ТекСтрокаБюджетЗакупок.КЗНачало;
			Движение.Содержание = "оплата верха";
			//Если ТипЗнч(ТекСтрокаБюджетЗакупок.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
			//Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)Бюджет] = ТекСтрокаБюджетЗакупок.Контрагент;
			//КонецЕсли;
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи ДДС", Истина)] = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("2133.04");
		КонецЕсли;
		// начислены транспортные жд
		Если НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = Нрег("Сверх цены") И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = Нрег("сверх цены (отк)") И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = Нрег("самовывоз") Тогда
			Если ТекСтрокаБюджетЗакупок.ЖД * Количество Тогда
				Движение = Движения.Учетный.Добавить();
				Движение.СчетКт = ПланыСчетов.Учетный.Счет9003();
				Движение.СчетДт = ПланыСчетов.Учетный.Счет6003();
				Движение.Период = ТекДата;
				...
				Движение.СценарийПлана = Сценарий;
				Движение.Сумма = ТекСтрокаБюджетЗакупок.ЖД * Количество;
				Движение.Содержание = "начислен транспорт ж/д";
				Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
				Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.СтатьиЗатрат.НайтиПоНаименованию("Транспортные расходы по реализации (Ж/д) (возмещаемые)");
				//Если ЗначениеЗаполнено(ТекСтрокаБюджетЗакупок.СтатьяБДДС) Тогда
				//	Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)Бюджет] = ТекСтрокаБюджетЗакупок.СтатьяБДДС;
				//КонецЕсли;
				ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, - ТекСтрокаБюджетЗакупок.ЖД * Количество);
			КонецЕсли;
		КонецЕсли;
		// начислены транспортные авто
		Если ТекСтрокаБюджетЗакупок.Авто * Количество Тогда
			НомерНом = ТекСтрокаБюджетЗакупок.Номенклатура.Д_НомерНоменклатуры;
			Если (НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = Нрег("Сверх цены")
				И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = Нрег("сверх цены (отк)") И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = Нрег("самовывоз"))
				ИЛИ (НЕ НомерНом = Перечисления.Д_НомерНоменклатуры.Номер1 И
				НЕ НомерНом = Перечисления.Д_НомерНоменклатуры.Номер1закуп И
				НЕ НомерНом = Перечисления.Д_НомерНоменклатуры.Номер1закупТранзит И
				НЕ НомерНом = Перечисления.Д_НомерНоменклатуры.Номер1ПодДокиЗАК И
				...
				Движение.СценарийПлана = Сценарий;
				Движение.Сумма = ТекСтрокаБюджетЗакупок.Авто * Количество;
				Движение.Содержание = "начислен транспорт Авто";
				Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
				//Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.ИздержкиОбращения.ТранспортныеАвто;
				//Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)Бюджет] = Справочники.ИздержкиОбращения.ТранспортныеАвто;
				ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, - ТекСтрокаБюджетЗакупок.Авто * Количество);
			КонецЕсли;
			
		КонецЕсли;
		
		// начислено сопровождение
		Если ТекСтрокаБюджетЗакупок.Сопровождение * Количество Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = ПланыСчетов.Учетный.Счет9003();
			Движение.СчетДт = ПланыСчетов.Учетный.Счет6003();
			Движение.Период = ТекДата;
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетЗакупок.Сопровождение * Количество;
			Движение.Содержание = "начислено сопровождение";
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.СтатьиЗатрат.НайтиПоКоду("2133.23");
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты бюджет", Истина)] = Справочники.СтатьиЗатрат.НайтиПоКоду("2133.23");
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, - ТекСтрокаБюджетЗакупок.Сопровождение * Количество);
		КонецЕсли;
		
		// di 22.04.14
		
		Если ТекСтрокаБюджетЗакупок.Дисконт * Количество Тогда
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетЗакупок.Дисконт * Количество;
			Движение.Содержание = "начислен дисконт";
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.СтатьиЗатрат.НайтиПоКоду("2138.11");
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, - ТекСтрокаБюджетЗакупок.Дисконт * Количество);
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетЗакупок.Дисконт * Количество;
			//Движение.КоличествоДт = Количество; // задвоене количества на счете
			Движение.Содержание = "начислена выручка в сумме дисконта";
			Если ТипЗнч(ТекСтрокаБюджетЗакупок.Контрагент) = Тип("СправочникСсылка.Предприятия") Тогда
				Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Предприятия", Истина)] = ТекСтрокаБюджетЗакупок.Контрагент;
			Иначе
		...
		Если ТекСтрокаБюджетЗакупок.КомпенсацияНалогов * Количество Тогда	
			// начислена компенсация налогов
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = ПланыСчетов.Учетный.Счет9003();
			Движение.СчетДт = ПланыСчетов.Учетный.Счет7601();
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетЗакупок.КомпенсацияНалогов * Количество;
			Движение.Содержание = "начислена компенсация налогов";
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			//Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.ИздержкиОбращения.КомпенсацияНалогов;
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, - ТекСтрокаБюджетЗакупок.КомпенсацияНалогов * Количество);							
		КонецЕсли;
		
		Если ТекСтрокаБюджетЗакупок.ВыплатаДисконта Тогда
			// выплачен дисконт
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = ПланыСчетов.Учетный.Счет7601();
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетЗакупок.ВыплатаДисконта;
			Движение.Содержание = "выплачен дисконт";
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи ДДС", Истина)] = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("2133.04");
		КонецЕсли;
		...
		//возмещены транспортные покупателем 
		//Если Возмещение * Количество Тогда
		//	ПроцентАвто = ?((ТекСтрокаБюджетЗакупок.Авто + ТекСтрокаБюджетЗакупок.ЖД), ТекСтрокаБюджетЗакупок.Авто / (ТекСтрокаБюджетЗакупок.Авто + ТекСтрокаБюджетЗакупок.ЖД), 0);
		//	ПроцентЖд = ?((ТекСтрокаБюджетЗакупок.Авто + ТекСтрокаБюджетЗакупок.ЖД), ТекСтрокаБюджетЗакупок.ЖД / (ТекСтрокаБюджетЗакупок.Авто + ТекСтрокаБюджетЗакупок.ЖД), 0);
		...
		Если ВозмещениеАвто Тогда
			...
			Если (НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = Нрег("Сверх цены")
				...
				Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
				//Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.ИздержкиОбращения.ТранспортныеАвто;
				ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, ВозмещениеАвто * Количество);
			КонецЕсли;
		КонецЕсли;
		
		Если НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = Нрег("Сверх цены") И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = Нрег("сверх цены (отк)") И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = Нрег("самовывоз") И ВозмещениеЖд Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = СчетПокупатель;
			Движение.СчетДт = ПланыСчетов.Учетный.Счет9003();
			Движение.Период = ТекДата;
			...
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.СтатьиЗатрат.НайтиПоНаименованию("Транспортные расходы по реализации (Ж/д) (возмещаемые)");
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, ВозмещениеЖд * Количество);
		...
		Если ВозмещениеСопровождения Тогда
			...
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.СтатьиЗатрат.НайтиПоКоду("2133.23");
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, ВозмещениеСопровождения * Количество);
		...
		// оплачены транспортные
		Если НЕ ТекСтрокаБюджетЗакупок.ОплатаТранспорта Тогда
			Если (ТекСтрокаБюджетЗакупок.Авто) * Количество И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = НРег("сверх цены")
				И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = НРег("сверх цены (отк)") И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = Нрег("самовывоз") Тогда
				//И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр2) = НРег("сверх цены (прямая)") Тогда
				Движение = Движения.Учетный.Добавить();
				Движение.СчетКт = ПланыСчетов.Учетный.Счет6003();
				Движение.СчетДт = ПланыСчетов.Учетный.Счет5101();
				...
				Движение.Содержание = "Оплачен транспорт авто";
				//Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)Бюджет] = Справочники.ИздержкиОбращения.ТранспортныеАвто;
				Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи ДДС", Истина)] = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("2133.21");
			КонецЕсли;
			
			Если НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = Нрег("Сверх цены") И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = НРег("сверх цены (отк)") И НЕ НРег(ТекСтрокаБюджетЗакупок.ВозмещениеТр) = Нрег("самовывоз") И (ТекСтрокаБюджетЗакупок.ЖД) * Количество Тогда
				Движение = Движения.Учетный.Добавить();
				Движение.СчетКт = ПланыСчетов.Учетный.Счет6003();
				Движение.СчетДт = ПланыСчетов.Учетный.Счет5101();
				...
				Движение.Сумма = (ТекСтрокаБюджетЗакупок.ЖД) * Количество;
				Движение.Содержание = "Оплачен транспорт ж/д";
				Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты бюджет", Истина)] = Справочники.СтатьиЗатрат.НайтиПоНаименованию("Транспортные расходы по реализации (Ж/д) (возмещаемые)");
				Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи ДДС", Истина)] = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("2133.21");
			КонецЕсли;
		КонецЕсли;
		
		// списана продукция по средней стоимости
		Если Количество * Себестоимость ИЛИ ТекСтрокаБюджетЗакупок.Количество Тогда
			Движение = Движения.Учетный.Добавить();
			...
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, - Количество * Себестоимость);
		КонецЕсли;
		// приход от покупателей
		Если ТекСтрокаБюджетЗакупок.ПоступлениеДС Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = СчетИсточник;
			...
			Движение.Сумма = ТекСтрокаБюджетЗакупок.ПоступлениеДС;
			Движение.Содержание = "оплата от покупателя";
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи ДДС", Истина)] = ?(ПустаяСтрока(ТекСтрокаБюджетЗакупок.СтатьяБДДС), Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("1100.01"), ТекСтрокаБюджетЗакупок.СтатьяБДДС);
			Если ТипЗнч(ТекСтрокаБюджетЗакупок.Контрагент) = Тип("СправочникСсылка.Предприятия") Тогда
				Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Предприятия", Истина)] = ТекСтрокаБюджетЗакупок.Контрагент;
			Иначе
				//Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)Бюджет] = ТекСтрокаБюджетЗакупок.Контрагент;
			КонецЕсли;
		КонецЕсли;
		// поступил товар от поставщика или ВНХ_Поставщика
		Если Количество * ТекСтрокаБюджетЗакупок.ЦенаЗакупки И ТекСтрокаБюджетЗакупок.Количество Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = ПланыСчетов.Учетный.Счет41();
			Движение.СчетДт = СчетПродавца;
			Движение.Период = ТекДата;
			Движение.Предприятия = Предприятие;
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = Количество * ТекСтрокаБюджетЗакупок.ЦенаЗакупки;
			Движение.КоличествоКт = ТекСтрокаБюджетЗакупок.Количество;
			Движение.Содержание = "поступил товар";
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			Если ТекСтрокаБюджетЗакупок.Покупной = Перечисления.Д_ПокупнойСпирт.Закупной Тогда
				Движение.СчетДт = ПланыСчетов.Учетный.Счет6001();
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, Справочники.Контрагенты.НайтиПоНаименованию("ЛЮКС", Истина));
			КонецЕсли;
		КонецЕсли;
		//оплата за закупной товар 28.05.13 счет 60.4 заменен на счет 60.3
		Если ТекСтрокаБюджетЗакупок.Количество * ТекСтрокаБюджетЗакупок.ЦенаЗакупки И СчетПродавца = ПланыСчетов.Учетный.Счет6003() Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = СчетПродавца;
			Движение.СчетДт = СчетИсточникПродавца;
			...
			Движение.Предприятия = Предприятие;
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаБюджетЗакупок.Количество * ТекСтрокаБюджетЗакупок.ЦенаЗакупки;
			Движение.Содержание = "Оплатили за покупной товар";
			//Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)] = ТекСтрокаБюджетЗакупок.Контрагент;
			СтатьяДДСЗакуп = БюджетныйНаСервере.ПолучитьСтатьюДДСДляЗакупнойНоменклатуры(ТекСтрокаБюджетЗакупок.Покупной);
			Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи ДДС", Истина)] = СтатьяДДСЗакуп;
		КонецЕсли;
		// начислены бонусы КС
		Если Количество Тогда
			
			СуммаКС = ТекСтрокаБюджетЗакупок.ОтчисленияКС;
			
			Движение = Движения.Учетный.Добавить();
			Движение.СчетКт = ПланыСчетов.Учетный.Счет9003();
			Движение.СчетДт = ПланыСчетов.Учетный.Счет7901();
			Движение.Период = ТекДата;
			Движение.Предприятия = Предприятие;
			...
			Движение.СценарийПлана = Сценарий;
			Акциз = БюджетныйНаСервере.ВернутьТекСтавку(ТекДата, ТекСтрокаБюджетЗакупок.Номенклатура); //необходимо получить ставку акциза
			СуммаБонусов = (Цена - Акциз - ТекСтрокаБюджетЗакупок.Верх - (ТекСтрокаБюджетЗакупок.ЖД + ТекСтрокаБюджетЗакупок.Авто) + Возмещение) * СуммаКС / 100 * Количество;
			Движение.Сумма = СуммаБонусов; 
			Движение.Содержание = "начислены бонусы";
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекСтрокаБюджетЗакупок.Номенклатура;
			//Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Справочники.ИздержкиОбращения.Бонусы;
			ДобавитьСтрокуПрибыли(ТекСтрокаБюджетЗакупок.Месяц, - СуммаБонусов);
			//Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)] = ТекСтрокаБюджетЗакупок.Контрагент;
		КонецЕсли;
...
Процедура ОпределитьКорсчет(ТекСтрокаБюджетСебестоимости, Движение, Счет, ПризнакСуммы = "ФР", ЭтоДоход = Ложь)
	...
	Иначе
		Если ЭтоДоход Тогда
			Движение[Счет] = ПланыСчетов.Учетный.Счет6201();
		Иначе	
