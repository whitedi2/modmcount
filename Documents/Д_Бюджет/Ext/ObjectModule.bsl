...
Процедура ОбъемПроизводства()
	...
	Для Каждого ТекСтрокаДвижения Из ОбъемПроизводства2 Цикл
		...
		Для каждого ТекСтрока Из ОтобранныеСтроки Цикл
			СуммаЗатрат = СуммаЗатрат + ТекСтрока.ПроцНачислено * (100 - ТекСтрокаДвижения.ПроцентНаПростой) / 100;		
		КонецЦикла; 
		...
		Если НЕ ТекСтрокаДвижения.НЕРаспределять Тогда
			...
			Для каждого ТекСтрока Из ОтобранныеСтроки Цикл
				СуммаЗатрат = СуммаЗатрат + ТекСтрока.ПроцНачислено * (ТекСтрокаДвижения.ПроцентЗатрат / 100) * (100 - ТекСтрокаДвижения.ПроцентНаПростой) / 100;		
			КонецЦикла;
...
Процедура ВводОстатков()
	...
	Для каждого ТабличноеПоле Из МассивТабличныхПолей Цикл
		Для Каждого ТекСтрокаВводОстатковСырья Из ТабличноеПоле Цикл
			...
			Если СчетДт = ПланыСчетов.Учетный.Счет00() Тогда
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, ТекСтрокаВводОстатковСырья.Субконто1);
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 2, ТекСтрокаВводОстатковСырья.Субконто2);
			иначе
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ТекСтрокаВводОстатковСырья.Субконто1);
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 2, ТекСтрокаВводОстатковСырья.Субконто2);
			Конецесли;
...
Процедура Закупки()
	...
	Для Каждого ТекСтрокаБюджетЗакупок Из БюджетЗакупок Цикл
		...
		Движение.Предприятия = Предприятие;
		Движение.СценарийПлана = Сценарий;
		Движение.Сумма = ТекСтрокаБюджетЗакупок.Количество * ТекСтрокаБюджетЗакупок.Цена;
		Движение.КоличествоДт = ТекСтрокаБюджетЗакупок.Количество;
		Движение.Содержание = "Поступление от поставщика";
		УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ТекСтрокаБюджетЗакупок.Номенклатура);
		УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, ТекСтрокаБюджетЗакупок.Контрагент);
		...
		Если СуммаФР Тогда
			...
			Иначе
				Если ТекСтрокаПрочие90.Статья = Справочники.СтатьиЗатрат.НайтиПоНаименованию("НДС_", Истина) ИЛИ
					...
					ТекСтрокаПрочие90.Статья = Справочники.СтатьиЗатрат.ПолучитьСтатьюНДС(Предприятие) 
					Тогда
					Движение.СчетКт = ПланыСчетов.Учетный.Счет6801();
...
Процедура Кредиты()
	...
	СтатьяПолученияКредита = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УчетнаяПолитикаПредприятия, "СтатьяПолучениеКредитовЗаймов");
	СтатьяПогашенияКредита = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УчетнаяПолитикаПредприятия, "СтатьяПогашениеКредитовЗаймов");
	СтатьяПогашенияПроцентовПоКредиту = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(УчетнаяПолитикаПредприятия, "СтатьяПогашениеПроцентовКредитовЗаймов");
	ПДР_РасходыНаУслугиБанков = Справочники.ПрочиеДоходыИРасходы.НайтиПоНаименованию("Расходы на услуги банков");
	...
	Для Каждого ТекСтрокаКредиты Из Кредиты Цикл
		...
		Если ТекСтрокаКредиты.Поступление Тогда
			...
			Движение.СчетКт = ?(ЗначениеЗаполнено(ТекСтрокаКредиты.Кредит.СчетУчета), ТекСтрокаКредиты.Кредит.СчетУчета, ПланыСчетов.Учетный.Счет6601());
			Движение.Период = ТекДата;
			Движение.Предприятия = Предприятие;
			...
			Движение.Содержание = ТекСтрокаКредиты.Основание + " (поступление кредитных средств)"; 
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, ТекСтрокаКредиты.Кредит.Кредитор);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 2, ТекСтрокаКредиты.Кредит);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, СтатьяПолученияКредита);
		КонецЕсли; 
		
		// гашение кредита
		Если ТекСтрокаКредиты.Гашение Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетДт = ?(ЗначениеЗаполнено(ТекСтрокаКредиты.Кредит.СчетУчета), ТекСтрокаКредиты.Кредит.СчетУчета, ПланыСчетов.Учетный.Счет6601());
			Движение.СчетКт = ПланыСчетов.Учетный.Счет5101();
			Движение.Период = ТекДата;
			Движение.Предприятия = Предприятие;
			...
			Движение.Содержание = ТекСтрокаКредиты.Основание + " (возврат кредитных средств)";  
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ТекСтрокаКредиты.Кредит.Кредитор);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 2, ТекСтрокаКредиты.Кредит);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, СтатьяПогашенияКредита);
			//ДобавитьСтрокуПрибыли(ТекСтрокаКредиты.Месяц, - ТекСтрокаКредиты.Гашение);
		КонецЕсли;		
		
		// начисление процентов по кредиту
		Если ТекСтрокаКредиты.ПроцНачислено Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетДт = ПланыСчетов.Учетный.Счет91();
			Движение.СчетКт = ?(ЗначениеЗаполнено(ТекСтрокаКредиты.Кредит.СчетУчетаПроцентов), ТекСтрокаКредиты.Кредит.СчетУчетаПроцентов, ПланыСчетов.Учетный.Счет6602());
			Движение.Период = ТекДата;
			Движение.Предприятия = Предприятие;
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаКредиты.ПроцНачислено;
			Движение.Содержание = ТекСтрокаКредиты.Основание + " (начисление процентов по кредиту)"; 
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, ТекСтрокаКредиты.Кредит.Кредитор);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 2, ТекСтрокаКредиты.Кредит);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ПДР_РасходыНаУслугиБанков);
		КонецЕсли; 
		
		// гашение процентов по кредиту
		Если ТекСтрокаКредиты.ПроцОплачено Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетДт = ?(ЗначениеЗаполнено(ТекСтрокаКредиты.Кредит.СчетУчетаПроцентов), ТекСтрокаКредиты.Кредит.СчетУчетаПроцентов, ПланыСчетов.Учетный.Счет6602());
			Движение.СчетКт = ПланыСчетов.Учетный.Счет5101();
			Движение.Период = ТекДата;
			Движение.Предприятия = Предприятие;
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаКредиты.ПроцОплачено;
			Движение.Содержание = ТекСтрокаКредиты.Основание + " (возврат процентов по кредиту)";  
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ТекСтрокаКредиты.Кредит.Кредитор);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 2, ТекСтрокаКредиты.Кредит);
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, СтатьяПогашенияПроцентовПоКредиту);
			//ДобавитьСтрокуПрибыли(ТекСтрокаКредиты.Месяц, - ТекСтрокаКредиты.ПроцОплачено);
		КонецЕсли;	
		
		Если ТекСтрокаКредиты.ЗатратыПоОформлению Тогда
			Движение = Движения.Учетный.Добавить();
			Движение.СчетДт = ПланыСчетов.Учетный.Счет91();
			Движение.СчетКт = ПланыСчетов.Учетный.Счет5101();
			Движение.Период = ТекДата;
			...
			Движение.СценарийПлана = Сценарий;
			Движение.Сумма = ТекСтрокаКредиты.ЗатратыПоОформлению;
			Движение.Содержание = ТекСтрокаКредиты.Основание + " (затраты по оформлению кредита)"; 
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1,  Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду("2210.04"));
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ПДР_РасходыНаУслугиБанков);
			//ДобавитьСтрокуПрибыли(ТекСтрокаКредиты.Месяц, - ТекСтрокаКредиты.ЗатратыПоОформлению);
		КонецЕсли;		
