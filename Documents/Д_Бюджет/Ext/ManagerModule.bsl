...
Процедура РасчитатьПродажи(ТабДок, ДокументОбъект, ТипБюджета = Неопределено) Экспорт
	...
	НачДата = НачалоМесяца(БюджетныйНаСервере.ВернутьРеквизит(ДокументОбъект.Сценарий, "ДатаНачала") + 60 * 60 * 24);;
	ЭтоБюджетПродаж = ТипБюджета = Перечисления.Д_ТипыБюджетов.БюджетПродаж;
	...
	ОбластьШапка.Параметры.ТипБюджета = ?(ЭтоБюджетПродаж, "продаж и дебиторской задолженности", "закупок");
	...
	Для каждого ТекМесяц Из ТекПериоды Цикл
		ТЗ = ?(ЭтоБюджетПродаж, ДокументОбъект.БюджетПродаж.Выгрузить(), ДокументОбъект.БюджетЗакупок.Выгрузить());
		...
		СтруктураПоискаПериода = Новый Структура("Месяц", ТеКмесяц);
		ОтобранныеСтроки = ?(ЭтоБюджетПродаж, ДокументОбъект.БюджетПродаж.НайтиСтроки(СтруктураПоискаПериода), ДокументОбъект.БюджетЗакупок.НайтиСтроки(СтруктураПоискаПериода));
		
		Для каждого Выборка2 Из ОтобранныеСтроки Цикл
			ТекСтрочка = ?(ЭтоБюджетПродаж, ДокументОбъект.БюджетПродаж.Индекс(Выборка2), ДокументОбъект.БюджетЗакупок.Индекс(Выборка2));
		...
		ТЗ.Свернуть("Месяц", "СопровождениеСумма, ДЗКонец, ДЗНачало, КЗНачало, КЗОплата, Количество, ПоступлениеДС, ЦенаСумма, ВерхСумма, ДисконтСумма, КомпенсацияНалоговСумма, ВыплатаДисконта, ЖДАвтоСумма, ВозмещениеТрСумма, ЦенаЗакупкиСумма, ЧистаяЦенаСумма"); 
	...
	Если ТипБюджета = Перечисления.Д_ТипыБюджетов.БюджетПродаж Тогда
...
Процедура РечатьСебестоимостьСерверНовый(ТабДок, ДокументОбъект, ТекБП, КорректировочныйБюджет = Ложь, ТекКоманда = "") Экспорт
	
	// 23.07.13 di 
	Если ТипЗнч(ТекБП) = Тип("СправочникСсылка.УтверждениеБюджета") Тогда
	...
	Если ДокументОбъект.ОбъемПроизводства.Количество() Тогда //себестоимость
		...
		НоменклатураПростой = Справочники.Номенклатура.НайтиПоНаименованию("Простой предприятия");
					...
					Если НЕ ЗначениеЗаполнено(ТекСтрока.Номенклатура) И Не ТолькоНаПростой Тогда //если распределять
						...
						Для ТекИндекс = 0 По ПроизводимыеНоменклатуры.Количество() - 1 Цикл							
							...
							Если ПроизводимыеНоменклатуры[ТекИндекс].ПроцентНаПростой И СписаниеНаПростой Тогда
								...
								ЗаполнитьЗначенияСвойств(НоваяСтрока, ТекСтрока);
								НоваяСтрока.Код = ТекСтрока.СтатьяЗатрат.Код;
								НоваяСтрока.Номенклатура = НоменклатураПростой;
...
Процедура Печать(ТабДок, ТекБП, ТипБюджета) Экспорт
	...
	ИначеЕсли ТекБП.ТипБюджета = Перечисления.Д_ТипыБюджетов.КорректировкиБюджета Тогда 
		...
		Иначе
			Документы.Д_Бюджет.РечатьСебестоимостьСерверНовый(ТабДок, ПараметрКоманды, ТекБП)
		КонецЕсли;
...
Процедура ДобавитьОперативныеБюджеты(МассивТЧ, Бюджет, БизнесПроцесс)
	Если ТипЗнч(БизнесПроцесс) = Тип("СправочникСсылка.УтверждениеБюджета") Тогда
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
...
Функция ПолучитьПодразделениеЗагрузкаЗатрат(Подразделение, ЦФО, НомерСтроки, Кто) Экспорт
...
Функция НайтиСтатьюВСебестоимости(СтатьяБДДС) Экспорт
...
Функция ПолучитьКонтрагентаПоКлючевымРеквизитам(НаименованиеКонтрагента, ИННКонтрагента) Экспорт
	...
	Запрос.УстановитьПараметр("ИНН", ?(ЗначениеЗаполнено(ИННКонтрагента), Строка(ИННКонтрагента), Неопределено));
