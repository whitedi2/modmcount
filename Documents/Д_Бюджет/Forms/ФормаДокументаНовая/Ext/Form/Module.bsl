...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ИспользоватьПроекты = Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройки("ИспользоватьПроекты");
	...
	Иначе
		Массив = Новый Массив;
		Массив.Добавить(Тип("СправочникСсылка.КлючиАналитикиБюджетов"));
		ОписаниеТиповЧ = Новый ОписаниеТипов(Массив);	                                  
	КонецЕсли;
	Элементы.ЗатратыКПКлючАналитикиСтрокиЗатрат.ОграничениеТипа = ОписаниеТиповЧ;
	Элементы.ЗатратыКлючАналитикиБюджета.ОграничениеТипа = ОписаниеТиповЧ;
	Элементы.БюджетЗакупокКлючАналитикиБюджета.ОграничениеТипа = ОписаниеТиповЧ;
	Элементы.БюджетПродажКлючАналитикиБюджета.ОграничениеТипа = ОписаниеТиповЧ;
...
Процедура СценарийПриИзменении(Элемент)
	...
	Элементы.БюджетЗакупок.ПодчиненныеЭлементы.БюджетЗакупокМесяц.СписокВыбора.Очистить();
	...
	Для Каждого ТекЭлементМесяц Из СписокМесяцев Цикл 
		...
		Элементы.БюджетЗакупок.ПодчиненныеЭлементы.БюджетЗакупокМесяц.СписокВыбора.Добавить(ТекЭлементМесяц.Значение, ТекЭлементМесяц.Представление);
...
Процедура ИзменитьТабличныеЧасти()
	...
	Если Объект.Месяц = Дата(3000, 12, 31) ИЛИ Объект.Месяц = 0 Тогда
		...
		Элементы.БюджетЗакупок.ОтборСтрок = Неопределено;
	...
	Иначе
		...
		Элементы.БюджетЗакупок.ОтборСтрок = ФиксСтруктураПоиска;
...
&НаКлиенте
Процедура КопироватьСтрокиЗакупки(Команда)
	
	КопироватьСтроки(Элементы.БюджетЗакупок, Объект.БюджетЗакупок);
...
&НаКлиенте
Процедура БюджетЗакупокПриОкончанииРедактирования(Элемент, НоваяСтрока, ОтменаРедактирования)
	
	Если НоваяСтрока И НЕ Объект.Месяц = Дата(3000, 12, 31) Тогда
		Объект.БюджетЗакупок[Элемент.ТекущаяСтрока].Месяц = Объект.Месяц;	
	КонецЕсли;
	//Объект.БюджетПродаж.Сортировать("Месяц Возр, НомерСтроки Возр");
	//Если Объект.БюджетПродаж[Элемент.ТекущаяСтрока].Месяц = Элементы.Месяц.СписокВыбора[1].Значение Тогда
	//	СформироватьОстаткиГП();
...
&НаКлиенте
Процедура СвернутьТЗКлиент(СвернутьДоПериода = Истина)
	
	ТЗРеализация.Очистить();
	
	Для каждого ТекСтрока Из Объект.БюджетПродаж Цикл
		
		Если СвернутьДоПериода Тогда
			СтруктураПоиска = Новый Структура("Контрагент, Номенклатура, Месяц, Подразделение", ТекСтрока.Контрагент, ТекСтрока.Номенклатура, ТекСтрока.Месяц, ТекСтрока.Подразделение);
		Иначе
			СтруктураПоиска = Новый Структура("Контрагент, Номенклатура, Подразделение", ТекСтрока.Контрагент, ТекСтрока.Номенклатура, ТекСтрока.Подразделение);
		КонецЕсли;
		
		ОтобранныеСтроки = ТЗРеализация.НайтиСтроки(СтруктураПоиска);
...
&НаКлиенте
Процедура СвернутьТЗКлиентЗакупки(СвернутьДоПериода = Истина)
	
	ТЗРеализация.Очистить();
	
	Для каждого ТекСтрока Из Объект.БюджетЗакупок Цикл
...
&НаКлиенте
Процедура ПросчитатьДЗИКЗЗакупки()
	
	//формируем список контрагентов, присутствующих в документе
	
	СвернутьТЗКлиентЗакупки();
	...
	Для каждого ТекПозиция Из ТЗРеализация Цикл
		...
		//находим первую строчку, т.е. остатки на начало
		СтруктураПоиска = Новый Структура("Месяц, Контрагент, Номенклатура, Подразделение", Элементы.Месяц.СписокВыбора[1].Значение, ТекПозиция.Контрагент, ТекПозиция.Номенклатура, ТекПозиция.Подразделение);
		ОтобранныеСтроки = Объект.БюджетЗакупок.НайтиСтроки(СтруктураПоиска);
		ДЗНачало = 0; Отгрузка = 0; Оплата = 0;
		КЗНачало = 0; НачисленоКЗ = 0; ОплаченоКЗ = 0;
		Если НЕ ОтобранныеСтроки.Количество() Тогда
			ДЗКонец = 0;		
...
&НаКлиенте
Процедура БюджетЗакупокПредоплатаПриИзменении(Элемент)
	
	Элементы.БюджетЗакупок.ТекущиеДанные.Отсрочка = 0;
	
КонецПроцедуры

&НаКлиенте
Процедура БюджетЗакупокОтсрочкаПриИзменении(Элемент)
	
	Элементы.БюджетЗакупок.ТекущиеДанные.Предоплата = 0;
...
Функция СменитьТип(ТипСправочника)
	...
	Если ТипСправочника = Перечисления.Д_ПризнакСписания.ФинРез Тогда
		НовыйТип = Справочники.ПрочиеДоходыИРасходы.ПустаяСсылка();
	КонецЕсли;
	Возврат НовыйТип;
КонецФункции
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	ИначеЕсли ИмяСобытия = "ПереносВБюджет" Тогда
		...
		ОбновитьНадписьНеперенесенных();
	ИначеЕсли ИмяСобытия = "ЗавершениеЗагрузкиИзФайла" Тогда
		ЗаполнитьТабличнуюЧастьПоПараметрамЗагрузки(Параметр);
	КонецЕсли; 
...
Процедура СинхронизироватьОстаткиСервер()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |				И Счет В ИЕРАРХИИ (&Счета),
	               |			,
	               |			Предприятия = &Предприятие
	               |				И ЛОЖЬ) КАК УчетныйОстатки
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	УчетныйОстатки.Счет
	               |
	...
	СчетаДляОстатков = Новый Массив;
	СчетаДляОстатков.Добавить(ПланыСчетов.Учетный.Счет43());
	СчетаДляОстатков.Добавить(ПланыСчетов.Учетный.Счет41());
	Запрос.УстановитьПараметр("Счета", СчетаДляОстатков);
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	Для Каждого ТекТЧ Из ТекущийОбъект.Метаданные().ТабличныеЧасти Цикл
		...
		Если НЕ ТекТЧ.Реквизиты.Найти("КлючАналитикиБюджета") = Неопределено И НЕ ТекТЧ.Реквизиты.Найти("УИД") = Неопределено И СоздаватьГрафикБюджета Тогда
...
Процедура СоздатьОбновитьКлючАналитики(СтрокаТЧ, ТекущийОбъект)
	Если ЗначениеЗаполнено(СтрокаТЧ.КлючАналитикиБюджета) И ТипЗнч(СтрокаТЧ.КлючАналитикиБюджета) = Тип("СправочникСсылка.КлючиАналитикиБюджетов") Тогда
...
Процедура ПечатьБюджета(Команда)
	...
	ПечатьДокументаСервер(ТабДок, Команда.Имя);
...
&НаСервере
Процедура ПечатьДокументаСервер(ТабДок, ТекКоманда)
	...
	Если ТекКоманда = "ПечатьЗакупок" Тогда 
		...
		Документы.Д_Бюджет.РасчитатьПродажи(ТабДок, Объект.Ссылка, Перечисления.Д_ТипыБюджетов.БюджетЗакупок);
	...
	ИначеЕсли ТекКоманда = "ПечатьЗакупок" Или ТекКоманда = "ПечатьПродаж" Тогда 
	...
	ИначеЕсли ТекКоманда = "ПечатьКорректировокЗатрат" Тогда 
		Документы.Д_Бюджет.РечатьСебестоимостьСерверНовый(ТабДок, Объект.Ссылка, Перечисления.Д_ТипыБюджетов.КорректировкиБюджета);	
	...
	ИначеЕсли ТекКоманда = "ПечатьЗатрат" Тогда 
		Документы.Д_Бюджет.РечатьСебестоимостьСерверНовый(ТабДок, Объект.Ссылка, Неопределено, КорректировочныйБюджет, ТекКоманда)
...
&НаКлиенте
Процедура БюджетЗакупокПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	
	Если ПустаяСтрока(Элементы.БюджетЗакупок.ТекущиеДанные.Контрагент) Тогда
		Элементы.БюджетЗакупок.ТекущиеДанные.Контрагент = ПустойКонтрагент();
	КонецЕсли;
	
	Если НоваяСтрока Тогда
	    Элементы.БюджетЗакупок.ТекущиеДанные.ОбъектПродажи = "Номенклатура";
		Элементы.БюджетЗакупок.ТекущиеДанные.Номенклатура = ПредопределенноеЗначение("Справочник.Номенклатура.ПустаяСсылка");	
	КонецЕсли;
...
&НаСервере
Процедура СортировкаСтрокЗакупки()
	ВремТЗ = Объект.БюджетЗакупок.Выгрузить();
	ВремТЗ.Сортировать("Месяц Возр, НомерСтроки Возр");
	Объект.БюджетЗакупок.Загрузить(ВремТЗ);
КонецПроцедуры
...
Процедура ПоступленияДСЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Для Каждого ТекСтрокаПродажи Из Объект.БюджетПродаж Цикл
		ТекСтрокаПродажи.ПоступлениеДС = 0;
	КонецЦикла;
	
	//формируем список контрагентов, присутствующих в документе
	
	СвернутьТЗКлиент(Ложь);
	...
	Для каждого ТекПозиция Из ТЗРеализация Цикл   	
		...
		Для каждого ТекСтрока2 Из ОтобранныеСтроки Цикл			
			Цена = ТекСтрока2.Цена + ТекСтрока2.Верх;
			ПоступлениеДС = ТекСтрока2.Количество * Цена;
			
			Если ТекСтрока2.Отсрочка = 0 Тогда 
				ТекСтрока2.ПоступлениеДС = ТекСтрока2.ПоступлениеДС + ПоступлениеДС;
			Иначе
				СрокНаступленияОплаты = ДобавитьМесяц(ТекСтрока2.Месяц, Цел(ТекСтрока2.Отсрочка / 30));
				
				Если Год(СрокНаступленияОплаты) > Год(Элементы.Месяц.СписокВыбора[Элементы.Месяц.СписокВыбора.Количество() - 1].Значение) Тогда
					Продолжить;
				КонецЕсли;
				
				МесяцНаступленияОплаты = Мин(НачалоМесяца(СрокНаступленияОплаты), Элементы.Месяц.СписокВыбора[Элементы.Месяц.СписокВыбора.Количество() - 1].Значение);     
				
				СтруктураПоиска = Новый Структура("Подразделение, Контрагент, Номенклатура, Месяц", 
				ТекСтрока2.Подразделение, 
				ТекСтрока2.Контрагент, 
				ТекСтрока2.Номенклатура,
				МесяцНаступленияОплаты);  
				
				ОтобранныеСтроки = Объект.БюджетПродаж.НайтиСтроки(СтруктураПоиска);  
				
				Если ОтобранныеСтроки.Количество() Тогда 	
					ОтобранныеСтроки[0].ПоступлениеДС = ОтобранныеСтроки[0].ПоступлениеДС + ПоступлениеДС;
				Иначе
					НоваяСтрокаБюджетаПродаж = Объект.БюджетПродаж.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаБюджетаПродаж, ТекСтрока2,, "Месяц, Цена, Количество, СтавкаНДС, Предоплата, Отсрочка");
					НоваяСтрокаБюджетаПродаж.Месяц = МесяцНаступленияОплаты;
					НоваяСтрокаБюджетаПродаж.ПоступлениеДС = ПоступлениеДС;
				КонецЕсли;
	...
	Объект.БюджетПродаж.Сортировать("Месяц Возр, Контрагент Возр, Номенклатура Возр");
...
&НаКлиенте
Процедура ПоступленияДСЗакупки(Команда)
	
	ПоказатьВопрос(Новый ОписаниеОповещения("ПоступленияДСЗакупкиЗавершение", ЭтотОбъект, Новый Структура("Команда", Команда)), "Все поступления будут пересчитаны. Продолжить?", РежимДиалогаВопрос.ДаНет);	
КонецПроцедуры

&НаКлиенте
Процедура ПоступленияДСЗакупкиЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если Команда.Имя = "ПоступленияДСЗакупки" Тогда
		КоллекцияСтрок = Объект.БюджетЗакупок;
	Иначе
		КоллекцияСтрок = Новый Массив;
		Для каждого ТекСтрока Из Элементы.БюджетЗакупок.ВыделенныеСтроки Цикл
			ТекСтрокаОбъект = Объект.БюджетЗакупок.НайтиПоИдентификатору(ТекСтрока);
			КоллекцияСтрок.Добавить(ТекСтрокаОбъект);	
		КонецЦикла; 	
	КонецЕсли;
	...
	Для каждого ТекПозиция Из ТЗРеализация Цикл   	
		...
		Для каждого ТекСтрока2 Из ОтобранныеСтроки Цикл			
			...
			Если ТекСтрока2.Отсрочка = 0 Тогда 
				ТекСтрока2.ПоступлениеДС = ПоступлениеДС;
			Иначе
				СрокНаступленияОплаты = ДобавитьМесяц(ТекСтрока2.Месяц, Цел(ТекСтрока2.Отсрочка / 30));
				...
				СтруктураПоиска = Новый Структура("Подразделение, Контрагент, Номенклатура, Количество, ЦенаЗакупки, Месяц", 
				ТекСтрока2.Подразделение, 
				ТекСтрока2.Контрагент, 
				ТекСтрока2.Номенклатура,
				ТекСтрока2.Количество,
				ТекСтрока2.ЦенаЗакупки,
				МесяцНаступленияОплаты);  
				
				ОтобранныеСтроки = Объект.БюджетЗакупок.НайтиСтроки(СтруктураПоиска);  
				
				Если ОтобранныеСтроки.Количество() Тогда 	
					ОтобранныеСтроки[0].ПоступлениеДС = ПоступлениеДС;
				Иначе
					НоваяСтрокаБюджетаЗакупок = Объект.БюджетЗакупок.Добавить();
					ЗаполнитьЗначенияСвойств(НоваяСтрокаБюджетаЗакупок, ТекСтрока2,, "Месяц, Цена, Количество, СтавкаНДС, Предоплата, Отсрочка");
					НоваяСтрокаБюджетаЗакупок.Месяц = МесяцНаступленияОплаты;
					НоваяСтрокаБюджетаЗакупок.ПоступлениеДС = ПоступлениеДС;
				КонецЕсли;
	...
	Объект.БюджетЗакупок.Сортировать("Месяц Возр, Контрагент Возр, Номенклатура Возр");
...
&НаКлиенте
Процедура СортироватьСтрокиЗакупки(Команда)
	СортировкаСтрокЗакупки();
КонецПроцедуры
...
&НаКлиенте
Процедура РасчетПоступленийПоСтрокеЗакупки(Команда)
	
	Если НЕ Элементы.БюджетЗакупок.ТекущиеДанные = Неопределено Тогда
		
		Для каждого ТекСтрока2 Из Элементы.БюджетЗакупок.ВыделенныеСтроки Цикл
			
			ТекСтрока2 = Элементы.БюджетЗакупок.ТекущиеДанные;
...
&НаСервере
Процедура ЗаполнитьТабличнуюЧастьПоПараметрамЗагрузки(СтруктураПараметровЗагрузки)
	
	ТекТЧ = Объект[СтруктураПараметровЗагрузки.ИмяТЧ];
	
	Для Каждого ТекЭлементМассиваСтрок Из СтруктураПараметровЗагрузки.МассивЗагружаемыхСтрок Цикл
        НоваяСтрокаТЧ = ТекТЧ.Добавить();
		НоваяСтрокаТЧ.Основание = ТекЭлементМассиваСтрок.Основание;
		
		Попытка
			НоваяСтрокаТЧ.Месяц = Дата(ТекЭлементМассиваСтрок.Месяц);
		Исключение
			Сообщить("Выбран некорректный месяц!");
		КонецПопытки;
		
		Попытка
			НоваяСтрокаТЧ.СуммаСеб = Число(ТекЭлементМассиваСтрок.СуммаФР);
		Исключение
			НоваяСтрокаТЧ.СуммаСеб = 0;
		КонецПопытки;
		
		Попытка
			НоваяСтрокаТЧ.СуммаБДДС = Число(ТекЭлементМассиваСтрок.СуммаБДДС);
		Исключение
			НоваяСтрокаТЧ.СуммаБДДС = 0;
		КонецПопытки;
		
		Если ЗначениеЗаполнено(ТекЭлементМассиваСтрок.ЦФО) Тогда                                    
			ЦФО = Справочники.Предприятия.НайтиПоНаименованию(ТекЭлементМассиваСтрок.ЦФО, Истина);
		Иначе 
			ЦФО = Объект.Предприятие;
		КонецЕсли;
		
		Если ЗначениеЗаполнено(ЦФО) Тогда
			Подразделение = Документы.Д_Бюджет.ПолучитьПодразделениеЗагрузкаЗатрат(ТекЭлементМассиваСтрок.Подразделение, ЦФО, НоваяСтрокаТЧ.НомерСтроки, "Подразделение"); 
		Иначе     
			Сообщить("Предприятие " + ЦФО + " не найдено в справочнике Предприятия (строка " + НоваяСтрокаТЧ.НомерСтроки + ")");
		КонецЕсли;
		
		Если СтруктураПараметровЗагрузки.ИмяТЧ = "ЗатратыКП" Тогда
			НоваяСтрокаТЧ.ВидДеятельности = Подразделение;
			НоваяСтрокаТЧ.ЦФО = ЦФО;
		ИначеЕсли СтруктураПараметровЗагрузки.ИмяТЧ = "Прочие90" Тогда
		...
		СтатьяБДДС = Справочники.СтатьиДвиженияДенежныхСредств.НайтиПоКоду(ТекЭлементМассиваСтрок.КодСтатьи);			
		НомерПризнака = 2;
		
		Если Не ЗначениеЗаполнено(СтатьяБДДС) Тогда
			СтатьяСебестоимости = Справочники.СтатьиЗатрат.НайтиПоКоду(ТекЭлементМассиваСтрок.КодСтатьи);
			
			Если Не ЗначениеЗаполнено(СтатьяСебестоимости) Тогда
				Сообщить ("Не найдена статья по коду " + ТекЭлементМассиваСтрок.КодСтатьи + " в справочнике СтатьиДДС и С/с (строка " + НоваяСтрокаТЧ.НомерСтроки + ")");
				СтатьяБДДС = "";
			КонецЕсли;
			
		иначе
			СтатьяСебестоимости = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(СтатьяБДДС, "СтатьяЗатрат");
			НомерПризнака = 1;
			
			Если Не ЗначениеЗаполнено(СтатьяСебестоимости) Тогда
				СтатьяСебестоимости = Справочники.ПрочиеДоходыИРасходы.НайтиПоКоду(ТекЭлементМассиваСтрок.КодСтатьи);
				
				Если Не ЗначениеЗаполнено(СтатьяСебестоимости) Тогда
					сообщить("Не найдено соответствие статьи затрат со статьей БДДС по коду " + ТекЭлементМассиваСтрок.КодСтатьи + " (строка " + НоваяСтрокаТЧ.НомерСтроки + ")");
					СтатьяСебестоимости = "";
				КонецЕсли;
		...
		Если СтруктураПараметровЗагрузки.ИмяТЧ = "Прочие90" Тогда
			НоваяСтрокаТЧ.Статья = СтатьяСебестоимости;
		Иначе 	
			НоваяСтрокаТЧ.СтатьяЗатрат = СтатьяСебестоимости;
		КонецЕсли;
		
		Если Не (СтруктураПараметровЗагрузки.ИмяТЧ = "ЗатратыКП" Или СтруктураПараметровЗагрузки.ИмяТЧ = "Прочие90") Тогда
			
			Если НомерПризнака = 1 Тогда
				НоваяСтрокаТЧ.Признак = Перечисления.Д_ПризнакСписания.Себестоимость;
			иначе
				НоваяСтрокаТЧ.Признак = Перечисления.Д_ПризнакСписания.ФинРез;
			КонецЕсли;

		КонецЕсли;	
		
		НоваяСтрокаТЧ.СтатьяБДДС = СтатьяБДДС;
		
		Если ЗначениеЗаполнено(ТекЭлементМассиваСтрок.Заявитель) Тогда
			Заявитель = Справочники.Предприятия.НайтиПоНаименованию(ТекЭлементМассиваСтрок.Заявитель, Истина);
			
			Если ЗначениеЗаполнено(Заявитель) Тогда  
				НоваяСтрокаТЧ.Заявитель = Заявитель;
				НоваяСтрокаТЧ.ПодразделениеЗаявителя = Документы.Д_Бюджет.ПолучитьПодразделениеЗагрузкаЗатрат(ТекЭлементМассиваСтрок.ПодразделениеЗаявителя, Заявитель, НоваяСтрокаТЧ.НомерСтроки, "Подразделение заявителя");
			Иначе     
				Сообщить("Предприятие " + Заявитель + " не найдено в справочнике Предприятия (строка " + НоваяСтрокаТЧ.НомерСтроки + ")");
			КонецЕсли;
						
		КонецЕсли;
		
		Если СтруктураПараметровЗагрузки.ИмяТЧ = "Затраты" Или СтруктураПараметровЗагрузки.ИмяТЧ = "ЗатратыКП" Тогда
			ИННКонтрагента = "";
			НаименованиеКонтрагента = "";
			
			Попытка
				ИННКонтрагентаЧисло = Число(ТекЭлементМассиваСтрок.Контрагент);
				ИННКонтрагента = СтрЗаменить(ТекЭлементМассиваСтрок.Контрагент, Символы.НПП, "");
			Исключение
				НаименованиеКонтрагента = ТекЭлементМассиваСтрок.Контрагент;
			КонецПопытки;
			
			НоваяСтрокаТЧ.Контрагент = Документы.Д_Бюджет.ПолучитьКонтрагентаПоКлючевымРеквизитам(НаименованиеКонтрагента, ИННКонтрагента);
			
			Если ЗначениеЗаполнено(НоваяСтрокаТЧ.Контрагент) Тогда
				НоваяСтрокаТЧ.Договор = Справочники.ДоговорыКонтрагентов.НайтиПоНаименованию(ТекЭлементМассиваСтрок.ДоговорКонтрагента, Истина,,НоваяСтрокаТЧ.Контрагент);
			КонецЕсли;
			
			НоваяСтрокаТЧ.КлючАналитикиБюджета = Справочники.КлючиАналитикиБюджетов.НайтиПоКоду(ТекЭлементМассиваСтрок.КлючАналитики);
		КонецЕсли;
...
Процедура ЗатратыПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	Если Копирование Тогда
		Элементы.Затраты.ТекущиеДанные.КлючАналитикиБюджета = Неопределено;
		Элементы.Затраты.ТекущиеДанные.УИД = Новый УникальныйИдентификатор;
	КонецЕсли;
КонецПроцедуры
