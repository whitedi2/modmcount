...
Процедура ЗагрузкаЗатрат()
	...
	Иначе  		
		...
		Данные = ПрочитатьДанныеФайла(Адрес, 12, РасширениеФайла); 		
		//!!!!!
		...
		Для ТекСтрока = 0 по СтрокаОкончания - СтрокаНачала Цикл
			...
			СтруктураСтрокиЗагрузки = Новый Структура;
			СтруктураСтрокиЗагрузки.Вставить("Месяц", СокрЛП(Данные[0][ТекСтрока]));
			СтруктураСтрокиЗагрузки.Вставить("КодСтатьи", СокрЛП(Данные[1][ТекСтрока]));
			СтруктураСтрокиЗагрузки.Вставить("СуммаФР", СокрЛП(Данные[2][ТекСтрока]));
			СтруктураСтрокиЗагрузки.Вставить("СуммаБДДС", СокрЛП(Данные[3][ТекСтрока]));
			СтруктураСтрокиЗагрузки.Вставить("ЦФО", СокрЛП(Данные[4][ТекСтрока]));
			СтруктураСтрокиЗагрузки.Вставить("Подразделение", СокрЛП(Данные[5][ТекСтрока]));
			СтруктураСтрокиЗагрузки.Вставить("Основание", СокрЛП(Данные[6][ТекСтрока]));	
			СтруктураСтрокиЗагрузки.Вставить("Заявитель", СокрЛП(Данные[7][ТекСтрока]));
			СтруктураСтрокиЗагрузки.Вставить("ПодразделениеЗаявителя", СокрЛП(Данные[8][ТекСтрока]));
			СтруктураСтрокиЗагрузки.Вставить("Контрагент", СокрЛП(Данные[9][ТекСтрока]));
			СтруктураСтрокиЗагрузки.Вставить("ДоговорКонтрагента", СокрЛП(Данные[10][ТекСтрока])); 
			СтруктураСтрокиЗагрузки.Вставить("КлючАналитики", СокрЛП(Данные[11][ТекСтрока]));
			
			МассивСтрок.Добавить(СтруктураСтрокиЗагрузки);
		КонецЦикла;
		
		//ЗаполнитьТабличнуюЧастьПоПараметрамЗагрузки(МассивСтрок);
		//XLSОбъект.Application.Quit();
		Оповестить("ЗавершениеЗагрузкиИзФайла", Новый Структура("ИмяТЧ, МассивЗагружаемыхСтрок", ВладелецФормы.Имя, МассивСтрок));
