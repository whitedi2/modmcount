...
Процедура ОбработкаКомандыЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		...
		Если ТипЗнч(ПараметрКоманды) = Тип("ДокументСсылка.Д_ЗаявкаНаКорректировкуБюджета") Тогда
			ТекФормаБП = ПолучитьФорму("БизнесПроцесс.Согласование2.Форма.ФормаНовый");
		Иначе
			ТекФормаБП = ПолучитьФорму("Справочник.СогласованиеОбщее.ФормаОбъекта");
		КонецЕсли;
...
Функция ПроверкаВозможностиЗапускаБП(ПараметрКоманды)
	...
	Для Каждого ТекДокумент Из Массив Цикл
		...
		//		Продолжить;
		//	КонецЕсли;	
		Если ТипЗнч(ТекДокумент) = Тип("ДокументСсылка.Д_ЗаявкаНаСогласованиеДоговора") Тогда
			Если ТекДокумент.ВидСЗ = Справочники.Д_ВидыВнутреннихДокументов.СогласованиеДоговораСтроительствоТендер Тогда
				Запрос = Новый Запрос;
				Запрос.Текст = 
				"ВЫБРАТЬ
				|	1 КАК Проверка
				...
				Запрос.УстановитьПараметр("Владелец", ТекДокумент);
				Если Запрос.Выполнить().Пустой() Тогда
					Сообщить("Необходимо прикрепить протокол тендерного комитета");
					Возврат Ложь;
				КонецЕсли;	
			КонецЕсли;	
		КонецЕсли;
...
Функция ТочкаМаршрутаДоработки(ПараметрКоманды)
	...
	Возврат ТочкаМаршрутаДоработки = Перечисления.СогласованиеОбщееТочкиМаршрута.Действие1;
