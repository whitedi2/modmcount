...
Процедура УУ_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Пока ВыбокаНомерСтроки.Следующий() Цикл
			...
			//		ТекСтрока.Продукция = ВыбокаПродукция.Продукция;
			//	КонецЕсли;
			//	Продолжить;
			//КонецЕсли;
			...
			Пока ВыборкаДетальныеЗаписи.Следующий() И ОсталосьСписать > 0 Цикл 
				...
				ЗапросПродукции.УстановитьПараметр("Номенклатура",ВыборкаДетальныеЗаписи.Номенклатура);
				ВыбокаПродукция = ЗапросПродукции.Выполнить().Выбрать();
				Если ВыбокаПродукция.Следующий() Тогда
					ТекСтрока.Продукция = ВыбокаПродукция.Продукция;
				КонецЕсли;
			...
			КонецЕсли;
				...
				Если ВыборкаДетальныеЗаписи.БазоваяЕдиницаИзмеренияИзВетис = ВыборкаДетальныеЗаписи.ЕдиницаИзмеренияНоменклатуры Тогда
					Коэф = 1;
				Иначе
					Если Не ЗначениеЗаполнено(ВыборкаДетальныеЗаписи.ФасовкаКоличествоЕдиницВУпаковке) ИЛИ 
						ВыборкаДетальныеЗаписи.ФасовкаЕдиницаИзмеренияПродукции <> ВыборкаДетальныеЗаписи.ЕдиницаИзмеренияВЕТИСВРегистреОстатков Тогда
						Запрос = Новый Запрос;
						Запрос.Текст = "ВЫБРАТЬ
						...
						КонецЦикла;
					Иначе Коэф = ВыборкаДетальныеЗаписи.ФасовкаКоличествоЕдиницВУпаковке;
					КонецЕсли;
					Если Коэф = 0 Тогда
						Коэф = 1;
					КонецЕсли;
