...
Процедура УУ_ПриСозданииНаСервереПосле(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	И ОстаткиПродукцииВЕТИС.ЗаписьСкладскогоЖурнала.СрокГодностиНачалоПериода > &ТекущаяДата
		               |
		               |УПОРЯДОЧИТЬ ПО
		...
		Для каждого ТекСтрока Из Объект.Товары Цикл
			...
			Для каждого ТекНайденнаяСтрока Из НайденныеСтроки Цикл
				...
				Если НужноКоличество > ТекНайденнаяСтрока.КоличествоВЕТИС Тогда
					НужноКоличество = НужноКоличество - ТекНайденнаяСтрока.КоличествоВЕТИС;
					ЗаполнитьЗначенияСвойств(ТекСтрока, ТекНайденнаяСтрока.ЗаписьСкладскогоЖурнала);
					ТекСтрока.Количество = ТекНайденнаяСтрока.КоличествоВЕТИС / Коэф;
					ТекСтрока.КоличествоВЕТИС = ТекНайденнаяСтрока.КоличествоВЕТИС;
					...
					Если ЗначениеЗаполнено(ТекСтрока.Номенклатура) И (ТекСтрока.Номенклатура.ЕдиницаИзмерения.Код = "796" ИЛИ ТекСтрока.ЕдиницаИзмерения = "шт") Тогда //если шт
						...
						Если НЕ Цел(ТекСтрока.Количество) = ТекСтрока.Количество Тогда
							ТекСтрока.КоличествоВЕТИС = ТекНайденнаяСтрока.КоличествоВЕТИС - (ТекСтрока.Количество - Цел(ТекСтрока.Количество)) * Коэф;
