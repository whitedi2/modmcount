...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ИспользоватьВНХВзаимозачеты = Не Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройки("сабСквозныеИсточнкиПоПредприятиям");
	...
	Если Проводки.Количество() Тогда
		...
		Если Проводки.Количество() = 1 И ТипЗнч(Проводки[0].КорСубконто3) = Тип("ДокументСсылка.УЧ_ВыплатаЗП") И ЗначениеЗаполнено(Проводки[0].КорСубконто3) И Проводки[0].КорСубконто3.СуммаДокумента = ЭтотОбъект.СуммаДокумента Тогда
...
Процедура ПроводкаУслуги(ЭтоРасход, ТекСтрока)
	...
	?(ЗначениеЗаполнено(Подразделение), Подразделение, ?(ЗначениеЗаполнено(ТекСтрока.Подразделение), ТекСтрока.Подразделение, ТекСтрока.ФинПодразделение)), 									//ПодразделениеДт
	, 									//КоличествоДт
	, 							//ВалютаДт
	, 					//ВалСуммаДт
	ТекСтрока.КорСчет,						//счетКт
...
Процедура ПроводкаРаспределениеЗП(ЭтоРасход, ТекСтрока, ТекСтрокаДока)
	...
	ТекСтрока.КорСубконто3,				//СубконтоКт3
	?(ЗначениеЗаполнено(ТекСтрока.Подразделение), ТекСтрока.Подразделение, ?(ЗначениеЗаполнено(ТекСтрока.ФинПодразделение), ТекСтрока.ФинПодразделение, Подразделение)),			//ПодразделениеКт
	ТекСтрока.Количество,									//КоличествоДт
	ТекСтрока.Валюта,									//ВалютаКт
	ТекСтрока.КорВалСумма,                                   //ВалСуммаКт
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Проводки.Количество() Тогда //продублировано с формы
		...
		ОписаниеТиповДоговор = Новый ОписаниеТипов(МассивТиповДоговор);
		...
		Для каждого ТекСтрока Из Проводки Цикл
			...
			Если ЗначениеЗаполнено(Договор) И ВариантыОтчетовВызовСервера.ТипСубконто(ТекСтрока.КорСчет, 2) = ОписаниеТиповДоговор Тогда
				ТекСтрока.КорСубконто2 = Договор;
			КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("Структура") И Не ДанныеЗаполнения.Свойство("Заявка") И Не ДанныеЗаполнения.Свойство("ДокументВыплаты") Тогда // из обработки платежей переводы
		...
		Если ДанныеЗаполнения.ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ВозмещениеОплат Тогда
			ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ВозмещениеПоступлений;
		ИначеЕсли ДанныеЗаполнения.ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ВозмещениеПоступлений Тогда
