...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Параметры.Свойство("ВидОперации") Тогда
		Если Параметры.ВидОперации = "КассовыйОрдер" Тогда
			...
			ВидОперацийДвиженияДС = НовыйОтбор.ПравоеЗначение;
			
			Заголовок = "Кассовые ордера";
			
			Если НЕ Элементы.Найти("ФормаСоздать") = Неопределено Тогда
				Элементы.ФормаСоздать.Видимость = Ложь;
			КонецЕсли;
			Если НЕ Элементы.Найти("ФормаГруппа2") = Неопределено Тогда
				Элементы.ФормаГруппа2.Видимость = Истина;
			КонецЕсли;
		ИначеЕсли Параметры.ВидОперации = "БанковскаяВыписка" Тогда
			
			НовыйОтбор = Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
			НовыйОтбор.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ВидОперации");
			НовыйОтбор.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
			НовыйОтбор.ПравоеЗначение = ПредопределенноеЗначение("Перечисление.ВидыОперацийДвиженияДС.БанковскаяВыписка");
			
			ВидОперацийДвиженияДС = НовыйОтбор.ПравоеЗначение;
			
			Заголовок = "Банковские выписки (ДДС)";
...
Процедура Поступление(Команда)
	Если Заголовок = "Кассовые ордера" Или Заголовок = "Кассовый ордер" Тогда
	...
	ИначеЕсли Заголовок = "Банковские выписки (ДДС)" Или Заголовок = "Банковская выписка" Тогда	
...
&НаКлиенте
Процедура ОткрытьОбработкуВзаиморасчетов(Команда)
	ОткрытьФорму("Обработка.УЧ_ОбработкаПлатежей.Форма");
КонецПроцедуры
