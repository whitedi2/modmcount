...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтоНовыйЧистый = Не ЗначениеЗаполнено(Объект.Ссылка) И Не ЗначениеЗаполнено(Параметры.Основание);
	...
	Элементы.ПроводкиКлючАналитикиБюджета.Видимость = ИспользоватьКлючиАналитикиБюджета Или ИспользоватьПроекты;
	...
	Элементы.ПроводкиКлючАналитикиБюджета.ОграничениеТипа = ОписаниеТиповЧ;
	Элементы.ПроводкиКлючАналитикиБюджета.Заголовок = ЗаголовокКлючаАналитики;
	...
	Если Параметры.Свойство("СозданиеПКОФорма2") Тогда
		Объект.ВидОперации = Перечисления.ВидыОперацийДвиженияДС.КассовыйОрдер;
		Объект.ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.Поступление;
		Если ЗначениеЗаполнено(Параметры.СозданиеПКОФорма2.ДокументУУ) Тогда
			Объект.ДокОснование = Параметры.СозданиеПКОФорма2.ДокументУУ;
		КонецЕсли;
		ЗаполнитьЗначенияСвойств(Объект,Параметры.СозданиеПКОФорма2);
		Если Объект.Проводки.Количество() Тогда
			Объект.Проводки[0].ФинСтатья = Параметры.СозданиеПКОФорма2.Субконто1;
			Объект.Проводки[0].КорСубконто1 = Параметры.СозданиеПКОФорма2.Контрагент;
			Объект.Проводки[0].Сумма = Объект.СуммаДокумента;
			Объект.Проводки[0].КорСчет = ПланыСчетов.Учетный.Счет6201();
			Если ЗначениеЗаполнено(Параметры.СозданиеПКОФорма2.ДокументУУ) Тогда
				Объект.Проводки[0].КорСубконто3 = Параметры.СозданиеПКОФорма2.ДокументУУ;
			Иначе
				Сообщить("Для текущего бухгалтерского документа (розничная продажа) не удалось создать и подобрать уплавленческий документ. Поле ""Документ"" для ПКО не заполнено. Необходимо подобрать документ вручную."); 
			КонецЕсли;	
		КонецЕсли;
	КонецЕсли;
...
Процедура ЗаполнитьОбратноеВНХ()
	...
	ЭтоВНХ = Объект.ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ОплатаВНХ ИЛИ Объект.ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.ПоступлениеВНХ ИЛИ Объект.ПриходРасход = Перечисления.ВидыОперацийПлатежноеПоручение.СнятиеНаличных;
	
	Если ЗначениеЗаполнено(Объект.Ссылка) И НЕ ПараметрыСеанса.ОграничиватьПредприятия И ЭтоВНХ Тогда
	...
	Если Не ЗначениеЗаполнено(ОбратноеВНХДвижение) И Параметры.Свойство("Основание") И ЗначениеЗаполнено(Параметры.Основание) Тогда
		ОбратноеВНХДвижение = УЧ_Сервер.ПодобратьВНХДвижение(Новый Структура("Ссылка, Дата, Организация, Контрагент, СуммаДокумента, ЭтоВНХ", Объект.Ссылка, Объект.Дата, Объект.Организация, Объект.Контрагент, Объект.СуммаДокумента, ЭтоВНХ), Объект.ПриходРасход, Неопределено);	
		ОбратноеВНХДвижение_ДоУдаления = ОбратноеВНХДвижение;
	КонецЕсли;
...
Процедура ИзменитьСчетСервер()
	...
	Иначе
		...
		Если ТипЗнч(Объект.Субконто2) = Тип("СправочникСсылка.БанковскиеСчета") И (Объект.Субконто2.Валютный ИЛИ (ЗначениеЗаполнено(Объект.Субконто2) И НЕ Объект.Субконто2.ВалютаДенежныхСредств = УЧ_Сервер.НациональнаяВалюта())) Тогда
			СчетВалютный = Истина;
			Объект.Счет = ПланыСчетов.Учетный.Счет5102();
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не ПараметрыЗаписи.Свойство("ВыполняетсяЗапись") Тогда
		ПараметрыЗаписи.Вставить("ВыполняетсяЗапись", Истина);
		
		Если БюджетныйНаКлиенте.ЕстьСвойствоОбъекта(ЭтотОбъект, "ДокументБУ") И ЗначениеЗаполнено(ЭтотОбъект.ДокументБУ) Тогда 
			СуммаОснованияБУ = БюджетныйНаСервере.ВернутьРеквизит(ЭтотОбъект.ДокументБУ, "СуммаДокумента");                    
			
			Если СуммаОснованияБУ <> Объект.СуммаДокумента Тогда
				ПараметрыЗаписи.Вставить("ВыполняетсяЗапись", Ложь);
			КонецЕсли;
	...
	Если Не ПараметрыЗаписи.ВыполняетсяЗапись Тогда  
		Отказ = Истина;
		Оповещение = Новый ОписаниеОповещения("ПередЗаписьюДДСЗавершение", ЭтотОбъект, ПараметрыЗаписи);
		ТекстВопроса = "Различаются суммы документа (" + Объект.СуммаДокумента + ") и документа-основания БУ (" + СуммаОснованияБУ + "). Записать документ?";
...
&НаКлиенте
Процедура ПередЗаписьюДДСЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Нет Тогда 
		ДополнительныеПараметры.ВыполняетсяЗапись = Ложь;
	Иначе     
		ДополнительныеПараметры.ВыполняетсяЗапись = Истина; 
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	Для Каждого СтрокаТаблицы Из Объект.Проводки Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.КорСчетУслуг) Тогда
			...
			СчетУслуг = БюджетныйНаСервере.ВернутьРеквизиты(СтрокаТаблицы.КорСчетУслуг, "Ссылка,Наименование,Код,Родитель,Вид,Забалансовый,Валютный,Количественный,Крахмал,УчетПоПодразделениям");
		...
		Если ЗначениеЗаполнено(СтрокаТаблицы.КорСчет791) Тогда
			...
			Счет79 = БюджетныйНаСервере.ВернутьРеквизиты(СтрокаТаблицы.КорСчет791, "Ссылка,Наименование,Код,Родитель,Вид,Забалансовый,Валютный,Количественный,Крахмал,УчетПоПодразделениям");
		...
		Если ПоЦФО Тогда
			Если ЗначениеЗаполнено(СтрокаТаблицы.ФинЦФО) Тогда
				...
				СтрокаТаблицы["ПодразделениеДоступность"] = МИН(?(Счет = Неопределено, Ложь, Счет.УчетПоПодразделениям), СтрокаТаблицы.ПодразделениеЦФОДоступность);
			Иначе
				СтрокаТаблицы.ПодразделениеЦФОДоступность = Объект.Предприятие.УчетПоПодразделениям;
				СтрокаТаблицы["ПодразделениеДоступность"] = МИН(?(Счет = Неопределено, Ложь, Счет.УчетПоПодразделениям), СтрокаТаблицы.ПодразделениеЦФОДоступность);
			КонецЕсли;
		Иначе
			СтрокаТаблицы["ПодразделениеДоступность"] = МИН(?(Счет = Неопределено, Ложь, Счет.УчетПоПодразделениям), Объект.Предприятие.УчетПоПодразделениям);
		КонецЕсли;
...
Процедура КонтрагентПриИзменении(Элемент)
	...
	Если ТипЗнч(Объект.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
		...
		Если Изменять ИЛИ ЭтоНовыйЧистый Тогда
			РеквизитыДоговора = сабОперОбщегоНазначения.ПолучитьДоговорКонтрагентаПоУмолчанию(Объект.Контрагент, ?(ЗначениеЗаполнено(Объект.Организация), Объект.Организация, Неопределено));
...
Процедура ОбновитьСвязиНаСервере()
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийДвиженияДС.БанковскаяВыписка") Тогда
		...
		Если ЗначениеЗаполнено(Объект.Организация) Тогда
			...
			Если Не Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройки("сабСквозныеИсточнкиПоПредприятиям") Тогда
...
&НаКлиенте
Процедура ОбратноеВНХДвижениеНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	
	ФормаСписка = ПолучитьФорму("Документ.УЧ_ДвижениеДС.ФормаВыбора", Новый Структура("РежимВыбора", Истина),Элемент);
	Если Не ФормаСписка.Элементы.Найти("ФормаСоздать") = Неопределено Тогда
		ФормаСписка.Элементы.ФормаСоздать.Видимость = Ложь;	
	КонецЕсли;
	
	ЭлементОтбора = ФормаСписка.Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = ?(ЭтоРасход, Новый ПолеКомпоновкиДанных("Приход"), Новый ПолеКомпоновкиДанных("Расход"));
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.Равно;
	ЭлементОтбора.Использование = Истина;
	ЭлементОтбора.ПравоеЗначение = Объект.СуммаДокумента;
	
	ЭлементОтбора = ФормаСписка.Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Дата");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.БольшеИлиРавно;
	ЭлементОтбора.Использование = Истина;
	ЭлементОтбора.ПравоеЗначение = НачалоДня(Объект.Дата);
	
	ЭлементОтбора = ФормаСписка.Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Дата");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно;
	ЭлементОтбора.Использование = Истина;
	ЭлементОтбора.ПравоеЗначение = КонецДня(Объект.Дата);
	
	НовыйОтборГруппа = ФормаСписка.Список.Отбор.Элементы.Добавить(Тип("ГруппаЭлементовОтбораКомпоновкиДанных"));
	НовыйОтборГруппа.ТипГруппы = ТипГруппыЭлементовОтбораКомпоновкиДанных.ГруппаИли;

	ЭлементОтбора = НовыйОтборГруппа.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("Ссылка");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно;
	ЭлементОтбора.Использование = Истина;
	ЭлементОтбора.ПравоеЗначение = ОбратноеВНХДвижение;
	
	ЭлементОтбора = НовыйОтборГруппа.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
	ЭлементОтбора.ЛевоеЗначение = Новый ПолеКомпоновкиДанных("ОбратноеВНХДвижение");
	ЭлементОтбора.ВидСравнения = ВидСравненияКомпоновкиДанных.МеньшеИлиРавно;
	ЭлементОтбора.Использование = Истина;
