...
&НаКлиенте
Процедура РеализацияДокументРеализацииПриИзменении(Элемент)
	
	ТД = Элементы.Реализация.ТекущиеДанные;
	Если ЗначениеЗаполнено(ТД.ДокументРеализации) Тогда
		Если ЗначениеЗаполнено(ТД.Счет) Тогда 
			ОписаниеОповещениеЗаполнениеСчета = Новый ОписаниеОповещения("ВыполнитьПослеОтветаЗаполнениеСчета",ЭтотОбъект);
			ПоказатьВопрос(ОписаниеОповещениеЗаполнениеСчета,"Перезаполнить счет на основании документа реализации?",РежимДиалогаВопрос.ДаНет);
		Иначе 
			ТД.Счет = ПолучитьСчетУчетаНаСервере(ТД.ДокументРеализации,ТД.Номенклатура);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ВыполнитьПослеОтветаЗаполнениеСчета(Результат,ДопПараметры) Экспорт
...
&НаСервереБезКонтекста
Функция ПолучитьСчетУчетаНаСервере(ДокументРеализации,Номенклатура)
	
	СтруктураДляПоиска = Новый Структура("Номенклатура",Номенклатура);
	МассивНайденныхСтрокПоНоменклатуре = ДокументРеализации.Товары.НайтиСтроки(СтруктураДляПоиска); 
