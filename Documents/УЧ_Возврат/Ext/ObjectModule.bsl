...
Процедура ЗаполнитьМассивСерий(МассивСерий,ТекСтрокаРеализация)
	...
	Если НайденныеСерии.Количество() > 1 Тогда //несколько серий
		Для каждого ТекНайдСерия Из НайденныеСерии Цикл
			МассивСерий.Добавить(Новый Структура("СерияНоменклатуры, Количество, СуммаОтгрузки, КоличествоКРХ", ТекНайдСерия.СерияНоменклатуры, ТекНайдСерия.Количество, 
			?(ТекСтрокаРеализация.Количество, ТекСтрокаРеализация.СуммаОтгрузки / ТекСтрокаРеализация.Количество, 0) * ТекНайдСерия.Количество,
			?(ТекСтрокаРеализация.Количество, ТекСтрокаРеализация.КоличествоКРХ / ТекСтрокаРеализация.Количество, 0) * ТекНайдСерия.Количество));
		КонецЦикла;	
	Иначе	
		МассивСерий.Добавить(Новый Структура("СерияНоменклатуры, Количество, СуммаОтгрузки, КоличествоКРХ", ТекСтрокаРеализация.СерияНоменклатуры, ТекСтрокаРеализация.Количество, ТекСтрокаРеализация.СуммаОтгрузки, ТекСтрокаРеализация.КоличествоКРХ));
	КонецЕсли;
	...
	Для Каждого ТекСтрокаРеализация Из Товары Цикл
		...
		//КонецЕсли;  
			...
			Если ТипЗнч(ТекСтрокаРеализация.Номенклатура) = Тип("СправочникСсылка.Номенклатура") И ТекСтрокаРеализация.Номенклатура.ТипНоменклатуры = Перечисления.сабТипыНоменклатуры.Материалы Тогда
				...
				//Движение.Сумма = ?(ТекИндекс = МассивСерий.Количество(), ТекСтрокаСерии.СуммаОтгрузки + СуммаОтклонение, ТекСтрокаСерии.СуммаОтгрузки);
				Движение.Содержание = ?(ТекФлаг60_79, "", Строка(Контрагент)) + ", Кол-во:" + ТекКоличество + " Цена:" + ТекСтрокаРеализация.Цена + ", " + Комментарий;
				//Движение.Содержание = ?(ТекФлаг60_79, "", Строка(Контрагент)) + ", Кол-во:" + ТекСтрокаСерии.Количество + " Цена:" + ТекСтрокаРеализация.Цена + ", " + Комментарий;
				...
				ЗаполнитьМассивСерий(МассивСерий,ТекСтрокаРеализация);
				СуммаОтклонение = ТекСтрокаРеализация.СуммаОтгрузки;
				КоличествоКРХОтклонение = ТекСтрокаРеализация.КоличествоКРХ;
				ТекИндекс = 0;
				
				Для каждого ТекСтрокаСерии Из МассивСерий Цикл
					ТекИндекс = ТекИндекс + 1;
					//СуммаОтклонение = СуммаОтклонение - ТекСтрокаСерии.СуммаОтгрузки;
					КоличествоКРХОтклонение = КоличествоКРХОтклонение - ТекСтрокаСерии.КоличествоКРХ;
					Движение = Движения.Учетный.Добавить();
					...
					//Движение.КоличествоДт = ТекКоличество;ТекСтрокаСерии.Количество 
					Движение.Сумма = Себестоимость * ТекСтрокаСерии.Количество;
					Движение.КоличествоДт = ТекСтрокаСерии.Количество;
					Если ТекСтрокаРеализация.Счет.Крахмал Тогда
						//Движение.КоличествоКРХДт = ТекСтрокаРеализация.КоличествоКРХ;
						Движение.КоличествоКРХДт = ?(ТекИндекс = МассивСерий.Количество(), ТекСтрокаСерии.КоличествоКРХ + КоличествоКРХОтклонение, ТекСтрокаСерии.КоличествоКРХ);
					КонецЕсли;
					...
					Движение.Содержание = ?(ТекФлаг60_79, "", Строка(Контрагент)) + ", Кол-во:" + ТекСтрокаСерии.Количество + " С/с:" + Строка(Формат(Себестоимость, "ЧДЦ=4"))  + ", " + Комментарий;
					...
					УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт,3,ТекСтрокаСерии.СерияНоменклатуры);
			...
			Иначе
				...
				//Если ТекСтрокаРеализация.Счет = ПланыСчетов.Учетный.Счет43() Тогда
					...
					//	Себестоимость = 0;			
					//КонецЕсли;
				//ИначеЕсли ТекСтрокаРеализация.Счет.ПринадлежитЭлементу(ПланыСчетов.Учетный.Счет10()) Тогда
					//СчетГП = ТекСтрокаРеализация.Счет;
					...
					//КонецЕсли;
				//Иначе
					СчетГП = ?(ЗначениеЗаполнено(ТекСтрокаРеализация.Счет),ТекСтрокаРеализация.Счет,ПланыСчетов.Учетный.Счет41());
				...
				Для каждого ТекСтрокаСерии Из МассивСерий Цикл
					...
					Если ФлагВалюты И СчетГП = ПланыСчетов.Учетный.Счет41() Тогда
						//Движение.ВалютнаяСуммаДт = Себестоимость * ТекКоличество;
						Движение.ВалютнаяСуммаДт = Себестоимость * ТекСтрокаСерии.Количество; 
						Движение.ВалютаКт = Валюта;
					КонецЕсли;
					Движение.Период = Дата;
					Движение.Предприятия = Предприятие;
					...
					УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,2,СтатьяФормальнаяСебестоимость);
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.УЧ_Реализация") Тогда
		...
		Для Каждого ТекСтрокаРеализация Из ДанныеЗаполнения.Товары Цикл
			...
			Если Не ЗначениеЗаполнено(НоваяСтрока.Счет) Тогда
				...
				Запрос.УстановитьПараметр("НомерСтрокиДок", ТекСтрокаРеализация.НомерСтроки);
				Запрос.УстановитьПараметр("Регистратор", ДанныеЗаполнения);
				РезультатЗапроса = Запрос.Выполнить();
				Выборка = РезультатЗапроса.Выбрать();
				Если Выборка.Следующий() Тогда
					НоваяСтрока.Счет = Выборка.СчетКт;
				КонецЕсли;
			КонецЕсли;
		КонецЦикла;
	КонецЕсли;
