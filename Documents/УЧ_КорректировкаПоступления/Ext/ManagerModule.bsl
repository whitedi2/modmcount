Процедура ЗаполнитьПоДокументуОснованию(Объект) Экспорт
	...
	Если ТипЗнч(Объект.ДокОснование) = Тип("ДокументСсылка.УЧ_ПоступлениеТоваров") Тогда
		...
		Запрос.Текст = "ВЫБРАТЬ
		               ...
		               |	) КАК УчетПартий,
		               |	УЧ_ПоступлениеТоваров.СерииНоменклатуры.(
		               |		Ссылка КАК Ссылка,
		               |		НомерСтроки КАК НомерСтроки,
		               |		Номенклатура КАК Номенклатура,
		               |		ДатаПроизводства КАК ДатаПроизводства,
		               |		Количество КАК Количество,
		               |		ИдентификаторСтрокиРеализации КАК ИдентификаторСтрокиРеализации,
		               |		НомерСтрокиРеализации КАК НомерСтрокиРеализации,
		               |		СерияНоменклатуры КАК СерияНоменклатуры
		...
		Если Выборка.Следующий() Тогда
			...
			ЗаполнитьЗначенияСвойств(Объект, Выборка,, "УчетПартий,СерииНоменклатуры");
			...
			Пока ВыборкаТЧ.Следующий() Цикл
				...
				НоваяСтрока.СерияНоменклатуры = ВыборкаТЧ.СерияНоменклатуры;
				НоваяСтрока.СчетУчета = ВыборкаТЧ.СчетУчета;
			КонецЦикла;
			...
			ВыборкаСерииНоменклатуры = Выборка.СерииНоменклатуры.Выбрать();
			Пока ВыборкаСерииНоменклатуры.Следующий() Цикл
				НоваяСтрока = Объект.СерииНоменклатуры.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрока,ВыборкаСерииНоменклатуры); 
				НоваяСтрока.КоличествоДоИзменения = ВыборкаСерииНоменклатуры.Количество;
			КонецЦикла;
			Для каждого СтрокаТовары Из Объект.Товары Цикл
				Если Не ЗначениеЗаполнено(СтрокаТовары.СерияНоменклатуры) Тогда
					СтруктураДляПоискаСерии = Новый Структура("Номенклатура,НомерСтрокиРеализации",СтрокаТовары.Номенклатура,СтрокаТовары.НомерСтроки);
					МассивНайденныхСтрок = Объект.СерииНоменклатуры.НайтиСтроки(СтруктураДляПоискаСерии);
					Если МассивНайденныхСтрок.Количество() > 1 Тогда
						СтрокаТовары.НесколькоСерий = Истина;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;		
		КонецЕсли;
