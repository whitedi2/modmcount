...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если Не ПеренесеноИзАстор Тогда
		...
		Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
			...
			Если НайденныеСерии.Количество() > 1 Тогда //несколько серий
				Для каждого ТекНайдСерия Из НайденныеСерии Цикл
					МассивСерий.Добавить(Новый Структура("СерияНоменклатуры, Количество, Цена, Сумма", ТекНайдСерия.СерияНоменклатуры, ТекНайдСерия.Количество, 0, 0))	;
				КонецЦикла;	
			Иначе	
				МассивСерий.Добавить(Новый Структура("СерияНоменклатуры, Количество, Цена, Сумма", ТекСтрокаТабличнаяЧасть.СерияНоменклатуры, ТекСтрокаТабличнаяЧасть.Количество, 0, 0));
			КонецЕсли;
			...
			Для каждого ТекСерия Из МассивСерий Цикл
				...
				Если ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.СчетУчета) Тогда
					Если УчетПоподразделениямСчетУчета Тогда
						СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, Подразделение, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Товар, Склад, СоответствиеСчета.Получить(ТекСтрокаТабличнаяЧасть.Товар), Подразделение, ТекСерия.СерияНоменклатуры);
					Иначе	
						СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Товар, Склад, СоответствиеСчета.Получить(ТекСтрокаТабличнаяЧасть.Товар), ТекСерия.СерияНоменклатуры);
					КонецЕсли;
				Иначе	
					Если УчетПоподразделениямСчетУчета Тогда
						СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Товар, Склад, Подразделение, ТекСерия.СерияНоменклатуры);
					Иначе	
						СтруктураПоиска = Новый Структура("Номенклатура, Склад, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Товар, Склад, ТекСерия.СерияНоменклатуры);
					КонецЕсли;
				КонецЕсли;
