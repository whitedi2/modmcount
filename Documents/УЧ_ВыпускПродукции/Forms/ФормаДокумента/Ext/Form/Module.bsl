...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацииВыпускаПродукции.ПроизводствоПоСпецификации") Тогда
		Объект.ИзменениеКоличестваМатериалов = Ложь;
		Элементы.ИзменениеКоличестваМатериалов.Доступность = Ложь;
		Элементы.МатериалыКоличество.ТолькоПросмотр = Ложь;
		Элементы.МатериалыНормативноеКоличество.Видимость = Ложь;
	Иначе 
		РучнаяКорректировкаРасходаВВыпускахПоСпецификации = Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройки("РучнаяКорректировкаРасходаВВыпускахПоСпецификации");
		Элементы.ИзменениеКоличестваМатериалов.Доступность = РучнаяКорректировкаРасходаВВыпускахПоСпецификации;
		Элементы.МатериалыКоличество.ТолькоПросмотр = НЕ Объект.ИзменениеКоличестваМатериалов;
		Элементы.МатериалыНормативноеКоличество.Видимость = Объект.ИзменениеКоличестваМатериалов;
		
		УсловноеОформлениеТЧМатериалы();
	КонецЕсли;
	
	//синхронизация серий
	Если сабОбщегоНазначенияПовтИсп.ПолучитьСерийныйУчет() Тогда
		сабОперОбщегоНазначения.СинхронизацияСерийПриСозданииФормы(Объект.Материалы,Объект.СерииНоменклатуры,"Материал");
	КонецЕсли;
...
Процедура ПересчитатьКоличествоПоСпецификации(ПервоначальноеКоличество, ТекУИД)
	...
	Для Каждого СтрокаМатериала Из СтрокиМатериалов Цикл
		...
		НормативноеКоличество = ?(КоличествоДо, СтрокаМатериала.НормативноеКоличество / КоличествоДо, СтрокаМатериала.НормативноеКоличество);
		СтрокаМатериала.НормативноеКоличество = НормативноеКоличество * ПервоначальноеКоличество;	
	КонецЦикла;	
	...
	Если ЗначениеЗаполнено(ТекСпецификация) Тогда
		...
		Для Каждого СоставСпецификаций Из НайденныйСоставСпецификаций Цикл
			...
			Если СоставПодчиненнойСпецификации.Количество() > 0 Тогда
				Для Каждого СтрокаПодчиненнойСпецификации Из СоставПодчиненнойСпецификации Цикл
					...
					Если СоставПодчиненнойСпецификации2.Количество() > 0 Тогда
						Для Каждого СтрокаПодчиненнойСпецификации2 Из СоставПодчиненнойСпецификации2 Цикл
								...
								Для Каждого СтрокаПодчиненнойСпецификации3 Из СоставПодчиненнойСпецификации3 Цикл
									...
									Если СоставПодчиненнойСпецификации4.Количество() > 0 Тогда
										Для Каждого СтрокаПодчиненнойСпецификации4 Из СоставПодчиненнойСпецификации4 Цикл
											...
											СтрокаТаблМатериалов.НормативноеКоличество = СтрокаПодчиненнойСпецификации4.Количество * ТекКоэф4;
									...
									Иначе
										...
										СтрокаТаблМатериалов.НормативноеКоличество = СтрокаПодчиненнойСпецификации3.Количество * ТекКоэф3;
							...
							Иначе
								...
								СтрокаТаблМатериалов.НормативноеКоличество = СтрокаПодчиненнойСпецификации2.Количество * ТекКоэф2;
					...
					Иначе
						...
						СтрокаТаблМатериалов.НормативноеКоличество = СтрокаПодчиненнойСпецификации.Количество * ТекКоэф1;
			...
			Иначе
				...
				СтрокаТаблМатериалов.НормативноеКоличество = СоставСпецификаций.Количество * ТекКоэф;
...
Процедура ТабличнаяЧастьПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	//Синхронизация серий 
	Для Каждого ТекущиеДанные Из Объект.Материалы Цикл
		Если сабОбщегоНазначенияПовтИсп.ПолучитьСерийныйУчет() Тогда
			сабОперОбщегоНазначенияНаКлиенте.ЗаполнитьНомерИсходнойСтрокиДляСерии(ТекущиеДанные);
		КонецЕсли;
	КонецЦикла;
...
Процедура СпособВыпускаПриИзменении(Элемент)
	...
	Если Объект.ВидОперации <> ПредопределенноеЗначение("Перечисление.ВидыОперацииВыпускаПродукции.ПроизводствоПоСпецификации") Тогда
		Объект.ИзменениеКоличестваМатериалов = Ложь;
		ИзменениеКоличестваМатериаловПриИзменении("");
		Элементы.ИзменениеКоличестваМатериалов.Доступность = Ложь;
		Элементы.МатериалыКоличество.ТолькоПросмотр = Ложь;
		Элементы.МатериалыНормативноеКоличество.Видимость = Ложь;
	КонецЕсли;
...
Процедура УстановитьВидимостьЭлементов()
	...
	Если (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацииВыпускаПродукции.ПроизводствоПоСпецификации")) 
		ИЛИ (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацииВыпускаПродукции.Комплектация"))
		ИЛИ (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацииВыпускаПродукции.Разукомплектация")) Тогда
		//Элементы.МатериалыСклад.Видимость = Ложь;
		//Элементы.МатериалыСчетСписания.Видимость = Ложь;
		...
		Если (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацииВыпускаПродукции.Комплектация"))
		ИЛИ (Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацииВыпускаПродукции.Разукомплектация")) Тогда
			Элементы.ТабличнаяЧастьСчетУчета.Видимость = Истина;			
		КонецЕсли;
		Элементы.Группа4.Видимость = Не СкрытьРесурсы;  
		Элементы.Группа13.Видимость = Объект.ПередачаВПереработку;
		Элементы.ТабличнаяЧастьСкрытьРесурсы.Пометка = СкрытьРесурсы;
...
Процедура ПередЗаписьюНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	//Синхронизация серий
	Если сабОбщегоНазначенияПовтИсп.ПолучитьСерийныйУчет() Тогда
		сабОперОбщегоНазначения.ПерезаполнитьТЧСерииНоменклатурыПередЗаписью(Объект.Материалы,ТекущийОбъект.СерииНоменклатуры,,"Материал");
...
Процедура МатериалыСерияНоменклатурыНачалоВыбора(Элемент, ДанныеВыбора, ВыборДобавлением, СтандартнаяОбработка)
	...
	//Если ТекДанные.НесколькоСерий Тогда
		...
		НовыйПараметрДокумент = Новый ПараметрВыбора("ДокументПодбора","УЧ_ВыпускПродукции");
		...
		МассивПараметрыВыбора.Добавить(НовыйПараметрДокумент);
		МассивПараметрыВыбора.Добавить(НовыйПараметрКоличество);
		...
		Если ТекДанные.НесколькоСерий Тогда
			...
			ТекФорма.ДокументПодбора = "УЧ_ВыпускПродукции";
			ТекФорма.ДокументСсылка = Объект.Ссылка;
		...
		Иначе
			...
			ПараметрыОтбора = Новый Структура("Владелец", ТекДанные.Материал);
			ПараметрыФормы.Вставить("Отбор", ПараметрыОтбора);
			ПараметрыФормы.Вставить("РежимВыбора",Истина);
			ПараметрыФормы.Вставить("МножественныйВыбор",Ложь);
			ПараметрыФормы.Вставить("ДокументПодбора","УЧ_ВыпускПродукции");
			ПараметрыФормы.Вставить("КоличествоПодбора",ТекДанные.Количество); 
			
			ПараметрыФормы.Вставить("ДатаОтгрузки",Объект.Дата);
			ПараметрыФормы.Вставить("Счет",ТекДанные.СчетУчета);  
			ПараметрыФормы.Вставить("Склад",?(ЗначениеЗаполнено(ТекДанные.Склад), ТекДанные.Склад, ?(ЗначениеЗаполнено(Объект.СкладМатериалов), Объект.СкладМатериалов, Объект.Склад)));
			ПараметрыФормы.Вставить("Предприятие",Объект.Предприятие);
			ПараметрыФормы.Вставить("ДокументСсылка", Объект.Ссылка);
			
			ОбработкаВыбора = Новый ОписаниеОповещения("ПриЗакрытииФормыВыбораСерииНоменклатуры", ЭтаФорма, "Подбор");
			
			ОткрытьФорму("Справочник.СерииНоменклатуры.Форма.ФормаВыбора",ПараметрыФормы,
...
&НаКлиенте
Процедура ПриЗакрытииФормыВыбораСерииНоменклатуры(Значение, ДопПараметры) Экспорт
    
    Если Значение = Неопределено Тогда
        Возврат;
    КонецЕсли;
    
    Если ЭтаФорма.ТекущийЭлемент.Имя = "Материалы" тогда
        Элементы.Материалы.ТекущиеДанные.СерияНоменклатуры = Значение;
    КонецЕсли;
...
Процедура ПослеЗаписиНаСервере(ТекущийОбъект, ПараметрыЗаписи)
	...
	//Синхронизация серий
	Если сабОбщегоНазначенияПовтИсп.ПолучитьСерийныйУчет() Тогда
		сабОперОбщегоНазначения.СинхронизацияСерийПослеЗаписиНаСервереФормы(Объект.Материалы,Объект.СерииНоменклатуры,"Материал");
	КонецЕсли;
...
Процедура МатериалыПередНачаломИзменения(Элемент, Отказ)
	...
	//Если Элемент.ТекущийЭлемент.Имя = "МатериалыКоличество" И Элемент.ТекущиеДанные.НесколькоСерий Тогда	
	//	ТекДанные = Элемент.ТекущиеДанные;
	//	Если ТекДанные.НесколькоСерий Тогда
	//		Отказ = Истина;
	//		МассивПараметрыВыбора = Новый Массив;
...
&НаКлиенте
Процедура МатериалыСерияНоменклатурыОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	ф=1;
КонецПроцедуры

&НаКлиенте
Процедура ИзменениеКоличества(Команда)
	// Вставить содержимое обработчика.
КонецПроцедуры

&НаКлиенте
Процедура ИзменениеКоличестваМатериаловПриИзменении(Элемент)
	Элементы.МатериалыКоличество.ТолькоПросмотр = НЕ Объект.ИзменениеКоличестваМатериалов;
	Элементы.МатериалыНормативноеКоличество.Видимость = Объект.ИзменениеКоличестваМатериалов;
...
Процедура УсловноеОформлениеТЧМатериалы()
	
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацииВыпускаПродукции.ПроизводствоПоСпецификации") Тогда
		
		УО = ЭтаФорма.УсловноеОформление;
		Фл = Ложь;
		Для каждого ТекЭл Из УО.Элементы Цикл
			Если ТекЭл.Представление = "МатериалыКоличество" Тогда
				ЭлементУО = ТекЭл;
				Фл = Истина;
			КонецЕсли;				
		КонецЦикла;
		Если Не Фл Тогда
			...
			ЭлементУО.Представление = "МатериалыКоличество";		
		КонецЕсли;
		
		
		Оформляемоеполе = ЭлементУО.Поля.Элементы.Добавить(); 
		Оформляемоеполе.Поле = Новый ПолеКомпоновкиДанных("МатериалыКоличество");
		Оформляемоеполе.Использование = ИСТИНА;  
		
		ЭлементУсловия = ЭлементУО.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
		ЭлементУсловия.ЛевоеЗначение   = Новый ПолеКомпоновкиДанных("Объект.Материалы.Количество");
		ЭлементУсловия.ПравоеЗначение = Новый ПолеКомпоновкиДанных("Объект.Материалы.НормативноеКоличество");
...
Процедура МатериалыПриИзменении(Элемент)
	...
	//Синхронизация серий 
	Если сабОбщегоНазначенияПовтИсп.ПолучитьСерийныйУчет() Тогда
		сабОперОбщегоНазначенияНаКлиенте.СинхронизироватьНомерСтрокиРеализацииДляТЧСерии(Элемент.ТекущиеДанные,Объект.Материалы,Объект.СерииНоменклатуры,"Материал");
	КонецЕсли;	
...
Процедура ТабличнаяЧастьПриИзменении(Элемент)
	...
	Для Каждого ТекущиеДанные Из Объект.Материалы Цикл
		Если сабОбщегоНазначенияПовтИсп.ПолучитьСерийныйУчет() Тогда
			сабОперОбщегоНазначенияНаКлиенте.СинхронизироватьНомерСтрокиРеализацииДляТЧСерии(ТекущиеДанные,Объект.Материалы,Объект.СерииНоменклатуры,"Материал");
		КонецЕсли;	
	КонецЦикла;
...
&НаКлиенте
Процедура СкрытьРесурсы(Команда)
	СкрытьРесурсы = НЕ СкрытьРесурсы;
	Элементы.Группа4.Видимость = Не СкрытьРесурсы;
	Элементы.ТабличнаяЧастьСкрытьРесурсы.Пометка = СкрытьРесурсы;
КонецПроцедуры

&НаКлиенте
Процедура ПередачаВПереработкуПриИзменении(Элемент)
