...
Процедура СформироватьДвиженияВыпускаПоСпецификации()
	...
	Если ВыпускПоПрямымЗатратам Тогда
		ТаблицаВыпуска.Свернуть("Номенклатура, Количество, УИД, СтатьяЗатрат", "Сумма"); //д1 перенес Количество в суммируемые
	Иначе	
		Запрос = Новый Запрос;
	...
	Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
		...
		Если ВыпускПоПрямымЗатратам Тогда
			...
			Для Каждого НайденнаяСтрокаВыпуска Из НайденныеСтрокиВыпуска Цикл
				...
				Движение2.СценарийПлана = Справочники.СценарииПланирования.НайтиПоНаименованию("Факт", Истина);
