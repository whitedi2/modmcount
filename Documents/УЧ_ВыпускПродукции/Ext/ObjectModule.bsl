...
Процедура СформироватьДвиженияКупажирования()
	...
	Счет_40 = ПланыСчетов.Учетный.ВыпускПродукции;
...
Процедура СформироватьДвиженияВыпускаПоСпецификации()
	...
	ВыпускПоПрямымЗатратам = ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.ПроизводствоПоСпецификации ИЛИ Материалы.Количество();
	...
	Для Каждого ТекСтрока Из Материалы Цикл
		...
		Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Номенклатура", Истина)] = ТекНоменклатура;
		ТекСтатья = ТекСтрока.СтатьяЗатрат;
		Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = ТекСтатья;
	...
	Если ВыпускПоПрямымЗатратам Тогда
		ТаблицаВыпуска.Свернуть("Номенклатура, Количество, УИД", "Сумма"); //д1 перенес Количество в суммируемые
	Иначе	
		Запрос = Новый Запрос;
	...
	Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
		...
		Движение.СубконтоДт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Склады", Истина)] = ТекСклад;
		...
		Если ВыпускПоПрямымЗатратам Тогда
			...
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Статьи затрат", Истина)] = Неопределено;
		Иначе
		...
		Если ВыпускПоПрямымЗатратам Тогда
			НайденныеСтрокиВыпуска = ТаблицаВыпуска.НайтиСтроки(Новый Структура("Номенклатура, Количество, УИД", ТекСтрокаТабличнаяЧасть.Номенклатура, ТекСтрокаТабличнаяЧасть.Количество, ТекСтрокаТабличнаяЧасть.УИД));
			
			Для Каждого НайденнаяСтрокаВыпуска Из НайденныеСтрокиВыпуска Цикл
				Движение.Сумма = НайденнаяСтрокаВыпуска.Сумма;	
			КонецЦикла;	
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказНаПроизводство") Тогда
		...
		Предприятие = ДанныеЗаполнения.Предприятие;
		Склад = ?(ЗначениеЗаполнено(ДанныеЗаполнения.ДокОснование.СкладГП), ДанныеЗаполнения.ДокОснование.СкладГП, Справочники.Склады.ПолучитьСкладПоЗначениюДопРеквизита("Склад ГП", Предприятие));
