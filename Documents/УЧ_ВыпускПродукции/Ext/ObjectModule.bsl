...
Процедура СформироватьДвиженияВыпускаПоСпецификации(Отказ)
	...
	Для Каждого ТекСтрока Из Материалы Цикл
		...
		Если ЗначениеЗаполнено(СтрокиТЧ[0].Склад) И Не ЗначениеЗаполнено(СкладМатериалов) Тогда
	...
	Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
		...
		ТекСкладГП = ?(ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.Склад), ТекСтрокаТабличнаяЧасть.Склад, Склад); 
		...
		УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт,2,ТекСкладГП);
		...
		Если ВыпускПоПрямымЗатратам Тогда
			...
			Для Каждого НайденнаяСтрокаВыпуска Из НайденныеСтрокиВыпуска Цикл
				...
				УЧ_Сервер.УстановитьСубконто(Движение2.СубконтоДт,Движение2.СчетДт,2,ТекСкладГП);
	...
	Для каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧастьДляРасчетаМинусов Цикл
		...
		Для каждого ТекСтрокаМинусов Из НайденныеСтрокиМинусов Цикл
			...
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,2,ТекСкладГП);
