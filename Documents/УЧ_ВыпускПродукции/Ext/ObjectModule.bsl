...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ИначеЕсли (ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.Комплектация) ИЛИ (ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.Разукомплектация) Тогда
		СформироватьДвиженияПоКомплектации(Отказ)
	Иначе 
...
Процедура СформироватьДвиженияПоКомплектации(Отказ)
	...
	Если ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.Комплектация Тогда
	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	...
	//Если НЕ НеКонтролироватьОстатки И НЕ Пользователи.РолиДоступны("допОтключитьКонтрольОстатков",,Ложь) Тогда
...
Иначе
	...
	Для каждого ТекСтрока Из ТабличнаяЧасть Цикл		
		ВыбСклад.Добавить(ТекСтрока.Склад);
		ВыбТовары.Добавить(ТекСтрока.Номенклатура);		
		ВыбСчет.Добавить(ТекСтрока.СчетУчета);
	КонецЦикла;
	...
	|	УЧ_ВыпускПродукцииТабличнаяЧасть.Ссылка.Предприятие,
	|	УЧ_ВыпускПродукцииТабличнаяЧасть.Склад,
	|	УЧ_ВыпускПродукцииТабличнаяЧасть.Ссылка.Подразделение КАК Подразделение,
	|	УЧ_ВыпускПродукцииТабличнаяЧасть.СчетУчета
	|ПОМЕСТИТЬ ТЧДока
	|ИЗ
	|	Документ.УЧ_ВыпускПродукции.ТабличнаяЧасть КАК УЧ_ВыпускПродукцииТабличнаяЧасть
	|ГДЕ
	...
	Для Каждого ТекСтрока Из Материалы Цикл
		...
		Если СтрокиТЧ.Количество() <> 0 Тогда
			...
			Если ВидОперации = Перечисления.ВидыОперацииВыпускаПродукции.Комплектация Тогда
				...
				Движение.СчетДт = СтрокиТЧ[0].СчетУчета;
				...
				Движение.КоличествоДт = ТекСтрока.Количество*ТабличнаяЧасть.Итог("Количество")/Материалы.Итог("Количество");
				...
				Движение.Содержание = "Комплектация";
				
			Иначе 
				
				Движение.СчетКт = СтрокиТЧ[0].СчетУчета;
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,1,СтрокиТЧ[0].Номенклатура);
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,2,Склад);
				
				Движение.КоличествоКт = ТекСтрока.Количество*ТабличнаяЧасть.Итог("Количество")/Материалы.Итог("Количество");
				
				СоответствиеСчета = ПоучитьСоответствияСчетовНоменклатуры();
				ТекСчетДт = ?(ЗначениеЗаполнено(ТекСтрока.СчетУчета), ТекСтрока.СчетУчета, СоответствиеСчета.Получить(ТекСтрока.Материал));
				Движение.СчетДт = ТекСчетДт;
				...
				Движение.Содержание = "Разукомплектация";
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Для каждого ТекСтрока Из НайденныеПустыеСтроки Цикл
		...
		Для каждого ТекСтрока2 Из НайденныеСтрокаНоменклатуры Цикл
			сабОбщегоНазначенияКлиентСервер.СообщитьОбОшибке(ЭтотОбъект, "Не заполнена колонка ""Материал"" в спецификации. Строка:" + Строка(ТекСтрока2.НомерСтроки), "ТабличнаяЧасть", Строка(ТекСтрока2.НомерСтроки), "Спецификация");
			Отказ = Истина;
		КонецЦикла; 
	КонецЦикла;
	
	//проверка на заполненность строк материалов
	Для каждого ТекСТрокаГП Из ТабличнаяЧасть Цикл
		НайденныеПустыеСтроки = Материалы.НайтиСтроки(Новый Структура("Материал, УИДТЧ", ТекСТрокаГП.Номенклатура, ТекСТрокаГП.УИД));
		Для каждого ТекСтрока Из НайденныеПустыеСтроки Цикл
			сабОбщегоНазначенияКлиентСервер.СообщитьОбОшибке(ЭтотОбъект, "В колонке ""Материал"" выбрана номенклатура совпадающая с Продукцией. Строка:" + Строка(ТекСтрока.НомерСтроки), "ТабличнаяЧасть", Строка(ТекСТрокаГП.НомерСтроки), "Номенклатура");
			Отказ = Истина;
		КонецЦикла;
	КонецЦикла;
...
Процедура СформироватьДвиженияВыпускаПоСпецификации(Отказ)
	...
	ПараметрыОбъекта = Новый Структура("Организация, Дата", Организация, Дата);
	СведенияОНоменклатуре = БухгалтерскийУчетПереопределяемый.ПолучитьСведенияОСпискеНоменклатуры(Материалы.ВыгрузитьКолонку("Материал"), ПараметрыОбъекта, Ложь, Ложь);
	ТекСтатьяЗатратПоУмолч = ОбщегоНазначенияКлиентСервер.ПредопределенныйЭлемент("Справочник.СтатьиЗатрат.СписаниеМатериалов");
	...
	|	УчетныйОбороты.Субконто3 КАК СерияНоменклатуры,
	...
	|			) КАК УчетныйОбороты
	|ГДЕ
	|	УчетныйОбороты.Регистратор <> &Ссылка
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ
	...
	Если Предприятие.УчетнаяПолитика.МетодикаКонтроляОтрицательныхОстатков = "Новая" Тогда	
		РасширенныйГоризонтПроверки = Истина;  
		ДатаГоризонтаПроверки = МоментКонца.Дата + 60*60*24*Предприятие.УчетнаяПолитика.ГоризонтКонтроляПоследовательности; 
	Иначе
	...
	Для Каждого ТекСтрока Из МатериалыТЧ Цикл
		...
		ТекСтатьяЗатрат = ТекСтрока.СтатьяЗатрат;
		Если Не ЗначениеЗаполнено(ТекСтатьяЗатрат) Тогда
			ТекСведения = СведенияОНоменклатуре.Получить(ТекСтрока.Материал);
			Если Не ТекСведения = Неопределено Тогда
				ТекСтатьяЗатрат = ТекСведения.СтатьяЗатрат;
				Если Не ЗначениеЗаполнено(ТекСтатьяЗатрат) Тогда
					ТекСтатьяЗатрат = ТекСтатьяЗатратПоУмолч;				
				КонецЕсли;
			КонецЕсли;
		КонецЕсли;
		...
		Для каждого ТекСерия Из МассивСерий Цикл
			...
			СтруктураПоиска = Новый Структура("Номенклатура, Склад", ТекСтрока.Материал, ТекСклад);
			
			Если ЗначениеЗаполнено(ТекСтрока.СчетУчета) Тогда
				СтруктураПоиска.Вставить("Счет", ТекСтрока.СчетУчета);			
			КонецЕсли;
			...
			Если ИспользоватьСерии И РассчитыватьСебестоимостьПоСериям Тогда
				НайденныеМатериалы = ТаблицаВыпуска.НайтиСтроки(Новый Структура("Номенклатура, СерияНоменклатуры", ТекСтрока.Материал, ТекСерия.СерияНоменклатуры));
			Иначе
			...
			УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт,2,ТекСтатьяЗатрат);
			...
			Если ВыпускПоПрямымЗатратам Тогда
				...
				СтрокаВыпуска.СтатьяЗатрат = ТекСтатьяЗатрат;
