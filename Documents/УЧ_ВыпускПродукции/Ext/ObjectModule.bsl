...
Процедура СформироватьДвиженияВыпускаПоСпецификации(Отказ)
	...
	|	ЕСТЬNULL(ОстаткиСвернуто.Количество, 0) - ТЧДока.Количество КАК КоличествоМинус,
	|	ТЧДока.Предприятие КАК Предприятие,
	|	ТЧДока.Подразделение КАК Подразделение,
	|	ОстаткиСвернуто.Склад КАК Склад,
	|	ТЧДока.Количество КАК КоличествоНужно,
	|	ЕСТЬNULL(ОстаткиСвернуто.Количество, 0) КАК КоличествоОстаток,
	...
	|			КОНЕЦ)
	|ГДЕ
	|	ЕСТЬNULL(ОстаткиСвернуто.Количество, 0) - ТЧДока.Количество < 0
		...
		Для каждого ТекСерия Из МассивСерий Цикл
			...
			Иначе	
				Если ЗначениеЗаполнено(ТекСтрока.СчетУчета) Тогда
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, СерияНоменклатуры", ТекСтрока.Материал, ТекСклад, ТекСтрока.СчетУчета, ТекСерия.СерияНоменклатуры);
				Иначе	
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, СерияНоменклатуры", ТекСтрока.Материал, ТекСклад, ТекСерия.СерияНоменклатуры);
				КонецЕсли;
			КонецЕсли;
