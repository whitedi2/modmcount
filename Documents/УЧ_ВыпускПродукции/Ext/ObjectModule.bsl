...
Процедура СформироватьДвиженияВыпускаПоСпецификации(Отказ)
	...
	|			ИНАЧЕ УчетныйОстатки.СуммаОстаток / УчетныйОстатки.КоличествоОстаток
	|		КОНЕЦ) КАК Цена,
	|	МИНИМУМ(УчетныйОстатки.Счет) КАК Счет,
	...
	|	УЧ_ВыпускПродукцииТабличнаяЧасть.Ссылка.Склад КАК Склад,
	|	УЧ_ВыпускПродукцииТабличнаяЧасть.Ссылка.Подразделение КАК Подразделение
	|ПОМЕСТИТЬ ТЧДока
	|ИЗ
	|	Документ.УЧ_ВыпускПродукции.ТабличнаяЧасть КАК УЧ_ВыпускПродукцииТабличнаяЧасть
	...
	Для Каждого ТекСтрока Из МатериалыТЧ Цикл
		...
		Движение.СчетКт = ?(ЗначениеЗаполнено(ТекСтрока.СчетУчета), ТекСтрока.СчетУчета, СоответствиеСчета.Получить(ТекСтрока.Материал));
		...
		Если ВыборкаДетальныеЗаписи.Количество() Тогда
			Если ТекСтрока.Количество = ВыборкаДетальныеЗаписи[0].Количество + КоличествоОстаток Тогда
				Движение.Сумма = ВыборкаДетальныеЗаписи[0].Сумма + СуммаОстаток;
			Иначе
				...
				Движение.Сумма = ТекЦена * ТекСтрока.Количество;
			КонецЕсли;
		...
		ИначеЕсли СуммаОстаток Тогда
			Если ТекСтрока.Количество = КоличествоОстаток Тогда
