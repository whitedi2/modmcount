...
Процедура СформироватьДвиженияВыпускаПоСпецификации(Отказ)
	...
	ТЗ_ОстаткиНаМоментПроведения = ОстаткиНаМоментПроведения(ВыбСчет, ВыбТовары, ВыбСклад, ИспользоватьСерии, РассчитыватьСебестоимостьПоСериям, СценарийПлана);
	...
	Для Каждого ТекСтрока Из МатериалыТЧ Цикл
		...
		Для каждого ТекСерия Из МассивСерий Цикл
			...
			ВыборкаДетальныеЗаписи = ТЗ_ОстаткиНаМоментПроведения.НайтиСтроки(СтруктураПоиска);
	...
	|ВЫБРАТЬ
	|	ТЧДока.Номенклатура КАК Номенклатура,
	|	ЕСТЬNULL(ОстаткиСвернуто.КоличествоОстаток, 0) КАК КоличествоМинус,
	|	ТЧДока.Предприятие КАК Предприятие,
	|	ТЧДока.Подразделение КАК Подразделение,
	|	ОстаткиСвернуто.Склад КАК Склад,
	|	ТЧДока.Количество КАК КоличествоНужно,
	...
	Запрос.УстановитьПараметр("ВыбСклады", ВыбСклад);
	Запрос.УстановитьПараметр("ВыбСчет", ВыбСчет);
	Запрос.УстановитьПараметр("ПозицияДокумента", ?(ЗначениеЗаполнено(Ссылка),МоментВремени().Дата, ТекущаяДата()) + 60*60*24*31);
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.УстановитьПараметр("Счет41", ПланыСчетов.Учетный.Счет41());
	Запрос.УстановитьПараметр("Счет10", ПланыСчетов.Учетный.Счет10());
	Запрос.УстановитьПараметр("Счет43", ПланыСчетов.Учетный.Счет43());
	...
	Если НЕ НеКонтролироватьОстатки И НЕ РольДоступна("допОтключитьКонтрольОстатков") Тогда
		...
		Для каждого ТекСтрокаМинус Из РезультатКонтроль Цикл
			Отказ = Истина;
			Сообщить(?(ТекСтрокаМинус.УчетПоПодразделениям, "По Подразделению " + Строка(ТекСтрокаМинус.Подразделение) + " ", "") + "На Складе """ + Строка(ТекСтрокаМинус.Склад) + """ номенклатуры """ + Строка(ТекСтрокаМинус.Номенклатура) + """" + ?(ИспользоватьСерии, " по серии """ + Строка(ТекСтрокаМинус.СерияНоменклатуры) + """", "") + " недостаток " + Строка(-ТекСтрокаМинус.КоличествоОстаток));
		КонецЦикла; 
		Если Отказ Тогда
			ДополнительныеСвойства.Вставить("НехваткаОстатков", Истина);		
		КонецЕсли
...
Функция ОстаткиНаМоментПроведения(ВыбСчет, ВыбТовары, ВыбСклад, ИспользоватьСерии, РассчитыватьСебестоимостьПоСериям, СценарийПлана)
	...
	Запрос.УстановитьПараметр("ВыбСклады", ВыбСклад);
	Запрос.УстановитьПараметр("ВыбСчет", ВыбСчет);
	Запрос.УстановитьПараметр("ПозицияДокумента", МоментВремени());
	Запрос.УстановитьПараметр("Ссылка", Ссылка);
	Запрос.УстановитьПараметр("Счет41", ПланыСчетов.Учетный.Счет41());
	Запрос.УстановитьПараметр("Счет10", ПланыСчетов.Учетный.Счет10());
	Запрос.УстановитьПараметр("Счет43", ПланыСчетов.Учетный.Счет43());
	...
	РезультатОстаткиМП = ЗАпрос.Выполнить();
	
	Возврат РезультатОстаткиМП.Выгрузить();
