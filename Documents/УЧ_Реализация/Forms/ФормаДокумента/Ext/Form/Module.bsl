...
Функция РассчитатьЦену(Номенклатура, Контрагент, Предприятие, Подразделение, Дата, Договор)
	...
	Если ЗначениеЗаполнено(Контрагент.ТипКлиента) Тогда 
		ТипЦен = Справочники.ТипыЦенНоменклатуры.НайтиПоРеквизиту("ТипКлиента", Контрагент.ТипКлиента, , ); 
	КонецЕсли;
	
	Если ЗначениеЗаполнено(Договор.ТипЦен) И (Договор.СрокДействияТипаЦен > ТекущаяДата() ИЛИ Не ЗначениеЗаполнено(Договор.СрокДействияТипаЦен)) Тогда 
