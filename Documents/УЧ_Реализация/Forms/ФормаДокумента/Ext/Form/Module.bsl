				...
				Если Не РезультатЗапроса.Пустой() Тогда
					...
					Если Объект.Контрагент.ОрганизацияВГОВНХ Тогда
						Если ЗначениеЗаполнено(Выборка.СчетВНХТоварныхВзаиморасчетов) Тогда
							Объект.Счет = Выборка.СчетВНХТоварныхВзаиморасчетов;
						КонецЕсли;
					КонецЕсли;
				КонецЕсли; 
			Иначе 
...
Процедура СчетПриИзменении(Элемент)
	...
	СчетПриИзмененииНаСервере(Не Элемент = Неопределено);
...
Процедура СчетПриИзмененииНаСервере(Изменять)
	...
	Если Объект.Счет = УЧ_Сервер.СчетПоКоду("79.01") ИЛИ Объект.Счет = УЧ_Сервер.СчетПоКоду("79.02") Тогда
		Если Изменять Тогда
			Если ТипЗнч(Объект.Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
...
Процедура сабПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Для Каждого СтрокаТаблицы Из Объект.Товары Цикл
		СтрокаТаблицы.Всего = СтрокаТаблицы.СуммаОтгрузки + ?(Объект.СуммаВключаетНДС, 0, СтрокаТаблицы.СуммаНДС);
	КонецЦикла;
...
Процедура СуммаВключаетНДСПриИзменении(Элемент)
	Для Каждого СтрокаТаблицы Из Объект.Товары Цикл
		СтрокаТаблицы.СуммаОтгрузки = СтрокаТаблицы.Количество * СтрокаТаблицы.Цена;
		сабОбщегоНазначенияБУХКлиентСервер.РассчитатьСуммуТабЧасти(СтрокаТаблицы, 0);
		сабОбщегоНазначенияБУХКлиентСервер.РассчитатьСуммуНДСТабЧасти(СтрокаТаблицы, Объект.СуммаВключаетНДС, Ложь);
		СтрокаТаблицы.Всего = СтрокаТаблицы.СуммаОтгрузки + ?(Объект.СуммаВключаетНДС, 0, СтрокаТаблицы.СуммаНДС);
		СтрокаТаблицы.Сумма = СтрокаТаблицы.СуммаОтгрузки;
		//сабОбщегоНазначенияБУХКлиентСервер.ПриИзмененииКоличествоЦена(ЭтаФорма, "Реализация");
...
Процедура ПриИзмененииВидаОперации(Изменять)
	...
	Элементы.Возвраты.Видимость = Ложь;
	...
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРеализация.Услуги") Тогда
		...
		Элементы.Группа30.Видимость = Истина;
		Элементы.ГруппаМатериалыЗаказчика.Видимость = Ложь; 
	ИначеЕсли Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРеализация.УслугиПоПереработке") Тогда
		Элементы.ГруппаУслуги.Видимость = Истина;
		Элементы.ГруппаТовары.Видимость = Ложь;
		Элементы.Группа9.Видимость = Ложь;
		Элементы.Группа34.Видимость = Ложь;
		...
		Элементы.Группа5.Видимость = Истина;
		Элементы.Группа30.Видимость = Истина;
		Элементы.ГруппаМатериалыЗаказчика.Видимость = Истина;
	...
	Иначе //товары
		...
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРеализация.ПродажаСРеализации") Тогда
			Элементы.Возвраты.Видимость = Истина;		
		КонецЕсли;
...
Функция РассчитатьЦену(Номенклатура, Контрагент, Предприятие, Подразделение, Дата, Договор)
	...
	Если ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") И ЗначениеЗаполнено(Контрагент.ТипКлиента) Тогда 
		ТипЦен = Справочники.ТипыЦенНоменклатуры.НайтиПоРеквизиту("ТипКлиента", Контрагент.ТипКлиента, , ); 
	КонецЕсли;
	
	Если ТипЗнч(Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") и ЗначениеЗаполнено(Договор.ТипЦен) И (Договор.СрокДействияТипаЦен > ТекущаяДата() ИЛИ Не ЗначениеЗаполнено(Договор.СрокДействияТипаЦен)) Тогда 
		ТипЦен = Договор.ТипЦен;		
	КонецЕсли;
	
	Если Не ТипЗнч(Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Или Не ЗначениеЗаполнено(ТипЦен) Тогда
		Возврат 0;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ Элементы.ГруппаМатериалыЗаказчика.Видимость Тогда
...
Процедура РеализацияСерияНоменклатурыНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	НовыйПараметрДата = Новый ПараметрВыбора("ДокументСсылка",Объект.Ссылка);
...
Функция ВернутьСчетПередачи()
	Возврат ПланыСчетов.Учетный.Счет4502();
КонецФункции
