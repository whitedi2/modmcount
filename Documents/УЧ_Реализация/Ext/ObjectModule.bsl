...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРеализация.ZОтчет Тогда	
		...
		КонецЕсли;
	КонецЕсли;
	Если Не НеКонтролироватьОстатки = Истина И Не ЭтоЗОтчетИНетТовародвижений И НЕ РольДоступна("допОтключитьКонтрольОстатков") Тогда
	...
	Для Каждого ТекСтрокаРеализация Из Товары Цикл
		...
		Если ТаблицаРозничныхПродаж.Количество() Тогда
			НайденныеСтрокиРозницы = ТаблицаРозничныхПродаж.НайтиСтроки(Новый Структура("Контрагент, Договор, Касса", ТекСтрокаРеализация.Контрагент, ТекСтрокаРеализация.Договор, ТекСтрокаРеализация.Касса));
		...
		Иначе //основная продукция и товары
			...
			Если ВидОперации = Перечисления.ВидыОперацийРеализация.ПередачаНаРеализацию Тогда 
				...
				//себестоимость
				СтруктураПоиска = Новый Структура("Номенклатура, Счет, Склад", ТекСтрокаРеализация.Номенклатура, ТекСтрокаРеализация.Счет, ТекСтрокаРеализация.Склад);
				Выборка = РезультатОстатки.НайтиСтроки(СтруктураПоиска);
				Если Выборка.Количество() Тогда
					Себестоимость = ?(Выборка[0].КоличествоОстаток, Окр(Выборка[0].СуммаОстаток / Выборка[0].КоличествоОстаток, 2), 0);
				Иначе
					Себестоимость = 0;
				...
				//Движение.Сумма = РасчетСебестоимости.РассчитатьСебестоимость(Дата,Организация,ТекСтрокаРеализация.Номенклатура,);
	...
	Если ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРеализация.ZОтчет") Тогда
		...
		Для каждого ТекСтрока Из ТЧКонрагенты Цикл
			...
			Если ТаблицаРозничныхПродаж.Количество() Тогда
				НайденныеСтрокиРозницы = ТаблицаРозничныхПродаж.НайтиСтроки(Новый Структура("Контрагент, Договор, Касса", ТекСтрока.Контрагент, ТекСтрока.Договор, ТекущаяКасса));
