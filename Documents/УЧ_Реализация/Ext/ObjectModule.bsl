			...
			Для каждого ТекСерия Из МассивСерий Цикл
				...
				Если ЗначениеЗаполнено(ТекСтрокаРеализация.Счет) Тогда
					Если УчетПоподразделениямСчетУчета Тогда
						СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение, Счет, СерияНоменклатуры", ТекСтрокаРеализация.Номенклатура, ТекСтрокаРеализация.Склад, Подразделение, ТекСтрокаРеализация.Счет, ТекСерия.СерияНоменклатуры);
					Иначе	
						СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, СерияНоменклатуры", ТекСтрокаРеализация.Номенклатура, ТекСтрокаРеализация.Склад, ТекСтрокаРеализация.Счет, ТекСерия.СерияНоменклатуры);
					КонецЕсли;
				Иначе
					Если УчетПоподразделениямСчетУчета Тогда
						СтруктураПоиска = Новый Структура("Номенклатура, Склад, Подразделение, СерияНоменклатуры", ТекСтрокаРеализация.Номенклатура, ТекСтрокаРеализация.Склад, Подразделение, ТекСерия.СерияНоменклатуры);
					Иначе	
						СтруктураПоиска = Новый Структура("Номенклатура, Склад, СерияНоменклатуры", ТекСтрокаРеализация.Номенклатура, ТекСтрокаРеализация.Склад, ТекСерия.СерияНоменклатуры);
					КонецЕсли;
				КонецЕсли;
