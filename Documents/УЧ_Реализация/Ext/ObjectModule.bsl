...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	НастройкиУчПолитики = БюджетныйНаСервере.ВернутьРеквизиты(Предприятие.УчетнаяПолитика, "РаспределятьПроводкиПоДокументамТоварооборота, СчетВНХВыручка, СчетВНХСебестоимость, УчетНДС, СтатьяРазницыЦенВГОПоступления"); 
	
	РаспределятьПроводкиПоДокументамТоварооборота = НастройкиУчПолитики.РаспределятьПроводкиПоДокументамТоварооборота;
	
	ВестиУчетНДС = НастройкиУчПолитики.УчетНДС;
	...
	КонецЕсли;
	КурсСписания = 1;
	КратностьВзаиморасчетов = ?(КратностьВзаиморасчетов, КратностьВзаиморасчетов, 1);
	
	СтатьяФормальнаяСебестоимость = Справочники.СтатьиЗатрат.НайтиПоНаименованию("Формальная себестоимость", Истина);;
	СтатьяВыручка = Справочники.СтатьиЗатрат.ПолучитьСтатьюВыручки(Предприятие);
	СтатьяНДС = Справочники.СтатьиЗатрат.ПолучитьСтатьюНДС(Предприятие);
	...
	ЭтоZОтчет = ВидОперации = Перечисления.ВидыОперацийРеализация.ZОтчет;
	...
	//Если Не НеКонтролироватьОстатки = Истина И Не ЭтоЗОтчетИНетТовародвижений И НЕ Пользователи.РолиДоступны("допОтключитьКонтрольОстатков",,Ложь) И НЕ ВидОперации = Перечисления.ВидыОперацийРеализация.ОсновныеСредства Тогда 
	Если Не НеКонтролироватьОстатки = Истина И Не ЭтоЗОтчетИНетТовародвижений И НЕ РольДоступна("допОтключитьКонтрольОстатков") И НЕ ВидОперации = Перечисления.ВидыОперацийРеализация.ОсновныеСредства Тогда
		
		//Контроль остатков с учетом бронирования{
		Если НастройкаБронированиеТоваров Тогда
			МассивСообщенийКонтроля = Новый Массив;
			ЗаказСБронированием = Неопределено;
			Если ДополнительныеСвойства.Свойство("ЗаказСБронированием") Тогда
				Если ЗначениеЗаполнено(ДополнительныеСвойства.ЗаказСБронированием) Тогда
					ЗаказСБронированием = ДополнительныеСвойства.ЗаказСБронированием;	
				КонецЕсли;
			КонецЕсли;
			УЧ_Сервер.КонтрольОстатковСУчетомБронирования(ЭтотОбъект, СтруктураИмен, СтруктураСоответствий,МассивСообщенийКонтроля,ЗаказСБронированием);
	...
	ОрганизацияПокупатель = ?(ТипЗнч(Контрагент) = Тип("СправочникСсылка.Организации"), Контрагент, ?(ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") И Контрагент.ОрганизацияВГОВНХ, Контрагент.ЭтоОрганизация, Справочники.Организации.ПустаяСсылка()));
	ЭтоПродажаВнутриПредприятия = (Предприятие = ОрганизацияПокупатель.Предприятие);
	СтатьяПереоценки91 = НастройкиУчПолитики.СтатьяРазницыЦенВГОПоступления;
	...
	ЭтоВНХ = (Счет = ПланыСчетов.Учетный.Счет7901() ИЛИ Счет = ПланыСчетов.Учетный.Счет7902());
	
	ЭтоПродажаСКомиссии = ВидОперации = Перечисления.ВидыОперацийРеализация.ПродажаСРеализации ИЛИ ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРеализация.ПродажаСРеализации");
	ЭтоПередачаНаКомиссию = ВидОперации = Перечисления.ВидыОперацийРеализация.ПередачаНаРеализацию ИЛИ ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийРеализация.ПередачаНаРеализацию");
	
	ТаблицаРозничныхПродаж = ПолучитьТаблицуРозничныхПродаж();
	
	МассивТЧ = Новый Массив;
	МассивТЧ.Добавить("Товары");
	МассивТЧ.Добавить("ТоварыВозвращенные");
	
	Для Каждого ИмяТЧ Из МассивТЧ Цикл
		Для Каждого ТекСтрокаРеализация Из ЭтотОбъект[ИмяТЧ] Цикл
			
			Если ИмяТЧ = "ТоварыВозвращенные" Тогда
				ЗнакВозврат = -1;			
			КонецЕсли;
			
			Если ЭтоZОтчет И ЭтоЗОтчетИНетТовародвижений Тогда
				Продолжить;		
			КонецЕсли;
			...
			Если ТипЗнч(ТекСтрокаРеализация.Номенклатура) = Тип("СправочникСсылка.ОсновныеСредства") Тогда
				...
				Если ЭтоВНХ Тогда
					Если Счет.ВидыСубконто.Количество() > 0 Тогда
						Субконто1Заполнено = Ложь;
						Если Счет.ВидыСубконто[0].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Организации")) И 
							ЗначениеЗаполнено(Контрагент) И ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
							ОрганизацияВНХ = Контрагент.ЭтоОрганизация;
							Если ЗначениеЗаполнено(ОрганизацияВНХ) Тогда
								УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ОрганизацияВНХ);
								Субконто1Заполнено = Истина;
							КонецЕсли;
						КонецЕсли;
						Если Не Субконто1Заполнено И Счет.ВидыСубконто[0].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Контрагенты")) Тогда
				...
				Движение.Сумма = ТекСтрокаРеализация.СуммаОтгрузки * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
				...
				//себестоимость
				СтрукутраПоиска = Новый Структура("Номенклатура, Счет, Склад", ТекСтрокаРеализация.Номенклатура, ПланыСчетов.Учетный.Счет01(), ТекСтрокаРеализация.Склад);
				...
				Если Выборка.Количество() Тогда
					...
					Себестоимость = Выборка[0].СуммаОстаток;
				Иначе
					Себестоимость = 0;
				КонецЕсли;
			...
			Иначе //основная продукция и товары
				...
				Если НайденныеСерии.Количество() > 1 Тогда //несколько серий
					...
					КонецЦикла;	
				Иначе	
					МассивСерий.Добавить(Новый Структура("СерияНоменклатуры, Количество, Сумма", ТекСтрокаРеализация.СерияНоменклатуры, ТекСтрокаРеализация.Количество, 0));
				КонецЕсли;
				...
				Для каждого ТекСерия Из МассивСерий Цикл
					...
					СтруктураПоиска = Новый Структура("Номенклатура", ТекСтрокаРеализация.Номенклатура);
					
					Если ЭтоПродажаСКомиссии Тогда
						СтруктураПоиска.Вставить("Склад", Контрагент);
					Иначе
						СтруктураПоиска.Вставить("Склад", ТекСтрокаРеализация.Склад);
					КонецЕсли;
					
					Если ЗначениеЗаполнено(ТекСтрокаРеализация.Счет) Тогда
						СтруктураПоиска.Вставить("Счет", ТекСтрокаРеализация.Счет);
					КонецЕсли;
					
					//Если ЭтоПродажаСКомиссии И ЗначениеЗаполнено(ТекСтрокаРеализация.СчетПередачи) Тогда
					//	СтруктураПоиска.Вставить("Счет", ТекСтрокаРеализация.СчетПередачи);
					//КонецЕсли;
					...
					КоличествоОстаток = 0;
					СуммаОстаток = 0;
					СчетУчетаОстатка = Неопределено;
					
					Для каждого ТекСтрока Из ВыборкаДетальныеЗаписи Цикл
						
						Если НЕ ЗначениеЗаполнено(ТекСтрокаРеализация.Счет) И НЕ СчетУчетаОстатка = Неопределено И НЕ СчетУчетаОстатка = ТекСтрока.Счет Тогда
							Прервать;					
						КонецЕсли;
						
						СчетУчетаОстатка = ТекСтрока.Счет;
						
						Если ЗначениеЗаполнено(ТекСтрокаРеализация.Счет) И НЕ ТекСтрокаРеализация.Счет = ТекСтрока.Счет Тогда
							Продолжить;					
						КонецЕсли;
						...
						Если Не ЗначениеЗаполнено(ТекСтрокаРеализация.Счет) Тогда
							СчетГП = ТекСтрока.Счет; 
						КонецЕсли;
						
					КонецЦикла;
					
					ЦенаОстаток = ?(КоличествоОстаток, СуммаОстаток / КоличествоОстаток, 0); 
					Себестоимость = ?(КоличествоОстаток = ТекСерия.Количество, СуммаОстаток, ЦенаОстаток*ТекСерия.Количество)  * КурсСписания * ЗнакВозврат;
					...
					Если ЭтоПередачаНаКомиссию Тогда
						Движение.СчетДт = ТекСтрокаРеализация.СчетПередачи;
					ИначеЕсли ЭтоВНХ И ЗначениеЗаполнено(НастройкиУчПолитики.СчетВНХСебестоимость) Тогда
						Если ЭтоПродажаВнутриПредприятия Тогда
							Движение.СчетДт = ПланыСчетов.Учетный.Счет4501(); //для товаров в рамках одного предприятия на товары
							...
							КонецЕсли;
						Иначе	
							Движение.СчетДт = НастройкиУчПолитики.СчетВНХСебестоимость;
						КонецЕсли;
					ИначеЕсли ЭтоПродажаВнутриПредприятия Тогда
					...
					Если ЭтоПередачаНаКомиссию Тогда
						...
						УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт,1,ТекСтрокаРеализация.Номенклатура);
						УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт,2,Контрагент);
					...
					УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,2,?(ЭтоПродажаСКомиссии, Контрагент, ТекСтрокаРеализация.Склад));
				...
				//выручка
				Если НЕ (ЭтоВНХ И ЗначениеЗаполнено(НастройкиУчПолитики.СчетВНХСебестоимость)) Тогда
					...
					Если СчетДт.ВидыСубконто.Количество() > 0 Тогда
						Субконто1Заполнено = Ложь;
						Если СчетДт.ВидыСубконто[0].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Организации")) И 
							ЗначениеЗаполнено(ТекКонтрагент) И ТипЗнч(ТекКонтрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
							ОрганизацияВНХ = ТекКонтрагент.ЭтоОрганизация;
							Если ЗначениеЗаполнено(ОрганизацияВНХ) Тогда
								УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ОрганизацияВНХ);
								Субконто1Заполнено = Истина;
							КонецЕсли;
						КонецЕсли;
						Если Не Субконто1Заполнено И СчетДт.ВидыСубконто[0].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Контрагенты")) Тогда
					...
					Если ЭтоВНХ И ЗначениеЗаполнено(НастройкиУчПолитики.СчетВНХВыручка) Тогда//подозреваем 99 счет, пооэтому без субконто
						Движение.СчетКт = НастройкиУчПолитики.СчетВНХВыручка;
					ИначеЕсли ЭтоПродажаВнутриПредприятия Тогда
					...
					Если УчитыватьНДС Тогда
						Движение.Сумма = ?(СуммаВключаетНДС, ТекСтрокаРеализация.СуммаОтгрузки, ТекСтрокаРеализация.СуммаОтгрузки + ТекСтрокаРеализация.СуммаНДС) * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
					Иначе
						Движение.Сумма = ТекСтрокаРеализация.СуммаОтгрузки * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
					КонецЕсли;
					Движение.НомерСтрокиДок = ТекСтрокаРеализация.НомерСтроки;
					...
					Если УчитыватьНДС И ТекСтрокаРеализация.СуммаНДС Тогда
						...
						//Иначе
						Если ЭтоВНХ И ЗначениеЗаполнено(НастройкиУчПолитики.СчетВНХВыручка) Тогда//подозреваем 99 счет, пооэтому без субконто
							Движение.СчетДт = НастройкиУчПолитики.СчетВНХВыручка;
						ИначеЕсли ЭтоПродажаВнутриПредприятия Тогда
							Движение.СчетДт = СчетПереоценки;
							УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт,1,СтатьяПереоценки91);
						Иначе	
				...
				Если НачислятьВерх И ТекСтрокаРеализация.СуммаВерха Тогда
					...
					Если  ВидВерха = 0 Тогда
						...
						СуммаВерха = ТекСтрокаРеализация.СуммаВерха * ТекСтрокаРеализация.Количество * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
					Иначе
						...
						СуммаВерха = ТекСтрокаРеализация.СуммаВерха/100 * ТекСтрокаРеализация.СуммаОтгрузки * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
					КонецЕсли;
				...
				Если НачислятьВерх И ТекСтрокаРеализация.ЦенаКлиента Тогда
					...
					СуммаВерха = (ТекСтрокаРеализация.Цена - ТекСтрокаРеализация.ЦенаКлиента) * ТекСтрокаРеализация.Количество * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
			...
			Для каждого ТекСтрокаВерха Из ПрочиеНачисления Цикл
				...
				Если ТекСтрокаВерха.ВидСуммы = "Сумма на единицу" Тогда
					Движение.Сумма = ТекСтрокаВерха.Сумма * ТекСтрокаРеализация.Количество * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
				...
				ИначеЕсли ТекСтрокаВерха.ВидСуммы = "% от суммы док-та" Тогда
					Движение.Сумма = (ТекСтрокаВерха.Сумма / 100) * ТекСтрокаРеализация.Сумма * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
					Движение.Содержание = "Начислено: " + Строка(ТекСтрокаВерха.Субконто2) + " " + Строка(ТекСтрокаВерха.Сумма) + "%";
				Иначе	
					Движение.Сумма = ТекСтрокаВерха.Сумма * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
	...
	Для Каждого ТекСтрокаУслуг Из Услуги Цикл
		...
		ТекКонтрагент = ?(ЗначениеЗаполнено(ТекСтрокаУслуг.Контрагент), ТекСтрокаУслуг.Контрагент, Контрагент);
		ТекДоговор = ?(ЗначениеЗаполнено(ТекСтрокаУслуг.Договор), ТекСтрокаУслуг.Договор, Договор);
		...
		Движение.Предприятия = Предприятие;
		Движение.СценарийПлана = СценарийФакт;
		Движение.Сумма = ?(СуммаВключаетНДС, ТекСтрокаУслуг.Сумма, ТекСтрокаУслуг.Сумма + ТекСтрокаУслуг.СуммаНДС) * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
		...
		Если УчитыватьНДС И ТекСтрокаУслуг.СуммаНДС Тогда
			...
			Если Движение.СчетДт.УчетПоПодразделениям Тогда
				Движение.ПодразделениеДт = ?(ЗначениеЗаполнено(Подразделение), Подразделение, ТекСтрокаУслуг.Подразделение);
			КонецЕсли;
		...
		Для каждого ТекСтрокаВерха Из ПрочиеНачисления Цикл
			...
			Если ТекСтрокаВерха.ВидСуммы = "Сумма на единицу" Тогда
				Движение.Сумма = ТекСтрокаВерха.Сумма * ТекСтрокаУслуг.Количество * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
				Движение.Содержание = "Начислено: " + Строка(ТекСтрокаВерха.Субконто2) + Строка(ТекСтрокаВерха.Сумма) + " на ед.";
			ИначеЕсли ТекСтрокаВерха.ВидСуммы = "% от суммы док-та" Тогда
				Движение.Сумма = (ТекСтрокаВерха.Сумма / 100) * ТекСтрокаУслуг.Сумма * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
				Движение.Содержание = "Начислено: " + Строка(ТекСтрокаВерха.Субконто2) + " " + Строка(ТекСтрокаВерха.Сумма) + "%";
			Иначе	
				Движение.Сумма = ТекСтрокаВерха.Сумма * КурсВалюты * ЗнакВозврат / КратностьВзаиморасчетов;
				Движение.Содержание = "Начислено: " + Строка(ТекСтрокаВерха.Субконто2);
	...
	Для Каждого ТекСтрокаМатериалы Из МатериалыЗаказчика Цикл
		Движение = Движения.Учетный.Добавить();
		Движение.Период = Дата;
		Движение.Предприятия = Предприятие; 
		Движение.СценарийПлана = СценарийФакт;
		...
		Движение.СчетДт = Счет;
		Движение.СчетКт = ТекСтрокаМатериалы.СчетУчета;
		...
		Если ЭтоВНХ Тогда
			...
			Если Счет.ВидыСубконто.Количество() > 0 Тогда
				...
				Если Счет.ВидыСубконто[0].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Организации")) И 
					...
					Если ЗначениеЗаполнено(ОрганизацияВНХ) Тогда
						УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, ОрганизацияВНХ); 
						УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 2, ОрганизацияВНХ);
						Субконто1Заполнено = Истина;
					КонецЕсли;
		...
		УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 2, Договор);  
		УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 3, Договор);
		...
		УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, ТекСтрокаМатериалы.Номенклатура);
		//УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 2, СтатьяДохода);
		
		Движение.Содержание = "Отчет давальцу о списанных в производство материалах";
		//Движение.Сумма = ТекСтрокаМатериалы.Сумма;
	КонецЦикла;

	Если ЭтоZОтчет Тогда
		
		ТекСуммаБП = ОплатаПлатежнымиКартами.Итог("Сумма");
		Если ТекСуммаБП Тогда
			Эквайринг = ОплатаПлатежнымиКартами[0].ЭквайринговыйТерминал;
		Иначе
			Эквайринг = Неопределено;
		КонецЕсли;
		
		ТЧКонрагенты = Товары.Выгрузить();
		ТЧКонрагенты.Свернуть("Контрагент, Договор, Касса", "СуммаОтгрузки");
		
		ТЧКонрагенты2 = Услуги.Выгрузить();
		ТЧКонрагенты2.Свернуть("Контрагент, Договор", "СуммаОтгрузки");
		
		Для каждого ТекСтрока Из ТЧКонрагенты2 Цикл
			НоваяСтрока = ТЧКонрагенты.Добавить();
		...
		Для каждого ТекСтрока Из ТЧКонрагенты Цикл
			ТекущаяКасса = ?(ЗначениеЗаполнено(ТекСтрока.Касса), ТекСтрока.Касса, Касса);
			ЕстьПродажа = Ложь;
			ТекСубконто3 = Ссылка;
			
			Если ТаблицаРозничныхПродаж.Количество() Тогда
				СтруктураОтбора = Новый Структура("Контрагент, Договор", ТекСтрока.Контрагент, ТекСтрока.Договор);
				Если ЗначениеЗаполнено(ТекущаяКасса) Тогда
					СтруктураОтбора.Вставить("Касса", ТекущаяКасса);				
				КонецЕсли;
				НайденныеСтрокиРозницы = ТаблицаРозничныхПродаж.НайтиСтроки(СтруктураОтбора);
				
				Если ЗначениеЗаполнено(ТекущаяКасса) И Не НайденныеСтрокиРозницы.Количество() Тогда //страхуемся от старых документов РознПродажа без указанных касс
					СтруктураОтбора.Удалить("Касса");
					НайденныеСтрокиРозницы = ТаблицаРозничныхПродаж.НайтиСтроки(СтруктураОтбора);
				КонецЕсли;
				...
				Для каждого ТекДок Из НайденныеСтрокиРозницы Цикл
					...
					Если ТекДок.СуммаБН Тогда
						...
						УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт, 1, ?(ЗначениеЗаполнено(Эквайринг), Эквайринг.Контрагент, Неопределено));
						УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт, 2, ?(ЗначениеЗаполнено(Эквайринг), Эквайринг.ДоговорКонтрагента, Неопределено));
					Иначе
						Движение.СчетДт = ?(ЗначениеЗаполнено(ТекущаяКасса), ТекущаяКасса.СчетУчета, ПланыСчетов.Учетный.Счет5001());
						УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт, 1, СтатьяДДС);
						УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт, 2, ТекущаяКасса);
					КонецЕсли;
					...
					Если РаспределятьПроводкиПоДокументамТоварооборота И НЕ Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Документ", Истина)) = Неопределено Тогда
						УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт, 3, ТекСубконто3);
					КонецЕсли;
					//Если ТаблицаРозничныхПродаж.Колонки.Найти("ПКОФорма2") <> Неопределено И ТекДок.ПКОФорма2 > 0 Тогда
					//	Движение.Сумма = ТекДок.СуммаДокумента - ТекДок.ПКОФорма2;
					//	ДвижениеПКОФорма2 = Движения.Учетный.Добавить();  
					//	ДвижениеПКОФорма2.СценарийПлана = СценарийФакт;
					//	ДвижениеПКОФорма2.СчетКт = Счет; 
					//	ДвижениеПКОФорма2.Период = Дата;
					//	УЧ_Сервер.УстановитьСубконто(ДвижениеПКОФорма2.СубконтоКт,ДвижениеПКОФорма2.СчетКт, 1, ?(ЗначениеЗаполнено(ТекСтрока.Контрагент), ТекСтрока.Контрагент, Контрагент));
					//	УЧ_Сервер.УстановитьСубконто(ДвижениеПКОФорма2.СубконтоКт,ДвижениеПКОФорма2.СчетКт, 2, ?(ЗначениеЗаполнено(ТекСтрока.Договор), ТекСтрока.Договор, Договор));
					//	Если РаспределятьПроводкиПоДокументамТоварооборота И НЕ Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Документ", Истина)) = Неопределено Тогда
					//		УЧ_Сервер.УстановитьСубконто(ДвижениеПКОФорма2.СубконтоКт,ДвижениеПКОФорма2.СчетКт, 3, ТекСубконто3);
					//	КонецЕсли;
					//	ДвижениеПКОФорма2.СчетДт = ?(ЗначениеЗаполнено(ТекущаяКасса), ТекущаяКасса.СчетУчета, ПланыСчетов.Учетный.Счет5001());
					//	УЧ_Сервер.УстановитьСубконто(ДвижениеПКОФорма2.СубконтоДт,ДвижениеПКОФорма2.СчетДт, 1, СтатьяДДС);
					//	УЧ_Сервер.УстановитьСубконто(ДвижениеПКОФорма2.СубконтоДт,ДвижениеПКОФорма2.СчетДт, 2, ТекущаяКасса); 
					//	ДвижениеПКОФорма2.Сумма = ТекДок.ПКОФорма2;
					//	ДвижениеПКОФорма2.Предприятия = Предприятие;
					//Иначе
			...
			Если Не ЕстьПродажа Тогда
				...
				ТекСуммаБП = ТекСуммаБП - ТекСтрока.СуммаОтгрузки;
				
				Движение = Движения.Учетный.Добавить();
				Движение.СценарийПлана = СценарийФакт;
				Если ТекСуммаБП >= 0 Тогда
					ТекСуммаБП = ТекСуммаБП - ТекСтрока.СуммаОтгрузки;
					Движение.СчетДт = ПланыСчетов.Учетный.НайтиПоКоду("57.03");
					УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт, 1, ?(ЗначениеЗаполнено(Эквайринг), Эквайринг.Контрагент, Неопределено));
					УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт, 2, ?(ЗначениеЗаполнено(Эквайринг), Эквайринг.ДоговорКонтрагента, Неопределено));
				...
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт, 1, ?(ЗначениеЗаполнено(ТекСтрока.Контрагент), ТекСтрока.Контрагент, Контрагент));
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт, 2, ?(ЗначениеЗаполнено(ТекСтрока.Договор), ТекСтрока.Договор, Договор));
				Если НЕ ЭтоЗОтчетИНетТовародвижений И РаспределятьПроводкиПоДокументамТоварооборота И НЕ Движение.СчетКт.ВидыСубконто.Найти(ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Документ", Истина)) = Неопределено Тогда
	...
	Если НЕ ДополнительныеСвойства.Свойство("НеИзменятьСтатусЗаказа") И ЗначениеЗаполнено(ДокОснование) И ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ЗаказКлиента") И Не ДокОснование.Статус = Перечисления.СтатусыЗаказовКлиентов.Отгружен Тогда		
		...
		Если ВозможноЧастичноеИсполнениеЗаказов Тогда
			...
			ТекДата = ?(ДокОснование.ДатаПоступления = Дата('00010101'), ДокОснование.Дата, ?(ДокОснование.ТранзитныйГруз, ДокОснование.ДатаТранзитнойОтгрузки, ДокОснование.ДатаПоступления));
	...
	Иначе
		Если Не Отказ И НастройкаБронированиеТоваров Тогда
			ЗаказСБронированием = Неопределено;
			Если ДополнительныеСвойства.Свойство("ЗаказСБронированием") Тогда
				Если ЗначениеЗаполнено(ДополнительныеСвойства.ЗаказСБронированием) Тогда
					ЗаказСБронированием = ДополнительныеСвойства.ЗаказСБронированием;	
				КонецЕсли;
			КонецЕсли;
			УЧ_Сервер.ЗачетБронированиеДляЗаказаОснования(Ссылка,ЗаказСБронированием);
		КонецЕсли;	 
	КонецЕсли;
	
	//распределяем проводки по долям! в случае наличия графика в договоре
	сабОперОбщегоНазначения.РаспределитьСуммыПоГрафикуДоговора(Движения, ЭтотОбъект);
	
	//распределяем по счетам
...
Функция ПолучитьТаблицуРозничныхПродаж()
	...
	Если НЕ ВидОперации = Перечисления.ВидыОперацийРеализация.ZОтчет Тогда
		Возврат Новый ТаблицаЗначений;	
	КонецЕсли;
	
	МассивКассZОтчета = Новый Массив;
	МассивКассZОтчета.Добавить(Ссылка.Касса);
	
	Для Каждого ТекСтрокаТовары Из Ссылка.Товары Цикл
		
		Если МассивКассZОтчета.Найти(ТекСтрокаТовары.Касса) = Неопределено Тогда
			МассивКассZОтчета.Добавить(ТекСтрокаТовары.Касса);
		КонецЕсли;
	...
	//	Если ОбщегоНазначения.ЕстьРеквизитОбъекта("СуммаДокументаФорма2", Метаданные.Документы.РозничнаяПродажа) Тогда
	...
	//               |	СУММА(ВЫБОР
	//               |			КОГДА РозничнаяПродажа.ОплатаФорма2
	//               |					И РозничнаяПродажа.СуммаДокументаФорма2 <> 0
	//               |				ТОГДА РозничнаяПродажа.СуммаДокументаФорма2
	//               |			ИНАЧЕ РозничнаяПродажа.СуммаДокумента
	...
	//               |		КОГДА РозничнаяПродажа.ОплатаФорма2
	//               |				И РозничнаяПродажа.СуммаДокументаФорма2 <> 0
	//               |			ТОГДА РозничнаяПродажа.СуммаДокументаФорма2 - ЕСТЬNULL(Возвраты.СуммаДокумента, 0)
	//               |		ИНАЧЕ РозничнаяПродажа.СуммаДокумента - ЕСТЬNULL(Возвраты.СуммаДокумента, 0)
	//               |	КОНЕЦ КАК СуммаДокумента,
	...
	//               |////////////////////////////////////////////////////////////////////////////////
	//               |ВЫБРАТЬ
	//               |	ВТИтоговая.Ссылка КАК Ссылка,
	//               |	ВТИтоговая.Контрагент КАК Контрагент,
	//               |	ВТИтоговая.Договор КАК Договор,
	//               |	ВТИтоговая.Касса КАК Касса,
	//               |	ВТИтоговая.СуммаДокумента КАК СуммаДокумента,
	//               |	ВТИтоговая.ДокументУУ КАК ДокументУУ,
	//               |	ВТИтоговая.СуммаБН КАК СуммаБН,
	//               |	ВЫБОР
	//               |		КОГДА ВТИтоговая.Ссылка.ОплатаФорма2
	//               |				И ВТИтоговая.СуммаБН > 0
	//               |				И ВТИтоговая.СуммаДокумента > ВТИтоговая.СуммаБН
	//               |			ТОГДА ВТИтоговая.СуммаДокумента - ВТИтоговая.СуммаБН
	//               |		ИНАЧЕ 0
	//               |	КОНЕЦ КАК ПКОФорма2
	//               |ИЗ
	...
	|					сабОбработкаДокументов.ДокументУУ = &ДокументУУ)
	|			ИЛИ РозничнаяПродажа.ОтчетОРозничныхПродажах В (&ДокументУУ))
	|	И РозничнаяПродажа.ВидОперации = &ВидОперацииВозврат И РозничнаяПродажа.Касса В (&КассыZОтчета)
	|
	|СГРУППИРОВАТЬ ПО
	|	РозничнаяПродажа.Основание,
	|	РозничнаяПродажа.Контрагент,
	...
	|					сабОбработкаДокументов.ДокументУУ = &ДокументУУ)
	|			ИЛИ РозничнаяПродажа.ОтчетОРозничныхПродажах В (&ДокументУУ))
	|	И РозничнаяПродажа.ВидОперации = &ВидОперацииПродажа И РозничнаяПродажа.Касса В (&КассыZОтчета)
	|
	|СГРУППИРОВАТЬ ПО
	|	РозничнаяПродажа.Ссылка,
	|	РозничнаяПродажа.Контрагент,
	...
	Запрос.УстановитьПараметр("КассыZОтчета", МассивКассZОтчета);
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Если ВидОперации = Перечисления.ВидыОперацийРеализация.ПередачаНаРеализацию ИЛИ ВидОперации = Перечисления.ВидыОперацийРеализация.ПродажаСРеализации Тогда
		Для каждого ТекСтрока Из Товары Цикл
			Если Не ЗначениеЗаполнено(ТекСтрока.СчетПередачи) Тогда
				ТекСтрока.СчетПередачи = ПланыСчетов.Учетный.Счет4502();		
			КонецЕсли;
		КонецЦикла;		
	КонецЕсли;
	...
	СуммаДокумента = ?(УчитыватьНДС И НЕ СуммаВключаетНДС, СуммаДокумента + СуммаНДСИтог, СуммаДокумента);
	...
	Если ЗначениеЗаполнено(ДокОснование) И ТипЗнч(ДокОснование) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		Если ДатаПоступления = Дата('00010101') Тогда
			Если ДокОснование.ТранзитныйГруз Тогда
				ДатаПоступления = ДокОснование.ДатаТранзитнойОтгрузки;
			Иначе
	...
	Для каждого ТекСтрока Из Услуги Цикл
		Если ТипЗнч(ТекСтрока.Субконто1) = Тип("СправочникСсылка.СтатьиЗатрат") Тогда
			СтатьиЗатратСтрока = СтатьиЗатратСтрока + Строка(ТекСтрока.Субконто1) + "; ";	
		ИначеЕсли ТипЗнч(ТекСтрока.Субконто2) = Тип("СправочникСсылка.СтатьиЗатрат") Тогда	
			СтатьиЗатратСтрока = СтатьиЗатратСтрока + Строка(ТекСтрока.Субконто2) + "; ";	
		КонецЕсли;
	КонецЦикла;
	
	Если Услуги.Количество() Тогда
		СтатьиЗатратСтрока = ЛЕВ(СтатьиЗатратСтрока, СтрДлина(СтатьиЗатратСтрока) - 2);	
	КонецЕсли;
...
Процедура ОбработкаУдаленияПроведения(Отказ)
	...
	Если Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройкиКонстанты("сабБронированиеТоваровВЗаказах") Тогда
		УЧ_Сервер.ЗачетБронированиеДляЗаказаОснования(Ссылка, ,Истина);
	КонецЕсли;
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
		...
		Счет 			= ПланыСчетов.Учетный.Счет7902(Предприятие);
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.УЧ_ПоступлениеТоваров") Тогда
		...
		Если ДанныеЗаполнения.СчетКонтрагента = ПланыСчетов.Учетный.Счет7902() ИЛИ ДанныеЗаполнения.СчетКонтрагента = ПланыСчетов.Учетный.Счет7901() Тогда
			Предприятие 	= ДанныеЗаполнения.Контрагент;
			Подразделение = ДанныеЗаполнения.ПодразделениеВн;
			ДокОснование 	= ДанныеЗаполнения.Ссылка;
			СуммаДокумента 	= ДанныеЗаполнения.СуммаДокумента;
			Склад 			= ДанныеЗаполнения.Склад;
			Счет 			= ПланыСчетов.Учетный.Счет7902(ДанныеЗаполнения.Предприятие);
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказКлиента") Тогда	
		...
		Иначе
			Если ДанныеЗаполнения.ТранзитныйГруз Тогда
				ДатаДокумента = ДанныеЗаполнения.ДатаТранзитнойОтгрузки;
			Иначе	
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ КонтролироватьЗаполнениеОбязательныхПолейВПоступлении = Неопределено И КонтролироватьЗаполнениеОбязательныхПолейВПоступлении  Тогда
		Если  Счет.ВидыСубконто.Количество() > 1 Тогда
			Если Не Счет.ВидыСубконто[1].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ВнутренниеДоговоры")) Тогда
