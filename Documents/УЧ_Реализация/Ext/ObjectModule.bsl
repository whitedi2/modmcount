...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	НастройкиУчПолитики = БюджетныйНаСервере.ВернутьРеквизиты(Предприятие.УчетнаяПолитика, "РаспределятьПроводкиПоДокументамТоварооборота, СчетВНХВыручка, СчетВНХСебестоимость, УчетНДС"); 
	...
	ОрганизацияПокупатель = ?(ТипЗнч(Контрагент) = Тип("СправочникСсылка.Организации"), Контрагент, ?(ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты"), Контрагент.ЭтоОрганизация, Справочники.Организации.ПустаяСсылка()));
	...
	Для Каждого ТекСтрокаРеализация Из Товары Цикл
		...
		Иначе //основная продукция и товары
			...
			Для каждого ТекСерия Из МассивСерий Цикл
				...
				Движение = Движения.Учетный.Добавить();
				Если ВидОперации = Перечисления.ВидыОперацийРеализация.ПередачаНаРеализацию Тогда
				...
				ИначеЕсли ЭтоВНХ И ЗначениеЗаполнено(НастройкиУчПолитики.СчетВНХСебестоимость) Тогда
					Если ЭтоПродажаВнутриПредприятия Тогда
						Движение.СчетДт = ПланыСчетов.Учетный.Счет45(); //для товаров в рамках одного предприятия на товары
				...
				КонецЕсли;
				Если ВидОперации = Перечисления.ВидыОперацийРеализация.ПередачаНаРеализацию Тогда
					Движение.СчетКт = ТекСтрокаРеализация.Счет;	
				Иначе
		...
		Для каждого ТекСтрока Из ТЧКонрагенты Цикл
			...
			Если ТаблицаРозничныхПродаж.Количество() Тогда
				...
				Если ЗначениеЗаполнено(ТекущаяКасса) И Не НайденныеСтрокиРозницы.Количество() Тогда //страхуемся от старых документов РознПродажа без указанных касс
					...
					НайденныеСтрокиРозницы = ТаблицаРозничныхПродаж.НайтиСтроки(СтруктураОтбора, ТекСтрока.Контрагент, ТекСтрока.Договор);
				КонецЕсли;
	...
	ОстаткиСУчетомДвиженийДокумента = УЧ_Сервер.ОстаткиСУчетомДвижений(ЭтотОбъект, СтруктураИмен, СтруктураСоответствий, ?(ЗначениеЗаполнено(Ссылка),МоментВремени().Дата, ТекущаяДата()) + 60*60*24*31);
	РезультатОстаткиСУчетомДвиженийДокумента = ОстаткиСУчетомДвиженийДокумента.РезультатОстатки;
	РезультатКонтрольСУчетомДвиженийДокумента = ОстаткиСУчетомДвиженийДокумента.РезультатКонтроль;
	...
	Если Не НеКонтролироватьОстатки = Истина И Не ЭтоЗОтчетИНетТовародвижений И НЕ РольДоступна("допОтключитьКонтрольОстатков") И НЕ ВидОперации = Перечисления.ВидыОперацийРеализация.ОсновныеСредства Тогда
		...
		Для каждого ТекСтрокаМинус Из РезультатКонтрольСУчетомДвиженийДокумента Цикл
			Отказ = Истина;
			Сообщить(?(ТекСтрокаМинус.УчетПоПодразделениям, "По Подразделению " + Строка(ТекСтрокаМинус.Подразделение) + " ", "") + "На Складе """ + Строка(ТекСтрокаМинус.Склад) + """ недостаток номенклатуры """ + Строка(ТекСтрокаМинус.Номенклатура) + " (" + Строка(ТекСтрокаМинус.Номенклатура.Код)  + ")"" в количестве " + Строка(-ТекСтрокаМинус.КоличествоОстаток));
		КонецЦикла; 
	...
	Если ВидОперации = Перечисления.ВидыОперацийРеализация.ПередачаНаРеализацию Тогда
		Для каждого ТекСтрока Из Товары Цикл
			Если Не ЗначениеЗаполнено(ТекСтрока.СчетПередачи) Тогда
				ТекСтрока.СчетПередачи = ПланыСчетов.Учетный.Счет45();		
			КонецЕсли;
		КонецЦикла;		
	КонецЕсли;
