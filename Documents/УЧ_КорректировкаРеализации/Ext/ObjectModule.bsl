Процедура ОбработкаПроведения(Отказ, РежимПроведения)
	...
	Для Каждого ТекСтрокаТабличнаяЧасть Из Товары Цикл
		...
		Если  НЕ ТекСтрокаТабличнаяЧасть.Количество - ТекСтрокаТабличнаяЧасть.КоличествоДоИзменения = 0 Тогда
			...
			Если ОтчетностьПодписана Тогда
				Движение.СчетДт = ПланыСчетов.Учетный.Счет91();
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 1, СтатьяПрочихДоходовИРасходов);
			Иначе	
		...
		Если УчитыватьНДС И (ТекСтрокаТабличнаяЧасть.СуммаНДС ИЛИ ТекСтрокаТабличнаяЧасть.СуммаНДСДоИзменения) Тогда
			...
			Если ОтчетностьПодписана Тогда
				...
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, СтатьяПрочихДоходовИРасходов);
			Иначе
