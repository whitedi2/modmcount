...
Функция ПечатьЗаказ(МассивОбъектов) Экспорт
	...
	Запрос = Новый Запрос(
		...
		|	ЗаказНаПеремещениеТабличнаяЧасть.КоличествоВУпаковке КАК КоличествоВУпаковке
		|ИЗ
	...
	Если НЕ ВыводитьУпаковки Тогда
		...
		ОбластьКолонкаТоваров.ШиринаКолонки = ОбластьКолонкаТоваров.ШиринаКолонки + Макет.Область("КолонкаУпаковок").ШиринаКолонки; 		
	...
	Пока ВыборкаПоДокументам.Следующий() Цикл
		...
		Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл   
				...
				Если ПолучитьВесовой(ВыборкаПоСтрокамТЧ.Номенклатура) тогда
					ОбластьСтрокаТаблицыНачало 			= Макет.ПолучитьОбласть("СтрокаТаблицыВес|НачалоСтроки");
					ОбластьСтрокаТаблицыКолонкаКодов 	= Макет.ПолучитьОбласть("СтрокаТаблицыВес|КолонкаКодов");
					ОбластьСтрокаТаблицыКолонкаУпаковок = Макет.ПолучитьОбласть("СтрокаТаблицыВес|КолонкаУпаковок");
					ОбластьСтрокаТаблицыКолонкаТоваров 	= Макет.ПолучитьОбласть("СтрокаТаблицыВес|КолонкаТоваров");
					ОбластьСтрокаТаблицыКонец 			= Макет.ПолучитьОбласть("СтрокаТаблицыВес|КонецСтроки");
				иначе
					ОбластьСтрокаТаблицыНачало 			= Макет.ПолучитьОбласть("СтрокаТаблицы|НачалоСтроки");
					ОбластьСтрокаТаблицыКолонкаКодов 	= Макет.ПолучитьОбласть("СтрокаТаблицы|КолонкаКодов");
					ОбластьСтрокаТаблицыКолонкаУпаковок = Макет.ПолучитьОбласть("СтрокаТаблицы|КолонкаУпаковок");
				...
				Пока ВыборкаСерии.Следующий() И ОсталосьСписатьКоличество > 0 Цикл
					...
					Для Каждого ВесоваяЕдиницаИзм Из МассивВесовыхЕдИзм Цикл
						Если ВесоваяЕдиницаИзм = ВыборкаПоСтрокамТЧ.ПредставлениеЕдиницыИзмеренияУпаковки Тогда
					...
					Если ВыводитьУпаковки Тогда
						...
						Если ЭтоВесоваяЕдИзмерения Тогда
							Сбор = "Вес: " + СписатьКоличество + " " + ВыборкаПоСтрокамТЧ.ПредставлениеЕдиницыИзмеренияУпаковки;    
							ОбластьСтрокаТаблицыКолонкаУпаковок.Параметры.КоличествоУпаковок = Сбор;
						Иначе 
							...
							Если КолУпак = 0 ИЛИ КолВУп = 0 ИЛИ КолВУп = 1 Тогда
								Сбор = Строка(СписатьКоличество) + " " + ВыборкаПоСтрокамТЧ.ПредставлениеЕдиницыИзмеренияУпаковки;
							Иначе   
								Если ОстОтУпак = 0 Тогда
									Сбор = Строка(КолУпак) + " кор.";
								Иначе
									Сбор = Строка(КолУпак) + " кор." + " + " + ОстОтУпак + " " + ВыборкаПоСтрокамТЧ.ПредставлениеЕдиницыИзмеренияУпаковки; 
								КонецЕсли;	
							КонецЕсли;	 						
							ОбластьСтрокаТаблицыКолонкаУпаковок.Параметры.КоличествоУпаковок = Сбор;
						КонецЕсли;	
