...
Процедура ТабличнаяЧастьКоличествоПриИзменении(Элемент)
	...
	Если ТекДанные.КоличествоВУпаковке = 0 ИЛИ ТекДанные.КоличествоВУпаковке = 1 Тогда
		...
		ТекДанные.КоличествоУпаковокПредставление = "" + КолУпаковок + " уп.,"  +
		КолЕдиниц + " " + ПолучитьПредставлениеЕдиницыУпаковки(ТекДанные.Номенклатура,ТекДанные.ЕдиницаИзмерения);
	...
	Иначе
		...
		ТекДанные.КоличествоУпаковокПредставление = "" + КолУпаковок + " " + ТекДанные.ЕдиницаИзмерения + ", " +
		КолЕдиниц + " " + ПолучитьПредставлениеЕдиницыУпаковки(ТекДанные.Номенклатура,ТекДанные.ЕдиницаИзмерения);
    КонецЕсли;
...
Процедура сабПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда 
		...
		|ГДЕ
		|	ПеремещениеТоваров.Заказ = &Заказ
		...
		|ГДЕ
		|	УЧ_ПеремещениеТоваров.ДокОснование = &Заказ
	...
	Если АльтернативнаяФорма Тогда 
		...
		Если Объект.ТабличнаяЧасть.Количество() > 1 Тогда
			ТЗНоменклатураУпаковка = Объект.ТабличнаяЧасть.Выгрузить(,"Номенклатура,ЕдиницаИзмерения");
			ВыборкаЕдиницыУпаковки = ПолучитьПредставлениеЕдиницыУпаковки(,,Ложь,ТЗНоменклатураУпаковка); 
			Для каждого СтрокаТовары Из Объект.ТабличнаяЧасть Цикл
				ВыборкаЕдиницыУпаковки.Сбросить();
				Если ВыборкаЕдиницыУпаковки.НайтиСледующий(Новый Структура("Номенклатура,Упаковка",СтрокаТовары.Номенклатура, СтрокаТовары.ЕдиницаИзмерения)) Тогда
					СтрокаТовары.КоличествоВУпаковкеПредставление = "(" +  Строка(СтрокаТовары.КоличествоВУпаковке) + " " + ВыборкаЕдиницыУпаковки.ЕдиницаИзмерения +")";
					
					Если СтрокаТовары.КоличествоВУпаковке = 0 ИЛИ СтрокаТовары.КоличествоВУпаковке = 1 Тогда
						КолУпаковок = 0;
						КолЕдиниц = СтрокаТовары.Количество;
						СтрокаТовары.КоличествоУпаковокПредставление = "" + КолУпаковок + " уп.,"  +
						КолЕдиниц + " " + ВыборкаЕдиницыУпаковки.УпаковкаПредставление;
					Иначе
						КолУпаковок = Цел(СтрокаТовары.Количество / СтрокаТовары.КоличествоВУпаковке);
						КолЕдиниц = СтрокаТовары.Количество - (КолУпаковок * СтрокаТовары.КоличествоВУпаковке);
						СтрокаТовары.КоличествоУпаковокПредставление = "" + КолУпаковок + " " + ВыборкаЕдиницыУпаковки.УпаковкаПредставление + ", " +
						КолЕдиниц + " " + ВыборкаЕдиницыУпаковки.ЕдиницаИзмерения;
					КонецЕсли;
				КонецЕсли;
			КонецЦикла;
		Иначе
			Для каждого СтрокаТовары Из Объект.ТабличнаяЧасть Цикл
				Если ЗначениеЗаполнено(СтрокаТовары.ЕдиницаИзмерения) И ЗначениеЗаполнено(СтрокаТовары.КоличествоВУпаковке) Тогда
					ПредставлениеЕдиницыУпаковки = ПолучитьПредставлениеЕдиницыУпаковки(СтрокаТовары.Номенклатура, СтрокаТовары.ЕдиницаИзмерения);  
					СтрокаТовары.КоличествоВУпаковкеПредставление = "(" +  Строка(СтрокаТовары.КоличествоВУпаковке) + " " + ПредставлениеЕдиницыУпаковки +")";
				Иначе
					СтрокаТовары.КоличествоВУпаковкеПредставление = Строка(СтрокаТовары.КоличествоВУпаковке);
				КонецЕсли; 
				...
				Если СтрокаТовары.КоличествоВУпаковке = 0 ИЛИ СтрокаТовары.КоличествоВУпаковке = 1 Тогда
					...
					КолЕдиниц = СтрокаТовары.Количество;
					СтрокаТовары.КоличествоУпаковокПредставление = "" + КолУпаковок + " уп.,"  +
					КолЕдиниц + " " + ПолучитьПредставлениеЕдиницыУпаковки(СтрокаТовары.Номенклатура,СтрокаТовары.ЕдиницаИзмерения);
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если НЕ МеждуПредприятиями Тогда
		Объект.ПредприятиеПолучатель = Объект.Предприятие;
...
Процедура ТабличнаяЧастьНоменклатураПриИзменении(Элемент)
	...
	Если АльтернативнаяФорма Тогда 	
		Если ЗначениеЗаполнено(ТекДанные.ЕдиницаИзмерения) И ЗначениеЗаполнено(ТекДанные.КоличествоВУпаковке) Тогда
			ПредставлениеЕдиницыУпаковки = ПолучитьПредставлениеЕдиницыУпаковки(ТекДанные.Номенклатура, ТекДанные.ЕдиницаИзмерения);  
			ТекДанные.КоличествоВУпаковкеПредставление = "(" +  Строка(ТекДанные.КоличествоВУпаковке) + " " + ПредставлениеЕдиницыУпаковки +")";
		Иначе
			ТекДанные.КоличествоВУпаковкеПредставление = Строка(ТекДанные.КоличествоВУпаковке);
		КонецЕсли;
...
Процедура ВидЗаказаПриИзменении(Элемент)
	...
	Иначе
		...
		Элементы.Договор.Видимость = МеждуПредприятиями;
...
Процедура ВидОперацииПриИзмененииНаСервере(Изменять)
	...
	МеждуПредприятиями = Объект.ВидОперации = Перечисления.ВидыЗаказовНаПеремещение.ВнутреннееПеремещение;
	Элементы.ПредприятиеПолучатель.Видимость = МеждуПредприятиями;
	Элементы.ПодразделениеПолучатель.Видимость = МеждуПредприятиями;
	Элементы.ОрганизацияПолучатель.Видимость = МеждуПредприятиями;
	Элементы.Контрагент.Видимость = МеждуПредприятиями;
	Элементы.Договор.Видимость = МеждуПредприятиями;
	Элементы.ПродажаВРозницу.Видимость = МеждуПредприятиями;
	Элементы.ТабличнаяЧастьОбновитьЦену.Видимость = МеждуПредприятиями;
	Элементы.ГруппаДополнительно.Видимость = МеждуПредприятиями;
...
Процедура ОрганизацияПолучательПриИзмененииСервер()
	...
	ТекАдрес = УправлениеКонтактнойИнформациейБП.КонтактнаяИнформацияНаДату(Объект.ОрганизацияПолучатель, ВидКонтактнойИнформации).Представление;
...
Процедура ЗаполнитьОстаткиИРезервы(РасчетПоСтроке = Ложь, СтруктураДанных = Неопределено)
	...
	ТаблицаОстатков = УЧ_Сервер.ПолучитьТаблицуОстатков(ОбъектМодуль, СтруктураИмен, СтруктураСоответствий, НачалоДня(ДатаОстатков));
	...
	Запрос.УстановитьПараметр("Период", НачалоДня(ДатаОстатков));
	Запрос.УстановитьПараметр("Предприятие", Объект.Предприятие);
	...
	Если РасчетПоСтроке Тогда
		...
		Для каждого ТекСтрокаНайденная Из НайденныеСтроки Цикл
			Если НачалоДня(ТекСтрокаНайденная.ДатаЗаказа) <= НачалоДня(ДатаОстатков) И НЕ ТипЗнч(ТекСтрокаНайденная.Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
	...
	Иначе
		...
		Для каждого ТекСтрока Из Объект.ТабличнаяЧасть Цикл
			...
			Для каждого ТекСтрокаНайденная Из НайденныеСтроки Цикл
				Если НачалоДня(ТекСтрокаНайденная.Дата) <= НачалоДня(ДатаОстатков) И НЕ ТипЗнч(ТекСтрокаНайденная.Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда
	...
	Для каждого ТекСтрока Из Выборка Цикл
		Если НачалоДня(ТекСтрока.Дата) <= НачалоДня(ДатаОстатков) И НЕ ТипЗнч(ТекСтрока.Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") Тогда		
			Продолжить;	
		КонецЕсли;
	...
	Если ТипЗнч(ТЗДанныеПодбора) = Тип("ТаблицаЗначений") Тогда
		Для Каждого СтрокаТовара Из ТЗДанныеПодбора Цикл
			...
			Иначе
				...
				Если АльтернативнаяФорма Тогда
					ТекДанные.КоличествоУпаковок = Цел(?(ТекДанные.КоэффициентПересчетаУпаковок = 0,0,ТекДанные.Количество / ТекДанные.КоэффициентПересчетаУпаковок));				ТекДанные.Сумма = ТекДанные.Количество * ТекДанные.Цена; 
					Если ЗначениеЗаполнено(ТекДанные.ЕдиницаИзмерения) И ЗначениеЗаполнено(ТекДанные.КоличествоВУпаковке) Тогда
						ПредставлениеЕдиницыУпаковки = ПолучитьПредставлениеЕдиницыУпаковки(ТекДанные.Номенклатура, ТекДанные.ЕдиницаИзмерения);  
						ТекДанные.КоличествоВУпаковкеПредставление = "(" +  Строка(ТекДанные.КоличествоВУпаковке) + " " + ПредставлениеЕдиницыУпаковки +")";
					Иначе
