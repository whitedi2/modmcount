...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Объект.ПодотчетноеЛицо = сабОбщегоНазначенияКлиентСервер.ПолучитьСотрудникаПоПользователю(Объект.Автор);
	КонецЕсли;
...
Процедура СписокДокОснованиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	Если СписокДокОснование.Количество() < 2 Тогда //если одна заявка, то сразу выбираем
		...
		Если НЕ ТекЗаявка = Неопределено Тогда
			Если СписокДокОснование.Количество() = 0 Тогда
				СписокДокОснование.Добавить(ТекЗаявка);
			Иначе
				СписокДокОснование[0].Значение = ТекЗаявка;
			КонецЕсли;
...
Функция ПроверитьЗаявкуНаОплату(ЗаявкаНаОплату)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВЫБОР
	               |		КОГДА Д_ТекущаяЗадача.ТочкаМаршрута = ЗНАЧЕНИЕ(БизнесПроцесс.Согласование1.ТочкаМаршрута.Действие5)
...
Функция ПолучитьРКО()
 	...
 	Возврат Константы.СотрудникРКО.Получить();	
