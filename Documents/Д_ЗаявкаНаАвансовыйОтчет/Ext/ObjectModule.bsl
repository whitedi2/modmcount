...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.Д_ЗаявкаНаФинансирование") Тогда
		...
		ПодотчетноеЛицо = ДанныеЗаполнения.Контрагент;	
		//УИДСтрокиДокОснования = ОтобранныеСтроки[0].УИДСтроки;
		Если ДанныеЗаполнения.ТабличнаяЧасть.Количество() Тогда
			Для каждого ТекСтрока Из ДанныеЗаполнения.ТабличнаяЧасть Цикл
				...
				НоваяСтрока.ИнвПроект = ТекСтрока.ИнвПроект;
				НоваяСтрока.Статья = ТекСтрока.СтатьяДДС;
				НоваяСтрока.Сумма = ТекСтрока.Сумма;
				НоваяСтрока.Описание = ДанныеЗаполнения.НазначениеПлатежа;
			КонецЦикла; 
		Иначе
			НоваяСтрока = Затраты.Добавить();
			НоваяСтрока.Предприятие = ДанныеЗаполнения.ЦФО;
			НоваяСтрока.Подразделение = ДанныеЗаполнения.Подразделение;
			НоваяСтрока.ИнвПроект = ДанныеЗаполнения.ИнвПроект;
			НоваяСтрока.Статья = ДанныеЗаполнения.СтатьяДДС;
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	ВидСЗ = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаАвансовыйОтчет");
