...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Иначе
		Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
			...
			Если Движение.СчетДт.Количественный Тогда
				Движение.КоличествоДт = ТекСтрокаТабличнаяЧасть.Количество; //?(ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.КоличествоПоПервичнымДокументам), ТекСтрокаТабличнаяЧасть.КоличествоПоПервичнымДокументам, ТекСтрокаТабличнаяЧасть.Количество);
			КонецЕсли;
			Движение.Сумма = ?(УчитыватьНДС, ?(СуммаВключаетНДС, ТекСтрокаТабличнаяЧасть.Сумма - ТекСтрокаТабличнаяЧасть.СуммаНДС, ТекСтрокаТабличнаяЧасть.Сумма), ?(СуммаВключаетНДС, ТекСтрокаТабличнаяЧасть.Сумма, ТекСтрокаТабличнаяЧасть.Сумма + ТекСтрокаТабличнаяЧасть.СуммаНДС)) * КурсВалюты;
	...
	Для Каждого СтрокаУслуг Из Услуги Цикл
		...
		ИначеЕсли СчетКонтрагента = ПланыСчетов.Учетный.Счет6005() Тогда
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)] = Контрагент;
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.ВнеобАктивы] = Договор;
		Иначе	
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Контрагенты", Истина)] = Контрагент;
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Договоры", Истина)] = Договор;
		КонецЕсли;
