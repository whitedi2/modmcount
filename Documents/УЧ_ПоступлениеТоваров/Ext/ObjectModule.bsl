...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	НастройкиУчПолитики = БюджетныйНаСервере.ВернутьРеквизиты(Предприятие.УчетнаяПолитика, "РаспределятьПроводкиПоДокументамТоварооборота, УчетНДС, СчетВНХВыручка, СчетВНХСебестоимость"); 
		...
		Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
			...
			Для каждого ТекСерия Из МассивСерий Цикл
				...
				Если ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.СчетЗатрат) Тогда
					...
					УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт,Движение.СчетДт,1,ТекСтрокаТабличнаяЧасть.СтатьяЗатрат);
