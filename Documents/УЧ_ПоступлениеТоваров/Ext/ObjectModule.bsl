...
Функция ПоучитьСоответствияСчетовНоменклатуры()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И ТИПЗНАЧЕНИЯ(УЧ_ПоступлениеТоваровТабличнаяЧасть.Номенклатура) <> ТИП(справочник.основныесредства)
	               |
	               |СГРУППИРОВАТЬ ПО
	               ...
	               |		КОГДА УЧ_ПоступлениеТоваровТабличнаяЧасть.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Учетный.ПустаяСсылка)
	               |			ТОГДА ВЫБОР
	               |					КОГДА сабСоответствиеСчетовИСубконто.СчетУУ ЕСТЬ NULL
	               |							ИЛИ сабСоответствиеСчетовИСубконто.СчетУУ = ЗНАЧЕНИЕ(ПланСчетов.Учетный.ПустаяСсылка)
	               |						ТОГДА &Счет41
	               |					ИНАЧЕ сабСоответствиеСчетовИСубконто.СчетУУ
	               |				КОНЕЦ
	               |		ИНАЧЕ УЧ_ПоступлениеТоваровТабличнаяЧасть.СчетУчета
	               |	КОНЕЦ
	               |ИЗ
	               |	Документ.УЧ_ПоступлениеТоваров.ТабличнаяЧасть КАК УЧ_ПоступлениеТоваровТабличнаяЧасть
	               |		ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.СчетаБухгалтерскогоУчетаОС.СрезПоследних(&ДатаДокумента, ) КАК СчетаБухгалтерскогоУчетаОССрезПоследних
	               |			ЛЕВОЕ СОЕДИНЕНИЕ РегистрСведений.сабСоответствиеСчетовИСубконто КАК сабСоответствиеСчетовИСубконто
	               |			ПО СчетаБухгалтерскогоУчетаОССрезПоследних.СчетУчета = сабСоответствиеСчетовИСубконто.СчетБУ
	               |		ПО УЧ_ПоступлениеТоваровТабличнаяЧасть.Ссылка.Организация = СчетаБухгалтерскогоУчетаОССрезПоследних.Организация
	               |			И УЧ_ПоступлениеТоваровТабличнаяЧасть.Номенклатура = СчетаБухгалтерскогоУчетаОССрезПоследних.ОсновноеСредство
	               |ГДЕ
	               |	УЧ_ПоступлениеТоваровТабличнаяЧасть.Ссылка = &Ссылка
	               |	И ТИПЗНАЧЕНИЯ(УЧ_ПоступлениеТоваровТабличнаяЧасть.Номенклатура) = ТИП(справочник.основныесредства)
	               |
	...
	НоменклатураДляСчетовУчета = Запрос.Выполнить().Выгрузить().ВыгрузитьКолонку("Номенклатура");
	
	НовоеСоответствие = сабОбщегоНазначения.ПолучитьСчетУчетаНоменклатуры(НоменклатураДляСчетовУчета, Организация, Склад);
	//НовоеСоответствие = сабОбщегоНазначения.ПолучитьСчетУчетаНоменклатуры(ТабличнаяЧасть.ВыгрузитьКолонку("Номенклатура"), Организация, Склад);
...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	ЭтоВНХ = (СчетКонтрагента = ПланыСчетов.Учетный.Счет7901() ИЛИ СчетКонтрагента = ПланыСчетов.Учетный.Счет7902());
	
	НастройкиУчПолитики = БюджетныйНаСервере.ВернутьРеквизиты(Предприятие.УчетнаяПолитика, "РаспределятьПроводкиПоДокументамТоварооборота, УчетНДС, СчетВНХВыручка, СчетВНХСебестоимость, СтатьяРазницыЦенВГОПоступления"); 
	...
	ВНХТовар = (СчетКонтрагента = ПланыСчетов.Учетный.Счет7902() ИЛИ СчетКонтрагента = ПланыСчетов.Учетный.Счет7901());	
	...
	Если ПоступлениеНаЗабаланс Тогда
		...
		Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
			...
			Движение.Сумма = ТекСтрокаТабличнаяЧасть.Сумма * КурсВалюты / КратностьВзаиморасчетов;
	...
	Иначе
		...
		ВыборкаОстаткиТоварыВПути = Новый ТаблицаЗначений;
		
		Если ЭтоВНХ И ЭтоПродажаВнутриПредприятия Тогда
			Запрос = Новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ
			...
			|	ВЫБОР
			|		КОГДА НЕ УчетныйОстатки.КоличествоОстаток = 0
			|			ТОГДА УчетныйОстатки.СуммаОстаток / УчетныйОстатки.КоличествоОстаток
			|		ИНАЧЕ 0
			|	КОНЕЦ КАК Цена
			|ИЗ
			...
			Запрос.УстановитьПараметр("СчетТоварыВПути", ПланыСчетов.Учетный.Счет45());
			...
			Результат = Запрос.Выполнить();
			ВыборкаОстаткиТоварыВПути = Результат.Выгрузить();
			
		КонецЕсли;
		
		Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
						
			ТекСтрокаЦена = ТекСтрокаТабличнаяЧасть.Цена;
			ТекСтрокаСумма = ТекСтрокаТабличнаяЧасть.Сумма;
			ТекСтрокаСуммаНДС = ТекСтрокаТабличнаяЧасть.СуммаНДС;
			
			Если ВыборкаОстаткиТоварыВПути.Количество() Тогда
				ВыборкаОстаткиТоварыВПутиНайденные = ВыборкаОстаткиТоварыВПути.НайтиСтроки(Новый Структура("Номенклатура", ТекСтрокаТабличнаяЧасть.Номенклатура));
				
				Если ВыборкаОстаткиТоварыВПутиНайденные.Количество() Тогда //корректировка по с/с ТоваровВПути
					ТекСтрокаЦена = ВыборкаОстаткиТоварыВПутиНайденные[0].Цена;
					ТекСтрокаСумма = ?(ТекСтрокаТабличнаяЧасть.Количество = ВыборкаОстаткиТоварыВПутиНайденные[0].КоличествоОстаток, ВыборкаОстаткиТоварыВПутиНайденные[0].СуммаОстаток, ТекСтрокаТабличнаяЧасть.Количество * ВыборкаОстаткиТоварыВПутиНайденные[0].Цена); 
					ВыборкаОстаткиТоварыВПутиНайденные[0].КоличествоОстаток = ВыборкаОстаткиТоварыВПутиНайденные[0].КоличествоОстаток - ТекСтрокаТабличнаяЧасть.Количество;
					Если Не ВыборкаОстаткиТоварыВПутиНайденные[0].КоличествоОстаток Тогда
						ТекСтрокаСумма = ВыборкаОстаткиТоварыВПутиНайденные[0].СуммаОстаток;
					КонецЕсли;
					ВыборкаОстаткиТоварыВПутиНайденные[0].СуммаОстаток = ВыборкаОстаткиТоварыВПутиНайденные[0].СуммаОстаток - ТекСтрокаСумма;
					ТекСтрокаСуммаНДС = 0;
				КонецЕсли;
				
			КонецЕсли;
			
			ТекСклад = ?(ЗначениеЗаполнено(Склад), Склад, ТекСтрокаТабличнаяЧасть.Склад); 
			ТекМОЛ = ?(ЗначениеЗаполнено(МОЛ), МОЛ, ТекСтрокаТабличнаяЧасть.МОЛ);
			
			ТекСуммаСтрока = ?(УчитыватьНДС, ?(СуммаВключаетНДС, ТекСтрокаСумма - ТекСтрокаСуммаНДС, ТекСтрокаСумма), ТекСтрокаСумма) * КурсВалюты / КратностьВзаиморасчетов;
			...
			Для каждого ТекСерия Из МассивСерий Цикл
				...
				Если ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.СчетУчета) Тогда
					СоответствиеСчета.Вставить(ТекСтрокаТабличнаяЧасть.Номенклатура, ТекСтрокаТабличнаяЧасть.СчетУчета);				
				КонецЕсли;
				...
				// аналитика контрагент-договор на субсчетах счета 10 {
				Если СчДебет.ВидыСубконто.Количество() > 1 И СчДебет.ВидыСубконто[1].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Контрагенты")) 
				...
				Если СчДебет.ВидыСубконто.Количество() > 2 И СчДебет.ВидыСубконто[2].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ДоговорыКонтрагентов")) 
					И ТипЗнч(Договор) = Тип("СправочникСсылка.ДоговорыКонтрагентов") Тогда  
					
					УЧ_Сервер.УстановитьСубконто(Движение.СубконтоДт, Движение.СчетДт, 3, Договор);
				КонецЕсли;
				//}	
				...
				Если ЭтоВНХ И ЗначениеЗаполнено(НастройкиУчПолитики.СчетВНХСебестоимость) Тогда
					
					Если ЭтоПродажаВнутриПредприятия Тогда
						Движение.СчетКт = ПланыСчетов.Учетный.Счет45(); //для товаров в рамках одного предприятия на товары
					...
					Иначе	
						Движение.СчетКт = НастройкиУчПолитики.СчетВНХСебестоимость;
					КонецЕсли;
				...
				Иначе
					...
					Если ВНХТовар Тогда
						...
						Если СчетКонтрагента.ВидыСубконто.Количество() > 0 Тогда 
							Субконто1Заполнено = Ложь;
							
							Если СчетКонтрагента.ВидыСубконто[0].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Организации")) И 
								...
								Если ЗначениеЗаполнено(ОрганизацияВНХ) Тогда
									УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт, Движение.СчетКт, 1, ОрганизацияВНХ);
									Субконто1Заполнено = Истина;
								КонецЕсли; 
								
							КонецЕсли; 
							
							Если Не Субконто1Заполнено И СчетКонтрагента.ВидыСубконто[0].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.Контрагенты")) Тогда
				...
				//если сразу списываем на затраты
				Если ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.СчетЗатрат) Тогда
					...
					//Если Движение.СчетКт.Количественный Тогда
					//	Движение.КоличествоКт = ТекСерия.Количество; //?(ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.КоличествоПоПервичнымДокументам), ТекСтрокаТабличнаяЧасть.КоличествоПоПервичнымДокументам, ТекСтрокаТабличнаяЧасть.Количество);
					//КонецЕсли;
					
					Движение.Сумма = ТекСумма;
					Движение.Содержание = "списано в расходы";
					
				КонецЕсли;
				
			КонецЦикла;
			
			Если УчитыватьНДС И ТекСтрокаСуммаНДС Тогда
				...
				Движение.Сумма = ТекСтрокаСуммаНДС * КурсВалюты  / КратностьВзаиморасчетов;
				
				Если ФлагВалюты И Движение.СчетКт.Валютный Тогда
					Движение.ВалютнаяСуммаКт = - Движение.Сумма; 
					Движение.ВалютаКт = Валюта;
				КонецЕсли;
			...
			Для каждого ТекСтрокаВерха Из Верхи Цикл
				...
				Движение.Предприятия = Предприятие;
				Движение.СценарийПлана = СценарийФакт;
				Движение.Сумма = ТекСтрокаВерха.Сумма * ТекСтрокаТабличнаяЧасть.Количество * КурсВалюты  / КратностьВзаиморасчетов;
				Движение.НомерСтрокиДок = ТекСтрокаТабличнаяЧасть.НомерСтроки;
			...
			Для каждого ТекСтрокаМенеджер Из ВерхиМенеджера Цикл
				...
				Движение.Предприятия = Предприятие;
				Движение.СценарийПлана = СценарийФакт;
				Движение.Сумма = ТекСтрокаМенеджер.Сумма * ТекСтрокаТабличнаяЧасть.Количество * КурсВалюты / КратностьВзаиморасчетов;
				Движение.НомерСтрокиДок = ТекСтрокаТабличнаяЧасть.НомерСтроки;
			...
			//	Движение.Предприятия = Предприятие;
			//	Движение.СценарийПлана = СценарийФакт;
			//	Движение.Сумма = ТекСтрокаТабличнаяЧасть.СуммаДоставки * КурсВалюты / КратностьВзаиморасчетов;
			//	Если ФлагВалюты Тогда
		...
		Если КорректироватьСебестоимостьПриВыходеОстаткаИзМинуса Тогда
			...
			ТаблицаОстатков = УЧ_Сервер.ПолучитьТаблицуОстатков(ЭтотОбъект, СтруктураИмен, СтруктураСоответствий,,Истина,Ложь);
			РезультатОстатки = ТаблицаОстатков.РезультатОстатки;
			РезультатКонтроль = ТаблицаОстатков.РезультатКонтрольМинусов;
			...
			Для каждого ТекСтрокаТабличнаяЧасть Из РезультатКонтроль Цикл
				...
				ТекСтрокаЦена = ТекСтрокаТабличнаяЧасть.ЦенаПоступления;
				Если ВыборкаОстаткиТоварыВПути.Количество() Тогда
					ВыборкаОстаткиТоварыВПутиНайденные = ВыборкаОстаткиТоварыВПути.НайтиСтроки(Новый Структура("Номенклатура", ТекСтрокаТабличнаяЧасть.Номенклатура));
					Если ВыборкаОстаткиТоварыВПутиНайденные.Количество() Тогда //корректировка по с/с ТоваровВПути
						ТекСтрокаЦена = ВыборкаОстаткиТоварыВПутиНайденные[0].Цена;
				...
				Если НЕ Окр(ТекСтрокаТабличнаяЧасть.СуммаКонечное - ТекСтрокаЦена * ТекСтрокаТабличнаяЧасть.КоличествоКонечное, 2) ИЛИ НЕ ТекСтрокаТабличнаяЧасть.КоличествоПоступления Тогда
					Продолжить;			
				КонецЕсли;
				...
				Если Движение.СчетДт.УчетПоПодразделениям Тогда
					Движение.ПодразделениеДт = Подразделение;
				КонецЕсли;
				Движение.СчетКт = ?(ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.Счет), ТекСтрокаТабличнаяЧасть.Счет, СоответствиеСчета.Получить(ТекСтрокаТабличнаяЧасть.Номенклатура));
				Если Движение.СчетКт.УчетПоПодразделениям Тогда
					Движение.ПодразделениеКт = Подразделение;
				КонецЕсли;	
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,1,ТекСтрокаТабличнаяЧасть.Номенклатура);
				...
				Движение.Сумма = ТекСтрокаТабличнаяЧасть.СуммаКонечное - ТекСтрокаЦена * ТекСтрокаТабличнаяЧасть.КоличествоКонечное;
				Движение.Содержание = "Корректировка до с/с последней поставки " + Формат(ТекСтрокаЦена, "ЧДЦ=2") ;
			КонецЦикла; 
		КонецЕсли;
	...
	Для Каждого СтрокаУслуг Из Услуги Цикл
		...
		Если ЭтоВНХ И ЗначениеЗаполнено(НастройкиУчПолитики.СчетВНХСебестоимость) Тогда//подозреваем 99 счет, пооэтому без субконто
		...
		Движение.Сумма = ?(УчитыватьНДС, ?(СуммаВключаетНДС, СтрокаУслуг.Сумма - СтрокаУслуг.СуммаНДС, СтрокаУслуг.Сумма), СтрокаУслуг.Сумма) * КурсВалюты / КратностьВзаиморасчетов;
		...
		Если УчитыватьНДС И СтрокаУслуг.СуммаНДС Тогда
			...
			Движение.Сумма = СтрокаУслуг.СуммаНДС * КурсВалюты / КратностьВзаиморасчетов;
	...
	//регистр данных поставок
	Движения.сабДанныеПоставок.Записывать = Истина;
	МассивНоменкл = Новый Массив;
	Для каждого ТекСтрока Из ТабличнаяЧасть Цикл
		Если Не ТипЗнч(ТекСтрока.Номенклатура) = Тип("СправочникСсылка.Номенклатура") Тогда
			Продолжить;
		КонецЕсли;
		Если МассивНоменкл.Найти(ТекСтрока.Номенклатура) = Неопределено Тогда
			Движение = Движения.сабДанныеПоставок.Добавить();
			Движение.Период = Дата;
			ЗаполнитьЗначенияСвойств(Движение, ЭтотОбъект);
			ЗаполнитьЗначенияСвойств(Движение, ТекСтрока);
			//обработка НДС +++
			Движение.ЦенаБезНДС = ?(УчитыватьНДС, ?(СуммаВключаетНДС, ТекСтрока.Цена - 
			УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекСтрока.Цена,Истина,ТекСтрока.СтавкаНДС.Ставка), ТекСтрока.Цена), ТекСтрока.Цена) * КурсВалюты / КратностьВзаиморасчетов; 
			
			Движение.Цена = ?(УчитыватьНДС, ?(Не СуммаВключаетНДС, ТекСтрока.Цена + 
			УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекСтрока.Цена,Ложь,ТекСтрока.СтавкаНДС.Ставка), ТекСтрока.Цена), ТекСтрока.Цена) * КурсВалюты / КратностьВзаиморасчетов;
			
			Движение.Сумма = ?(УчитыватьНДС, ?(Не СуммаВключаетНДС, ТекСтрока.Сумма + 
			УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекСтрока.Сумма,Ложь,ТекСтрока.СтавкаНДС.Ставка), ТекСтрока.Сумма), ТекСтрока.Сумма) * КурсВалюты / КратностьВзаиморасчетов;
			//---
			Если Не ЗначениеЗаполнено(Движение.Склад) Тогда
				Движение.Склад = Склад;		
			КонецЕсли;
			МассивНоменкл.Добавить(ТекСтрока.Номенклатура);		
		КонецЕсли;
	КонецЦикла;
	
	//Заполнить субконто для ВНХ 
	//Для каждого СтрДвижение Из Движения.Учетный Цикл
	//
	//	
	//
	//КонецЦикла;
...
Процедура ОбработкаПроверкиЗаполнения(Отказ, ПроверяемыеРеквизиты)
	...
	Если НЕ КонтролироватьЗаполнениеОбязательныхПолейВПоступлении = Неопределено И КонтролироватьЗаполнениеОбязательныхПолейВПоступлении Тогда
		...
		Если  СчетКонтрагента.ВидыСубконто.Количество() > 1 Тогда
			Если Не СчетКонтрагента.ВидыСубконто[1].ВидСубконто.ТипЗначения.СодержитТип(Тип("СправочникСсылка.ВнутренниеДоговоры")) Тогда
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для каждого ТекСтрока Из ТабличнаяЧасть Цикл
		...
		КонецЕсли; 
		КраткийСостав = КраткийСостав + Строка(ТекСтрока.Номенклатура) + "(" + Формат(ТекСтрока.Количество, "ЧДЦ=0; ЧГ=") + "), ";
		
		Если ЗначениеЗаполнено(ТекСтрока.СтатьяЗатрат) Тогда
			ТекСтрока.Субконто1 = ТекСтрока.СтатьяЗатрат;	
		КонецЕсли;
		
		Если ЗначениеЗаполнено(СчетЗатрат) Тогда
			ТекСтрока.СчетЗатрат = СчетЗатрат;
			//ТекСтрока.КорПодразделение = КорПодразделение;
			ТекСтрока.Субконто1 = Субконто1;
			ТекСтрока.Субконто2 = Субконто2;
			ТекСтрока.Субконто3 = Субконто3;
		КонецЕсли;	
...
Процедура ОбработкаЗаполнения(ДанныеЗаполнения, ТекстЗаполнения, СтандартнаяОбработка)
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.УЧ_Реализация") Тогда
		...
		РеквизитКонтрагент = ДанныеЗаполнения.Контрагент;
		...
		Запрос.УстановитьПараметр("ИНН", РеквизитКонтрагент.ИНН);
		Запрос.УстановитьПараметр("КПП", РеквизитКонтрагент.КПП);
		...
		Организация = ?(ТипЗнч(ДанныеЗаполнения.Контрагент) = Тип("СправочникСсылка.Контрагенты"), ДанныеЗаполнения.Контрагент.ЭтоОрганизация, Ложь);
		
		РеквизитОрганизация = ДанныеЗаполнения.Организация;
		...
		Предприятие = СтруктураРесурсов.Предприятие;
		...
		НомерВходящегоДокумента = ДанныеЗаполнения.Ссылка.Номер;
		ДатаВходящегоДокумента = ДанныеЗаполнения.Ссылка.Дата;
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.УЧ_ПеремещениеТоваров") Тогда
		...
		Если ДанныеЗаполнения.ВидОперации = Перечисления.ВидыПеремещений.ПеремещениеНаПредприятие Тогда
			СчетКонтрагента = ПланыСчетов.Учетный.Счет7902(Предприятие);
	...
	Если ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
		...
		СчетКонтрагента 			= ДанныеЗаполнения.Договор.СчетВзаиморасчетов;
