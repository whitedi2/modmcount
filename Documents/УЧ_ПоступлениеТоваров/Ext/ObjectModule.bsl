...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Иначе
		Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
			...
			ТекСуммаСтрока = ?(УчитыватьНДС, ?(СуммаВключаетНДС, ТекСтрокаТабличнаяЧасть.Сумма - ТекСтрокаТабличнаяЧасть.СуммаНДС, ТекСтрокаТабличнаяЧасть.Сумма), ТекСтрокаТабличнаяЧасть.Сумма) * КурсВалюты;
			...
			Если УчитыватьНДС И ТекСтрокаТабличнаяЧасть.СуммаНДС Тогда
				...
				Движение.Сумма = ТекСтрокаТабличнаяЧасть.СуммаНДС * КурсВалюты;
		...
		Если КорректироватьСебестоимостьПриВыходеОстаткаИзМинуса Тогда
			...
			|							ТОГДА УЧ_ПоступлениеТоваровТабличнаяЧасть.Сумма - УЧ_ПоступлениеТоваровТабличнаяЧасть.СуммаНДС
			|						ИНАЧЕ УЧ_ПоступлениеТоваровТабличнаяЧасть.Сумма
			|					КОНЕЦ
			|			ИНАЧЕ ВЫБОР
			|					КОГДА &СуммаВключаетНДС
			...
			|		ИНАЧЕ УЧ_ПоступлениеТоваровТабличнаяЧасть.СчетУчета
			|	КОНЕЦ КАК СчетУчета,
			|	УЧ_ПоступлениеТоваровТабличнаяЧасть.СчетЗатрат КАК СчетЗатрат,
			|	УЧ_ПоступлениеТоваровТабличнаяЧасть.СерияНоменклатуры КАК СерияНоменклатуры
			|ПОМЕСТИТЬ ТЧДока
			|ИЗ
			|	Документ.УЧ_ПоступлениеТоваров.ТабличнаяЧасть КАК УЧ_ПоступлениеТоваровТабличнаяЧасть
			|ГДЕ
			...
			|		ИНАЧЕ УЧ_ПоступлениеТоваровТабличнаяЧасть.СчетУчета
			|	КОНЕЦ,
			|	УЧ_ПоступлениеТоваровТабличнаяЧасть.СчетЗатрат,
			|	УЧ_ПоступлениеТоваровТабличнаяЧасть.СерияНоменклатуры
			|;
			|
			|////////////////////////////////////////////////////////////////////////////////
			|ВЫБРАТЬ
			...
			|	ТЧДока.СчетЗатрат КАК СчетЗатрат,
			...
			|			И (ВЫБОР
			|				КОГДА НЕ ТЧДока.СчетУчета = ЗНАЧЕНИЕ(ПланСчетов.Учетный.ПустаяСсылка)
			|					ТОГДА ТЧДока.СчетУчета = Остатки.Счет
			|				ИНАЧЕ ИСТИНА
			|			КОНЕЦ)
			|			И (ВЫБОР
			...
			Для каждого ТекСтрокаТабличнаяЧасть Из РезультатКонтроль Цикл
				...
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,1,ТекСтрокаТабличнаяЧасть.Номенклатура);
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,2,ТекСтрокаТабличнаяЧасть.Склад);
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,1,ТекСтрокаТабличнаяЧасть.СерияНоменклатуры);
	...
	Для Каждого СтрокаУслуг Из Услуги Цикл
		...
		Движение.Сумма = ?(УчитыватьНДС, ?(СуммаВключаетНДС, СтрокаУслуг.Сумма - СтрокаУслуг.СуммаНДС, СтрокаУслуг.Сумма), СтрокаУслуг.Сумма) * КурсВалюты;;
		...
		Если УчитыватьНДС И СтрокаУслуг.СуммаНДС Тогда
			...
			Движение.Сумма = СтрокаУслуг.СуммаНДС * КурсВалюты;
	...
	Для каждого ТекСтрока Из ТабличнаяЧасть Цикл
		...
		Если МассивНоменкл.Найти(ТекСтрока.Номенклатура) = Неопределено Тогда
			...
			УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекСтрока.Цена,Истина,ТекСтрока.СтавкаНДС.Ставка), ТекСтрока.Цена), ТекСтрока.Цена) * КурсВалюты; 
			
			Движение.Цена = ?(УчитыватьНДС, ?(Не СуммаВключаетНДС, ТекСтрока.Цена + 
			УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекСтрока.Цена,Ложь,ТекСтрока.СтавкаНДС.Ставка), ТекСтрока.Цена), ТекСтрока.Цена) * КурсВалюты;
			
			Движение.Сумма = ?(УчитыватьНДС, ?(Не СуммаВключаетНДС, ТекСтрока.Сумма + 
			УчетНДСКлиентСервер.РассчитатьСуммуНДС(ТекСтрока.Сумма,Ложь,ТекСтрока.СтавкаНДС.Ставка), ТекСтрока.Сумма), ТекСтрока.Сумма) * КурсВалюты;
			//---
