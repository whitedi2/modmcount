...
Функция ПечатьПоступлениеТоваров(МассивОбъектов, ОбъектыПечати) Экспорт
	...
	|	ЗапросПоТоварам.СерияНоменклатуры КАК СерияНоменклатуры,
	...
	Пока Шапка.СледующийПоЗначениюПоля("Ссылка") Цикл
		...
		//Код, артикул, серии номенклатуры 
		Если СерииНоменклатурыИспользуются ИЛИ ВыводитьКоды тогда
				
			ОбластьШапкаНачало		        = Макет.ПолучитьОбласть("ШапкаСКодом|Начало");
			ОбластьШапкаКодАртикул          = Макет.ПолучитьОбласть("ШапкаСКодом|КодАртикул");
			ОбластьШапкаТовар         		= Макет.ПолучитьОбласть("ШапкаСКодом|Товар");
			ОбластьШапкаСерияНоменклатуры   = Макет.ПолучитьОбласть("ШапкаСКодом|СерияНоменклатуры");
			ОбластьШапкаДанные          	= Макет.ПолучитьОбласть("ШапкаСКодом|Данные");  
			
			ОбластьСтрокаНачало        		= Макет.ПолучитьОбласть("СтрокаСКодом|Начало");
			ОбластьСтрокаКодАртикул         = Макет.ПолучитьОбласть("СтрокаСКодом|КодАртикул");
			ОбластьСтрокаТовар         		= Макет.ПолучитьОбласть("СтрокаСКодом|Товар");
			ОбластьСтрокаСерияНоменклатуры  = Макет.ПолучитьОбласть("СтрокаСКодом|СерияНоменклатуры");
			ОбластьСтрокаДанные          	= Макет.ПолучитьОбласть("СтрокаСКодом|Данные"); 
			
			ОбластьПодвалНачало        		= Макет.ПолучитьОбласть("ПодвалСКодом|Начало");
			ОбластьПодвалКодАртикул         = Макет.ПолучитьОбласть("ПодвалСКодом|КодАртикул");
			ОбластьПодвалТовар         		= Макет.ПолучитьОбласть("ПодвалСКодом|Товар");
			ОбластьПодвалСерияНоменклатуры  = Макет.ПолучитьОбласть("ПодвалСКодом|СерияНоменклатуры");
			ОбластьПодвалДанные          	= Макет.ПолучитьОбласть("ПодвалСКодом|Данные");
			...
			Если ВыводитьКоды Тогда 			
				Если ДополнительнаяКолонкаПечатныхФормДокументов = Перечисления.ДополнительнаяКолонкаПечатныхФормДокументов.Артикул Тогда
					ОбластьШапкаКодАртикул.Параметры.ИмяКодАртикул = "Артикул";
				ИначеЕсли ДополнительнаяКолонкаПечатныхФормДокументов = Перечисления.ДополнительнаяКолонкаПечатныхФормДокументов.Код Тогда
					ОбластьШапкаКодАртикул.Параметры.ИмяКодАртикул = "Код";
				КонецЕсли;	
				ТабличныйДокумент.Присоединить(ОбластьШапкаКодАртикул);			
			КонецЕсли;
			
			ТабличныйДокумент.Присоединить(ОбластьШапкаТовар);
				
			Если СерииНоменклатурыИспользуются тогда				
				ТабличныйДокумент.Присоединить(ОбластьШапкаСерияНоменклатуры);				
			КонецЕсли;
			...
			Пока Шапка.Следующий() Цикл
				...
				ОбластьСтрокаНачало.Параметры.НомерСтроки = НомерСтроки;
				ТабличныйДокумент.Вывести(ОбластьСтрокаНачало);
				Если ВыводитьКоды тогда
					ОбластьСтрокаКодАртикул.Параметры.КодАртикул = Шапка.КодАртикул;
					ТабличныйДокумент.Присоединить(ОбластьСтрокаКодАртикул); 
				КонецЕсли;
				ОбластьСтрокаТовар.Параметры.Товар = СокрЛП(Шапка.Товар);
				ТабличныйДокумент.Присоединить(ОбластьСтрокаТовар);     
				Если СерииНоменклатурыИспользуются тогда
					ОбластьСтрокаСерияНоменклатуры.Параметры.СерияНоменклатуры = Шапка.СерияНоменклатуры; 
					ТабличныйДокумент.Присоединить(ОбластьСтрокаСерияНоменклатуры);  
				КонецЕсли;
				ОбластьСтрокаДанные.Параметры.Заполнить(Шапка);
			...
			ТабличныйДокумент.Вывести(ОбластьПодвалНачало);
			Если ВыводитьКоды тогда
				ТабличныйДокумент.Присоединить(ОбластьПодвалКодАртикул); 
			КонецЕсли;
			ТабличныйДокумент.Присоединить(ОбластьПодвалТовар);     
			Если СерииНоменклатурыИспользуются тогда
				ТабличныйДокумент.Присоединить(ОбластьПодвалСерияНоменклатуры);  
			КонецЕсли;  
			
			ОбластьПодвалДанные.Параметры.Всего = ОбщегоНазначенияБПВызовСервера.ФорматСумм(Сумма);
			ОбластьПодвалДанные.Параметры.ВсегоНДС = ОбщегоНазначенияБПВызовСервера.ФорматСумм(СуммаНДС);
			ОбластьПодвалДанные.Параметры.НДС = ?(Шапка.СуммаВключаетНДС, "В том числе НДС:", "Сумма НДС:");
