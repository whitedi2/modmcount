...
Функция ПечатьТОРГ4(МассивОбъектов) Экспорт
	...
	Запрос = Новый Запрос("ВЫБРАТЬ РАЗРЕШЕННЫЕ
		...
		|		1                                      КАК Коэффициент,
		|			Документ.Количество * Документ.Номенклатура.ВесБрутто                                      КАК МассаБрутто,
...
Процедура СформироватьТекстЗапросаТОРГ2(ТекстЗапроса)
	...
	|ГДЕ
	|	УЧ_ПоступлениеТоваровТабличнаяЧасть.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.ТипыНоменклатуры.Товар)
