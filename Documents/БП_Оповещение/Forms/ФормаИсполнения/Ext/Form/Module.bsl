...
Функция ВыполненоСервер(ПроверкаУспешно, ВставлятьКомм)
	...
	Если ТипЗнч(Документ) = Тип("ЗадачаСсылка.Задача") И ТипСообщенияИзменеикСрока И ПроверкаУспешно Тогда
...
Процедура ГиперссылкаНажатие(Элемент, СтандартнаяОбработка)
	...
	//ИначеЕсли ТипЗнч(Объект.Гиперссылка) = Тип("БизнесПроцессСсылка.УтверждениеБюджета") Тогда
	//	СтандартнаяОбработка = Ложь;
	//	
