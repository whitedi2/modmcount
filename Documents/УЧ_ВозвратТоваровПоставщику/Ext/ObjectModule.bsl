...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если Не НеКонтролироватьОстатки = Истина И НЕ РольДоступна("допОтключитьКонтрольОстатков") Тогда
	...
	Для Каждого ТекСтрокаТабличнаяЧасть Из Товары Цикл
		...
		//Доработка проведения для нескольких серий+++  
		Если сабОбщегоНазначенияПовтИсп.ПолучитьСерийныйУчет() И 
			сабОбщегоНазначенияПовтИсп.ПолучитьНаличиеСерийногоУчетаДляСчета(СчетКт) Тогда 
			КоличествоПоСтроке = ?(ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.КоличествоПоПервичнымДокументам), ТекСтрокаТабличнаяЧасть.КоличествоПоПервичнымДокументам, ТекСтрокаТабличнаяЧасть.Количество);
			СуммаПоСтроке = ?(ЗначениеЗаполнено(ТекСтрокаТабличнаяЧасть.СуммаПоПервичнымДокументам), ТекСтрокаТабличнаяЧасть.СуммаПоПервичнымДокументам, ТекСтрокаТабличнаяЧасть.Сумма) * КурсВалюты;
			...
			Если НайденныеСерии.Количество() > 1 Тогда //несколько серий
				Для каждого ТекНайдСерия Из НайденныеСерии Цикл
					МассивСерий.Добавить(Новый Структура("СерияНоменклатуры, Количество, Сумма", ТекНайдСерия.СерияНоменклатуры, ТекНайдСерия.Количество, ?(КоличествоПоСтроке, СуммаПоСтроке / КоличествоПоСтроке, 0) * ТекНайдСерия.Количество))	;
				КонецЦикла;	
			Иначе	
				МассивСерий.Добавить(Новый Структура("СерияНоменклатуры, Количество, Сумма", ТекСтрокаТабличнаяЧасть.СерияНоменклатуры, КоличествоПоСтроке, СуммаПоСтроке));
			КонецЕсли;
			
			СуммаОтклонение = СуммаПоСтроке;
			ТекИндекс = 0;
			...
			ДвижениеСчетКТ = СоответствиеСчета.Получить(ТекСтрокаТабличнаяЧасть.Номенклатура);
				...
				//	СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, Подразделение, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, ТекСтрокаТабличнаяЧасть.Склад, ДвижениеСчетКТ, Подразделение);
				//Иначе	
				//	СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, ТекСтрокаТабличнаяЧасть.Склад, ДвижениеСчетКТ);
				//КонецЕсли;
				//ВыборкаДетальныеЗаписи = РезультатОстатки.НайтиСтроки(СтруктураПоиска);
				//Если ВыборкаДетальныеЗаписи.Количество() Тогда
				//	ОбщаяСуммаКорректировка = Окр(ВыборкаДетальныеЗаписи[0].Цена * КоличествоПоСтроке - СуммаПоСтроке,2);
				//	ДвижениеСодержание = "Корректировка до с/с " + Формат(ВыборкаДетальныеЗаписи[0].Цена, "ЧДЦ=2") ;
				//Иначе
				//	ОбщаяСуммаКорректировка = Окр(0 - СуммаПоСтроке,2);
			...
			Для каждого ТекСерия Из МассивСерий Цикл
				...
				Если ФлагВалюты Тогда
					Движение.ВалютнаяСуммаДт = ТекСерия.Сумма; 
				...
				УчетПоподразделениямСчетУчета = СоответствиеУчетаПоПодразделениям.Получить(СчетКт);
				Если УчетПоподразделениямСчетУчета Тогда
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, Подразделение, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, ТекСтрокаТабличнаяЧасть.Склад, ДвижениеСчетКТ, Подразделение, ТекСерия.СерияНоменклатуры);
				Иначе	
					СтруктураПоиска = Новый Структура("Номенклатура, Склад, Счет, СерияНоменклатуры", ТекСтрокаТабличнаяЧасть.Номенклатура, ТекСтрокаТабличнаяЧасть.Склад, ДвижениеСчетКТ, ТекСерия.СерияНоменклатуры);
				КонецЕсли;
				ВыборкаДетальныеЗаписи = РезультатОстатки.НайтиСтроки(СтруктураПоиска);
				
				Если ВыборкаДетальныеЗаписи.Количество() Тогда
					ДвижениеСумма = Окр(ВыборкаДетальныеЗаписи[0].Цена * Движение.КоличествоКт - СуммаПоЦене,2);
					ДвижениеСодержание = "Корректировка до с/с " + Формат(ВыборкаДетальныеЗаписи[0].Цена, "ЧДЦ=2") ;
				Иначе
					ДвижениеСумма = Окр(0 - СуммаПоЦене,2);
					ДвижениеСодержание = "Корректировка до с/с 0" ;
				КонецЕсли; 
				//ДвижениеСумма = ?(ТекИндекс = МассивСерий.Количество(),ОбщаяСуммаКорректировка, ДвижениеСумма);
				//ОбщаяСуммаКорректировка = ОбщаяСуммаКорректировка - ДвижениеСумма;
				Если ДвижениеСумма Тогда
					Движение = Движения.Учетный.Добавить();
					Движение.Период = Дата;
					Движение.СценарийПлана = Справочники.СценарииПланирования.НайтиПоНаименованию("Факт", Истина);
					Движение.Предприятия = Предприятие;
					Движение.СчетДт = Справочники.УчетныеПолитики.СчетКорректировкиСеб(Предприятие);
