...
Функция ПечатьТОРГ12(МассивОбъектов) Экспорт
	...
	|	И (&НеПроверятьПроведение
	|			ИЛИ ДокументКПечати.Проведен)
	|
	|УПОРЯДОЧИТЬ ПО
	|	Ссылка
	|;
	...
	Если РольДоступна("сабСопровождающийТТ") Или БюджетныйНаСервере.РольАдминаДоступнаСервер() Тогда
		НеПроверятьПроведение = Истина;
	Иначе
		НеПроверятьПроведение = Ложь;
	КонецЕсли;	
	Запрос.УстановитьПараметр("НеПроверятьПроведение", НеПроверятьПроведение);
...
Функция ПолучитьДанныеДляПечатнойФормыСчетФактура(ПараметрыПечати, МассивОбъектов) Экспорт
	...
	Если ПараметрыПечати.Свойство("ПечатьЕГАИС") Тогда // д1 для печати по алкокодам
		ПоместитьВременнуюТаблицуТоваров_ЕГАИС(МенеджерВременныхТаблиц);
	Иначе
	...
	|	ТаблицаДокументаУслуги.Номенклатура.НаименованиеПолное КАК НоменклатураНаименование,
	...
	|	ВЫБОР
	|		КОГДА ТаблицаДокументаУслуги.ЦенаВключаетНДС
	|			ТОГДА (ТаблицаДокументаУслуги.Сумма - ТаблицаДокументаУслуги.СуммаНДС) / ТаблицаДокументаУслуги.Количество
	|		ИНАЧЕ ТаблицаДокументаУслуги.Цена
	|	КОНЕЦ КАК Цена,
	|	ТаблицаДокументаУслуги.НомерСтроки КАК НомерСтроки,
	|	ТаблицаДокументаУслуги.СтранаПроисхождения,
	|	ТаблицаДокументаУслуги.НомерГТД
	|ИЗ
...
//	                                                    ТаблицаДанныхДокументов с полями: Ссылка.
//
Процедура ПоместитьВременнуюТаблицуТоваров_ЕГАИС(МенеджерВременныхТаблиц) Экспорт
	...
	|	ТТНИсходящаяЕГАИСТовары.АлкогольнаяПродукция КАК Номенклатура,
	|	СУММА(ТТНИсходящаяЕГАИСТовары.Количество) КАК Количество,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.ЕдиницаИзмерения КАК Упаковка,
	|	СУММА(ТТНИсходящаяЕГАИСТовары.Количество) КАК Количество1,
	|	ТТНИсходящаяЕГАИСТовары.Цена,
	|	СУММА(ТТНИсходящаяЕГАИСТовары.Сумма) КАК Сумма,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.СтавкаНДС КАК СтавкаНДС,
	|	СУММА(ТТНИсходящаяЕГАИСТовары.Сумма / (100 + СоответствиеНоменклатурыЕГАИС.Номенклатура.СтавкаНДС.Ставка) * СоответствиеНоменклатурыЕГАИС.Номенклатура.СтавкаНДС.Ставка) КАК СуммаНДС,
	|	ДанныеДокументов.ЦенаВключаетНДС,
	|	ВЫБОР
	|		КОГДА СоответствиеНоменклатурыЕГАИС.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.сабТипыНоменклатуры.Товар)
	|			ТОГДА ИСТИНА
	|		ИНАЧЕ ЛОЖЬ
	|	КОНЕЦ КАК ЭтоТовар,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.НомерГТД КАК НомерГТД,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.Страна КАК СтранаПроисхождения,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения
	|ПОМЕСТИТЬ ТаблицаТоваров
	|ИЗ
	...
	|				ПО ТТНИсходящаяЕГАИСТовары.АлкогольнаяПродукция = СоответствиеНоменклатурыЕГАИС.АлкогольнаяПродукция
	|			ПО УЧ_ВозвратТоваровПоставщику.ТТНИсходящаяЕГАИС = ТТНИсходящаяЕГАИСТовары.Ссылка
	|		ПО ДанныеДокументов.Ссылка = УЧ_ВозвратТоваровПоставщику.Ссылка
	|
	|СГРУППИРОВАТЬ ПО
	|	УЧ_ВозвратТоваровПоставщику.Ссылка,
	|	ТТНИсходящаяЕГАИСТовары.НомерСтроки,
	|	ТТНИсходящаяЕГАИСТовары.АлкогольнаяПродукция,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.ЕдиницаИзмерения,
	|	ТТНИсходящаяЕГАИСТовары.Цена,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.СтавкаНДС,
	|	ДанныеДокументов.ЦенаВключаетНДС,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.НомерГТД,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.Страна,
	|	СоответствиеНоменклатурыЕГАИС.Номенклатура.ЕдиницаИзмерения,
	|	ВЫБОР
	|		КОГДА СоответствиеНоменклатурыЕГАИС.Номенклатура.ТипНоменклатуры = ЗНАЧЕНИЕ(Перечисление.сабТипыНоменклатуры.Товар)
	|			ТОГДА ИСТИНА
...
Процедура ПоместитьВременнуюТаблицуУслуг(МенеджерВременныхТаблиц)
	...
	|	ТаблицаУслуг.Номенклатура.ЕдиницаИзмерения КАК Упаковка,
	|	ТаблицаУслуг.Количество КАК Количество1,
	|	ТаблицаУслуг.Цена,
	|	ТаблицаУслуг.Сумма,
	...
	|	ДанныеДокументов.ЦенаВключаетНДС,
	|	ЛОЖЬ КАК ЭтоТовар,
	|	ТаблицаУслуг.Номенклатура.НомерГТД КАК НомерГТД,
	|	ТаблицаУслуг.Номенклатура.Страна КАК СтранаПроисхождения
	|ПОМЕСТИТЬ ТаблицаУслуг
	|ИЗ
	|	Документ.УЧ_ВозвратТоваровПоставщику.Услуги КАК ТаблицаУслуг
	|		ВНУТРЕННЕЕ СОЕДИНЕНИЕ ТаблицаДанныхДокументов КАК ДанныеДокументов
	|		ПО ТаблицаУслуг.Ссылка = ДанныеДокументов.Ссылка";
...
Функция ПолучитьДанныеДляПечатнойФормыТранспортнаяНакладная(МассивДокументов) Экспорт
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЕСТЬNULL(МагазиныКонтактнаяОтправитель.Представление, """") КАК ПунктРазгрузки
	               |ИЗ
	               |	Документ.УЧ_ВозвратТоваровПоставщику КАК ВозвратТоваровПоставщику
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Справочник.Контрагенты.КонтактнаяИнформация КАК МагазиныКонтактнаяОтправитель
	               |		ПО ВозвратТоваровПоставщику.Контрагент = МагазиныКонтактнаяОтправитель.Ссылка
	               |			И (МагазиныКонтактнаяОтправитель.Вид = ЗНАЧЕНИЕ(Справочник.ВидыКонтактнойИнформации.ФактАдресКонтрагента))
	               |ГДЕ
...
// Функция формирует печатную форму документа.
//
Функция ПечатьТОРГ12_ЕГАИС(МассивОбъектов) Экспорт
	...
	|		СоответствиеНоменклатурыЕГАИС.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмерения,
	|		1 КАК КоэффициентМест,
	|		СоответствиеНоменклатурыЕГАИС.Номенклатура.ЕдиницаИзмерения КАК ЕдиницаИзмеренияМест,
	|		ТТНИсходящаяЕГАИСТовары.Количество * СоответствиеНоменклатурыЕГАИС.Номенклатура.ВесБрутто КАК МассаБрутто,
	|		СоответствиеНоменклатурыЕГАИС.Номенклатура.СтавкаНДС КАК СтавкаНДС,
	|		ТТНИсходящаяЕГАИСТовары.Цена КАК Цена,
	|		СУММА(ТТНИсходящаяЕГАИСТовары.Количество) КАК Количество,
	|		СУММА(ТТНИсходящаяЕГАИСТовары.Количество) КАК КоличествоМест,
	|		СУММА(ТТНИсходящаяЕГАИСТовары.Сумма) КАК Сумма,
	|		СУММА(ТТНИсходящаяЕГАИСТовары.Сумма / (100 + СоответствиеНоменклатурыЕГАИС.Номенклатура.СтавкаНДС.Ставка) * СоответствиеНоменклатурыЕГАИС.Номенклатура.СтавкаНДС.Ставка) КАК СуммаНДС,
	...
	|			ПО УЧ_ВозвратТоваровПоставщику.ТТНИсходящаяЕГАИС = ТТНИсходящаяЕГАИСТовары.Ссылка
	|	ГДЕ
	|		УЧ_ВозвратТоваровПоставщику.Ссылка В(&МассивДокументов)
	|		И УЧ_ВозвратТоваровПоставщику.Ссылка.Проведен
	|	
	|	СГРУППИРОВАТЬ ПО
	|		УЧ_ВозвратТоваровПоставщику.Ссылка,
	|		ТТНИсходящаяЕГАИСТовары.АлкогольнаяПродукция,
	...
	|		ТТНИсходящаяЕГАИСТовары.Цена,
	|		ТТНИсходящаяЕГАИСТовары.НомерСтроки,
	|		ТТНИсходящаяЕГАИСТовары.Количество * СоответствиеНоменклатурыЕГАИС.Номенклатура.ВесБрутто) КАК ВложенныйЗапрос
	|
	|УПОРЯДОЧИТЬ ПО
	|	ВложенныйЗапрос.Ссылка,
	|	НомерСтроки
