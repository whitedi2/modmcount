...
Процедура Печать(ТабДок, Ссылка, РецензентКомментарий = Неопределено, МассивОткрытыхИсторий = Неопределено, МассивОткрытыхИсторийАвтора = Неопределено, МобильныйКлиент = Ложь) Экспорт
	...
	Если МобильныйКлиент Тогда
		Макет = Документы.Д_СлужебнаяЗаписка.ПолучитьМакет("Печать_мобКлиент");
	Иначе
	...
	Пока Выборка.Следующий() Цикл
		...
		Если Выборка.ВидСЗ = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаСогласованиеДоговора") Тогда
		...
		Иначе
			...
			Если Выборка.ВидСЗ = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("Приказ") Тогда
				ОбластьЗаголовок.Параметры.Ссылка = "Приказ " + Прав(Выборка.Ссылка, 34);
			Иначе	
				ОбластьЗаголовок.Параметры.Ссылка =Строка(ТипЗнч(Выборка.Ссылка)) + " №" + Выборка.Номер + " от " + Формат(Выборка.Дата, "ДФ=dd.MM.yyyy");
			КонецЕсли;
			...
			ОбластьШапка.Параметры.ВидСЗ = ?(Выборка.ВидСЗ = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаКадровоеДвижение"), "" + Выборка.ВидСЗ + " (" + Выборка.ВидКадровогоДвижения + ")", Выборка.ВидСЗ);
			...
			//	
			//	ОбластьКомандировка.Параметры.ДлительностьКомандировки = Строка(Окр((Выборка.ДатаОкончания - Выборка.ДатаНачала) / (60*60*24),0) + 1) + " дн. (с " + Формат (Выборка.ДатаНачала, "ДФ=dd.MM.yyyy") + " по " + Формат (Выборка.ДатаОкончания, "ДФ=dd.MM.yyyy") + ")";
			//	
			...
			Если Выборка.ВидСЗ = Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("ЗаявкаНаКадровоеДвижение") Тогда
...
Процедура ОбработкаПолученияФормы(ВидФормы, Параметры, ВыбраннаяФорма, ДополнительнаяИнформация, СтандартнаяОбработка)
	Если ВидФормы = "ФормаСписка" ИЛИ ВидФормы = "ФормаВыбора" Тогда
		...
		ПараметрыСеанса.СтруктураПараметровФормСпискаДокументооборота = Новый ФиксированнаяСтруктура(Новый Структура("ВидВнутреннегоДокумента, ИмяДокумента, ФормаВыбора", Справочники.Д_ВидыВнутреннихДокументов.ПолучитьЭлементПредопределенный("СлужебнаяЗаписка"), "Д_СлужебнаяЗаписка", ВидФормы = "ФормаВыбора"));
		ВыбраннаяФорма = "Документ.Д_СлужебнаяЗаписка.Форма.ФормаСпискаБП";
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбработкаПолученияПредставления(Данные, Представление, СтандартнаяОбработка)
	...
	Если Не ТекРекв = Неопределено Тогда
		Представление = ?(ЗначениеЗаполнено(ТекРекв.Комментарий), ТекРекв.Комментарий + " (" + Строка(ТипЗнч(Данные.Ссылка)) + ") " + Строка(ТекРекв.Номер) + " от " + Формат(ТекРекв.Дата, "ДФ=dd.MM.yyyy"), Строка(ТипЗнч(Данные.Ссылка)) + " " + Строка(ТекРекв.Номер) + " от " + Формат(ТекРекв.Дата, "ДФ=dd.MM.yyyy"));	
	КонецЕсли;
КонецПроцедуры
...
Процедура ПечатьФорматируемогоДокумента(ТабДок, Ссылка, РецензентКомментарий = Неопределено, МассивОткрытыхИсторий = Неопределено, МассивОткрытыхИсторийАвтора = Неопределено, МобильныйКлиент = Ложь) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		//	БПСервер.НовыйПараграфТекстФД(ФормДок, "(" +Строка(?(Выборка.НеДействующий, "Не действующий", "Действующий"))+")");
		//КонецЕсли;
		
		СтруктураПоиска = Новый Структура("Заявка", Выборка);
		
		БПСервер.НовыйПараграфТекстФД(ФормДок, "Содержание:", Истина);
