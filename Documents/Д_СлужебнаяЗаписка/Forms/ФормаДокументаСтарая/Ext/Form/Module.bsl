...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Объект.ВидСЗ = ПредопределенноеЗначение("Справочник.Д_ВидыВнутреннихДокументов.ЗаявкаНаКадровоеДвижение") Тогда
...
Процедура ПриОткрытии(Отказ)
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если НЕ ТекБП = Неопределено И ТипЗнч(ТекБП) = Тип("БизнесПроцессСсылка.Согласование3") Тогда
		...
		ИначеЕсли НЕ ТекБП = Неопределено И ТипЗнч(ТекБП) = Тип("БизнесПроцессСсылка.Согласование2") Тогда
			ТекБПСтруРекв = БюджетныйНаСервере.ВернутьРеквизиты(ТекБП, "ОснованиеЗаблокирован, Стартован");
			ВсемСразу = Неопределено;
		ИначеЕсли НЕ ТекБП = Неопределено И ТипЗнч(ТекБП) = Тип("БизнесПроцессСсылка.СогласованиеОбщее") Тогда
			ТекБПСтруРекв = БюджетныйНаСервере.ВернутьРеквизиты(ТекБП, "ОснованиеЗаблокирован, Стартован");
			ВсемСразу = Неопределено;
		КонецЕсли;
	Иначе
...
Процедура СоздатьБПИнтерактивно()
	Если НЕ Объект.ВидСЗ = Справочники.Д_ВидыВнутреннихДокументов.КорректировкаБюджета Тогда
		НовыйБП = БизнесПроцессы.Согласование3.СоздатьБизнесПроцесс();	
	Иначе
		НовыйБП = БизнесПроцессы.Согласование2.СоздатьБизнесПроцесс();	
	КонецЕсли;
	
	НовыйБП.Автор = ПараметрыСеанса.ТекущийПользователь;
	НовыйБП.Описание = Объект.Комментарий;
	НовыйБП.Дата = ТекущаяДата();
	НовыйБП.Заявка = Объект.Ссылка;
	НовыйБП.Предприятие = Объект.Предприятие;
	//добавляем "кому" в список рецензентов
	Если НЕ ПустаяСтрока(Объект.Кому) Тогда
		Если ТипЗнч(Объект.Кому) = Тип("СправочникСсылка.Пользователи") Тогда
			НоваяСтрока = НовыйБП.ДопСогласование.Добавить();
			НоваяСтрока.СубъектСогласования = Объект.Кому;
		Иначе
			Для каждого ТекПользователь Из Объект.Рецензенты Цикл
				НоваяСтрока = НовыйБП.ДопСогласование.Добавить();
				НоваяСтрока.СубъектСогласования = ТекПользователь.Пользователь;
			КонецЦикла; 
		КонецЕсли;
		//НоваяСтрока.ТочкаДействия = ВернутьТочкуДействия();
		//НоваяСтрока.Автор = БюджетныйНаСервере.ПолучитьПользователя();
	КонецЕсли;
	
	Если НЕ Объект.ВидСЗ = Справочники.Д_ВидыВнутреннихДокументов.КорректировкаБюджета Тогда	
		Для каждого ТекАдресат Из Объект.Адресаты Цикл
			НоваяСтрока = НовыйБП.Адресаты.Добавить();
			НоваяСтрока.СубъектСогласования = ТекАдресат.Пользователь;
		КонецЦикла;
		НовыйБП.ОтправлятьВсем = Объект.ОтправлятьВсем;
		НовыйБП.ВидПроизвольнойСЗ = Объект.ВидПроизвольнойСЗ;
	КонецЕсли;
	
	
	//НовыйБП.Стартован = Объект.Предприятие;
	НовыйБП.Записать();
	НовыйБП.Старт();
...
Процедура СоздатьБП(Команда)
	...
	Если Объект.СЗ.Количество() И НЕ ПустаяСтрока(Объект.Текстовка) Тогда
		Предупреждение("Документ имеет заполненную корректировочную и произвольную форму. Запуск БП невозможен."); 			
		Возврат;	
	КонецЕсли;
	
	Если ПоискУжеЗапущенныхБП() Тогда
		Предупреждение("Документ уже имеет запущенный бизнес-процесс. Запуск нескольких БП по одному документу невозможен."); 			
		Возврат;
	КонецЕсли;
	
	Если Объект.Рецензенты.Количество() > 3 И НаправлятьНаУтверждение = "По очереди" Тогда
	...
	Если ПроверитьЗаполнение() И Вопрос("После старта процесса согласования изменения в документе станут невозможны. Уверены что хотите стартовать бизнес процесс?", РежимДиалогаВопрос.ДаНет) = КодВозвратаДиалога.Да Тогда
		Если Записать() Тогда
			СоздатьБПИнтерактивно();
			ПоказатьОповещениеПользователя("Запущен бизнес процесс утверждения:",
			,
			Объект.Ссылка,
...
Процедура УбратьФормуКорректировки()
	Если НЕ ПравоДоступа("ИнтерактивноеДобавление", Метаданные.БизнесПроцессы.Согласование2) Тогда
...
Процедура СЗСтатьяБДДСПриИзменении(Элемент)
	...
	//Если ЗначениеЗаполнено(СтрукрутаСтатьи.Статья) Тогда
...
Процедура ПроизвольнаяСЗПриИзменении(Элемент)
	...
	Если Объект.ВидСЗ = ПредопределенноеЗначение("Справочник.Д_ВидыВнутреннихДокументов.ЗаявкаНаСогласованиеДоговора") Тогда
		НаправлятьНаУтверждение = "Всем сразу";
		Объект.ОтправлятьВсем = Истина;
	КонецЕсли;	
	
	Элементы.Группа22.Видимость = (Объект.ВидСЗ = ПредопределенноеЗначение("Справочник.Д_ВидыВнутреннихДокументов.ЗаявкаНаСогласованиеДоговора"));
	Элементы.Комментарий.Видимость = НЕ (Объект.ВидСЗ = ПредопределенноеЗначение("Справочник.Д_ВидыВнутреннихДокументов.ЗаявкаНаСогласованиеДоговора"));	
	Элементы.КомментарийСтр.Видимость = (Объект.ВидСЗ = ПредопределенноеЗначение("Справочник.Д_ВидыВнутреннихДокументов.ЗаявкаНаСогласованиеДоговора"));	
	Элементы.Кому.Заголовок = ?(Объект.ВидСЗ = ПредопределенноеЗначение("Справочник.Д_ВидыВнутреннихДокументов.ЗаявкаНаСогласованиеДоговора"), "Инициатор", "Кому");
	Элементы.Группа7.Заголовок = ?(Объект.ВидСЗ = ПредопределенноеЗначение("Справочник.Д_ВидыВнутреннихДокументов.ЗаявкаНаСогласованиеДоговора"), "Предмет договора", "Текст документа");
...
Процедура ПредприятиеПриИзменении(Элемент)
	...
	Если НЕ СтруктураДанных = Неопределено Тогда
		...
		Если НЕ Элемент = Неопределено И НЕ СтруктураДанных.УчетПоПодразделениям Тогда
...
Процедура ПредприятиеНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	СтандартнаяОбработка = Ложь;
	ДоступныПП = БюджетныйНаСервере.ПолучитьПредприятия();
	ТекПП = ?(Объект.Предприятие.Пустая(), ?(ДоступныПП.Количество(), ДоступныПП[0], Объект.Предприятие), Объект.Предприятие);
	ОткрытьФорму("Справочник.Предприятия.ФормаВыбора", Новый Структура("ТекущаяСтрока", ТекПП), Элемент);
	УстановитьСвязь();
КонецПроцедуры
