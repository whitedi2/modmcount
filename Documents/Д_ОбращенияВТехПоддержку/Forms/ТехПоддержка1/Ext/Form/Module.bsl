...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	КонецЦикла; 
	ЭтоАдминистратор = БюджетныйНаСервере.РольАдминаДоступнаСервер() ИЛИ РольДоступна("сабСотрудникИТ");
	
	Элементы.Группа3.ОтображениеСтраниц = ?(ЭтоАдминистратор,ОтображениеСтраницФормы.ЗакладкиСверху, ОтображениеСтраницФормы.Нет);
	...
	Список.Параметры.УстановитьЗначениеПараметра("ВсеЗаявки", ЭтоАдминистратор);
...
Процедура ХронометражСписокВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	Если Поле.Имя = "ХронометражСписокКомандаСписка" Тогда
		БПСервер.ПаузаСтартХронометраж(Элементы.ХронометражСписок.ТекущиеДанные.Период, Элементы.ХронометражСписок.ТекущиеДанные.Предмет, Элементы.ХронометражСписок.ТекущиеДанные.КомандаСписка);
		Элементы.ХронометражСписок.Обновить();
		СтандартнаяОбработка = Ложь;
	КонецЕсли;
КонецПроцедуры
...
Процедура ОбновитьСводкуНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	СУММА(ОтчетОПроделаннойРаботеРасшифровкаРаботы.Длительность) КАК ТекущийМесяцЧасы,
	               |	СвязьРодителей.Родитель КАК Проект
	               |ПОМЕСТИТЬ ВыполненоРабот
	               |ИЗ
	               |	Справочник.ОтчетОПроделаннойРаботе.РасшифровкаРаботы КАК ОтчетОПроделаннойРаботеРасшифровкаРаботы
	               |		ЛЕВОЕ СОЕДИНЕНИЕ СвязьРодителей КАК СвязьРодителей
	               |		ПО ОтчетОПроделаннойРаботеРасшифровкаРаботы.Проект = СвязьРодителей.Проект
	               |ГДЕ
	               |	ОтчетОПроделаннойРаботеРасшифровкаРаботы.Ссылка.Дата МЕЖДУ &ДатаНачала И &ДатаОкончания
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	СвязьРодителей.Родитель
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	СвязьРодителей.Родитель КАК Проект,
	               |	МАКСИМУМ(СвязьРодителей.Родитель.ТекущийТариф.ОбъемРабот) КАК ТарифЧасы,
	               |	МАКСИМУМ(ВыполненоРабот.ТекущийМесяцЧасы) КАК ТекущийМесяцЧасы,
	               |	СУММА(ВЫБОР
	               |			КОГДА Задача.ТочкаМаршрута = ЗНАЧЕНИЕ(БизнесПроцесс.ЗаявкиАдминистратору.ТочкаМаршрута.Действие2)
	               |				ТОГДА 1
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) КАК ЗадачиНаСогласовании,
	               |	СУММА(ВЫБОР
	               |			КОГДА Задача.ТочкаМаршрута = ЗНАЧЕНИЕ(БизнесПроцесс.ЗаявкиАдминистратору.ТочкаМаршрута.Действие3)
	               |				ТОГДА 1
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) КАК ЗадачиНаРассмотрении,
	               |	СУММА(ВЫБОР
	               |			КОГДА Задача.ТочкаМаршрута = ЗНАЧЕНИЕ(БизнесПроцесс.ЗаявкиАдминистратору.ТочкаМаршрута.Действие4)
	               |				ТОГДА 1
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) КАК ЗадачиВРаботе,
	               |	СУММА(ВЫБОР
	               |			КОГДА Задача.ТочкаМаршрута = ЗНАЧЕНИЕ(БизнесПроцесс.ЗаявкиАдминистратору.ТочкаМаршрута.Действие6)
	               |				ТОГДА 1
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) КАК ЗадачиНаПроверке,
	               |	СУММА(ВЫБОР
	               |			КОГДА Задача.ТочкаМаршрута = ЗНАЧЕНИЕ(БизнесПроцесс.ЗаявкиАдминистратору.ТочкаМаршрута.Действие5)
	               |				ТОГДА 1
	               |			ИНАЧЕ 0
	               |		КОНЕЦ) КАК ЗадачиНаПроверкеАвтором
	               |ИЗ
	               |	Документ.Д_ОбращенияВТехПоддержку КАК Д_ОбращенияВТехПоддержку
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Задача.Задача КАК Задача
	               |		ПО Д_ОбращенияВТехПоддержку.Ссылка = Задача.Заявка
	               |			И (Задача.Выполнена = ЛОЖЬ)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ СвязьРодителей КАК СвязьРодителей
	               |		ПО Д_ОбращенияВТехПоддержку.Проект = СвязьРодителей.Проект
	               |		ЛЕВОЕ СОЕДИНЕНИЕ ВыполненоРабот КАК ВыполненоРабот
	               |		ПО Д_ОбращенияВТехПоддержку.Проект = ВыполненоРабот.Проект
	               |ГДЕ
	               |	Д_ОбращенияВТехПоддержку.ПометкаУдаления = ЛОЖЬ
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	СвязьРодителей.Родитель
	               |
	...
	Для каждого ТекСтрока Из Выборка Цикл
		...
		НоваяСтрока.ПроцентВыполнения = ?(НоваяСтрока.ТарифЧасы, НоваяСтрока.ТекущийМесяцЧасы / НоваяСтрока.ТарифЧасы, 0) * 100;
		НоваяСтрока.ЗадачиВсего = НоваяСтрока.ЗадачиНаСогласовании + НоваяСтрока.ЗадачиНаРассмотрении + НоваяСтрока.ЗадачиВРаботе + НоваяСтрока.ЗадачиНаПроверке + НоваяСтрока.ЗадачиНаПроверкеАвтором;
	КонецЦикла; 
КонецПроцедуры
