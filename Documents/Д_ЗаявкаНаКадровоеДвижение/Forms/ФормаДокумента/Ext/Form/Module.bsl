...
Процедура ВидКадровогоДвиженияПриИзменении(Элемент)
	...
	Иначе
		...
		Если НЕ Элемент = Неопределено Тогда
			Объект.ДолжностьРегл = ПредопределенноеЗначение("Справочник.Д_Должности.ПустаяСсылка");
			Объект.ДолжностьУпр = ПредопределенноеЗначение("Справочник.Д_Должности.ПустаяСсылка");
		КонецЕсли;
	КонецЕсли;
...
Процедура ДолжностьУпрНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ВыбранноеЗначение = Неопределено Тогда
		...
		иначеЕсли ВыбранноеЗначение.Значение = "Выбрать должность из справочника..." Тогда
			ОткрытьФорму("Справочник.Д_Должности.ФормаВыбора", Новый Структура("ТекущаяСтрока", Объект.ДолжностьУпр), Элемент) ;		
		КонецЕсли;	
	КонецЕсли;
КонецПроцедуры
...
Процедура ДолжностьРеглНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	Если НЕ ВыбранноеЗначение = Неопределено Тогда
		...
		иначеЕсли ВыбранноеЗначение.Значение = "Выбрать должность из справочника..." Тогда
			ОткрытьФорму("Справочник.Д_Должности.ФормаВыбора", Новый Структура("ТекущаяСтрока", Объект.ДолжностьРегл), Элемент) ;		
		КонецЕсли;	
	КонецЕсли;
КонецПроцедуры
