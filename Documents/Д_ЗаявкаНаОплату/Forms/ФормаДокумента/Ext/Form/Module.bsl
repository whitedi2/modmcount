...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		БПСервер.ДобавитьКомандыПроцессов(ЭтаФорма, "Группа16", Объект.Ссылка);
...
Процедура РасчитатьЛимиты(СтатьяДДС, ТекСумма, СуммаПлана, ЦФО, Подразделение, ИнвПроект, Валюта, СчитатьПодразделения)
	...
	Пока ВыборкаДанные.Следующий() Цикл
		...
		ТекущийЛимит = ВыборкаДанные.Текущий;
		...
		ТекПодразделениеВыборки = ?(ЗначениеЗаполнено(ВыборкаДанные.Подразделение), ВыборкаДанные.Подразделение, ПустоеПодразделение); 
		
		Если ВыборкаДанные.СтатьяДДС = СтатьяДДС И ВыборкаДанные.ЦФО = ЦФО И ?(СчитатьПодразделения, ТекПодразделениеВыборки = Подразделение, Истина) И ?(ИнвПроект = Неопределено, Истина, ВыборкаДанные.ИнвПроект = ИнвПроект) Тогда
			Лимит = Лимит + ?(ЗначениеЗаполнено(ТекущийЛимит), ТекущийЛимит, 0);
...
Процедура ОтменитьСтроку(Команда)
	...
	ЕстьПППоЗаявке = БПСервер.ПолучитьПППоЗаявке(ДанныеСтроки.ЗаявкаНаФинансирование, ДанныеСтроки.УИДСтроки);
