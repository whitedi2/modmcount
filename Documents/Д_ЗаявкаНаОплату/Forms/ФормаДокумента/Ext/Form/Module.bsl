...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Иначе
		...
		ИначеЕсли Объект.Автор = БюджетныйНаСервере.ПолучитьПользователя() 
						И (БПСервер.ПолучитьТекущуюТочкуМаршрута(Объект.Ссылка) = БизнесПроцессы.Согласование1.ТочкиМаршрута.Действие1
						   ИЛИ БПСервер.ПолучитьТекущуюТочкуМаршрута(Объект.Ссылка) = БизнесПроцессы.Согласование1.ТочкиМаршрута.Действие2) Тогда
			Элементы.ДатаОплаты.Доступность = Истина;
		КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Для каждого ТекСтрока Из Объект.ЗаявкаБезнал Цикл
		...
		Если ТекСтрока.ВыдачаВПодОтчет Тогда
			ТекСтрока.СчетВзаиморасчетов = ПредопределенноеЗначение("ПланСчетов.Учетный.ПОКассаУч");
		Иначе
	...
	Если НоваяСтрока Тогда
		...
		Если НЕ Копирование Тогда
			...
			Если ТипЗнч(Элементы.Заявка.ТекущиеДанные.Источник) = Тип("СправочникСсылка.БанковскиеСчета") Тогда
				Элементы.Заявка.ТекущиеДанные.ВалютныйСчет = БюджетныйНаСервере.ВернутьРеквизиты(Элементы.Заявка.ТекущиеДанные.Источник, "ВидСчета").ВидСчета = ПредопределенноеЗначение("Перечисление.ГД_ВидыИсточников.ВалютныйРасчетныйСчет");
...
Процедура ЗаполнитьСтрокиТЧ()
	...
	Для каждого ТекСтрока Из Объект.ЗаявкаБезнал Цикл
		...
		Если ТипЗнч(ТекСтрока.Источник) = Тип("СправочникСсылка.БанковскиеСчета")  Тогда
			ТекСтрока.ВалютныйСчет = БюджетныйНаСервере.ВернутьРеквизиты(ТекСтрока.Источник, "ВидСчета").ВидСчета = ПредопределенноеЗначение("Перечисление.ГД_ВидыИсточников.ВалютныйРасчетныйСчет");
	...
	Если ЗначениеЗаполнено(Объект.Источник) Тогда
		Если ТипЗнч(Объект.Источник) = Тип("СправочникСсылка.БанковскиеСчета")  Тогда
			ВалютныйСчет = БюджетныйНаСервере.ВернутьРеквизиты(Объект.Источник, "ВидСчета").ВидСчета = ПредопределенноеЗначение("Перечисление.ГД_ВидыИсточников.ВалютныйРасчетныйСчет");
...
Процедура ОткрепитьЗаявки(ТекущийОбъект)
	...
	Для каждого ТекСтрока Из ОтмененныеЗаявки Цикл
		Если ЗначениеЗаполнено(ТекСтрока.ЗаявкаНаФинансирование) Тогда
			...
			//	СтрокиГрафика2 = РегистрыСведений.сабГрафикПлатежей.СоздатьНаборЗаписей();
			//	СтрокиГрафика2.Отбор.Документ.Установить(ТекСтрока.ЗаявкаНаФинансирование);
			//	СтрокиГрафика2.Отбор.УИДСтрокиДокумента.Установить(ТекСтрока.УИДСтроки);
			//	СтрокиГрафика2.Прочитать();
			//	Для каждого ТекСтрокаГрафика Из СтрокиГрафика2 Цикл
			//		ТекСтрокаГрафика.ТочкаМаршрута = БизнесПроцессы.Согласование1.ТочкиМаршрута.Старт;
			//	КонецЦикла;
			//	СтрокиГрафика2.Записать();
			//Иначе
				ТекОб = ТекСтрока.ЗаявкаНаФинансирование.ПолучитьОбъект();
				ТекОб.РеестрЗаявок = Неопределено;
				ТекОб.Записать();
		...
		Если ЗначениеЗаполнено(ТекСтрока.ЗаявкаНаФинансирование) И ЗначениеЗаполнено(ТекСтрока.УИДСтроки) Тогда
			...
			Для каждого ТекЗапись Из НаборЗаписей Цикл
				Если ТекСтрока.УИДСтроки = ТекЗапись.УИДСтрокиДокумента Тогда
					...
					//ТекЗапись.ТочкаМаршрута = БизнесПроцессы.Согласование1.ТочкиМаршрута.Действие4;			
