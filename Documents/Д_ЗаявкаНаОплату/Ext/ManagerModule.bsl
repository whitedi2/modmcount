...
Процедура Печать(ТабДок, Ссылка, ТекМакет = "ПечатьБезНалБух", РецензентКомментарий = Неопределено, МассивОткрытыхИсторий = Неопределено, МассивОткрытыхИсторийАвтора = Неопределено) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		ТЗ_ЗаявкаБезнал = Выборка.ЗаявкаБезнал.Выгрузить();
		
		Для Каждого ТекСтрокаЗаявкаБезнал Из ТЗ_ЗаявкаБезнал Цикл
			ТекВалютаОплаты = ТекСтрокаЗаявкаБезнал.Валюта;
			ТекВалютаЦФО = ТекСтрокаЗаявкаБезнал.ЦФО.ОсновнаяВалютаУчета;
			
			Если ЗначениеЗаполнено(ТекВалютаОплаты) И ЗначениеЗаполнено(ТекВалютаЦФО) И Не ТекВалютаОплаты = ТекВалютаЦФО Тогда
...
Функция РассчитатьЛимитыПоЗаявке(Ссылка)
	...
	Запрос.Текст = "ВЫБРАТЬ РАЗРЕШЕННЫЕ
	               ...
	               |		ВложенныйЗапрос.ЦФО КАК ЦФО,
	               |		ВЫБОР
	               |			КОГДА ВложенныйЗапрос.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	               |					ИЛИ ВложенныйЗапрос.Подразделение ЕСТЬ NULL
	               |				ТОГДА НЕОПРЕДЕЛЕНО
	               ...
	               |			КОНЕЦ)
	               |			И (ВЫБОР
	               |				КОГДА ТекЗаявка.Подразделение = НЕОПРЕДЕЛЕНО
	               |					ТОГДА Заявки.Подразделение = НЕОПРЕДЕЛЕНО
	               |				ИНАЧЕ ТекЗаявка.Подразделение = Заявки.Подразделение
	               |			КОНЕЦ)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ (ВЫБРАТЬ
	               |			СУММА(-ВЫБОР
	               |					КОГДА БюджетныйОбороты.СценарийПлана = &СценарийГод
	               ...
	               |			ВЫБОР
	               |				КОГДА БюджетныйОбороты.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	               |						ИЛИ БюджетныйОбороты.Подразделение ЕСТЬ NULL
	               |					ТОГДА НЕОПРЕДЕЛЕНО
	               |				ИНАЧЕ БюджетныйОбороты.Подразделение
	               |			КОНЕЦ КАК Подразделение,
	               ...
	               |		ГДЕ
	               |			ВЫБОР
	               |					КОГДА &ЕстьОтборПоПодразделениям
	               |						ТОГДА ВЫБОР
	               |								КОГДА БюджетныйОбороты.Подразделение = ЗНАЧЕНИЕ(Справочник.СтруктураПредприятия.ПустаяСсылка)
	               |										ИЛИ БюджетныйОбороты.Подразделение ЕСТЬ NULL
	               |									ТОГДА ИСТИНА
	               |								ИНАЧЕ БюджетныйОбороты.Подразделение В ИЕРАРХИИ (&ПодразделенияТекЗаявки)
	               |							КОНЕЦ
	               |					ИНАЧЕ ИСТИНА
	               |				КОНЕЦ
	               |		
	               ...
	               |			КОНЕЦ)
	               |			И (ВЫБОР
	               |				КОГДА ВложенныйЗапрос.Подразделение = НЕОПРЕДЕЛЕНО
	               |					ТОГДА ИСТИНА
	               |				ИНАЧЕ ТекЗаявка.Подразделение = ВложенныйЗапрос.Подразделение
	               |			КОНЕЦ)
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	...
	ИначеЕсли ТипЗнч(Ссылка) = Тип("ДокументСсылка.Д_ЗаявкаНаФинансирование") Тогда
		...
		Если ПодразделенияЗаявки.Найти(Ссылка.Подразделение) = Неопределено И ЗначениеЗаполнено(Ссылка.Подразделение) Тогда
			ПодразделенияЗаявки.Добавить(Ссылка.Подразделение);
		КонецЕсли;
	...
	Запрос.УстановитьПараметр("Предприятие", Ссылка.ЦФО);
	...
	Запрос.УстановитьПараметр("ЕстьОтборПоПодразделениям", ПодразделенияЗаявки.Количество());
	...
	//ЗапросБюдж = Новый Запрос;
	//ЗапросБюдж.Текст = "";
	//ЗапросБюдж.УстановитьПараметр("Дата1", Дата1);
	//ЗапросБюдж.УстановитьПараметр("Дата2", Дата2);
	//ЗапросБюдж.УстановитьПараметр("ТекЗаявка", Ссылка);
	//ЗапросБюдж.УстановитьПараметр("Предприятие", Ссылка);
	//ЗапросБюдж.УстановитьПараметр("СценарийГод", СтруктураБюджетов.ГодовойБюджет);
	//ЗапросБюдж.УстановитьПараметр("СценарийКвартал", СтруктураБюджетов.КвартальныйБюджет);
	//ЗапросБюдж.УстановитьПараметр("СценарийМесяц", СтруктураБюджетов.МесячныйБюджет);
	//ЗапросБюдж.УстановитьПараметр("СценарийВариант", СценарийВариант);
	//ЗапросБюдж.УстановитьПараметр("СценарийАктуальный", АктуальныйСценарий);
	//ЗапросБюдж.УстановитьПараметр("ДатаКурсаБюджСц1", АктуальныйСценарий.АктуальнаяДата);
	//ЗапросБюдж.УстановитьПараметр("ЕстьОтборПоПодразделениям", ПодразделенияЗаявки.Количество());
	//ЗапросБюдж.УстановитьПараметр("ПодразделенияТекЗаявки", ПодразделенияЗаявки);
	//ЗапросБюдж.УстановитьПараметр("НацВалюта", УЧ_Сервер.НациональнаяВалюта());
	//ЗапросБюдж.УстановитьПараметр("ПланПоСЗ", Справочники.СценарииПланирования.НайтиПоНаименованию("План по СЗ", Истина));
	//ЗапросБюдж.УстановитьПараметр("Счет50", ПланыСчетов.Учетный.Счет50());
	//ЗапросБюдж.УстановитьПараметр("Счет51", ПланыСчетов.Учетный.Счет51());
	//
	//РезультатБюдж = ЗапросБюдж.Выполнить();
	//ТЗ_Бюдж = РезультатБюдж.Выгрузить();
