...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Если Не ЗначениеЗаполнено(Объект.Ссылка) Тогда
		Объект.Субконто_НДФЛ =  Справочники.ВидыНалоговИПлатежейВБюджет.НайтиПоНаименованию("НДФЛ исчисленный налоговым агентом", Истина);
		Объект.Субконто_ПФР = Справочники.ВидыНалоговИПлатежейВБюджет.НайтиПоНаименованию("Страховые взносы в ПФР", Истина);
		Объект.Субконто_ФСС = Справочники.ВидыНалоговИПлатежейВБюджет.НайтиПоНаименованию("Страховые взносы в ФСС", Истина);
		Объект.Субконто_ФССНесч = Справочники.ВидыНалоговИПлатежейВБюджет.НайтиПоНаименованию("Страховые взносы в ФСС от несчастных случаев", Истина);
		Объект.Субконто_ФФОМС = Справочники.ВидыНалоговИПлатежейВБюджет.НайтиПоНаименованию("Страховые взносы в ФФОМС", Истина);
	КонецЕсли;
	//--
...
Процедура СчетНачисленияПриИзменении(Элемент)
	...
	Если Не ЗначениеЗаполнено(Объект.КорПодразделение) И Не Элемент = Неопределено Тогда
...
Процедура ТабличнаяЧастьПриНачалеРедактирования(Элемент, НоваяСтрока, Копирование)
	...
	Если НоваяСтрока Тогда 
		Элемент.ТекущиеДанные.УИДСтроки = Новый УникальныйИдентификатор;
		//Элемент.ТекущиеДанные.ВидНачисления = ПредопределенноеЗначение("Справочник.ВидыНачисленийОплатыТруда.Оклад");
	ИначеЕсли Не ЗначениеЗаполнено(Элемент.ТекущиеДанные.УИДСтроки) Тогда
		Элемент.ТекущиеДанные.УИДСтроки = Новый УникальныйИдентификатор;
		НайденныеСтрокиУдержаний = Объект.Удержания.НайтиСтроки(Новый Структура("ВидНачисления, Сотрудник", Элемент.ТекущиеДанные.ВидНачисления, Элемент.ТекущиеДанные.Сотрудник));
		
		Для Каждого ТекНайденнаяСтрокаУдержаний Из НайденныеСтрокиУдержаний Цикл
			ТекНайденнаяСтрокаУдержаний.УИДСтрокиНачислений = Элемент.ТекущиеДанные.УИДСтроки; 
		КонецЦикла;
...
Процедура ТабличнаяЧастьДнейПриИзменении(Элемент)
	...
	СтруктураОклада = сабОбщегоНазначения.ПолучитьСуммуНаСервере(Объект.Предприятие, Объект.Подразделение, ТекДанные.Сотрудник, Объект.Дата, ТекДанные.ВидНачисления);
	...
	Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийНачисленияЗП.НачислениеПоТарифу") Тогда
		ТекДанные.Сумма = Окр(ТекДанные.Часов * ТекДанные.Тариф, -1, РежимОкругления.Окр15как20);
		//Если ТекДанные.Сумма < ТекДанные.Часов * ТекДанные.бм_Тариф Тогда
		//	ТекДанные.Сумма = ТекДанные.Сумма + 10;		
		//КонецЕсли;
	КонецЕсли;
...
Процедура ТабличнаяЧастьВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле.Имя = "ТабличнаяЧастьУдержано" Тогда
		...
		Если ЗначениеЗаполнено(ТекДанные.УИДСтроки) Тогда 
			НайденныеСтроки = Объект.Удержания.НайтиСтроки(Новый Структура("ВидНачисления, Сотрудник, УИДСтрокиНачислений", ТекДанные.ВидНачисления, ТекДанные.Сотрудник, ТекДанные.УИДСтроки));
			ТекФорма.УИДСтрокиНачислений = ТекДанные.УИДСтроки; 
		Иначе
			НайденныеСтроки = Объект.Удержания.НайтиСтроки(Новый Структура("ВидНачисления, Сотрудник, УИДСтрокиНачислений", ТекДанные.ВидНачисления, ТекДанные.Сотрудник, ТекДанные.УИДСтроки));
		КонецЕсли;
...
Процедура ОбработкаОповещения(ИмяСобытия, Параметр, Источник)
	...
	Если ИмяСобытия = "УЧ_НачисленияЗПОбновитьУдержания" Тогда
		...
		Если ТипЗнч(Параметр) = Тип("Структура") Тогда
			НайденныеСтроки = Объект.Удержания.НайтиСтроки(Новый Структура("ВидНачисления, Сотрудник, УИДСтрокиНачислений", Параметр.Отбор.ВидНачисления, Параметр.Отбор.Сотрудник, Параметр.Отбор.УИДСтрокиНачислений));
...
Процедура ОбновитьСуммыУдержаний()
	...
	Для каждого ТекСтрока Из Объект.ТабличнаяЧасть Цикл
		...
		Если ЗначениеЗаполнено(ТекСтрока.УИДСтроки) Тогда
			НайденныеСтроки = Объект.Удержания.НайтиСтроки(Новый Структура("ВидНачисления, Сотрудник, УИДСтрокиНачислений", ТекСтрока.ВидНачисления, ТекСтрока.Сотрудник, ТекСтрока.УИДСтроки));
		Иначе
...
Процедура ТабличнаяЧастьВидНачисленияПриИзменении(Элемент)
	...
	Для Каждого ТекСтрокаНачисления Из Объект.ТабличнаяЧасть Цикл 
		
		Если ЗначениеЗаполнено(ТекСтрокаНачисления.УИДСтроки) Тогда			
			НайденныеСтрокиУдержаний = Объект.Удержания.НайтиСтроки(Новый Структура("ВидНачисления, Сотрудник, УИДСтрокиНачислений", ТекСтрокаНачисления.ВидНачисления, ТекСтрокаНачисления.Сотрудник,  ТекСтрокаНачисления.УИДСтроки));
		Иначе
			НайденныеСтрокиУдержаний = Объект.Удержания.НайтиСтроки(Новый Структура("ВидНачисления, Сотрудник", ТекСтрокаНачисления.ВидНачисления, ТекСтрокаНачисления.Сотрудник));
		КонецЕсли;
		
		Для Каждого ТекНайденнаяСтрокаУдержаний Из НайденныеСтрокиУдержаний Цикл
			НоваяСтрокаТЧ_Удержания = ТЧ_Удержания.Добавить();
			ЗаполнитьЗначенияСвойств(НоваяСтрокаТЧ_Удержания, ТекНайденнаяСтрокаУдержаний);
		КонецЦикла;
	...
	Для Каждого ТекСтрокаТЧ_Удержания Из ТЧ_Удержания Цикл
		НоваяСтрокаУдержания = Объект.Удержания.Добавить(); 
		НоваяСтрокаУдержания.ВидНачисления = ТекСтрокаТЧ_Удержания.ВидНачисления;
		НоваяСтрокаУдержания.ВидУдержания = ТекСтрокаТЧ_Удержания.ВидУдержания;
		НоваяСтрокаУдержания.Сотрудник = ТекСтрокаТЧ_Удержания.Сотрудник;
		НоваяСтрокаУдержания.Сумма = ТекСтрокаТЧ_Удержания.Сумма;
		НоваяСтрокаУдержания.УИДСтрокиНачислений = ТекСтрокаТЧ_Удержания.УИДСтрокиНачислений; 
	КонецЦикла;
...
&НаКлиенте
Процедура ТабличнаяЧастьТарифПриИзменении(Элемент)
	
	Элементы.ТабличнаяЧасть.ТекущиеДанные.Сумма = Окр(Элементы.ТабличнаяЧасть.ТекущиеДанные.Тариф * Элементы.ТабличнаяЧасть.ТекущиеДанные.Часов, -1, РежимОкругления.Окр15как20);
	//Если Элементы.ТабличнаяЧасть.ТекущиеДанные.Сумма < Элементы.ТабличнаяЧасть.ТекущиеДанные.бм_Тариф * Элементы.ТабличнаяЧасть.ТекущиеДанные.Часов Тогда
	//	Элементы.ТабличнаяЧасть.ТекущиеДанные.Сумма = Элементы.ТабличнаяЧасть.ТекущиеДанные.Сумма + 10;		
	//КонецЕсли;
