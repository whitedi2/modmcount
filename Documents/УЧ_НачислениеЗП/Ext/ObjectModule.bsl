...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	//Иначе
		...
		Счет70 = ПланыСчетов.Учетный.Счет70();
		
		Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл
			СчетПоВидуНачисленияТекущейСтроки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекСтрокаТабличнаяЧасть.ВидНачисления, "СчетНачисления");
			...
			Если ТипЗнч(ТекСтрокаТабличнаяЧасть.Сотрудник) = Тип("СправочникСсылка.Сотрудники") ИЛИ ЭтоСотрудник(ТекСтрокаТабличнаяЧасть.Сотрудник) Тогда
				Движение.СчетКт = ?(ЗначениеЗаполнено(СчетПоВидуНачисленияТекущейСтроки), СчетПоВидуНачисленияТекущейСтроки, Счет70);
			...
			Иначе 
				Движение.СчетКт = ?(ЗначениеЗаполнено(СчетПоВидуНачисленияТекущейСтроки), СчетПоВидуНачисленияТекущейСтроки, ?(ЗначениеЗаполнено(СчетУчетаФизичЛиц), СчетУчетаФизичЛиц, Счет70));
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,1,ТекСтрокаТабличнаяЧасть.Сотрудник);
				УЧ_Сервер.УстановитьСубконто(Движение.СубконтоКт,Движение.СчетКт,2,ТекСтрокаТабличнаяЧасть.ВидНачисления);
			КонецЕсли;
	...
	Для каждого ТекСтрока1 Из ТабличнаяЧасть Цикл
		...
		Если ЗначениеЗаполнено(ТекСтрока1.УИДСтроки) Тогда
			НайденныеСтроки = Удержания.НайтиСтроки(Новый Структура("ВидНачисления, Сотрудник, УИДСтрокиНачислений", ТекСтрока1.ВидНачисления, ТекСтрока1.Сотрудник, ТекСтрока1.УИДСтроки)); 
		Иначе
		...
		Для каждого ТекСтрока Из НайденныеСтроки Цикл
			СчетПоВидуУдержанияТекущейСтроки = ОбщегоНазначения.ЗначениеРеквизитаОбъекта(ТекСтрока.ВидУдержания, "СчетНачисления");
			...
			КонецЕсли;
			Движение.СчетКт = ?(ЗначениеЗаполнено(СчетПоВидуУдержанияТекущейСтроки), СчетПоВидуУдержанияТекущейСтроки, ПланыСчетов.Учетный.Счет7303());
...
Процедура ПередЗаписью(Отказ, РежимЗаписи, РежимПроведения)
	...
	Для Каждого ТекСтрока Из ТабличнаяЧасть Цикл
		...
		Если Не ЗначениеЗаполнено(ТекСтрока.УИДСтроки) Тогда
			...
			НайденныеСтрокиУдержаний = Удержания.НайтиСтроки(Новый Структура("ВидНачисления, Сотрудник", ТекСтрока.ВидНачисления, ТекСтрока.Сотрудник));
			
			Для Каждого ТекНайденнаяСтрокаУдержаний Из НайденныеСтрокиУдержаний Цикл
				ТекНайденнаяСтрокаУдержаний.УИДСтрокиНачислений = ТекСтрока.УИДСтроки; 
			КонецЦикла;   
