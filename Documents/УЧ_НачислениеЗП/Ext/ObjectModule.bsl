...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	//Иначе
		...
		Для Каждого ТекСтрокаТабличнаяЧасть Из ТабличнаяЧасть Цикл		 	
			...
			Движение.СчетКт = ПланыСчетов.Учетный.Счет70();
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Сотрудники", Истина)] = ТекСтрокаТабличнаяЧасть.Сотрудник;
			Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.НайтиПоНаименованию("Виды начислений оплаты труда", Истина)] = ТекСтрокаТабличнаяЧасть.ВидНачисления;
			Если Движение.СчетКт.ВидыСубконто.Количество() > 2 Тогда
				//Движение.СубконтоКт[ПланыВидовХарактеристик.ВидыСубконто.Подразделения] = ТекСтрокаТабличнаяЧасть.Подразделение;
			КонецЕсли;
			Если Движение.СчетКт.УчетПоПодразделениям Тогда
	...
	МассивСтруктурНалогов.Добавить(Новый Структура("Сумма, СчетКт, Статья25, Содержание, Налог", "СуммаНДФЛ", ПланыСчетов.Учетный.НайтиПоКоду("68.02"), Справочники.СтатьиЗатрат.НайтиПоКоду("2070.04"), "начислен НДФЛ", Субконто_НДФЛ));
	МассивСтруктурНалогов.Добавить(Новый Структура("Сумма, СчетКт, Статья25, Содержание, Налог", "СуммаПФР", ПланыСчетов.Учетный.НайтиПоКоду("69"), Справочники.СтатьиЗатрат.НайтиПоКоду("2070.03"), "начислен ПФР", Субконто_ПФР));
	МассивСтруктурНалогов.Добавить(Новый Структура("Сумма, СчетКт, Статья25, Содержание, Налог", "СуммаФСС", ПланыСчетов.Учетный.НайтиПоКоду("69"), Справочники.СтатьиЗатрат.НайтиПоКоду("2070.03"), "начислен ФСС", Субконто_ФСС));
	МассивСтруктурНалогов.Добавить(Новый Структура("Сумма, СчетКт, Статья25, Содержание, Налог", "СуммаФССНесчСл", ПланыСчетов.Учетный.НайтиПоКоду("69"), Справочники.СтатьиЗатрат.НайтиПоКоду("2070.03"), "начислен ФСС (несч сл)", Субконто_ФССНесч));
	МассивСтруктурНалогов.Добавить(Новый Структура("Сумма, СчетКт, Статья25, Содержание, Налог", "СуммаФФОМС", ПланыСчетов.Учетный.НайтиПоКоду("69"), Справочники.СтатьиЗатрат.НайтиПоКоду("2070.03"), "начислен ФФОМС", Субконто_ФФОМС));
	...
	КонецЦикла;
	ТчВрем.Свернуть("Организация, КорСчет, КорСубконто1, КорСубконто2, КорСубконто3, КорПодразделение", "Сумма, СуммаНДФЛ, СуммаПФР, СуммаФСС, СуммаФССНесчСл, СуммаФФОМС, Удержано");
	...
	Для каждого ТекНалогСтру Из МассивСтруктурНалогов Цикл
		Для каждого ТекСтрока Из ТчВрем Цикл
			...
			Если ТекСтрокаТабличнаяЧасть.КорСчет = ПланыСчетов.Учетный.НайтиПоКоду("25") Тогда
			...
			ИначеЕсли ТекСтрокаТабличнаяЧасть.КорСчет = ПланыСчетов.Учетный.НайтиПоКоду("20") Тогда
			...
			Если Движение.СчетДт.УчетПоПодразделениям Тогда
				Движение.ПодразделениеДт = ТекСтрока.КорПодразделение;
			КонецЕсли;
			...
			Если Движение.СчетКт.УчетПоПодразделениям Тогда
				Движение.ПодразделениеКт = ТекСтрока.КорПодразделение;
			КонецЕсли;
	...
	Для каждого ТекСтрока1 Из ТабличнаяЧасть Цикл
		...
		Для каждого ТекСтрока Из НайденныеСтроки Цикл
			...
			Если Движение.СчетДт.УчетПоПодразделениям Тогда
				Движение.ПодразделениеДт = ТекСтрока1.КорПодразделение;
			КонецЕсли;
			...
			Если Движение.СчетКт.УчетПоПодразделениям Тогда
				Движение.ПодразделениеКт = ТекСтрока1.КорПодразделение;
			КонецЕсли;
