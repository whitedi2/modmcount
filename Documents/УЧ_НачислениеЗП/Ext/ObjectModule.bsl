...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	КонецЦикла;
	ТчВрем.Свернуть("Организация, КорСчет, КорСубконто1, КорСубконто2, КорСубконто3, КорПодразделение", "Сумма, СуммаНДФЛ, СуммаПФР, СуммаФСС, СуммаФССНесчСл, СуммаФФОМС, СуммаВзносыПоЕдиномуТарифу, Удержано");
	...
	Для каждого ТекНалогСтру Из МассивСтруктурНалогов Цикл
		Для каждого ТекСтрока Из ТчВрем Цикл
			...
			Если ТекСтрокаТабличнаяЧасть.КорСчет = ПланыСчетов.Учетный.НайтиПоКоду("25") ИЛИ ТекСтрокаТабличнаяЧасть.КорСчет = ПланыСчетов.Учетный.НайтиПоКоду("26") ИЛИ ТекСтрокаТабличнаяЧасть.КорСчет = ПланыСчетов.Учетный.НайтиПоКоду("44") Тогда
