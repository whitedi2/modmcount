...
Процедура ОбработкаКоманды(ПараметрКоманды, ПараметрыВыполненияКоманды)
	...
	Если Отказ.Признак = "##УжеСоздан" Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("ОбработкаКомандыЗавершение", ЭтотОбъект, Новый Структура("Отказ, ПараметрыВыполненияКоманды, ПараметрКоманды", Отказ, ПараметрыВыполненияКоманды, ПараметрКоманды)), "На основании данного документа уже введен документ " + Отказ.Ссылка + ". Открыть созданный документ?", РежимДиалогаВопрос.ДаНетОтмена);
	ИначеЕсли Отказ.Признак = "##НеПроведен" Тогда
		ВызватьИсключение "Текущий документ не проведен. Ввод на основании не возможен.";
	Иначе
