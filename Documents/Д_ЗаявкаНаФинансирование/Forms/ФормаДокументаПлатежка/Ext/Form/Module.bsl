...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка) Экспорт
	...
	Если НЕ ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Запрос.Текст = 
		"ВЫБРАТЬ
		|	ДолжностиПользователяСрезПоследних.СтруктурнаяЕдиница КАК СтруктурнаяЕдиница
		|ИЗ
		...
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			Объект.Подразделение = ВыборкаДетальныеЗаписи.СтруктурнаяЕдиница;
		КонецЦикла;
		
		Если Не ЗначениеЗаполнено(Объект.СтавкаНДС) Тогда
			Объект.СтавкаНДС = Справочники.СтавкиНДС.СтавкаБезНДС();		
		КонецЕсли;
	Иначе
		...
		|	сабСтатусыЗаявокНаОплату.ДатаОплалы КАК ДатаОплалы
		|ИЗ
		|	РегистрСведений.сабСтатусыЗаявокНаОплату КАК сабСтатусыЗаявокНаОплату
		|ГДЕ
		|	сабСтатусыЗаявокНаОплату.Документ = &Документ";
		...
		Пока Выборка.Следующий() Цикл
			ДатаОплаты = Выборка.ДатаОплалы;	
		КонецЦикла;
		
		БПСервер.ДобавитьКомандыПроцессов(ЭтаФорма, "Группа7", Объект.Ссылка);
	...
	Если Не ДатаОплаты = Дата('00010101') Тогда
		СуммаОплаченоСтрока = СуммаОплаченоСтрока + " (от " + Формат(ДатаОплаты, "ДФ=dd.MM.yyyy") + ")";	
	КонецЕсли;
...
Процедура ПриОткрытии(Отказ)
	...
	НомерВходящегоДокументаПриИзменении(Неопределено);
...
Функция ПолучитьДанныеКонтрагентПриИзменении(Контрагент, ВидОплаты)
	...
	ИначеЕсли ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") ИЛИ ТипЗнч(Контрагент) = Тип("СправочникСсылка.Организации") Тогда
		...
		Если ЗначениеЗаполнено(Контрагент) И ТипЗнч(Контрагент) = Тип("СправочникСсылка.Контрагенты") Тогда
			СтруктураДанные.Вставить("ОплатаНалога", ВидОплаты = Перечисления.ВидыОперацийПлатежноеПоручение.ПеречислениеНалога Или Контрагент.ГосударственныйОрган);
		Иначе
...
Процедура УстановитьДоступностьФормы()
	...
	Если (НЕ ТекБП = Неопределено ИЛИ ЗначениеЗаполнено(Объект.РеестрЗаявок)) Тогда
		...
		ИсключитениеДляАдминистратораВРеестеНастройка = Справочники.сабМониторВнедрения.ПолучитьЗначениеНастройки("РазрешитьРедактированиеДокументовПослеОплатыАдминистратору");
		
		БюджетныйНаСервере.ФормаТолькоПросмотр(Объект, ЭтаФорма, ТекДоступность, ,НЕ ЗначениеЗаполнено(Объект.РеестрЗаявок) Или ИсключитениеДляАдминистратораВРеестеНастройка);
	КонецЕсли;
	...
	Элементы.КлючАналитикиБюджета.Видимость = ИспользоватьКлючиАналитикиБюджета Или ИспользоватьПроекты;
	
	Если ИспользоватьПроекты Тогда
		Массив = Новый Массив;
		Массив.Добавить(Тип("СправочникСсылка.Проекты"));
		ОписаниеТиповЧ = Новый ОписаниеТипов(Массив);
		ЗаголовокКлючаАналитики = "Проект";
	Иначе
		Массив = Новый Массив;
		Массив.Добавить(Тип("СправочникСсылка.КлючиАналитикиБюджетов"));
		ОписаниеТиповЧ = Новый ОписаниеТипов(Массив);
		ЗаголовокКлючаАналитики = "Ключ аналитики";
	КонецЕсли;
	
	Элементы.КлючАналитикиБюджета.ОграничениеТипа = ОписаниеТиповЧ;
	Элементы.КлючАналитикиБюджета.Заголовок = ЗаголовокКлючаАналитики;
...
Процедура РасчитатьЛимиты(СтатьяДДС, ТекСумма, СуммаПлана, ЦФО, Подразделение, СчитатьПодразделения)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |						ИЛИ СценарийПлана В (&СценарийПланПоСЗ)))
	               |					И ВЫБОР
	...
	Запрос.УстановитьПараметр("СценарийПланПоСЗ", Справочники.СценарииПланирования.ПланПоСЗ());
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если ЗначениеЗаполнено(Объект.РеестрЗаявок) И Модифицированность И Не БюджетныйНаСервере.РольДоступнаИлиПолныеПрава("сабФинансистРасширенный") Тогда
	...
	ЗаявкиПоСчетам = ПолучитьЗаявкиПоСчетам(Объект.ДатаВходящегоДокумента, Объект.НомерВходящегоДокумента, Объект.Ссылка);
	Если Модифицированность И НЕ ПараметрыЗаписи.Свойство("ПропускатьКонтроль2") И ЗаявкиПоСчетам.Количество() Тогда
		Отказ = Истина;
		
		СтрокаОплат = "";
		Для каждого ТекСтрокаОплат Из ЗаявкиПоСчетам Цикл
			СтрокаОплат = СтрокаОплат + "
			|" + Строка(ТекСтрокаОплат);		
		КонецЦикла;
		
		ПоказатьВопрос(Новый ОписаниеОповещения("ПередЗаписьюЗавершение2", ЭтотОбъект), "По дату и номеру счета в заявке уже есть созданные заявки:" + СтрокаОплат + "
...
&НаСервереБезКонтекста
Функция ПолучитьЗаявкиПоСчетам(Дата, Номер, Ссылка)
	
	МассивЗаявок = Новый Массив;
	
	Если Дата = Дата('00010101') Или Не ЗначениеЗаполнено(Номер) Тогда
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |ГДЕ
	               |	Д_ЗаявкаНаФинансирование.НомерВходящегоДокумента = &НомерВходящегоДокумента
	               |	И Д_ЗаявкаНаФинансирование.ДатаВходящегоДокумента = &ДатаВходящегоДокумента
	               |	И Д_ЗаявкаНаФинансирование.ПометкаУдаления = ЛОЖЬ
	               |	И НЕ Д_ЗаявкаНаФинансирование.Ссылка = &Ссылка";
	...
	Пока Выборка.Следующий() Цикл
		МассивЗаявок.Добавить(Выборка.Ссылка);	
	КонецЦикла;
...
Процедура ПередЗаписьюЗавершение2(РезультатВопроса, ДополнительныеПараметры) Экспорт
	...
	Если РезультатВопроса = КодВозвратаДиалога.Да  Тогда
		Записать(Новый Структура("ПропускатьКонтроль2", Истина));			
	КонецЕсли;
...
&НаСервере
Процедура ПредприятиеПриИзмененииНаСервере(Изменять, ИмяРеквизита = "")
	
	Если Изменять И Не ИмяРеквизита = "ЦФО" Тогда
		Объект.ЦФО = Объект.Предприятие;	
	КонецЕсли;
	
	Если Изменять И (Не ЗначениеЗаполнено(Объект.Подразделение) Или Не Объект.Подразделение.Предприятие = Объект.ЦФО) И ЗначениеЗаполнено(Объект.ЦФО) Тогда
...
Процедура УстановитьДоступность(Изменять)
	...
	Если Изменять Тогда
		...
		Если Объект.ВидОперации = ПредопределенноеЗначение("Перечисление.ВидыОперацийПлатежноеПоручение.ПеречислениеНалога") Тогда
			...
			Объект.СтавкаНДС = Справочники.СтавкиНДС.НайтиПоНаименованию("Без НДС", Истина);
			Объект.СуммаНДС = 0;
...
Процедура СформироватьНазначениеПлатежа(ИзменениеСуммы = Ложь) Экспорт
	...
	РеквизитСуммы = ?(Элементы.ВалютнаяСумма.Видимость, Объект.ВалютнаяСумма, Объект.Сумма);
...
Процедура ЦФОПриИзменении(Элемент)
	...
	ПредприятиеПриИзмененииНаСервере(Не Элемент = Неопределено, Элемент.Имя);
	ВалютнаяСуммаКонтрагентаПриИзмененииНаСервере(Не Элемент = Неопределено, ?(Элемент = Неопределено, "", Элемент.Имя));
...
Процедура ВалютнаяСуммаКонтрагентаПриИзмененииНаСервере(Изменять, ИмяЭлемента, НомерСтрокиТЧ = Неопределено)
	...
	Если Изменять Тогда
		...
		НуженПересчет = (ЗначениеЗаполнено(Объект.ВалютаКонтрагента) И Не Объект.ВалютаКонтрагента = НацВалюта) Или (ЗначениеЗаполнено(Объект.Валюта) И Не Объект.Валюта = НацВалюта) Или (ЗначениеЗаполнено(ВалютаЦФО) И Не ВалютаЦФО = НацВалюта);
		Если Не НуженПересчет Тогда
			Возврат;		
		КонецЕсли;
		
		Если ЗначениеЗаполнено(Объект.ВалютаКонтрагента) И ЗначениеЗаполнено(АктуальныйСценарий) И (ИмяЭлемента = "СчетКонтрагента" Или ИмяЭлемента = "СчетКонтрагента1" Или ИмяЭлемента = "ВалютнаяСуммаКонтрагента" Или ИмяЭлемента = "Договор") Тогда
			...
			Курс_ВалКонтрагента_ВалЦФО = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, Объект.ВалютаКонтрагента, ?(ЗначениеЗаполнено(ВалютаЦФО), ВалютаЦФО, НацВалюта), ТекущаяДата()).Курс;
			...
			КонецЕсли;
		ИначеЕсли ЗначениеЗаполнено(АктуальныйСценарий) И ЗначениеЗаполнено(Объект.Валюта) И (ИмяЭлемента = "БанковскийСчет" Или ИмяЭлемента = "Источник" Или ИмяЭлемента = "ВалютнаяСумма") Тогда
			Курс_ВалИсточника_ВалЦФО = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, Объект.Валюта, ?(ЗначениеЗаполнено(ВалютаЦФО), ВалютаЦФО, НацВалюта), ТекущаяДата()).Курс;
		...
		ИначеЕсли ИмяЭлемента = "ТабличнаяЧастьВалютнаяСуммаКонтрагента" Тогда
			...
			Курс_ВалКонтрагента_ВалИсточника = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, Объект.ВалютаКонтрагента, Объект.Валюта, ТекущаяДата()).Курс;
			Курс_ВалКонтрагента_ВалЦФО = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, Объект.ВалютаКонтрагента, ?(ЗначениеЗаполнено(ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета), ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета, НацВалюта), ТекущаяДата()).Курс;
		...
		ИначеЕсли ИмяЭлемента = "ТабличнаяЧастьВалСумма" Тогда
			ТекДанныеТЧ = Объект.ТабличнаяЧасть[НомерСтрокиТЧ - 1];
			Курс_ВалИсточника_ВалЦФО = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, Объект.Валюта, ?(ЗначениеЗаполнено(ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета), ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета, НацВалюта), ТекущаяДата()).Курс;
			Курс_ВалИсточника_ВалКонтрагента = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, Объект.Валюта, Объект.ВалютаКонтрагента, ТекущаяДата()).Курс;
		...
		ИначеЕсли ИмяЭлемента = "ТабличнаяЧастьСумма" Тогда
			ТекДанныеТЧ = Объект.ТабличнаяЧасть[НомерСтрокиТЧ - 1];
			Курс_ВалЦФО_ВалИсточника = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ?(ЗначениеЗаполнено(ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета), ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета, НацВалюта), Объект.Валюта, ТекущаяДата()).Курс;
			Курс_ВалЦФО_ВалКонтрагента = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ?(ЗначениеЗаполнено(ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета), ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета, НацВалюта), Объект.ВалютаКонтрагента, ТекущаяДата()).Курс;
		...
		ИначеЕсли ИмяЭлемента = "Курс" Или ИмяЭлемента = "ВалютнаяСумма" Тогда
			Объект.Сумма = Объект.ВалютнаяСумма * Объект.Курс;
		ИначеЕсли ИмяЭлемента = "КурсКонтрагента" Или ИмяЭлемента = "ВалютнаяСуммаКонтрагента" Тогда
			Объект.Сумма = Объект.ВалютнаяСуммаКонтрагента * Объект.КурсКонтрагента;
		Иначе
			Курс_ВалЦФО_ВалИсточника = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ?(ЗначениеЗаполнено(ВалютаЦФО), ВалютаЦФО, НацВалюта), Объект.Валюта, ТекущаяДата()).Курс;
			Курс_ВалЦФО_ВалКонтрагента = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ?(ЗначениеЗаполнено(ВалютаЦФО), ВалютаЦФО, НацВалюта), Объект.ВалютаКонтрагента, ТекущаяДата()).Курс;
...
&НаСервере
Процедура ОбновитьМСП()
	Если ТипЗнч(Объект.Контрагент) = Тип("СправочникСсылка.Контрагенты") И (Метаданные.Имя = "БухгалтерияСтроительнойОрганизацииКОРП" ИЛИ Метаданные.Имя = "БухгалтерияСтроительнойОрганизации") Тогда
		Объект.ОрганизацияСМП = Объект.Контрагент.ОрганизацияСМП;
		Элементы.МСП.Видимость = Истина;
	Иначе
		Объект.ОрганизацияСМП = Ложь;
		Элементы.МСП.Видимость = Ложь;
	КонецЕсли;
КонецПроцедуры

&НаКлиенте
Процедура НомерВходящегоДокументаПриИзменении(Элемент)
	Если ЗначениеЗаполнено(Объект.НомерВходящегоДокумента) Или Не Объект.ДатаВходящегоДокумента = Дата('00010101') Тогда
		Элементы.ГруппаВхНомерДата.Заголовок = "Счет от поставщика №" + Строка(Объект.НомерВходящегоДокумента) + " от " + Формат(Объект.ДатаВходящегоДокумента, "ДФ=dd.MM.yyyy");
	Иначе
		Элементы.ГруппаВхНомерДата.Заголовок = "Счет от поставщика (не указан)";		
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьНазначение(Команда)
	...
	Объект.НазначениеПлатежа = "Оплата по счету №" + Строка(Объект.НомерВходящегоДокумента) + " от " + Формат(Объект.ДатаВходящегоДокумента, "ДФ=dd.MM.yyyy");
	//НазначениеПлатежа = Лев(НазначениеПлатежа, СтрДлина(НазначениеПлатежа) - 2);
	Объект.НазначениеПлатежа = Объект.НазначениеПлатежа;
	СформироватьНазначениеПлатежа(Истина);

	//Документы.Д_ЗаявкаНаФинансирование.СформироватьНазначениеПлатежа(Объект, НазначениеПлатежа, Ложь,,Новый Структура("СуммаДокумента", "Сумма"));
