...
Процедура УстановитьДоступностьФормы()
	...
	Если ЗначениеЗаполнено(Объект.Ссылка) Тогда
		...
		Если НЕ ТекБП = Неопределено И ТипЗнч(ТекБП) = Тип("БизнесПроцессСсылка.СогласованиеОбщее") Тогда
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Если Не Расшифровка Тогда
		...
		//НоваяСтрока.СтатьяДДС = Объект.СтатьяДДС;
		//НоваяСтрока.Сумма = Объект.Сумма;
		//НоваяСтрока.СтавкаНДС = Объект.СтавкаНДС;
		//НоваяСтрока.СуммаНДС = Объект.СуммаНДС;
...
Процедура ПроверкаПринадлежностиСчетаНаСервере(Изменять, Реквизит, ИмяЭлемента)
	...
	Элементы.БанковскийСчетВалютаДенежныхСредств.Видимость = Элементы.ВалютнаяСумма.Видимость;	
...
Процедура ПредприятиеПриИзмененииНаСервере(Изменять)
	...
	Если Изменять И НЕ ЗначениеЗаполнено(Объект.ЦФО) Тогда
...
Процедура СуммаОплаченоСтрокаНажатие(Элемент, СтандартнаяОбработка)
	...
	Если Объект.ТипИсточника = ПредопределенноеЗначение("Перечисление.Д_ИсточникиСредств.БезНал") Тогда
		ТекФорма = ПолучитьФорму("Документ.ПлатежноеПоручение.ФормаСписка");
		...
		ТекФорма.Открыть();
	Иначе
		ТекФорма = ПолучитьФорму("Документ.УЧ_ДвижениеДС.ФормаСписка");
		НовыйОтбор = ТекФорма.Список.Отбор.Элементы.Добавить(Тип("ЭлементОтбораКомпоновкиДанных"));
...
Процедура СчетКонтрагентаПриИзмененииНаСервере(Изменять)
	...
	Если ЗначениеЗаполнено(Объект.Валюта) И ЗначениеЗаполнено(Объект.СчетКонтрагента) И ЗначениеЗаполнено(ВалютаКонтрагента) Тогда
		Если НЕ Объект.Валюта = ВалютаКонтрагента Тогда
			Элементы.ВалютнаяСуммаКонтрагента.Видимость = Истина;
			ВалютаКонтрагента = ВалютаКонтрагента;
...
Процедура ВалютнаяСуммаКонтрагентаПриИзмененииНаСервере(Изменять, ИмяЭлемента, НомерСтрокиТЧ = Неопределено)
	...
	Если Изменять Тогда
		...
		Если ИмяЭлемента = "СчетКонтрагента" Или ИмяЭлемента = "СчетКонтрагента1" Или ИмяЭлемента = "ВалютнаяСуммаКонтрагента" Тогда
			Если ЗначениеЗаполнено(ВалютаКонтрагента) И ЗначениеЗаполнено(АктуальныйСценарий) Тогда
				Курс_ВалКонтрагента_ВалИсточника = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ВалютаКонтрагента, Объект.Валюта, ТекущаяДата()).Курс;
				Курс_ВалКонтрагента_ВалЦФО = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ВалютаКонтрагента, ?(ЗначениеЗаполнено(ВалютаЦФО), ВалютаЦФО, УЧ_Сервер.НациональнаяВалюта()), ТекущаяДата()).Курс;
				Если Объект.ВалютнаяСуммаКонтрагента И Курс_ВалКонтрагента_ВалЦФО И Курс_ВалКонтрагента_ВалИсточника Тогда
		...
		ИначеЕсли ИмяЭлемента = "БанковскийСчет" Или ИмяЭлемента = "Источник" Или ИмяЭлемента = "ВалютнаяСумма" Тогда
			Если ЗначениеЗаполнено(АктуальныйСценарий) Тогда
				...
				Курс_ВалИсточника_ВалКонтрагента = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, Объект.Валюта, ВалютаКонтрагента, ТекущаяДата()).Курс;
				Если Объект.ВалютнаяСумма И Курс_ВалИсточника_ВалЦФО И Курс_ВалИсточника_ВалКонтрагента Тогда
		...
		ИначеЕсли ИмяЭлемента = "ТабличнаяЧастьВалютнаяСуммаКонтрагента" Тогда
			...
			Курс_ВалКонтрагента_ВалИсточника = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ВалютаКонтрагента, Объект.Валюта, ТекущаяДата()).Курс;
			Курс_ВалКонтрагента_ВалЦФО = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ВалютаКонтрагента, ?(ЗначениеЗаполнено(ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета), ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета, УЧ_Сервер.НациональнаяВалюта()), ТекущаяДата()).Курс;
		...
		ИначеЕсли ИмяЭлемента = "ТабличнаяЧастьСумма" Тогда
			...
			Курс_ВалЦФО_ВалКонтрагента = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ?(ЗначениеЗаполнено(ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета), ТекДанныеТЧ.Предприятие.ОсновнаяВалютаУчета, УЧ_Сервер.НациональнаяВалюта()), ВалютаКонтрагента, ТекущаяДата()).Курс;
		...
		Иначе
			...
			Курс_ВалЦФО_ВалКонтрагента = РасчетыБюджет.ПолучитьПлановыйКурс(АктуальныйСценарий, ?(ЗначениеЗаполнено(ВалютаЦФО), ВалютаЦФО, УЧ_Сервер.НациональнаяВалюта()), ВалютаКонтрагента, ТекущаяДата()).Курс;
...
Процедура ЗаполнитьДобавленныеКолонкиТаблиц()
	...
	Для Каждого ТекСтрокаТЧ Из Объект.ТабличнаяЧасть Цикл
		ТекстрокаТЧ.ВалютнаяСуммаКонтрагентаДоступность = Ложь;

		Если ЗначениеЗаполнено(Объект.Валюта) И ЗначениеЗаполнено(Объект.СчетКонтрагента) И ЗначениеЗаполнено(ВалютаКонтрагента) Тогда
			
			Если НЕ Объект.Валюта = ВалютаКонтрагента Тогда
				ТекстрокаТЧ.ВалютнаяСуммаКонтрагентаДоступность = Истина;
	...
	Элементы.ВалютнаяСуммаКонтрагента.Видимость = ЕстьВалКонтрагента;
