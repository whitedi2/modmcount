...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	Элементы.ДатаДоставки2.Видимость = Объект.КроссДокинг;
КонецПроцедуры
...
Процедура ПриЗаписиНаСервере(Отказ, ТекущийОбъект, ПараметрыЗаписи)
	...
	НаборЗаписей = РегистрыСведений.сабМаршрутыДоставки.СоздатьНаборЗаписей();
	НаборЗаписей.Отбор.Документ.Установить(Объект.Ссылка);
	НаборЗаписей.Записать();
	НомерВОчереди = 1;
	Для каждого Маршрут из МаршрутДоставкиФ цикл   	
		НоваяЗапись = НаборЗаписей.Добавить();    
		НоваяЗапись.Документ = Объект.Ссылка; 
		НоваяЗапись.МашрутДоставки = Маршрут.Значение; 
		НоваяЗапись.НомерВОчереди = НомерВОчереди;
...
Процедура ЗаполнитьНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	И НЕ ЗаказКлиентаДок.СпособДоставки = &СпособДоставки
	               |	И ВЫБОР
	               |			КОГДА ЗаказКлиентаДок.ТранзитныйГруз
	               |					И НЕ ЗаказКлиентаДок.ДатаТранзитнойОтгрузки = ДАТАВРЕМЯ(1, 1, 1)
	               |				ТОГДА ЗаказКлиентаДок.ДатаТранзитнойОтгрузки <= &ДатаДокумента
	               |			ИНАЧЕ ЗаказКлиентаДок.ДатаПоступления <= &ДатаДокумента
	               |		КОНЕЦ
	               |	И ЗаказКлиентаДок.Проведен = ИСТИНА
	               |	И сабМаршрутныйЛистТабличнаяЧасть.Ссылка ЕСТЬ NULL
	               |	И ВЫБОР
	               |			КОГДА &НетМаршрутов
	               |				ТОГДА ИСТИНА
				   |			ИНАЧЕ ЗаказКлиентаДок.МаршрутДоставки В (&МаршрутыДоставки)
	               |		КОНЕЦ
	               |	И ВЫБОР
	               |			КОГДА &ГрафикДоставки = ЗНАЧЕНИЕ(Справочник.ГрафикиДоставки.ПустаяСсылка)
	               |				ТОГДА ИСТИНА
				   ...
				   |			КОГДА &НетМаршрутов 
	               |				ТОГДА ИСТИНА
				   |			ИНАЧЕ ЗаказНаВозвратОтКлиента.МаршрутДоставки В (&МаршрутыДоставки)
	               |		КОНЕЦ
	               |	И ВЫБОР
	               |			КОГДА &ГрафикДоставки = ЗНАЧЕНИЕ(Справочник.ГрафикиДоставки.ПустаяСсылка)
	               |				ТОГДА ИСТИНА
	               ...
	               |
	               |ВЫБРАТЬ
	               |	ВложенныйМаршрутныйЛист.Ссылка,
	               |	ВложенныйМаршрутныйЛист.СуммаДокумента,
	               |	ВложенныйМаршрутныйЛист.Статус,
	               |	NULL,
	               |	NULL,
	               |	ВложенныйМаршрутныйЛист.Вес,
	               |	NULL,
	               |	ВложенныйМаршрутныйЛист.Подразделение,
	               |	NULL,
	               |	NULL
	               |ИЗ
	               |	Документ.сабМаршрутныйЛист КАК ВложенныйМаршрутныйЛист
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.сабМаршрутныйЛист.ТабличнаяЧасть КАК сабМаршрутныйЛистТабличнаяЧасть
	               |		ПО ВложенныйМаршрутныйЛист.Ссылка = сабМаршрутныйЛистТабличнаяЧасть.ЗаказКлиента
	               |			И (НЕ сабМаршрутныйЛистТабличнаяЧасть.Ссылка = &ЭтаСсылка)
	               |			И (сабМаршрутныйЛистТабличнаяЧасть.Ссылка.ПометкаУдаления = ЛОЖЬ)
	               |ГДЕ
	               |	ВложенныйМаршрутныйЛист.Ссылка.КроссДокинг = ИСТИНА
	               |	И ВложенныйМаршрутныйЛист.Предприятие = &Предприятие
	               |	И НАЧАЛОПЕРИОДА(ВложенныйМаршрутныйЛист.ДатаПоступления, ДЕНЬ) <= &ДатаДокумента
	               |	И ВложенныйМаршрутныйЛист.Проведен = ИСТИНА
	               |	И сабМаршрутныйЛистТабличнаяЧасть.Ссылка ЕСТЬ NULL
	               |	И ВложенныйМаршрутныйЛист.Ссылка <> &ЭтаСсылка
	               |	И ВложенныйМаршрутныйЛист.Статус В(&СтатусыМЛ)
				   //|	И ВЫБОР
				   //|			КОГДА &МаршрутДоставки = ЗНАЧЕНИЕ(Справочник.МаршрутыДоставки.ПустаяСсылка)
				   //|				ТОГДА ИСТИНА
				   //|			ИНАЧЕ ВложенныйМаршрутныйЛист.МаршрутДоставки = &МаршрутДоставки
				   //|		КОНЕЦ
	               |	И &КроссДокинг = ЛОЖЬ
	               |
	               |СГРУППИРОВАТЬ ПО
	...
	СтатусыМЛ.Добавить(Перечисления.СтатусыМаршрутныхЛистов.ВРаботе);
	СтатусыМЛ.Добавить(Перечисления.СтатусыМаршрутныхЛистов.Собран);
	...
	Запрос.УстановитьПараметр("НетМаршрутов", (МаршрутДоставкиФ.Количество()=0));
	Запрос.УстановитьПараметр("МаршрутыДоставки", МаршрутДоставкиФ);
	...
	Запрос.УстановитьПараметр("КроссДокинг", Объект.КроссДокинг);
	Запрос.УстановитьПараметр("СтатусыМЛ", СтатусыМЛ);
			...
			Если ТипЗнч(ТекСтрока.ЗаказКлиента) = Тип("ДокументСсылка.ЗаказНаВозвратОтКлиента") Тогда
				...
				Если ЗначениеЗаполнено(ТекСтрока.ЗаказКлиента.Организация) Тогда
					...
					Если ТекСтрока.ЗаказКлиента.БезвозмезднаяПередача Тогда
						...
						НоваяРеализация.Дата = ?(Объект.КроссДокинг, Объект.ДатаПоступления, Объект.Дата);
			...
			ИначеЕсли ТипЗнч(ТекСтрока.ЗаказКлиента) = Тип("ДокументСсылка.ЗаказКлиента") Тогда
				...
				Если ЗначениеЗаполнено(ТекСтрока.ЗаказКлиента.Организация) Тогда
					...
					Если ТекСтрока.ЗаказКлиента.БезвозмезднаяПередача Тогда
						...
						НоваяРеализация.Записать(?(ПроводитьДокументы, РежимЗаписиДокумента.Проведение, РежимЗаписиДокумента.Запись));//всегда непроведенным, т.к. нужно закрывать смену вручную
						ТекСтрока.Реализация = НоваяРеализация.Ссылка; 
					ИначеЕсли ТекСтрока.ЗаказКлиента.ПродажаВРозницу ИЛИ ЭтоНаличнаяОплата(ТекСтрока.ЗаказКлиента.Договор) Тогда
						НоваяРеализация = Документы.РозничнаяПродажа.СоздатьДокумент();
						НоваяРеализация.Заполнить(ТекСтрока.ЗаказКлиента);
						НоваяРеализация.ДополнительныеСвойства.Вставить("НеИзменятьСтатусЗаказа", Истина);
						НоваяРеализация.Дата = ?(Объект.КроссДокинг, Объект.ДатаПоступления, Объект.Дата);
					...
					Иначе
						...
						Если СоздаватьСчетНаОплатуПокупателю Тогда
							...
							НовыйСчет.Дата = ?(Объект.КроссДокинг, Объект.ДатаПоступления, Объект.Дата);
							Попытка
						...
						НовыйСчФ = Документы.СчетФактураВыданный.СоздатьДокумент();
						НовыйСчФ.Дата = ?(Объект.КроссДокинг, Объект.ДатаПоступления, Объект.Дата);
						НовыйСчФ.Заполнить(НоваяРеализация.Ссылка);
						НовыйСчФ.Записать(?(ПроводитьДокументы, РежимЗаписиДокумента.Проведение, РежимЗаписиДокумента.Запись));
					КонецЕсли;   
...
&НаСервере
Функция ЭтоНаличнаяОплата(Договор)
	
	Если ЗначениеЗаполнено(Договор) Тогда
		Если Договор.ПриоритетныйПорядокРасчетов = Перечисления.Д_ИсточникиСредств.Нал Тогда
			Возврат Истина;
		КонецЕсли;
	КонецЕсли;
	Возврат Ложь;
...
Процедура СинхронизацияСерийПриЗаписиНаСервереФормы(ДокументБУ,ДокументУУ)
	...
	Если ОбщегоНазначенияБП.ЕстьТабЧастьДокумента("СерииНоменклатуры",ДокументБУОбъект.Метаданные())Тогда 
		...
		Попытка
			...
			ДокументБУОбъект.ДополнительныеСвойства.Вставить("НеИзменятьСтатусЗаказа", Истина);
...
Процедура ТабличнаяЧастьЗаказКлиентаНачалоВыбора(Элемент, ДанныеВыбора, СтандартнаяОбработка)
	...
	СписокВыбора.Добавить("Маршрутные листы");
...
Процедура ТабличнаяЧастьЗаказКлиентаНачалоВыбораЗавершение(ВыбранныйЭлемент, ДополнительныеПараметры) Экспорт
	...
	ИначеЕсли ТекЗнач.Значение = "Маршрутные листы" Тогда
		
		СписокКД = ВыбратьКроссДокинг();
		...
		ЭлементОтбора.ПравоеЗначение = СписокКД;
		...
		//ОбработкаВыбора = Новый ОписаниеОповещения("ПриЗакрытииФормыВыбораВложенногоМЛ", ЭтаФорма, "Подбор");
		
		ОткрытьФорму("Документ.сабМаршрутныйЛист.Форма.ФормаВыбора",ПараметрыФормы,
...
Процедура ПриОткрытии(Отказ)
	...
	Если Объект.КроссДокинг тогда 
		Элементы.КроссДокингСтраница.Видимость = Истина;
	иначе
		Элементы.КроссДокингСтраница.Видимость = Ложь;
	КонецЕсли;
	...
	СписокМаршрутов = ПолучитьМаршрутыДоставки();
	Если СписокМаршрутов.Количество() тогда
		Для каждого Маршрут из СписокМаршрутов цикл
			МаршрутДоставкиФ.Добавить(Маршрут.Значение);		
		КонецЦикла;
	//иначе   
	//	Если ЗначениеЗаполнено(Объект.МаршрутДоставки) тогда
	//		МаршрутДоставкиФ.Добавить(Объект.МаршрутДоставки);
	//	КонецЕсли;
	КонецЕсли;
...
Процедура МаршрутДоставкиПриИзменении(Элемент)
	Если Объект.ТабличнаяЧасть.Количество() Тогда
		ПоказатьВопрос(Новый ОписаниеОповещения("МаршрутДоставкиПриИзмененииЗавершение", ЭтаФорма), "Очистить табличную часть?", РежимДиалогаВопрос.ДаНет);
	КонецЕсли;
КонецПроцедуры
...
Процедура ЗаполнитьЗначенияСтрок(ТекЗаказ = Неопределено)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ВЫБОР
	               |		КОГДА ЗаказКлиентаДок.ТранзитныйГруз
	               |			ТОГДА ЗаказКлиентаДок.ДатаТранзитнойОтгрузки
	               |		ИНАЧЕ ЗаказКлиентаДок.ДатаПоступления
	               ...
	               |	ЗаказПоставщику.Ссылка,
	               |	ВЫБОР
	               |		КОГДА ЗаказПоставщику.ТранзитныйГруз
	               |			ТОГДА ЗаказПоставщику.ДатаТранзитногоПоступления
	               |		ИНАЧЕ ЗаказПоставщику.ДатаПоступления
	               |	КОНЕЦ,
	               |	ЗаказПоставщику.ПокупкаВРозницу,
	               |	ЗаказПоставщику.АдресДоставки,
	               |	ЗаказПоставщику.БезвозмезднаяПередача,
	               ...
	               |ВЫБРАТЬ
	               |	ВложенныйМаршрутныйЛист.Ссылка,
	               |	ВложенныйМаршрутныйЛист.ДатаПоступления,
	               |	NULL,
	               |	ВЫРАЗИТЬ(ВложенныйМаршрутныйЛист.АдресТочкиПерегрузки КАК СТРОКА(200)),
	               |	NULL,
	               |	ВложенныйМаршрутныйЛист.СуммаДокумента,
	               |	ВложенныйМаршрутныйЛист.Статус,
	               |	NULL,
	               ...
	               |	Документ.сабМаршрутныйЛист КАК ВложенныйМаршрутныйЛист
	               |ГДЕ
	               |	ВложенныйМаршрутныйЛист.Ссылка В(&Заказы)
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ВложенныйМаршрутныйЛист.Ссылка,
	               |	ВложенныйМаршрутныйЛист.СуммаДокумента,
...
Процедура ТабличнаяЧастьПриАктивизацииЯчейки(Элемент)
	...
	Если Не Элемент = Неопределено И Не Элемент.ТекущиеДанные = Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Элемент.ТекущиеДанные.ЗаказКлиента) = Тип("ДокументСсылка.сабМаршрутныйЛист") тогда
			//Элемент.ТекущиеДанные.Статус = ;
		КонецЕсли;
...
Процедура ТабличнаяЧастьЗаказКлиентаПриИзменении(Элемент)
	...
	Если Не Элементы.ТабличнаяЧасть.ТекущиеДанные = Неопределено Тогда
		...
		ИначеЕсли ТипЗнч(Элементы.ТабличнаяЧасть.ТекущиеДанные.ЗаказКлиента) = Тип("ДокументСсылка.ЗаказПоставщику") Или ТипЗнч(Элементы.ТабличнаяЧасть.ТекущиеДанные.ЗаказКлиента) = Тип("ДокументСсылка.ЗаказНаПеремещение") Тогда	
			Элементы.ТабличнаяЧасть.ТекущиеДанные.Статус = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовПоставщикам.ПустаяСсылка");
		ИначеЕсли ТипЗнч(Элементы.ТабличнаяЧасть.ТекущиеДанные.ЗаказКлиента) = Тип("ДокументСсылка.сабМаршрутныйЛист") Тогда
			Объект.КроссДокинг = Истина;
		КонецЕсли;
...
Процедура ПроверитьОстаткиНаСервере()
	...
	Запрос.Текст = "ВЫБРАТЬ
				   ...
				   |	КоличествоВДокументе.КонечныйОстаток - ЕСТЬNULL(ВТРезервыИтог.КоличествоРезерв, 0) КАК ФактОстаток
	               |ИЗ
	               |	КоличествоВДокументе КАК КоличествоВДокументе
	...
	Пока Выборка.Следующий() цикл
		...
		НоваяСтр = ЗаказыПревышениеОстатка.Добавить();
		НоваяСтр.Заказ = Выборка.ЗаказКлиента; 
		НоваяСтр.Номенклатура = Выборка.Номенклатура;
		НоваяСтр.КоличествоВЗаказе = Выборка.Количество;
		НоваяСтр.ФактическийОстаток = Выборка.ФактОстаток;
	КонецЦикла;
...
Процедура ПроверитьОстатки(Команда)
	...
	ЗаказыПревышениеОстатка.Очистить();
	
	ПроверитьОстаткиНаСервере();       
	
	Если ЗаказыПревышениеОстатка.Количество() > 0 тогда
		АдресТаблицы = ПоместитьЗаказыВХранилище();	
		ДопПараметры = Новый Структура("АдресТаблицы", АдресТаблицы);
		
		ОткрытьФорму("Документ.сабМаршрутныйЛист.Форма.ФормаПревышениеОстатка", ДопПараметры, ЭтаФорма,,,,,РежимОткрытияОкнаФормы.БлокироватьОкноВладельца);
	КонецЕсли;
...
Функция ПоместитьЗаказыВХранилище()
	Возврат ПоместитьВоВременноеХранилище(ЗаказыПревышениеОстатка.Выгрузить(), УникальныйИдентификатор);
КонецФункции
...
Процедура ОформитьВСДНаСервере(Реализация = Неопределено,АдресВх)
	...
	Для каждого ДокументБух Из МассивРеализаций Цикл
		...
		Если ТипЗнч(ДокументБух) = Тип("ДокументСсылка.РеализацияТоваровУслуг") И ЗначениеЗаполнено(ОбъектВетис.ДокументОснование.Заказ) И ЗначениеЗаполнено(ОбъектВетис.ДокументОснование.Заказ.ПодразделениеКонтрагента) Тогда
			Запрос = Новый Запрос;
			Запрос.Текст = "ВЫБРАТЬ
		...
		Пока ВыбокаНомерСтроки.Следующий() Цикл
			...
			Если ОсталосьСписать > 0 Тогда
				НоваяСтрокаТЧ = ОбъектВетис.Товары.Добавить();
				ЗаполнитьЗначенияСвойств(НоваяСтрокаТЧ,ТекСтрока,,"НомерСтроки,ЕдиницаИзмеренияВЕТИС,ЗаписьСкладскогоЖурнала,ИдентификаторСтроки,СрокГодностиНачалоПериода");
				НоваяСтрокаТЧ.ИдентификаторСтроки = Новый УникальныйИдентификатор;
			...
			КонецЕсли;
		КонецЦикла;
		ОбъектВетис.Товары.Сортировать("Номенклатура, ДатаПроизводстваНачалоПериода");  
		//ТС
		Запрос = Новый Запрос;
		Запрос.Текст = "ВЫБРАТЬ
		...
		Запрос.УстановитьПараметр("Реализация", ОбъектВетис.ДокументОснование);
		Результат = Запрос.Выполнить();
		Выборка = Результат.Выбрать(); 
		Если Выборка.Следующий() Тогда
		...
		#Область ИсходныйКодЗаполненияВетис
...
Процедура ЗаполнитьТоварыВЕТИС(ДокументОбъект, ДанныеЗаполнения)
	...
	#Область ИсходныйКодЗаполнитьТоварыВетис
	////Реквизиты основания
	//МетаданныеОснования  = ДанныеЗаполнения.Метаданные();
	//ИмяОснования         = МетаданныеОснования.Имя;
	//
...
&НаСервере
Функция ВыбратьКроссДокинг()
	
	Запрос= Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	ВложенныйМаршрутныйЛист.Ссылка КАК Ссылка
		           |ИЗ
	               |	Документ.сабМаршрутныйЛист КАК ВложенныйМаршрутныйЛист
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.сабМаршрутныйЛист.ТабличнаяЧасть КАК сабМаршрутныйЛистТабличнаяЧасть
	               |		ПО ВложенныйМаршрутныйЛист.Ссылка = сабМаршрутныйЛистТабличнаяЧасть.ЗаказКлиента
	...
	Пока Выборка.Следующий() цикл
		НайденныеСтроки = Объект.ТабличнаяЧасть.НайтиСтроки(Новый Структура("ЗаказКлиента", Выборка.Ссылка));
		Если НайденныеСтроки.Количество() = 0 тогда
...
Процедура КроссДокингПриИзменении(Элемент)
	...
	Если Объект.КроссДокинг тогда 
		Элементы.КроссДокингСтраница.Видимость = Истина;
		//Элементы.Группа7.ТекущаяСтраница = Элементы.КроссДокингСтраница;
	иначе
		Элементы.КроссДокингСтраница.Видимость = Ложь;
	//	Элементы.Группа7.ТекущаяСтраница = Элементы.Основное;
	КонецЕсли;
...
Процедура ДатаДоставкиПриИзменении(Элемент)
	...
	Если Не НачалоДня(Объект.Дата) = НачалоДня(Объект.ДатаПоступления) И Объект.КроссДокинг Тогда
		...
		Если ЕстьРеализации Тогда
			ПоказатьВопрос(Новый ОписаниеОповещения("ДатаДоставкиПриИзмененииЗавершение", ЭтаФорма), "По заказам найдены документы отгрузки. Изменить в документах Дату на Дату доставки?", РежимДиалогаВопрос.ДаНет);
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Процедура ДатаДоставкиПриИзмененииЗавершение(РезультатВопроса, ДополнительныеПараметры) Экспорт
	
	Если РезультатВопроса = КодВозвратаДиалога.Да Тогда
		СинхронизироватьДатуВДокументах();			
	КонецЕсли;

КонецПроцедуры

&НаСервере
Процедура СинхронизироватьДатуВДокументах()
	
	Для каждого ТекСтрока Из Объект.ТабличнаяЧасть Цикл
		Если ЗначениеЗаполнено(ТекСтрока.Реализация) И Не НачалоДня(ТекСтрока.Реализация.Дата) = НачалоДня(Объект.ДатаПоступления) Тогда
			ТекОб = ТекСтрока.Реализация.ПолучитьОбъект();
			ТекОб.Дата = Объект.ДатаПоступления;
			Если ТекОб.Проведен = Истина Тогда
			...
			Если ТипЗнч(ТекСтрока.Реализация) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
				...
				Запрос.УстановитьПараметр("ДокументОснование", ТекСтрока.Реализация);
				Результат = Запрос.Выполнить();
				Выборка = Результат.Выбрать();
				Пока Выборка.Следующий() Цикл
				...
				Если ЗначениеЗаполнено(ТекСтрока.Реализация.СчетНаОплатуПокупателю) Тогда
					ТекОб = ТекСтрока.Реализация.СчетНаОплатуПокупателю.ПолучитьОбъект();
					ТекОб.Дата = Объект.ДатаПоступления;
					Если ТекОб.Проведен = Истина Тогда
						ТекОб.Записать(РежимЗаписиДокумента.Проведение);
					Иначе
...
&НаКлиенте
Процедура ПриЗакрытииФормыВыбораВложенногоМЛ(Значение, ДопПараметры) Экспорт
	...
	НоваяСтр.ЗаказКлиента = Значение;
...
&НаСервере
Процедура ПриЗакрытииФормыВыбораВложенногоМЛНаСервере(Ссылка)
	...
	//               |	сабМаршрутныйЛистТабличнаяЧасть.НомерСтроки КАК НомерСтроки,
	//               |	сабМаршрутныйЛистТабличнаяЧасть.ЗаказКлиента КАК ЗаказКлиента,
	//               |	сабМаршрутныйЛистТабличнаяЧасть.Сумма КАК Сумма,
	//               |	сабМаршрутныйЛистТабличнаяЧасть.Реализация КАК Реализация,
	//               |	сабМаршрутныйЛистТабличнаяЧасть.Статус КАК Статус,
...
&НаСервере
Функция ПолучитьМаршрутыДоставки()
	
	СписокМаршрутов = Новый СписокЗначений;
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	               |	сабМаршрутыДоставки.МашрутДоставки КАК МашрутДоставки,
	...
	Пока Выборка.Следующий() цикл
		СписокМаршрутов.Добавить(Выборка.МашрутДоставки);
	КонецЦикла; 
