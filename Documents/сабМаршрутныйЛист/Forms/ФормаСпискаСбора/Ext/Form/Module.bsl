...
Процедура ЗаполнитьДанныеМЛ(Заказ = Неопределено, Реализация = Неопределено)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	РозничнаяПродажаСерииНоменклатуры.ДатаПроизводства,
	               |	РозничнаяПродажаСерииНоменклатуры.СерияНоменклатуры,
	               |	РозничнаяПродажаТовары.Ссылка.НомерЧекаККМ
	               |ИЗ
	               |	Документ.РозничнаяПродажа.Товары КАК РозничнаяПродажаТовары
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РозничнаяПродажа.СерииНоменклатуры КАК РозничнаяПродажаСерииНоменклатуры
	               |		ПО РозничнаяПродажаТовары.Ссылка = РозничнаяПродажаСерииНоменклатуры.Ссылка
	               ...
	               |	ЕСТЬNULL(Реализации.НомерЧекаККМ, 0) КАК НомерЧекаККМ
	               |ИЗ
	               ...
	               |	ЕСТЬNULL(Реализации.НомерЧекаККМ, 0)
	               |ИЗ
	               |	Реализации КАК Реализации
	...
	|	НомерСтрокиРеализации", "УПОРЯДОЧИТЬ ПО Номенклатура");	
	КонецЕсли;
	...
	Для каждого ТекСтрока Из ТЧЗаказыВрем Цикл
		...
		ОсталосьРаспределитьКоличество = ТекСтрока.Количество;
			Для каждого ТекнайдСтрока Из НайденныеСтроки Цикл
				ОсталосьРаспределитьКоличество = ОсталосьРаспределитьКоличество - ТекнайдСтрока.Количество; 
				Если ТекнайдСтрока.Количество >= ТекКоличЗаказ Тогда
					ТекнайдСтрока.КоличествоЗаказ = ТекКоличЗаказ;
					ТекКоличЗаказ = 0; 
					ОсталосьРаспределитьКоличество = 0;
				Иначе
					ТекнайдСтрока.КоличествоЗаказ = ?(ОсталосьРаспределитьКоличество > 0,ТекнайдСтрока.Количество,ТекКоличЗаказ);
...
Процедура ПрименитьИзменения(Команда) 
	...
	ЕстьОСГНижеНормыТекстВопроса = ПроверитьОСГНаСоответствиеНормам(Истина); 
...
&НаСервере
Функция ПроверитьОСГНаСоответствиеНормам(ПроверкаПримененияИзменений = Ложь) 
	
	ТекстВопроса = Неопределено;
	ТЗ_СтрокиЗаказов = ТЧСтрокиЗаказов.Выгрузить();
	ШрифтЗаголовкаПредупреждения = Новый Шрифт(,,Истина);	
	МассивСтрокПредупреждения = Новый Массив();

	Запрос = Новый Запрос;
	Запрос.Текст = 
	"ВЫБРАТЬ
	|	ТЧСтрокиЗаказов.Номенклатура КАК Номенклатура,
	|	ТЧСтрокиЗаказов.НоменклатураВесовой КАК НоменклатураВесовой,
	...
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	|	ВТТЧСтрокиЗаказов.Номенклатура КАК Номенклатура,
	|	ВТТЧСтрокиЗаказов.ЗаказКлиента.Представление КАК ПредставлениеЗаказа
	|ИЗ
	|	ВТТЧСтрокиЗаказов КАК ВТТЧСтрокиЗаказов
	|ГДЕ
	|	ВТТЧСтрокиЗаказов.НоменклатураВесовой = ИСТИНА
	|	И ВТТЧСтрокиЗаказов.Количество = ВТТЧСтрокиЗаказов.КоличествоЗаказ
	|	И &ПроверкаПримененияИзменений = ИСТИНА
	|;
	|
	|////////////////////////////////////////////////////////////////////////////////
	|ВЫБРАТЬ РАЗЛИЧНЫЕ
	...
	|	ВТТЧСтрокиЗаказов КАК ВТТЧСтрокиЗаказов
	|ГДЕ
	|	ВТТЧСтрокиЗаказов.СерияНоменклатуры <> ЗНАЧЕНИЕ(Справочник.СерииНоменклатуры.ПустаяСсылка)
	|	И ВТТЧСтрокиЗаказов.ОСГПроценты < 60";
	Запрос.УстановитьПараметр("ТЧСтрокиЗаказов", ТЗ_СтрокиЗаказов); 
	Запрос.УстановитьПараметр("ПроверкаПримененияИзменений", ПроверкаПримененияИзменений);
	...
	Если Не РезультатЗапроса[1].Пустой() Тогда
		...
		МассивСтрокПредупреждения.Добавить(Новый ФорматированнаяСтрока("Для следующих весовых номенклатур фактический вес равен весу в заказе: ", ШрифтЗаголовкаПредупреждения));
		МассивСтрокПредупреждения.Добавить(Символы.ПС);
		
		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл
			МассивСтрокПредупреждения.Добавить(Строка(ВыборкаДетальныеЗаписи.Номенклатура) + " (" + ВыборкаДетальныеЗаписи.ПредставлениеЗаказа + ") " + Символы.ПС);
		КонецЦикла;  
	...
	Если Не РезультатЗапроса[2].Пустой() Тогда
		...
		Если МассивСтрокПредупреждения.Количество() Тогда 
			МассивСтрокПредупреждения.Добавить(Символы.ПС);
		КонецЕсли;
		
		МассивСтрокПредупреждения.Добавить(Новый ФорматированнаяСтрока("Есть выбранные серии с оставшимся сроком годности (ОСГ) ниже нормы в следующих заказах: ", ШрифтЗаголовкаПредупреждения));
		МассивСтрокПредупреждения.Добавить(Символы.ПС);

		Пока ВыборкаДетальныеЗаписи.Следующий() Цикл 
			МассивСтрокПредупреждения.Добавить(ВыборкаДетальныеЗаписи.ПредставлениеЗаказа + " " + Символы.ПС); 
		КонецЦикла;  
		
	КонецЕсли;                                               
	
	Если МассивСтрокПредупреждения.Количество() Тогда
		МассивСтрокПредупреждения.Добавить(Новый ФорматированнаяСтрока("Применить изменения?", ШрифтЗаголовкаПредупреждения));
		ТекстВопроса = Новый ФорматированнаяСтрока(МассивСтрокПредупреждения); 
	КонецЕсли;
...
&НаКлиенте
Процедура ТЧСтрокиЗаказовПриАктивизацииСтроки(Элемент)
	
	Если Элемент.ТекущиеДанные <> Неопределено Тогда 		
	//	Элементы.ТЧСтрокиЗаказов.ТолькоПросмотр = Элемент.ТекущиеДанные.НомерЧекаККМ <> 0;
	Элементы.ТЧСтрокиЗаказовКоличество.ТолькоПросмотр = Элемент.ТекущиеДанные.НомерЧекаККМ <> 0;	
	Иначе
	//	Элементы.ТЧСтрокиЗаказов.ТолькоПросмотр = Ложь;
	Элементы.ТЧСтрокиЗаказовКоличество.ТолькоПросмотр = Ложь;
	КонецЕсли;

КонецПроцедуры

&НаСервереБезКонтекста
Функция ЕстьОстатокПоСерии(Заказ,СерияНоменклатуры,Номенклатура,Период)

	УстановитьПривилегированныйРежим(Истина);
	Предприятие = Неопределено;
	Если ОбщегоНазначенияБП.ЕстьРеквизитДокумента("Предприятие",Заказ.Метаданные()) Тогда
		Если ЗначениеЗаполнено(Заказ.Предприятие) Тогда
			Предприятие = Заказ.Предприятие;
		КонецЕсли;
	КонецЕсли;
	
	Склад = Неопределено;
	Если ОбщегоНазначенияБП.ЕстьРеквизитДокумента("Склад",Заказ.Метаданные()) Тогда
		Если ЗначениеЗаполнено(Заказ.Склад) Тогда
			Склад = Заказ.Склад;
		КонецЕсли;
	КонецЕсли;
...
&НаКлиенте
Асинх Процедура ТЧСтрокиЗаказовСерияНоменклатурыОбработкаВыбора(Элемент, ВыбранноеЗначение, ДополнительныеДанные, СтандартнаяОбработка)
	
	ПараметрПериод = ?(ЗначениеЗаполнено(ДатаОтгрузки),ДатаОтгрузки,ТекущаяДата());
	ТД = Элементы.ТЧСтрокиЗаказов.ТекущиеДанные;
	Если Не ЕстьОстатокПоСерии(ТД.ЗаказКлиента,ВыбранноеЗначение,ТД.Номенклатура,ПараметрПериод) Тогда
		ИсходнаяСерия = ТД.СерияНоменклатуры;
		ИсходноеИзменениеСтроки = ТД.ЕстьИзменениеСтроки;
		ИсходнаяДоступность = Элементы.ТЧСтрокиЗаказовПрименить.Доступность;
		КнопкиВопроса = Новый СписокЗначений;
		КнопкиВопроса.Добавить(КодВозвратаДиалога.Да,"Подтвердить выбор"); 
		КнопкиВопроса.Добавить(КодВозвратаДиалога.Нет,"Отменить выбор");   
		Обещание = ВопросАсинх(НСтр("ru='По выбранной серии нет остатков'"), КнопкиВопроса);
		Результат = Ждать Обещание;
		Если Результат = КодВозвратаДиалога.Нет Тогда 
			ТД.СерияНоменклатуры = ИсходнаяСерия;
			ТЧСтрокиЗаказовСерияНоменклатурыПриИзменении(Элемент);
			ТД.ЕстьИзменениеСтроки = ИсходноеИзменениеСтроки;
			Элементы.ТЧСтрокиЗаказовПрименить.Доступность = ИсходнаяДоступность;
			ТекСтрокаСписка = Элементы.Список.ТекущаяСтрока;
			ТекСтрокаСпискаСамовывоз = Элементы.СписокЗаказов.ТекущаяСтрока;
			ТекСтрокаСпискаПеремещение = Элементы.СписокЗаказовПеремещение.ТекущаяСтрока;
			Если Не ЗначениеЗаполнено(ТД.СерияНоменклатуры) Тогда
				ТД.ОСГ = ""; 
				ТД.ОСГПроценты = 0;
			Иначе
				СтруктураОСГ = ЗаполнитьОСГДляСерии(ТД.СерияНоменклатуры, ДатаОтгрузки);
				ТД.ОСГ = СтруктураОСГ.ОСГ;
				ТД.ОСГПроценты = СтруктураОСГ.ОСГПроцент;
			КонецЕсли;
		КонецЕсли; 
	КонецЕсли;
