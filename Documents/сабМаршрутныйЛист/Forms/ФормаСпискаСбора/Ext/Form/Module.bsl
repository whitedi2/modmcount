...
Процедура ЗаполнитьСерииДляНоменклатурыНаСервере(ЭтоКомандаДляВыбраннойНоменклатуры,ЕстьИзменения,ВидТЧ)
	...
	|       ГДЕ ЕСТЬNULL(УчетныйОстатки.КоличествоОстаток, 0) > 0
	|		УПОРЯДОЧИТЬ ПО НомерСтроки, ДатаПроизводства
	|ИТОГИ
...
Процедура ЗаполнитьСерииДляВсейНоменклатурыДляВсехЗаказовНаСервере(ЕстьИзменения,ВидТЧ)
	...
	|                   И Субконто3 <> Значение(Справочник.СерииНоменклатуры.ПустаяСсылка) 
	|					И Субконто1 В
	|						(&СписокНоменклатуры)) КАК УчетныйОстатки
	|		УПОРЯДОЧИТЬ ПО Номенклатура, ДатаПроизводства
	|ИТОГИ
	|ПО
	|	Номенклатура,
	|	Склад"; 
			...
			Пока ВыборкаСклад.Следующий() Цикл
				...
				Пока ВыборкаСерии.Следующий() Цикл
					Если ВыборкаСерии.КоличествоОстаток < 0 Тогда
						Продолжить;
					КонецЕсли;
					СтруктураДляОтбора = Новый Структура("Номенклатура, СерияНоменклатуры",ВыборкаСерии.Номенклатура, Справочники.СерииНоменклатуры.ПустаяСсылка());
					...
					ОсталосьРаспределить = ВыборкаСерии.КоличествоОстаток;  
					Для Каждого СтрокаТЗ Из ТЗСтрок Цикл
						Если СтрокаТЗ.Склад <> ВыборкаСерии.Склад Тогда
							Продолжить;
						КонецЕсли;
						...
						СтрокаТЗ.СтрокаНоменклатуры.СерияНоменклатуры = ВыборкаСерии.СерияНоменклатуры; 
						СтрокаТЗ.СтрокаНоменклатуры.ДатаПроизводства = ВыборкаСерии.ДатаПроизводства;
