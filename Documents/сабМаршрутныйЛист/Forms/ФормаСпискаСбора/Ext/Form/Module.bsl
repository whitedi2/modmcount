...
Процедура ИзменитьКоличествоВРеализации(Документ)
	...
	ТЧПоНоменклатуре.Свернуть("Номенклатура, ЗаказКлиента, Реализация, Контрагент, Точка, НомерСтрокиРеализации, ЕдиницаИзмерения, ИдентификаторСтрокиРеализации, ДатаПроизводства",
	...
	Если ТипЗнч(Документ) = Тип("ДокументСсылка.РеализацияТоваровУслуг") Тогда
		...
		Для каждого ТекСтрока Из НайденныеСтрокиПоНоменклатуре Цикл
			...
			Если Не ТекСтрока.ДатаПроизводства = Дата('00010101') Тогда
				ДокОб.Товары[НомерСтроки - 1].СерияНоменклатуры = Справочники.СерииНоменклатуры.СоздатьПолучитьСерию(ТекСтрока.Номенклатура, ТекСтрока.ДатаПроизводства);
			КонецЕсли;
