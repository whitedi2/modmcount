...
Процедура УУ_ОбработкаЗаполнения(ДанныеЗаполнения, СтандартнаяОбработка)
	...
	ИначеЕсли ТипЗнч(ДанныеЗаполнения) = Тип("ДокументСсылка.УЧ_ВыпускПродукции") Тогда
		...
		ЭтотОбъект.Склад = ДанныеЗаполнения.СкладМатериалов; 
		...
		//	НоваяСтрокаТовары.Номенклатура = ТекСтрокаТЧ.Номенклатура;
		//	НоваяСтрокаТовары.Количество = ТекСтрокаТЧ.Количество; 
		//	НоваяСтрокаТовары.СчетУчета = Счет_10_01;
		//	НоваяСтрокаТовары.СчетПередачи = Счет_10_07;
		//КонецЦикла;
		
		Для Каждого ТекСтрокаТЧ Из ДанныеЗаполнения.Материалы Цикл
			НоваяСтрокаТовары = ЭтотОбъект.Товары.Добавить();
			НоваяСтрокаТовары.Номенклатура = ТекСтрокаТЧ.Материал;
			НоваяСтрокаТовары.Количество = ТекСтрокаТЧ.Количество; 
			НоваяСтрокаТовары.СчетУчета = Счет_10_01;
			НоваяСтрокаТовары.СчетПередачи = Счет_10_07;
		КонецЦикла;
