...
Процедура ОбработкаПроведения(Отказ, Режим)
	...
	Если ВозможноЧастичноеИсполнениеЗаказов ИЛИ (Не ЗначениеЗаполнено(Статус) ИЛИ Статус = Перечисления.СтатусыЗаказовКлиентов.Новый Или Статус = Перечисления.СтатусыЗаказовКлиентов.КОтгрузке) Тогда
		...
		Если ВозможноЧастичноеИсполнениеЗаказов И (Статус = Перечисления.СтатусыЗаказовКлиентов.Отгружен Или Статус = Перечисления.СтатусыЗаказовКлиентов.Отменен) Тогда
			Движения.Записать();
...
Процедура ПриКопировании(ОбъектКопирования)
	...
	//заполнения "Продажа в розницу" если в договоре наличный платеж
	Если ЗначениеЗаполнено(Договор) Тогда
		Если Договор.ПриоритетныйПорядокРасчетов = Перечисления.Д_ИсточникиСредств.Нал Тогда
			ПродажаВРозницу = Истина; 
		Иначе
			ПродажаВРозницу = Ложь;
		КонецЕсли;
	КонецЕсли;
