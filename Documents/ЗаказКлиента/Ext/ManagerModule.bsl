...
Функция ПечатьЗаказ(МассивОбъектов, ОбъектыПечати = Неопределено, ПечатьКомплекта = Ложь) Экспорт
	...
	Если сабОбщегоНазначенияПовтИсп.ПолучитьСерийныйУчет() Тогда
		...
		|	ЗаказКлиентаТабличнаяЧасть.Ссылка.Склад КАК Склад
		|ИЗ
	...
	КонецЦикла;
		...
		Пока ВыборкаПоДокументам.Следующий() Цикл
			...
			ОбластьЗаголовок.Параметры.Склад = ВыборкаПоДокументам.Ссылка.Склад;
			ТабличныйДокумент.Вывести(ОбластьЗаголовок);
			...
			Пока ВыборкаПоСтрокамТЧ.Следующий() Цикл 
				Если ПолучитьВесовой(ВыборкаПоСтрокамТЧ.Номенклатура) тогда
					ОбластьСтрокаНомера  = Макет.ПолучитьОбласть("СтрокаВес|НомерСтроки");
					ОбластьСтрокаКодов   = Макет.ПолучитьОбласть("СтрокаВес|КолонкаКодов");
					ОбластьСтрокаДанных  = Макет.ПолучитьОбласть("СтрокаВес|Данные");
					ОбластьСтрокаСкидок  = Макет.ПолучитьОбласть("СтрокаВес|Скидка");
					ОбластьСтрокаСуммы   = Макет.ПолучитьОбласть("СтрокаВес|Сумма");
				Иначе
					ОбластьСтрокаНомера  = Макет.ПолучитьОбласть("Строка|НомерСтроки");
					ОбластьСтрокаКодов   = Макет.ПолучитьОбласть("Строка|КолонкаКодов");
					ОбластьСтрокаДанных  = Макет.ПолучитьОбласть("Строка|Данные");
				...
				Пока ВыборкаСерии.Следующий() И ОсталосьСписатьКоличество > 0 Цикл
					...
					Если ВыводитьУпаковки Тогда
						Если ВыборкаСерии.КоличествоУпаковок = 0 ИЛИ ВыборкаСерии.КоличествоВУпаковке = 0 
							ИЛИ ВыборкаСерии.КоличествоВУпаковке = 1 Тогда 
							КолЕдиницСбор = СписатьКоличество;
							КоличествоУпСбор = 0;
						Иначе
							КоличествоУпСбор = Цел(СписатьКоличество / ВыборкаСерии.КоличествоВУпаковке);   //расчет по упаковкам, далее выведение по условиям весовой или нет
							КолВУпак = КоличествоУпСбор * ВыборкаСерии.КоличествоВУпаковке;                 //основной код который выполняется всегда
							КолЕдиницСбор = СписатьКоличество - КолВУпак; 
						КонецЕсли;
						
						ЕдИзмУпаковки = ПолучитьЕдИзмУпаковки(ВыборкаПоСтрокамТЧ.Номенклатура, ВыборкаПоСтрокамТЧ.ЕдиницаИзмерения); 
						Если КолЕдиницСбор <> 0 Тогда
							ШтСбор = " + " + Строка(КолЕдиницСбор) + " " + ВыборкаПоСтрокамТЧ.ЕдиницаИзмерения;
						Иначе
							ШтСбор = "";
						КонецЕсли;
						
						Если ЭтоВесоваяЕдИзмерения Тогда 								
							Сбор = "Вес: " + Строка(КоличествоУПСбор) + ?(ЕдИзмУпаковки = Неопределено, " упак.", " " + ЕдИзмУпаковки) + ШтСбор;   
					...
					КолВУпаковке = ВыборкаСерии.КоличествоВУпаковке; 
				КонецЦикла; 
				...
				Если ОсталосьСписатьКоличество > 0 Тогда
					...
					Если ВыводитьУпаковки Тогда
						Если ВыборкаСерии.КоличествоУпаковок = 0 ИЛИ ВыборкаСерии.КоличествоВУпаковке = 0 
							...
							КолЕдиницСбор = СписатьКоличество;
							КоличествоУпСбор = 0;
						Иначе                                                                      //выполняется в случае если, заказ отгружен, но в заказе поменяли количество или отгрузили меньше чем заказе
							КоличествоУпСбор = Цел(ОсталосьСписатьКоличество / КолВУпаковке);      //расчет по упаковкам, далее выведение по условиям весовой или нет
							КолВУпак = КоличествоУпСбор * КолВУпаковке;
							КолЕдиницСбор = ОсталосьСписатьКоличество - КолВУпак; 
						КонецЕсли;
...
Функция ПечатьКоммерческоеПредложение(ПараметрыКоманды) Экспорт
	...
	|	ЗаказКлиентаТабличнаяЧасть.Номенклатура.ФайлКартинки КАК НоменклатураФайлКартинки
	|ИЗ
	|	Документ.ЗаказКлиента.ТабличнаяЧасть КАК ЗаказКлиентаТабличнаяЧасть
	...
	Если ПараметрыКоманды[0].Ссылка.Организация.ФайлЛоготип.ПутьКФайлу <> "" Тогда
		Если НЕ ПараметрыКоманды[0].Ссылка.Организация.ФайлЛоготип.ПометкаУдаления Тогда
			
			ДвоичныеДанныеФайла                   = РаботаСФайлами.ДвоичныеДанныеФайла(ПараметрыКоманды[0].Ссылка.Организация.ФайлЛоготип);	
			...
			Шапка.Рисунки.Логотип.Картинка = Картинка;
	...
	Для каждого Выборка Из Результат Цикл
		...
		Строка.Параметры.РекомендуемаяДляЛПУ = Выборка.Цена;
		Строка.Параметры.СуммаЛота = Выборка.Сумма;
		Итого = Итого + Выборка.Сумма;
		ИтогоВТомЧислеНДС = ИтогоВТомЧислеНДС + Выборка.СуммаНДС;
		
		Если НЕ Выборка.НоменклатураФайлКартинки.ПометкаУдаления Тогда
			
			Если Выборка.НоменклатураФайлКартинки <> Справочники.НоменклатураПрисоединенныеФайлы.ПустаяСсылка() Тогда
			
			ДвоичныеДанныеФайла                   = РаботаСФайлами.ДвоичныеДанныеФайла(Выборка.НоменклатураФайлКартинки);	
			Если ТипЗнч(ДвоичныеДанныеФайла)      = Тип("Картинка") Тогда
				Картинка                          = ДвоичныеДанныеФайла;
			ИначеЕсли ТипЗнч(ДвоичныеДанныеФайла) = Тип("ДвоичныеДанные") Тогда
				Картинка                          = Новый Картинка(ДвоичныеДанныеФайла);
			...
			Строка.Рисунки.СхемаКонтура.Картинка = Картинка;
	...
	Подвал.Параметры.ИтогоВТомЧислеНДС = ИтогоВТомЧислеНДС;
	Подвал.Параметры.Всего = "Всего наименований "+(НомерСтроки-1)+", на сумму "+Итого+" рублей";		
	ТабДок.Вывести(Подвал);
...
Функция ПолучитьВесовой(Номенклатура) 
	Запрос = Новый Запрос;
