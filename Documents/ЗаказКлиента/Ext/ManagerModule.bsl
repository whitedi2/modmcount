...
Функция КалькуляцияИМаржаЗаказа(Ссылка) Экспорт
	...
	Пока Выборка.Следующий() Цикл
		...
		|  0
		|ИЗ
		|	РегистрСведений.ПлановаяСебестоимость.СрезПоследних(&ДатаОкончания, ) КАК ПлановаяСебестоимостьСрезПоследних
		|ГДЕ
		|	ПлановаяСебестоимостьСрезПоследних.Номенклатура В(&Номенклатура)";
		//}
