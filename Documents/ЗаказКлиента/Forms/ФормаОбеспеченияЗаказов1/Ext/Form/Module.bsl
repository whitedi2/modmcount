...
Процедура ЗаполнитьОстаткиИРезервы(Заказ, РасчетПоСтроке = Ложь, СтруктураДанных = Неопределено, ТЧ)
	...
	Если РасчетПоСтроке Тогда
		...
		Для каждого ТекСтрокаНайденная Из НайденныеСтроки Цикл
			Если НачалоДня(ТекСтрокаНайденная.ДатаЗаказа) <= НачалоДня(Объект.Дата) И НЕ ТипЗнч(ТекСтрокаНайденная.Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") И НЕ ТипЗнч(ТекСтрокаНайденная.Заказ) = Тип("ДокументСсылка.ЗаказНаВозвратОтКлиента") Тогда
	...
	Иначе
		...
		Для каждого ТекСтрока Из ТЧ Цикл
			...
			Для каждого ТекСтрокаНайденная Из НайденныеСтроки Цикл
				Если НачалоДня(ТекСтрокаНайденная.Дата) <= НачалоДня(Объект.Дата) И НЕ ТипЗнч(ТекСтрокаНайденная.Заказ) = Тип("ДокументСсылка.ЗаказПоставщику") И НЕ ТипЗнч(ТекСтрокаНайденная.Заказ) = Тип("ДокументСсылка.ЗаказНаВозвратОтКлиента") Тогда
