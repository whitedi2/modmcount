...
Процедура ЗаполнитьЗадолженностьКонтрагента(ТекОбъект = Неопределено)
	...
	Иначе
		...
		//Выборка.Следующий();
		//ЗадолженностьНадпись = "Задолженность по оплате " + Выборка.ДниПросрочки + " дн.";  
		//Если ТекОбъект = Неопределено Тогда
...
Процедура УстановитьЗависимыеДанныеВТЧ(ТекДанные) Экспорт
	...
	//	КонецЕсли;
	//Иначе
		ЦенаКЗаполнению = РассчитатьЦену(ТекДанные.Номенклатура, Объект.Контрагент, Объект.Предприятие, Объект.Подразделение, ?(ЗначениеЗаполнено(Объект.ДатаПоступления), Объект.ДатаПоступления, Объект.Дата), Объект.Договор);
		//Если ЦенаКЗаполнению <> 0 Тогда
			ТекДанные.Цена = ЦенаКЗаполнению;
		//КонецЕсли;
	//КонецЕсли;	
...
Функция РассчитатьЦену(Номенклатура, Контрагент, Предприятие, Подразделение, Дата, Договор)
	...
	Если ЗначениеЗаполнено(Договор.ТипЦен) И (Договор.СрокДействияТипаЦен > Дата ИЛИ Не ЗначениеЗаполнено(Договор.СрокДействияТипаЦен)) Тогда 
