...
Процедура ЗаполнитьОстаткиИРезервы(РасчетПоСтроке = Ложь, СтруктураДанных = Неопределено)
	...
	Запрос.Текст = "ВЫБРАТЬ
	               ...
	               |	ЗаказКлиентаТабличнаяЧасть.Ссылка КАК Заказ
	               |ПОМЕСТИТЬ Врем1
	               |ИЗ
	               |	Документ.ЗаказКлиента.ТабличнаяЧасть КАК ЗаказКлиентаТабличнаяЧасть
	               |ГДЕ
	               |	ЗаказКлиентаТабличнаяЧасть.Склад = &Склад
	               |	И ЗаказКлиентаТабличнаяЧасть.Номенклатура В(&Номенклатура)
	               |	И ЗаказКлиентаТабличнаяЧасть.Ссылка.Проведен = ИСТИНА
	               |	И ЗаказКлиентаТабличнаяЧасть.Ссылка.Предприятие = &Предприятие
	               |	И НЕ ЗаказКлиентаТабличнаяЧасть.Ссылка.Статус В (&Статус)
	               |	И НЕ ЗаказКлиентаТабличнаяЧасть.Ссылка = &ЭтаСсылка
	               |
	               |СГРУППИРОВАТЬ ПО
	               |	ЗаказКлиентаТабличнаяЧасть.Номенклатура,
	               |	ЗаказКлиентаТабличнаяЧасть.Склад,
	               |	ЗаказКлиентаТабличнаяЧасть.Ссылка
	               |;
	               |
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	Врем1.Заказ КАК Заказ
	               |ПОМЕСТИТЬ Врем2
	               |ИЗ
	               |	Врем1 КАК Врем1
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.УЧ_Реализация КАК УЧ_Реализация
	               |		ПО Врем1.Заказ = УЧ_Реализация.ДокОснование
	               |			И (УЧ_Реализация.ДокОснование.Проведен = ИСТИНА)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РеализацияТоваровУслуг КАК РеализацияТоваровУслуг
	               |		ПО Врем1.Заказ = РеализацияТоваровУслуг.Заказ
	               |			И (РеализацияТоваровУслуг.Проведен = ИСТИНА)
	               |		ЛЕВОЕ СОЕДИНЕНИЕ Документ.РозничнаяПродажа КАК РозничнаяПродажа
	               |		ПО Врем1.Заказ = РозничнаяПродажа.Заказ
	               |			И (РозничнаяПродажа.Проведен = ИСТИНА)
	               |ГДЕ
	               ...
	               |////////////////////////////////////////////////////////////////////////////////
	               |ВЫБРАТЬ
	               |	Врем1.Количество КАК Количество,
	               |	Врем1.Номенклатура КАК Номенклатура,
	               |	Врем1.Склад КАК Склад,
	               |	Врем1.Заказ КАК Заказ
	               |ИЗ
	               |	Врем1 КАК Врем1
	               |		ВНУТРЕННЕЕ СОЕДИНЕНИЕ Врем2 КАК Врем2
	               |		ПО Врем1.Заказ = Врем2.Заказ";
	
	Запрос.УстановитьПараметр("Склад", Объект.Склад);
	Запрос.УстановитьПараметр("Номенклатура", Объект.ТабличнаяЧасть.Выгрузить(,"Номенклатура"));
	Запрос.УстановитьПараметр("ДатаЗаказа", ?(ЗначениеЗаполнено(Объект.Ссылка), Объект.Дата, ТекущаяДата()));
	...
	Если РасчетПоСтроке Тогда
		...
		СтруктураДанных.КоличествоОстаток = СтруктураДанных.КоличествоНаОстатках - СтруктураДанных.КоличествоВРезервах - СтруктураДанных.КоличествоУпаковок;
	...
	Иначе
		...
		Для каждого ТекСтрока Из Объект.ТабличнаяЧасть Цикл
			...
			Для каждого ТекСтрокаНайденная Из НайденныеСтроки Цикл
				...
				ЗаполнитьЗначенияСвойств(НовВремСтрока, ТекСтрокаНайденная);
			КонецЦикла;	
		КонецЦикла;
		
		Для каждого ТекСтрока Из Объект.ТабличнаяЧасть Цикл
			ТекСтрока.КоличествоОстаток = ТекСтрока.КоличествоНаОстатках - ТекСтрока.КоличествоВРезервах - ТекСтрока.КоличествоУпаковок; 
		КонецЦикла;
	КонецЕсли;
...
Процедура ПередЗаписью(Отказ, ПараметрыЗаписи)
	...
	Для Каждого СтрокаТовары Из Объект.ТабличнаяЧасть Цикл
		...
		Если ЗначениеЗаполнено(СтрокаТовары.Кратность) И СтрокаТовары.Количество % СтрокаТовары.Кратность <> 0 Тогда
			сабОбщегоНазначенияКлиентСервер.СообщитьОбОшибке(Объект,"Количество " + СтрокаТовары.Количество + " не кратно " + СтрокаТовары.Кратность + ".", "Объект.ТабличнаяЧасть", СтрокаТовары.НомерСтроки, "КоличествоУпаковок", Отказ);
		КонецЕсли;
...
Процедура ТабличнаяЧастьНоменклатураПриИзменении(Элемент)
	...
	ТекДанные = Элементы.ТабличнаяЧасть.ТекущиеДанные;
	СтруктураДанных =Новый Структура("Номенклатура, Склад, КоличествоОстаток, КоличествоНаОстатках, КоличествоВРезервах, КоличествоУпаковок", 
	ТекДанные.Номенклатура, ТекДанные.Склад, ТекДанные.КоличествоОстаток, ТекДанные.КоличествоНаОстатках,
	ТекДанные.КоличествоВРезервах, ТекДанные.КоличествоУпаковок);
...
Процедура РассчитатьКоличество(ТекДанные = Неопределено)
	...
	Если ЗначениеЗаполнено(ТекДанные.Кратность) И ТекДанные.Количество % ТекДанные.Кратность <> 0 Тогда
		сабОбщегоНазначенияКлиентСервер.СообщитьОбОшибке(Объект,"Количество " + ТекДанные.Количество + " не кратно " + ТекДанные.Кратность + ".", "Объект.ТабличнаяЧасть", ТекДанные.НомерСтроки, "КоличествоУпаковок", Истина);
	КонецЕсли;
...
Процедура СтатусОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	...
	Если НЕ ВыбранноеЗначение = ПредопределенноеЗначение("Перечисление.СтатусыЗаказовКлиентов.КОтгрузке") Тогда
		СтандартнаяОбработка = Ложь;
...
Процедура КонтрагентПриИзмененииСервер(Изменять = Истина)
	...
	Если Изменять Тогда
		Объект.ПродажаВРозницу = Объект.Контрагент.ПриоритетныйПорядокРасчетов = Перечисления.Д_ИсточникиСредств.Нал;
	...
	Запрос = Новый Запрос;
	Запрос.Текст = "ВЫБРАТЬ
	|	ПодразденияКонтрагентов.Ссылка КАК Ссылка,
	|	ПодразденияКонтрагентов.Адрес КАК Адрес
	|ИЗ
	|	Справочник.ПодразденияКонтрагентов КАК ПодразденияКонтрагентов
	|ГДЕ
	|	ПодразденияКонтрагентов.Владелец = &Владелец";
...
Процедура АдресДоставкиОбработкаВыбора(Элемент, ВыбранноеЗначение, СтандартнаяОбработка)
	Если ВыбранноеЗначение = "Подразделения контрагента..." Тогда
		...
		ОткрытьФорму("Справочник.ПодразденияКонтрагентов.ФормаВыбора", ПараметрыФормы,,,,, Новый ОписаниеОповещения("АдресДоставкиОбработкаВыбораЗавершение", ЭтаФорма), РежимОткрытияОкнаФормы.БлокироватьВесьИнтерфейс);
	КонецЕсли;
КонецПроцедуры
...
Процедура АдресДоставкиПриИзмененииНаСервере(Изменять = Истина)
	...
	|	Справочник.ПодразденияКонтрагентов КАК ПодразденияКонтрагентов
	|ГДЕ
	|	ПодразденияКонтрагентов.Адрес = &Адрес
	|	И ПодразденияКонтрагентов.Владелец = &Владелец";
